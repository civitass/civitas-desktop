 ...
 *   bun packages/e2e/src/runner.ts --binary ./target/release/civitas
 *
 * Environment:
 *   CIVITAS_BASE_URL  — API base URL (default: http://localhost:3030)
 *   CIVITAS_BINARY    — path to civitas binary (default: civitas)
 */
...
const baseUrl =
  getFlag("--base-url") ??
  process.env.CIVITAS_BASE_URL ??
  "http://localhost:3030";
const binary =
  getFlag("--binary") ?? process.env.CIVITAS_BINARY ?? "civitas";
...
console.log("╔══════════════════════════════════════╗");
console.log("║   civitas e2e test runner         ║");
console.log("╚══════════════════════════════════════╝");
console.log("");
...
if (suite === "app") {
  ...
  console.log(
    "  cd apps/civitas-app-tauri && bun run test:e2e\n"
  );
