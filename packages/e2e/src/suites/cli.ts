...
const tests: TestDef[] = [
  {
    ...
    fn: async (exec, bin) => {
      ...
      if (!stdout.includes("civitas")) throw new Error("missing 'civitas' in output");
    },
  },
  {
    name: "doctor checks",
  ...
  // Regression: f1a7e87a6 added `civitas search` — daemon-free local
  ...
  // who do `civitas search "thing"` from their shell.
  {
    name: "search --help",
    fn: async (exec, bin) => {
  ...
  // Regression: `civitas search` against a non-existent data dir
  // must fail loudly with the documented error path, not crash with a
  // panic or hang. Pre-fix shape of the error: "no civitas database
  // at <path>. run `civitas record` first."
  {
    name: "search bails clearly on missing db",
    fn: async (exec, bin) => {
      // Random tmp dir guaranteed not to contain db.sqlite.
      const { stdout, exitCode } = await exec(
        `${bin} search --data-dir /tmp/nonexistent-civitas-e2e-${Date.now()} --limit 1 2>&1`
      );
      if (exitCode === 0)
        throw new Error("search exited 0 with no database — should bail");
      if (!stdout.toLowerCase().includes("no civitas database"))
...
if (import.meta.main) {
  const bin = process.argv[2] ?? "civitas";
  console.log(`\nCLI tests for ${bin}\n`);
