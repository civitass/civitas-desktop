...
async function buildAndDeploy(ip: string): Promise<void> {
  const binary = `${REPO_ROOT}/target/release/civitas`;
  const debugBinary = `${REPO_ROOT}/target/debug/civitas`;
  ...
  if (await Bun.file(binary).exists()) {
    ...
    throw new Error(
      "No civitas binary found. Run `cargo build -p civitas-engine --bin civitas` first."
    );
  }
  ...
  await scp(ip, src, "/tmp/civitas");
  await ssh(ip, "chmod +x /tmp/civitas");
}
...
async function startServer(ip: string, port = 3030): Promise<void> {
  await ssh(
    ip,
    `export PATH="/usr/local/bin:/tmp:$PATH" HOME=/Users/admin && nohup /tmp/civitas record --disable-audio --port ${port} > /tmp/civitas.log 2>&1 & echo $!`,
    { timeout: 10_000 }
  );
...
async function stopServer(ip: string): Promise<void> {
  await ssh(ip, "pkill -f 'civitas record' 2>/dev/null || true");
