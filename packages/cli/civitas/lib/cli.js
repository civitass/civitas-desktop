...
if (!bin) {
  ...
  console.error(
    `civitas: no prebuilt binary for ${key}. ` +
      `reinstall with: npm i -g civitas`,
  );
  process.exit(1);
}
...
const child = spawn(bin, process.argv.slice(2), {
  stdio: "inherit",
  env: { ...process.env, CIVITAS_DISTRIBUTION: process.env.CIVITAS_DISTRIBUTION || "cli" },
});
let forwardingSignal = null;

child.on("error", (error) => {
  console.error(`civitas: failed to spawn binary: ${error.message}`);
  process.exit(1);
});
