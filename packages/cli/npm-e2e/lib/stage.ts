...
export const PACKAGES: PlatformPackage[] = [
  {
    dir: "civitas-darwin-arm64",
    name: "@civitas/cli-darwin-arm64",
    os: "darwin",
    arch: "arm64",
    bin: "bin/civitas",
  ...
  {
    dir: "civitas-darwin-x64",
    name: "@civitas/cli-darwin-x64",
    os: "darwin",
    arch: "x64",
    bin: "bin/civitas",
  ...
  {
    dir: "civitas-linux-x64",
    name: "@civitas/cli-linux-x64",
    os: "linux",
    arch: "x64",
    bin: "bin/civitas",
  ...
  {
    dir: "civitas-win32-x64",
    name: "@civitas/cli-win32-x64",
    os: "win32",
    arch: "x64",
    bin: "bin/civitas.exe",
...
function copyPackages(version: string): string {
  ...
  wrapperJson.bin = { civitas: WRAPPER.bin };
...
function stageBinaries(stagedCliRoot: string, binaryPath: string, host: PlatformPackage): void {
  ...
  for (const pkg of PACKAGES) {
    ...
    if (pkg === host) {
      ...
      fs.writeFileSync(binPath, "civitas placeholder for local npm smoke tests\n");
    } else {
      writeExecutable(
        binPath,
        "#!/bin/sh\n" +
          "echo 'civitas placeholder for local npm smoke tests' >&2\n" +
          "exit 1\n",
      );
    }
  }
...
export function validateStage(stagedCliRoot: string, host: PlatformPackage): void {
  ...
  if (wrapperJson.bin?.civitas !== WRAPPER.bin) {
    fail("wrapper package must expose bin.civitas = lib/cli.js");
  }
...
export function defaultBinaryPath(): string {
  ...
  return path.join(REPO_ROOT, "target", host.cargoTarget, "release", `civitas${ext}`);
}
...
export async function cmdBuild(): Promise<void> {
  ...
  console.log(
    path.join(
      ...
      host.cargoTarget,
      "release",
      process.platform === "win32" ? "civitas.exe" : "civitas",
    ),
  );
}
