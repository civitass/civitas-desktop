...
export async function testRecord(ctx: TestContext): Promise<void> {
  ...
  const dataDir = path.join(ctx.workDir, "civitas-data");
  ...
  if (earlyExit) fail(`npx civitas@latest record exited before ${ctx.livenessSeconds}s`);
  ...
  if (process.platform === "win32") {
    ...
    const wrapper = descendants(child.pid || 0).find((row) =>
      row.command.includes("civitas/lib/cli.js"),
    );
  ...
  if (leftovers.length > 0) {
    fail(
      `civitas record left ${leftovers.length} process(es) running after SIGTERM:\n` +
