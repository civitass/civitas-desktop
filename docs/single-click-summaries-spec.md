...
**Source:** Historical product-design notes, privacy-sanitized for publication
**Date:** Feb 19, 2026
**Status:** Draft
...
Civitas can remember the screen and audio context a person explicitly permits,
but many people still do not know what to ask of that local history. The
current empty chat state shows a few auto-suggestions ("What did I do in the
last hour?"), but they are small, text-only, and easy to ignore. People who do
not know what to ask may simply leave.
...
**Core tension:** Civitas collects 2K-13K+ memories per day, but the value is locked behind a blank text box.

---

## Solution

**"Single-Click Summaries"** — a prompt template library and visual builder that makes civitas's value immediately obvious. Instead of a blank chat, users see curated one-click actions that generate structured summaries from their data.
...
**Left Panel — Filters:**
- **Time Period:** Predefined ranges with live memory counts from civitas DB
...
- **"Save as Template" persistence:** Where to store? Options: (a) localStorage — simplest, lost on reinstall, (b) civitas DB — survives reinstalls, (c) as a Pipe — enables scheduling.

### Pipes Integration
- **Scheduled follow-up:** A useful summary should be reusable as a scheduled
  Pipe after a good chat, without requiring the user to rebuild its filters and
  prompt.
