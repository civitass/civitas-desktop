...
| `js/log-injection` (×3) | `apps/civitas-app-tauri/e2e/tests/websocket-performance.js` | Server-supplied values were logged unescaped. Each value is now flattened to one line and truncated where it enters the socket handler rather than at each `console.log`, which also covered two reporting paths the per-call-site version had missed — the end-of-run error summary and the JSON parse handler. Frame counters are coerced with `Number()`. |
