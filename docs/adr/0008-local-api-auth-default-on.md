...
- The credential is generated per install and stored in the encrypted Civitas
  secret store backed by the OS credential vault.
...
A missing or unreadable credential produces a visible authorization failure
rather than an open server or silent key rotation. Rotating the owner key
invalidates configured clients and requires reconnection. Local clients must be
treated as privileged because a process that can read their configuration may
also read any credential stored there.
