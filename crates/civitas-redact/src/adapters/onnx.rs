...
//! Local ONNX-runtime inference of the pinned screenpipe PII redactor.
//!
...
//!
//! The model is downloaded only after explicit user opt-in. Its
//! upstream CC BY-NC 4.0 license is separate from Civitas' MIT source
//! license; see `docs/MODEL_CATALOG.md`.
//!
//! ## BIO decoding
//!
...
impl OnnxConfig {
    ...
    /// Immutable Hugging Face revision containing the audited ONNX
    /// artifacts. A model bump requires a deliberate URL, checksum, license,
    /// and [`ONNX_REDACTOR_VERSION`] review.
    pub const HF_REPO_BASE: &'static str =
        "https://huggingface.co/screenpipe/pii-redactor/resolve/5c907008e2a2ad394712496a26819ad28dd44662/v45_phase5_pruned";
    ...
    ///
    /// Every cached or newly downloaded artifact must match its pinned
    /// SHA-256 before it can be loaded.
    pub async fn ensure_model_present(&self) -> Result<(), RedactError> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|error| RedactError::Runtime(format!("v45 HTTP client: {error}")))?;

        for (filename, expected_sha) in Self::FILES {
            let target = self.model_dir.join(filename);
            if target.exists() {
                if !sha256_matches(&target, expected_sha)? {
            ...
            if civitas_core::network::network_mode_denies_remote() {
                return Err(RedactError::Runtime(format!(
                    "{filename} download blocked by CIVITAS_NETWORK_MODE=deny"
                )));
            }
...
// and falls through to the next on-device adapter.
// =========================================================================
