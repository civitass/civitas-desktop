...
//! [civitas-pii-bench-image](https://github.com/civitass/autowork-pii-bench-image)
...
//! quoting.
//!
//! Bench accuracy on `civitas-pii-bench-image` val (221 images):
...
//! recipe: `civitas-pii-bench-image/training/export_fp16.py`.
...
/// `civitas-pii-bench-image/src/score.py::CANONICAL_LABELS`.
#[cfg(feature = "onnx-cpu")]
...
pub struct RfdetrConfig {
    /// Path to `rfdetr_vN.onnx`. We default to `~/.civitas/models/rfdetr_v12.onnx`
...
impl RfdetrConfig {
    ...
    /// Immutable Hugging Face revision for the canonical ONNX. A model
    /// bump requires a deliberate URL, checksum, license, and
    /// [`RFDETR_VERSION`] review.
    pub const HF_DOWNLOAD_URL: &'static str =
        "https://huggingface.co/screenpipe/pii-image-redactor/resolve/d4bb93370fc3fe3c36007bef22f71bd0e97a0951/rfdetr_v12.onnx";
    ...
    pub async fn ensure_model_present(&self) -> Result<(), RedactError> {
        self.ensure_model_present_with_remote_allowed(
            !civitas_core::network::network_mode_denies_remote(),
        )
        .await
    }

    async fn ensure_model_present_with_remote_allowed(
        &self,
        remote_allowed: bool,
    ) -> Result<(), RedactError> {
        ...
        if !remote_allowed {
            return Err(RedactError::Runtime(
                "RF-DETR model download blocked by CIVITAS_NETWORK_MODE=deny".to_string(),
            ));
        }
        ...
        civitas_core::network::authorize_url(
            ...
            Self::HF_DOWNLOAD_URL,
        )
        .map_err(|error| RedactError::Runtime(error.to_string()))?;
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|error| RedactError::Runtime(format!("rfdetr HTTP client: {error}")))?;
        let resp = client
...
mod tests {
    ...
    #[test]
    fn default_path_lives_under_civitas_dir() {
        let p = RfdetrConfig::default_model_path();
        let s = p.to_string_lossy();
        assert!(s.contains(".civitas/models/rfdetr_v12.onnx"));
    }
    ...
    #[tokio::test]
    async fn wrong_checksum_fails_closed_without_a_network_request() {
        use tempfile::tempdir;
        let d = tempdir().unwrap();
        ...
        let error = cfg
            .ensure_model_present_with_remote_allowed(false)
            .await
            .unwrap_err();
        assert!(
            error
                .to_string()
                .contains("blocked by CIVITAS_NETWORK_MODE=deny"),
