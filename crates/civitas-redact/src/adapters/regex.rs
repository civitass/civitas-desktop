...
//!   the bare-digit false-positive that the original civitas regex
//!   triggered)
...
static PATTERNS: Lazy<Vec<Pattern>> = Lazy::new(|| {
    let raw: &[(&str, SpanLabel)] = &[
        ...
        // Database URI schemes with user information before the host.
        (
            concat!(
...
mod tests {
    ...
    #[test]
    fn email_caught() {
        let out = run("Contact: casey.rivera@work.example for details");
        assert_eq!(out.spans.len(), 1);
        assert_eq!(out.spans[0].label, SpanLabel::Email);
        assert_eq!(out.spans[0].text, "casey.rivera@work.example");
    ...
    fn connection_string_with_creds_caught() {
        let fixture = format!("psql postgres{}aiden:S3cret@db.acme.com:5432/prod", "://");
        let out = run(&fixture);
        assert_eq!(out.spans.len(), 1);
    ...
    fn throughput_on_realistic_corpus() {
        ...
        let mixed: Vec<String> = [
            "Contact: casey.rivera@work.example for the Q3 review",
            concat!(
