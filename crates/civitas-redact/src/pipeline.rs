...
//! 3. **Local AI fallback** (ONNX / OPF) runs on inputs the regex pass
                ...
                match ai.redact(&current.redacted).await {
                    Ok(ai_out) => {
                        let redacted = if ai_out.spans.is_empty() {
                            // A span-less adapter returns redacted text
                            // only. It has already applied the label
                            // policy, so trust its output
...
mod tests {
    ...
    async fn span_less_ai_output_is_trusted() {
        // A span-less adapter must have its output used verbatim because
        // it applied the policy internally. UppercaseAi
