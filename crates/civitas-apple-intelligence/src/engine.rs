...
/// Queries civitas data and processes it with Foundation Models.
///
/// Higher-level function that:
/// 1. Fetches recent data from the civitas HTTP API
/// 2. Feeds it to the on-device model
/// 3. Returns structured analysis
#[cfg(feature = "civitas-query")]
pub async fn query_civitas_with_ai(
    civitas_port: u16,
    query: &str,
    hours_back: u32,
    ...
    let ocr_url = format!(
        ...
        civitas_port, start_str, end_str
    );
    ...
    let instructions = "You are an AI assistant that analyzes a user's screen activity \
        and audio transcriptions captured by Civitas. \
...
/// Fetch screen frames from civitas, compute time per app, and
...
pub async fn analyze_app_usage_time(
    ...
    let instructions = "You analyze how a user spends time on their computer based on \
        application usage data captured by Civitas. \
...
mod tests {
    ...
    #[test]
    fn test_generate_text_civitas_context() {
        if check_availability() != Availability::Available {
            println!("Skipping: Foundation Models not available");
            return;
        }
...
"#;
        ...
        let result = generate_text(
            Some(
                "You analyze a developer's recent screen activity and meetings captured by Civitas. \
    ...
    fn test_benchmark_sequential_requests() {
        ...
        // 4. Long context (simulating civitas data)
    ...
    #[tokio::test]
    async fn test_real_civitas_query() {
        if check_availability() != Availability::Available {
            println!("Skipping: Foundation Models not available");
            return;
        }

        // Check if civitas is running
        let client = reqwest::Client::new();
        let health = client.get("http://localhost:3030/health").send().await;
        if health.is_err() || !health.unwrap().status().is_success() {
            println!("Skipping: civitas server not running on localhost:3030");
            return;
        }

        println!("=== Real Civitas + Foundation Models Integration ===\n");
        ...
        let r1 = generate_text(
            Some("You analyze screen activity and audio from Civitas. Give a concise summary of what the user has been doing. Max 5 bullet points."),
