# civitas-apple-intelligence

On-device AI processing for civitas using Apple's Foundation Models framework (macOS 26+).
...
Civitas records everything on your screen and audio. Processing this data with AI to extract action items, summaries, and insights currently requires sending data to cloud APIs. Foundation Models lets us do this **entirely on-device** — your data never leaves your machine.

## Architecture

```
┌─────────────────────────────────────────────┐
│  Rust (civitas-apple-intelligence)       │
...
```rust
use civitas_apple_intelligence::{check_availability, Availability};
...
```rust
use civitas_apple_intelligence::generate_text;
...
```rust
use civitas_apple_intelligence::generate_json;
...
### Query civitas data (feature: `civitas-query`)

```rust
// Fetches recent data from civitas HTTP API and processes with Foundation Models
let result = query_civitas_with_ai(3030, "What did I work on today?", 6).await?;
```
...
cargo test -p civitas-apple-intelligence -- --nocapture
```
...
```bash
cargo test -p civitas-apple-intelligence test_benchmark -- --nocapture
```
...
- [ ] Integration with civitas notification system (daily digest at 6pm)
