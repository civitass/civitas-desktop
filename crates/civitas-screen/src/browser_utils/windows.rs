        ...
        match root_ele.find_first(TreeScope::Subtree, &condition) {
            Ok(ele) => {
                ...
                if let Ok(address_bar) = ele.find_first(TreeScope::Subtree, &control_condition) {
                    debug!("browser address-bar candidate found");
                        ...
                        if let Ok(url_text) = value.get_string() {
                            if !url_text.is_empty() {
                                ...
                                if Url::parse(&full_url).is_ok() {
                                    debug!("browser URL passed local format validation");
                                    return Ok(Some(full_url));
                                } else {
                                    debug!("address-bar value was not a valid URL");
                                }
                            }
                        }
                    }
            ...
            Err(_) => {
                debug!(pid, "failed to find browser process window");
                return Ok(None);
            }
        }
        Ok(None)
