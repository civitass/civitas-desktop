            ...
            if frame_proxy.get_role().await? == Role::Frame {
                ...
                if let Ok(title) = frame_proxy.name().await {
                    if title == window_title {
                        debug!("found browser frame by title match");
                ...
                {
                    debug!("found browser frame by active state");
                    return Ok(Some(frame_proxy));
                }
            }
        }
        ...
        match Self::find_browser_process(conn, &root, pid).await {
            Ok((browser_proxy, _)) => {
                    ...
                    Self::find_active_frame(conn, &browser_proxy, window_title).await
                {
                    ...
                    {
                        Ok(Some(url)) => {
                            debug!("found active browser URL");
                            Ok(Some(url))
                        }
                        Ok(None) => {
                        ...
                        Err(_) => {
                            error!("failed to resolve active browser URL relation");
                            Ok(None)
                        }
                    }
                } else {
            ...
            Err(_) => {
                error!("failed to find browser process through accessibility");
                Ok(None)
            }
        }
    }
...
impl BrowserUrlDetector for LinuxUrlDetector {
    fn get_active_url(
        ...
        .join()
        .map_err(|_| anyhow!("browser URL detector thread panicked"))?
    }
}
