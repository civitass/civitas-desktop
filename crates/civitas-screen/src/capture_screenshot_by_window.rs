...
use civitas_core::window_pattern::{self, WindowPattern};
...
enum CaptureError {
    NoWindows,
    CaptureBackendError(CaptureBackend),
}
...
impl fmt::Display for CaptureError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            CaptureError::CaptureBackendError(backend) => write!(
                f,
                "window capture backend failed ({})",
                backend.diagnostic_code()
            ),
        }
    }
}
...
impl From<XCapError> for CaptureError {
    fn from(_error: XCapError) -> Self {
        debug!(
            backend = CaptureBackend::Xcap.diagnostic_code(),
            "window capture backend failed"
        );
        CaptureError::CaptureBackendError(CaptureBackend::Xcap)
    }
}

#[cfg(target_os = "macos")]
impl From<SckXCapError> for CaptureError {
    fn from(_error: SckXCapError) -> Self {
        debug!(
            backend = CaptureBackend::ScreenCaptureKit.diagnostic_code(),
            "window capture backend failed"
        );
        CaptureError::CaptureBackendError(CaptureBackend::ScreenCaptureKit)
    }
}

#[cfg(target_os = "macos")]
impl From<XcapXCapError> for CaptureError {
    fn from(_error: XcapXCapError) -> Self {
        debug!(
            backend = CaptureBackend::Xcap.diagnostic_code(),
            "window capture backend failed"
        );
...
static SKIP_APPS: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    HashSet::from([
        ...
        "TheBoringNotch",
        "Grammarly Desktop",
        // Civitas's own UI should never be captured
        "civitas",
        "civitas - Development",
        "civitas beta",
    ])
...
impl WindowFilters {
    ...
    /// `AppName::WindowTitle` scope (parsed in `civitas-core::window_pattern`);
    ...
    ///
    /// Delegates to [`civitas_a11y::url_filter::is_url_blocked`] so the
    /// vision path and the a11y tree walker agree on what "blocked" means.
    pub fn is_url_blocked(&self, url: &str) -> bool {
        civitas_a11y::url_filter::is_url_blocked(url, &self.ignored_urls)
    }
...
/// Skips overlay windows (layer > 0), system apps, and civitas's own UI.
#[cfg(target_os = "macos")]
pub fn find_topmost_pid_on_monitor(
    ...
    for w in cg_windows {
        ...
        // Skip civitas's own UI
        if w.owner_name.to_lowercase().contains("civitas") {
            continue;
        }
                    ...
                    if let Some(pid) = window.pid().ok() {
                        if let Some(exe_name) = get_process_exe_name(pid as u32) {
                            debug!(
                                ...
                                "window application metadata substituted"
                            );
                            exe_name
                        } else {
                            app_name
...
pub fn get_excluded_sck_window_ids(window_filters: &WindowFilters) -> Vec<u32> {
    ...
    let windows = match SckWindow::all() {
        ...
        Err(_error) => {
            debug!(
                backend = CaptureBackend::ScreenCaptureKit.diagnostic_code(),
                operation = "enumerate_filter_candidates",
    ...
    if !excluded.is_empty() {
        debug!(
            ...
            "resolved privacy-filter window exclusions"
        );
    }
...
pub async fn capture_all_visible_windows(
    ...
    for window_data in windows_data {
        ...
        let is_focused = if is_focused && overlay_pids.contains(&(process_id as u32)) {
            debug!(
                reason = "overlay_layer",
                "window focus classification demoted"
            );
            false
        } else {
            is_focused
        ...
        // Safety-net: always exclude civitas's own UI regardless of exact app name variant
        let is_civitas_ui = app_name.to_lowercase().contains("civitas");
        let is_valid = !is_civitas_ui
        ...
        if is_valid {
                    ...
                    .any(|&browser| app_name.to_lowercase().contains(browser))
            {
                ...
                match detector.get_active_url(&app_name, process_id, &window_name) {
                    ...
                    Err(_error) => {
                        debug!(
                            operation = "active_browser_url",
                            "browser metadata lookup failed"
                        );
                        None
                    }
                }
            ...
            if let Some(ref url) = browser_url {
                if window_filters.is_url_blocked(url) {
                    tracing::info!(reason = "blocked_url", "privacy filter skipped a window");
                    continue;
                }
            }
                ...
                && window_filters.is_title_suggesting_blocked_url(&window_name)
            {
                tracing::info!(
                    ...
                    "privacy filter skipped a window"
                );
                continue;
            }
...
mod tests {
    ...
    mod topmost_tests {
        ...
        #[test]
        fn test_topmost_skips_civitas_ui() {
            let windows = vec![
                make_window(999, 0, 0, 0, 1920, 1080, "civitas", "Main"),
                make_window(100, 0, 0, 0, 1920, 1080, "WezTerm", "π - brain"),
            ];
            assert_eq!(
                find_topmost_pid_on_monitor(&windows, &monitor_1080p()),
        ...
        #[test]
        fn test_topmost_skips_civitas_dev() {
            let windows = vec![
                make_window(999, 0, 0, 0, 1920, 1080, "civitas - Development", "Overlay"),
