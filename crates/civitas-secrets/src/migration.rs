...
/// Migrate legacy secret files from `~/.civitas` into the unified SecretStore.
///
/// - `*-oauth.json` files are migrated to `oauth:{name}` keys
/// - `connections.json` entries are migrated to `cred:{name}` keys
/// - A plaintext source is deleted only after encrypted read-back verification
...
pub async fn migrate_legacy_secrets(
    ...
    let fixed = fix_secret_file_permissions(civitas_dir);
    if fixed > 0 {
        info!("fixed permissions on {} secret files", fixed);
    }

    // Migrate *-oauth.json files
    if let Ok(entries) = std::fs::read_dir(civitas_dir) {
                ...
                match std::fs::read(&path) {
                    Ok(contents) => {
                        ...
                        if !should_import {
                            if let Err(e) = std::fs::remove_file(&path) {
                                report.errors.push(format!(
                                    "{}: already migrated but plaintext cleanup failed: {}",
                                    filename, e
                                ));
                            } else {
                                info!("removed migrated legacy secret {}", filename);
                            }
                            continue;
                        }
                        ...
                        if let Err(e) = store.set(&store_key, &contents).await {
                            ...
                            if !verified {
                                report.errors.push(format!(
                                    "{}: encrypted read-back verification failed",
                                    filename
                                ));
                                continue;
                            }
                            if let Err(e) = std::fs::remove_file(&path) {
                                report.errors.push(format!(
                                    "{}: migrated but plaintext cleanup failed: {}",
                                    filename, e
                                ));
                                continue;
                            }
                            ...
                            info!(
                                "migrated {} -> {} and removed plaintext",
                                filename, store_key
                            );
                        }
                    }
    ...
    let connections_path = civitas_dir.join("connections.json");
                ...
                match serde_json::from_str::<serde_json::Value>(&contents) {
                    Ok(json) => {
                        if let Some(obj) = json.as_object() {
                            let mut connection_migration_failed = false;
                            for (name, value) in obj {
                                ...
                                if should_import && !value_bytes.is_empty() {
                                    if let Err(e) = store.set(&cred_key, &value_bytes).await {
                                        connection_migration_failed = true;
                                        report
                                            .errors
                                        ...
                                        let verified = store
                                            ...
                                            .ok()
                                            .flatten()
                                            .is_some_and(|stored| stored == value_bytes);
                                        if !verified {
                                            connection_migration_failed = true;
                                            report.errors.push(format!(
                                                "connections.json/{}: encrypted read-back verification failed",
                                                name
                                            ));
                                            continue;
                                        }
                            ...
                            if !connection_migration_failed {
                                if let Err(e) = std::fs::remove_file(&connections_path) {
                                    report.errors.push(format!(
                                        "connections.json: migrated but plaintext cleanup failed: {}",
                                        e
                                    ));
                                } else {
                                    info!("removed migrated plaintext connections.json");
                                }
                            }
                        }
                    }
...
/// Walk `civitas_dir` and set 0o600 on any file matching secret-like patterns.
/// Returns the count of files whose permissions were fixed.
pub fn fix_secret_file_permissions(civitas_dir: &Path) -> usize {
    let mut count = 0;
    ...
    let whatsapp_dir = civitas_dir.join("whatsapp-session");
    let whatsapp_marker = civitas_dir.join(".whatsapp-perms-fixed");
...
mod tests {
    ...
    async fn make_store() -> SecretStore {
        ...
        SecretStore::new(pool, [42u8; 32]).await.unwrap()
    }
    ...
    async fn test_migrate_connections_json() {
        ...
        assert!(!dir_path.join("connections.json").exists());
    }
    ...
    async fn test_migrate_skips_already_migrated() {
        ...
        // First migration moves the value and removes the plaintext source.
        ...
        // Second migration has no plaintext work to do.
