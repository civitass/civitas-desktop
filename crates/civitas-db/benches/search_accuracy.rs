...
// Run: cargo bench --bench search_accuracy -p civitas-db
//
...
const DB_PATH: &str = "~/.civitas/db.sqlite";
...
fn bench_search_accuracy(c: &mut Criterion) {
    ...
    let pool = rt.block_on(async {
        SqlitePoolOptions::new()
            ...
            .await
            .expect("Failed to connect to database. Make sure civitas has been run.")
    });
