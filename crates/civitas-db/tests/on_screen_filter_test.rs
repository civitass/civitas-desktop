...
mod tests {
    use chrono::Utc;
    use civitas_db::{DatabaseManager, ElementSource};
    ...
    /// `civitas_a11y::tree::AccessibilityTreeNode` so the db's private
    ...
    #[tokio::test]
    async fn search_elements_finds_cjk_text_with_spurious_spaces() {
        let db = setup_test_db().await;
        insert_frame_with_element(&db, "Feishu", "项 目 进 度 已更新", Some(true)).await;
        settle().await;
        ...
        assert_eq!(total, 1);
        assert!(
            texts.contains(&"项 目 进 度 已更新"),
            "CJK element text should be searchable through elements_fts shadow tokens, got {:?}",
            texts
        );
    }
