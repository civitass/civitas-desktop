...
///
/// Run with: cargo test --package civitas-db --test untranscribed_chunks_test -- --nocapture
#[cfg(test)]
mod tests {
    use chrono::{Duration, Utc};
    use civitas_db::{AudioDevice, ContentType, DatabaseManager, DeviceType, SearchResult};
    ...
    async fn test_meeting_transcript_query_merges_live_and_background_paths() {
        ...
        assert_eq!(rows[1].speaker_name.as_deref(), Some("Casey"));
