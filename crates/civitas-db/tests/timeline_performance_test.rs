...
mod timeline_performance_tests {
    ...
    async fn insert_frames_with_ocr(
        ...
        for i in 0..count {
            ...
            let ocr_text = format!(
                "Frame {} - Application: VSCode - Window: main.rs - civitas - \
                 ...
                 Finder - Documents/projects/civitas - 42 items, 1.2 GB available \
                 ...
                 Terminal output: Compiling civitas v0.2.76 warning: unused variable \
                 Browser tabs: GitHub - civitas/civitas - Stack Overflow - rust async \
    ...
    async fn setup_file_db(path: &str) -> DatabaseManager {
        ...
        // DatabaseManager::new expects a bare filesystem path (it prepends the
        // `sqlite:` scheme itself). Passing a `sqlite:`-prefixed URL here made
        // ensure_db_parent_dir treat the URL as a relative path, creating a
        // literal `sqlite:/tmp/` directory (plus the startup integrity-check
        // marker) inside the crate directory on every test run.
    ...
    async fn test_file_based_db_performance() {
        let db_path = "/tmp/civitas_test_perf.db";
    ...
    /// Run with: cargo test -p civitas-db --test timeline_performance_test test_stress -- --nocapture --ignored
    #[tokio::test]
