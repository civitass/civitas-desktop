...
///
/// Replays all embeddings from `~/.civitas/db.sqlite` through different
/// clustering strategies and scores each one. No production code is touched.
///
/// Run with: cargo test -p civitas-db --test speaker_benchmark -- --nocapture --ignored
///
/// Ground truth: locally named speakers + temporal consistency
