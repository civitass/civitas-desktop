...
async fn low_tier_db_initializes_successfully() {
    let config = DbConfig::for_tier(civitas_config::DeviceTier::Low);
...
async fn mid_tier_db_initializes_successfully() {
    let config = DbConfig::for_tier(civitas_config::DeviceTier::Mid);
    assert_eq!(config.mmap_size, 0); // mmap disabled to prevent DB corruption
...
async fn write_pool_size_increased_for_all_tiers() {
    // Regression test for issue CIVITAS-CLI-FZ: connection pool timeout
    ...
    let mid_config = DbConfig::for_tier(civitas_config::DeviceTier::Mid);
    assert!(
    ...
    let low_config = DbConfig::for_tier(civitas_config::DeviceTier::Low);
    assert!(
