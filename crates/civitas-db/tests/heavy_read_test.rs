...
//!   cargo test -p civitas-db --test heavy_read_test -- --nocapture

use chrono::{Duration as ChronoDuration, Utc};
use civitas_db::{ContentType, DatabaseManager};
use std::sync::Arc;
...
async fn open_real_db() -> Arc<DatabaseManager> {
    let db_path = civitas_core::paths::default_civitas_data_dir().join("db.sqlite");
...
/// still go through.
///
/// Requires the real production DB at `~/.civitas/db.sqlite` — ignored
/// in CI where that doesn't exist. Run locally with:
///   cargo test -p civitas-db --test heavy_read_test -- --ignored --nocapture
#[tokio::test]
#[ignore = "requires real ~/.civitas/db.sqlite"]
