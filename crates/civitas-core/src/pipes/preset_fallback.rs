...
//!
//! State is persisted to `~/.civitas/ai_preset_fallback.json` so it survives
//! restarts.
...
impl PresetFallbackRegistry {
    ...
    pub fn new(civitas_dir: &Path) -> Self {
        let persist_path = civitas_dir.join("ai_preset_fallback.json");
