...
//! External memory sync — write a civitas-owned digest of memories
...
//! file, it produces a markdown digest, wraps it in a civitas-owned
//! marker block, and writes it atomically.
//!
//! The scheduler that decides *when* to call us lives in
//! `civitas-engine`, next to the DB it queries. Splitting the two
...
//! out a region civitas fully owns; everything outside is preserved
...
/// inside the block changes incompatibly so older civitas builds can
/// detect a newer block and refuse to rewrite it.
pub const RENDER_SCHEMA: u32 = 1;

/// Start sentinel for the civitas-owned region. Both halves include
/// the schema version so a future format change is visible to anyone
/// reading the file.
pub fn marker_start() -> String {
    format!("<!-- civitas-memories:start v{} -->", RENDER_SCHEMA)
}

pub fn marker_end() -> String {
    "<!-- civitas-memories:end -->".to_string()
}
...
/// `civitas-core` from needing to depend on `civitas-db`.
...
pub struct Destination {
    ...
    /// keeps the outer file small and hand-editable while civitas owns
...
impl Destination {
    pub const CLAUDE_CODE: Destination = Destination {
        ...
        filename: "CLAUDE.md",
        sidecar_filename: Some("civitas-memories.md"),
    };
...
pub fn render_block_body(entries: &[MemoryEntry], dest: &Destination) -> String {
    if let Some(sidecar) = dest.sidecar_filename {
        ...
        return format!(
            "## civitas memories\n\n\
            civitas writes durable memory facts to the sidecar below. \
            Manage them in the civitas app — the file is rewritten \
            on every sync.\n\n\
            @{}\n",
            sidecar
        );
...
pub fn render_digest(entries: &[MemoryEntry], dest: &Destination) -> String {
    ...
    out.push_str(&format!(
        "## civitas memories\n\n\
        Auto-synced by civitas from this user's local memory store. \
    ...
    if sorted.is_empty() {
        out.push_str("_no memories yet — civitas will populate this on the next sync._\n");
        return out;
    }
...
/// Remove every Civitas-owned marker block while preserving all surrounding
/// user content byte-for-byte. An unterminated block is treated as owning the
/// remainder of the file so a partial write cannot strand memory content.
pub fn strip_managed_blocks(existing: &str) -> (String, bool) {
    let start = marker_start();
    let end = marker_end();
    ...
    while let Some(start_index) = remaining.find(&start) {
        ...
        if let Some(relative_end) = remaining[after_start..].find(&end) {
            let after_end = after_start + relative_end + end.len();
...
/// contents. Used for civitas-owned sidecar files where there's no
...
/// Remove Civitas-managed blocks from an assistant instruction file without
/// deleting or reformatting any user-authored content around them.
pub fn remove_managed_blocks_atomic(target_path: &Path) -> std::io::Result<bool> {
    let existing = match std::fs::read_to_string(target_path) {
        Ok(existing) => existing,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(false),
        Err(error) => return Err(error),
    };
    let (next, changed) = strip_managed_blocks(&existing);
    if !changed {
        return Ok(false);
    }
    write_file_if_changed(target_path, &next)
...
/// Delete a Civitas-owned sidecar. Missing files are already clean.
pub fn remove_owned_file(target_path: &Path) -> std::io::Result<bool> {
...
mod tests {
    ...
    fn block_body_for_claude_is_import_reference_not_full_digest() {
        ...
        assert!(
            body.contains("@civitas-memories.md"),
    ...
    fn sidecar_path_set_for_claude_unset_for_codex() {
        ...
        assert_eq!(
            ...
            Some(home.join("civitas-memories.md"))
        );
    ...
    #[test]
    fn strip_managed_blocks_preserves_user_content_and_removes_duplicates() {
        let block = |body: &str| format!("{}\n{}\n{}", marker_start(), body, marker_end());
        let existing = format!(
            "# user preface\n\n{}\n\nuser middle\n\n{}\n\nuser ending\n",
            block("private memory one"),
            block("private memory two")
        );

        let (next, changed) = strip_managed_blocks(&existing);

        assert!(changed);
        assert!(next.contains("# user preface"));
        assert!(next.contains("user middle"));
        assert!(next.contains("user ending"));
        assert!(!next.contains("private memory"));
        assert!(!next.contains(&marker_start()));
        assert!(!next.contains(&marker_end()));
    }

    #[test]
    fn strip_managed_blocks_removes_unterminated_tail() {
        let existing = format!("# user preface\n\n{}\nprivate memory", marker_start());
        let (next, changed) = strip_managed_blocks(&existing);
        assert!(changed);
        assert_eq!(next, "# user preface\n\n");
    }

    #[test]
    fn strip_managed_blocks_is_noop_without_marker() {
        let existing = "# user file\n";
        assert_eq!(
            strip_managed_blocks(existing),
    ...
    fn write_atomic_full_creates_new_file() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("civitas-memories.md");
    ...
    fn write_atomic_full_overwrites_entire_file() {
        // Sidecar files are civitas-owned end-to-end — no splice.
    ...
    fn write_atomic_leaves_no_temp_sibling_after_success() {
        ...
        assert!(
            entries.iter().all(|n| !n.contains(".civitas-tmp")),
    ...
    #[test]
    fn remove_managed_blocks_atomic_cleans_only_owned_content() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("AGENTS.md");
        std::fs::write(&target, "# user rules\n").unwrap();
        write_atomic(&target, "private memory").unwrap();

        assert!(remove_managed_blocks_atomic(&target).unwrap());
        let after = std::fs::read_to_string(&target).unwrap();
        assert!(after.contains("# user rules"));
        assert!(!after.contains("private memory"));
        assert!(!after.contains(&marker_start()));
        assert!(!remove_managed_blocks_atomic(&target).unwrap());
    }

    #[test]
    fn remove_owned_file_is_idempotent() {
        let dir = tempfile::tempdir().unwrap();
        let sidecar = dir.path().join("civitas-memories.md");
        std::fs::write(&sidecar, "private memory").unwrap();
        assert!(remove_owned_file(&sidecar).unwrap());
