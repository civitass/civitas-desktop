...
/// Target bitrate for Apple's HEVC VideoToolbox encoder. Hardware encoding
/// keeps capture responsive and lets the macOS release ship an LGPL-only
/// FFmpeg build without libx265.
#[cfg(any(target_os = "macos", test))]
pub fn video_quality_to_videotoolbox_bitrate(quality: &str) -> &'static str {
    match quality {
        "low" => "5M",
        "high" => "20M",
        "max" => "40M",
        _ => "10M", // "balanced" or any unknown
    }
}
...
pub async fn start_ffmpeg_process(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        info!(
            "FFmpeg VideoToolbox encoding: quality={}, bitrate={}",
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        args.extend_from_slice(&[
            ...
            "-crf",
            crf,
            // Disable B-frames: libx265 default buffering shifts PTS and makes
            // the timeline seek to the wrong screenshot.
            "-x265-params",
            "bframes=0",
        ]);
    }
...
mod tests {
    ...
    #[test]
    fn videotoolbox_bitrates_are_monotonic_and_have_a_safe_default() {
        assert_eq!(video_quality_to_videotoolbox_bitrate("low"), "5M");
        assert_eq!(video_quality_to_videotoolbox_bitrate("balanced"), "10M");
        assert_eq!(video_quality_to_videotoolbox_bitrate("high"), "20M");
        assert_eq!(video_quality_to_videotoolbox_bitrate("max"), "40M");
        assert_eq!(video_quality_to_videotoolbox_bitrate("unknown"), "10M");
    }
}
