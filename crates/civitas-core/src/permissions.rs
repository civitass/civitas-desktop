...
pub fn preflight_check(need_screen: bool, need_audio: bool) -> bool {
    ...
    if !ok {
        eprintln!();
        eprintln!("run \"civitas\" again after granting permissions.");
...
/// Tauri app: on macOS 15+, skips `capture_probe()` to avoid a false positive —
/// Apple changed `CGWindowListCreateImage` to return the app's own windows even
/// without Screen Recording permission, so a non-NULL result no longer implies
/// access. This applies to debug builds too: macOS grants development, beta,
/// and production bundle identities independently, so assuming a developer
/// already has permission recreates the exact stale-status bug this function
/// exists to prevent.
#[cfg(target_os = "macos")]
