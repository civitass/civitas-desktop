...
lazy_static! {
    static ref PII_PATTERNS: Vec<(Regex, &'static str)> = vec![
        ...
        // Database connection strings with user information before the host.
        (Regex::new(concat!(
            r"(?i)(?:postgres|postgresql|mysql|mariadb|mongodb|mongodb\+srv|redis|rediss|amqp|amqps)",
            r"://",
            r"[^:]+:[^@]+@[^\s]+"
        )).unwrap(), "CONNECTION_STRING"),
        ...
        // shows the full secret in plaintext — common civitas failure
...
mod tests {
    ...
    fn credential_url(scheme: &str, username: &str, password: &str, host_and_path: &str) -> String {
        format!("{scheme}{}{}:{password}@{host_and_path}", "://", username)
    }

    fn provider_token(provider: &str, environment: &str, body: &str) -> String {
        format!("{provider}_{environment}_{body}")
    }

    fn slack_token(kind: &str, body: &str) -> String {
        format!("{kind}-{body}")
    }
    ...
    fn test_password_context_master_password() {
        ...
        assert_eq!(
            remove_pii(concat!("Master Password: ", "bitwarden123")),
            "Master Password: [PASSWORD]"
        );
    }
    ...
    fn test_stripe_key_detection() {
        ...
        let webhook_secret = concat!("whsec_", "BnlsS222bkRvslineRtsak70Nxo10467");
        assert!(contains_pii(webhook_secret));
        assert_eq!(get_pii_type(webhook_secret), Some("STRIPE_KEY".to_string()));

        // Stripe secret keys
        assert!(contains_pii(&provider_token(
            "sk",
            "live",
            "51ABC123def456GHI789"
        )));
        assert!(contains_pii(&provider_token(
            "sk",
            "test",
        ...
        let result = remove_pii(&format!("STRIPE_WEBHOOK_SECRET={webhook_secret}"));
    ...
    fn test_supabase_key_detection() {
        ...
        let result = remove_pii(concat!("Key: sb_publishable_", "ltaNA123456789"));
    ...
    fn test_env_variable_secrets() {
        ...
        let stripe_secret = provider_token("sk", "live", "abc123def456");
        assert!(contains_pii(&format!("STRIPE_SECRET_KEY={stripe_secret}")));
        ...
        let stripe_secret = provider_token("sk", "live", "abc123def456ghi");
        let result = remove_pii(&format!("export STRIPE_SECRET_KEY={stripe_secret}"));
        assert!(
    ...
    fn test_openai_key_detection() {
        ...
        let result = remove_pii(concat!("OPENAI_API_KEY=sk-proj-", "abc123def456ghi789jkl"));
        assert!(
    ...
    fn test_private_key_detection() {
        assert!(contains_pii(concat!("-----BEGIN ", "RSA PRIVATE KEY-----")));
        assert!(contains_pii(concat!(
            "-----BEGIN ",
            "OPENSSH PRIVATE KEY-----"
        )));
        assert!(contains_pii(concat!("-----BEGIN ", "PGP PRIVATE KEY-----")));
        assert!(contains_pii(concat!("-----BEGIN ", "EC PRIVATE KEY-----")));
        assert!(contains_pii(concat!("-----BEGIN ", "DSA PRIVATE KEY-----")));

        let result = remove_pii(concat!("Key: -----BEGIN ", "RSA PRIVATE KEY-----"));
    ...
    fn test_connection_string_detection() {
        // PostgreSQL
        assert!(contains_pii(&credential_url(
            "postgres",
            "user",
            "password123",
            "localhost:5432/mydb"
        ...
        assert!(contains_pii(&credential_url(
            ...
            "",
            "secret",
            "secure-redis.com:6380"
        )));

        let result = remove_pii(&format!(
            "DATABASE_URL={}",
            credential_url("postgres", "user", "secret", "host/db")
        ));
        assert!(
    ...
    fn test_url_with_credentials() {
        ...
        let result = remove_pii(&format!(
            "API: {}",
            credential_url("https", "apikey", "secret", "service.com/v1")
        ));
        assert!(!result.contains("secret"));
    }
    ...
    fn test_slack_token_detection() {
        assert!(contains_pii(&slack_token(
            "xoxb",
        ...
        assert!(contains_pii(concat!(
            "xapp-",
            "1-A12345678-1234567890123-abcdef"
        )));

        let result = remove_pii(&format!(
            "SLACK_TOKEN={}",
            slack_token("xoxb", "123456789012-1234567890123-abc")
        ));
    ...
    fn test_discord_token_detection() {
        ...
        let _discord_token = concat!(
            "MjM4NDk0NzU2NTIxMzc3Nzky.",
            "CunGFQ.wUILz7z6HoJzVeq6pyHPmVgQgV4"
        );
        // Note: This is a fake token format
        assert!(contains_pii(concat!(
            "NTk0NzU2NTIxMzc3Nzk2NDEy.",
            "XQ0bXQ.vHc7MzY3NDk0NzU2NTIxMzc3Nzk"
        )));
    }

    #[test]
    fn test_gitlab_token_detection() {
        assert!(contains_pii(concat!("glpat-", "xxxxxxxxxxxxxxxxxxxx")));
        assert!(contains_pii(concat!("glcbt-", "xxxxxxxxxxxxxxxxxxxx")));

        let result = remove_pii(concat!("GITLAB_TOKEN=glpat-", "abc123def456ghi789jkl012"));
    ...
    fn test_performance_with_all_new_patterns() {
        ...
        let input = format!(
            ...
            credential_url("postgres", "user", "pass", "host/db"),
            slack_token("xoxb", "token-here-1234567890")
        );
