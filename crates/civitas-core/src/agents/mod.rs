...
pub trait AgentExecutor: Send + Sync {
    ...
    /// `provider` overrides the user's selected provider (for example
    /// `"anthropic"`, `"openai"`, `"openrouter"`, `"bedrock"`, or local).
    /// If `None`, the local gateway resolves the user's default profile.
    ...
    async fn run(
        ...
        // Scoped token exposed to the agent's tools. Automated workflows
        // receive a `sp_pipe_*` token instead of the unrestricted device key.
    ...
    async fn run_streaming(
        ...
        // Chat/session that owns this run (e.g. `pipe:<name>`). The Pi executor
        // exports it as `CIVITAS_SESSION_ID` so local API calls can be
        // correlated without sending the label to a remote host.
        _session_owner: Option<&str>,
        // Scoped token for tool-originated local API calls.
