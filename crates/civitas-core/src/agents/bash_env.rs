...
//! then reads the skill file, learns about `$CIVITAS_LOCAL_API_KEY`, and
...
//! civitas API (localhost:3030). Non-civitas calls pass through in interactive
//! chat without the token and are rejected in automated workflows. Mixed-host
//! curl commands are rejected so a global `-H` can never leak the token to a
//! second transfer.
//!
...
//!
//! Reads `$CIVITAS_LOCAL_API_KEY` only — every spawn path (Tauri chat,
//! core pipe-executor) is now contractually required to export it.
//! `CIVITAS_API_AUTH_KEY` was a historical second name from when the two
...
///
/// Only matches literal HTTP URLs for `localhost:3030`, `127.0.0.1:3030`, or
/// `[::1]:3030`. If none match, interactive curl runs unchanged. A command
/// containing both a Civitas URL and another URL is rejected.
pub const WRAPPER_SCRIPT: &str = r#"# civitas — auto-injected by pi-agent bash subshells (do not edit by hand)
# Transparently adds Authorization: Bearer to curl calls that target the
# local civitas API, tags them with x-civitas-session (the chat/pipe
# that owns this agent, from CIVITAS_SESSION_ID) for local correlation,
# and — when
# CIVITAS_FILTER_PII=1 — rewrites any /search URL to include filter_pii=1
# so responses are PII-redacted before Pi sees them. Interactive chat may use
# external curl without leaking the token; automated workflows set
# CIVITAS_PIPE_SANDBOX=1 and external curl is rejected.
#
# Regenerated on every pi-agent spawn from civitas-core::agents::bash_env.

# Hide inference credentials from the agent's bash. Pi reads the dedicated
# inference key in its parent process and does NOT need it in tool shells.
# Leaving the historical cloud key exposed bit a real user (justinspillers,
# 2026-05-05): the agent saw an env var named "CIVITAS_API_KEY", used
...
# the agent can see is CIVITAS_LOCAL_API_KEY, which is correct.
unset CIVITAS_API_KEY
unset CIVITAS_INFERENCE_API_KEY

_sp_auth_key() {
  # spawn paths guarantee CIVITAS_LOCAL_API_KEY is set (see pi.rs).
  printf '%s' "${CIVITAS_LOCAL_API_KEY:-}"
}

curl() {
  local key sid has_local=0 has_other_url=0 add_filter=0 arg
  local -a out=() hdrs=()
  key="$(_sp_auth_key)"
  # Chat/session this agent runs under. The label is attached only to literal
  # Civitas loopback calls and never to an external request.
  sid="${CIVITAS_SESSION_ID:-}"
  if [ "${CIVITAS_FILTER_PII:-}" = "1" ]; then
    add_filter=1
  fi

  for arg in "$@"; do
    case "$arg" in
      http://localhost:3030|http://localhost:3030/*|http://localhost:3030\?*|http://localhost:3030\#*|\
      http://127.0.0.1:3030|http://127.0.0.1:3030/*|http://127.0.0.1:3030\?*|http://127.0.0.1:3030\#*|\
      http://\[::1\]:3030|http://\[::1\]:3030/*|http://\[::1\]:3030\?*|http://\[::1\]:3030\#*)
        has_local=1
  ...
  if [ "$has_local" = "1" ] && [ "$has_other_url" = "1" ]; then
    printf '%s\n' "civitas curl wrapper: mixed local and non-Civitas URLs are blocked" >&2
    return 64
  fi

  if [ "$has_local" = "1" ]; then
    [ -n "$key" ] && hdrs+=(-H "Authorization: Bearer $key")
    [ -n "$sid" ] && hdrs+=(-H "x-civitas-session: $sid")
    command curl "${hdrs[@]}" "${out[@]}"
  elif [ "${CIVITAS_PIPE_SANDBOX:-}" = "1" ]; then
    printf '%s\n' "civitas workflow sandbox: external curl is blocked; use an explicitly declared local connection proxy" >&2
    return 64
  else
    command curl "${out[@]}"
  fi
...
/// Path to the bash wrapper under `~/.civitas/<WRAPPER_RELATIVE_PATH>`,
/// relative to the supplied civitas data dir.
pub fn wrapper_path_under(civitas_dir: &Path) -> PathBuf {
    civitas_dir.join(WRAPPER_RELATIVE_PATH)
}

/// Write the wrapper into the given civitas data dir if the on-disk
...
/// ```no_run
/// # use std::path::Path;
/// let sp_dir = Path::new("/Users/me/.civitas");
/// let p = civitas_core::agents::bash_env::ensure_wrapper(sp_dir).unwrap();
/// // cmd.env("BASH_ENV", p);
/// ```
pub fn ensure_wrapper(civitas_dir: &Path) -> Result<PathBuf> {
    let path = wrapper_path_under(civitas_dir);
...
/// Resolve `~/.civitas` using the same logic the rest of the app uses,
...
mod tests {
    ...
    fn wrapper_script_reads_canonical_env_var_name() {
        assert!(WRAPPER_SCRIPT.contains("CIVITAS_LOCAL_API_KEY"));
        ...
        assert!(
            !WRAPPER_SCRIPT.contains("CIVITAS_API_AUTH_KEY"),
    ...
    fn wrapper_script_unsets_cloud_token_env_var() {
        // The agent must not see CIVITAS_API_KEY (cloud JWT) — it's
        // distinct from CIVITAS_LOCAL_API_KEY (local sp-<uuid>) and
        ...
        assert!(
            WRAPPER_SCRIPT.contains("unset CIVITAS_API_KEY"),
            "wrapper must unset CIVITAS_API_KEY in pi-agent subshells"
        );
        assert!(
            WRAPPER_SCRIPT.contains("unset CIVITAS_INFERENCE_API_KEY"),
            "tool shells must not inherit the unrestricted inference credential"
        );
    }
    ...
    fn wrapper_script_injects_only_for_localhost_3030() {
        ...
        for needle in ["localhost:3030", "127.0.0.1:3030", r"\[::1\]:3030"] {
            assert!(
    ...
    fn wrapper_script_references_filter_pii_env() {
        assert!(
            WRAPPER_SCRIPT.contains("CIVITAS_FILTER_PII"),
    ...
    fn wrapper_script_tags_session_owner() {
        assert!(
            WRAPPER_SCRIPT.contains("CIVITAS_SESSION_ID"),
            "wrapper must read the session id env var"
        );
        assert!(
            WRAPPER_SCRIPT.contains("x-civitas-session"),
            "wrapper must send the owner header so navigations route to the right chat"
        );
    }

    /// End-to-end: the shim adds `x-civitas-session: <id>` to local API
    /// calls when `CIVITAS_SESSION_ID` is set, and never leaks it to
    ...
    fn shim_tags_session_header_for_local_only() {
        ...
        let status = Command::new("bash")
            ...
            .env("CIVITAS_LOCAL_API_KEY", "sp-test")
            .env("CIVITAS_SESSION_ID", "conv-abc-123")
            .arg("-c")
        ...
        assert!(
            local.contains("x-civitas-session: conv-abc-123"),
        ...
        assert!(
            !ext.contains("x-civitas-session"),
            "owner header must not leak to third-party hosts; got: {ext}"
        );

        // A single curl invocation with multiple hosts is rejected even in
        // interactive mode: curl applies global headers to every transfer.
        let status = Command::new("bash")
            .env("PATH", format!("{}:/usr/bin:/bin", fake_curl_dir.display()))
            .env("BASH_ENV", &wrapper)
            .env("CURL_ARGV_FILE", tmp.path().join("mixed.argv"))
            .env("CIVITAS_LOCAL_API_KEY", "sp-test")
            .arg("-c")
            .arg("curl http://localhost:3030/health https://example.com/collect")
            .status()
            .unwrap();
    ...
    #[test]
    #[cfg(unix)]
    fn workflow_shim_blocks_external_curl() {
        use std::os::unix::fs::PermissionsExt;
        use std::process::Command;
        ...
        std::fs::create_dir_all(&fake_curl_dir).unwrap();
        let fake_curl = fake_curl_dir.join("curl");
        std::fs::write(&fake_curl, "#!/usr/bin/env bash\nexit 0\n").unwrap();
        let mut perms = std::fs::metadata(&fake_curl).unwrap().permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(&fake_curl, perms).unwrap();

        for command in [
            "curl https://example.com/private-upload",
            "curl http://localhost:11435/notify",
            "curl http://127.0.0.1:6379/",
        ] {
            let status = Command::new("bash")
                .env("PATH", format!("{}:/usr/bin:/bin", fake_curl_dir.display()))
                .env("BASH_ENV", &wrapper)
                .env("CIVITAS_PIPE_SANDBOX", "1")
                .arg("-c")
                .arg(command)
                .status()
                .unwrap();
            assert_eq!(status.code(), Some(64), "{command}");
        }
    }
