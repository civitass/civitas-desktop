...
//! The core recording settings type shared across all civitas components.

use serde::{Deserialize, Serialize};

/// Consent contract for the optional Smart PII model downloads.
///
/// Increment this when the model, upstream license, or material download
/// behavior changes. Existing consent then becomes stale and the model workers
/// remain disabled until the user reviews the new terms.
pub const SMART_PII_MODEL_LICENSE_VERSION: u32 = 1;
...
/// - **CLI**: built from command-line args or loaded from `~/.civitas/config.toml`
...
pub struct RecordingSettings {
    ...
    /// In-memory Deepgram credential loaded from the OS-vault-backed secret
    /// store. Legacy values can still deserialize for one-way migration, but
    /// this field is never serialized back into settings.
    #[serde(rename = "deepgramApiKey", default, skip_serializing)]
    ...
    #[serde(rename = "smartPiiModelLicenseAcceptedVersion", default)]
    pub smart_pii_model_license_accepted_version: u32,
    ...
    // ── Local identity and direct provider configuration ───────────────
    ...
    /// In-memory OpenAI-compatible transcription credential. Legacy values
    /// deserialize for migration and are never serialized back into settings.
    ...
    /// In-memory custom headers loaded from the encrypted credential store.
    /// Header values may contain secrets, so they are never serialized.
    ...
    /// In-memory local API key. Legacy values deserialize for one-way
    /// migration; the persisted value lives in the encrypted secret store.
    #[serde(rename = "apiKey", default, skip_serializing)]
    pub api_key: String,
}

impl RecordingSettings {
    /// Resolve source-level audio consent while retaining the former combined
    /// switch as a migration and global-pause boundary.
    pub fn effective_audio_capture_sources(&self) -> (bool, bool) {
        let (capture_microphone, capture_system_audio) =
            match (self.capture_microphone, self.capture_system_audio) {
                // The old Audio Recording control explicitly disclosed both
                // microphone and system capture, so preserving that exact
                // choice is safe for stores written before the split.
                (None, None) => (!self.disable_audio, !self.disable_audio),
                (microphone, system_audio) => {
                    (microphone.unwrap_or(false), system_audio.unwrap_or(false))
                }
            };
        ...
        if self.disable_audio {
            ...
            (capture_microphone, capture_system_audio)
        }
    }

    /// True when the global pause is active or neither source is allowed.
    pub fn effective_disable_audio(&self) -> bool {
        let (capture_microphone, capture_system_audio) = self.effective_audio_capture_sources();
        !capture_microphone && !capture_system_audio
    }
    ...
    /// Returns the explicit OCR language preference, falling back to the
    /// historic shared speech-language field for pre-migration settings.
    pub fn effective_ocr_languages(&self) -> &[String] {
        self.ocr_languages.as_deref().unwrap_or(&self.languages)
    }
}
...
impl Default for RecordingSettings {
    fn default() -> Self {
        Self {
            // Consumer-safe baseline: capture classes require an explicit user
            // choice before either source starts.
            disable_audio: true,
            ...
            analytics_enabled: false, // opt-in: disabled by default
...
mod tests {
    ...
    fn deserializes_from_empty_json() {
        ...
        assert_eq!(settings.ocr_languages, None);
        assert!(settings.effective_ocr_languages().is_empty());
    }

    #[test]
    fn audio_source_consent_supports_all_four_combinations() {
        for (microphone, system_audio, expected) in [
            ...
            let settings: RecordingSettings = serde_json::from_value(serde_json::json!({
                ...
                "captureSystemAudio": system_audio
            }))
            .unwrap();
            assert_eq!(settings.effective_audio_capture_sources(), expected);
            assert_eq!(
                settings.effective_disable_audio(),
    ...
    #[test]
    fn legacy_combined_audio_consent_migrates_without_partial_source_leakage() {
        let legacy: RecordingSettings = serde_json::from_str(r#"{"disableAudio":false}"#).unwrap();
        assert_eq!(legacy.capture_microphone, None);
        assert_eq!(legacy.capture_system_audio, None);
        assert_eq!(legacy.effective_audio_capture_sources(), (true, true));

        let microphone_only: RecordingSettings =
            serde_json::from_str(r#"{"disableAudio":false,"captureMicrophone":true}"#).unwrap();
        assert_eq!(
            microphone_only.effective_audio_capture_sources(),
            (true, false)
        );

        let globally_paused: RecordingSettings = serde_json::from_str(
            r#"{"disableAudio":true,"captureMicrophone":true,"captureSystemAudio":true}"#,
        )
        .unwrap();
        assert_eq!(
            globally_paused.effective_audio_capture_sources(),
            (false, false)
        );
    }

    #[test]
    fn legacy_language_setting_is_used_for_ocr_until_explicitly_migrated() {
        let legacy: RecordingSettings =
            serde_json::from_str(r#"{"languages":["chinese-simplified"]}"#).unwrap();
        assert_eq!(legacy.ocr_languages, None);
        assert_eq!(
            legacy.effective_ocr_languages(),
            &["chinese-simplified".to_string()]
        );

        let independent: RecordingSettings = serde_json::from_str(
            r#"{"languages":["english"],"ocrLanguages":["chinese-traditional"]}"#,
        )
        .unwrap();
        assert_eq!(
            independent.effective_ocr_languages(),
            &["chinese-traditional".to_string()]
        );

        let automatic: RecordingSettings =
            serde_json::from_str(r#"{"languages":["english"],"ocrLanguages":[]}"#).unwrap();
        assert!(automatic.effective_ocr_languages().is_empty());
    }
    ...
    fn deserializes_legacy_extra_fields() {
        ...
        assert_eq!(settings.vocabulary[0].word, "civitas");
    ...
    fn deserializes_real_store_bin_shape() {
        ...
        let json = r#"{
            ...
            "aiPresets": [{"name": "test"}],
            "showCivitasShortcut": "Super+Alt+S",
    ...
    fn flatten_simulation() {
        ...
        struct FakeSettingsStore {
            ...
            overlay_mode: String,
            #[serde(rename = "showCivitasShortcut", default)]
            show_civitas_shortcut: String,
        ...
        assert_eq!(store.show_civitas_shortcut, "Super+Alt+S");
