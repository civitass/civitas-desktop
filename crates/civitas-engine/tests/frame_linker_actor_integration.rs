...
use chrono::Utc;
use civitas_db::{DatabaseManager, InsertUiEvent, UiEventType};
use civitas_engine::frame_linker::{DropReason, EventPersisted, FrameCaptured};
use civitas_engine::frame_linker_actor::{
...
async fn fresh_db() -> Arc<DatabaseManager> {
    let db = DatabaseManager::new("sqlite::memory:", Default::default())
        .await
        .unwrap();
    sqlx::migrate!("../civitas-db/src/migrations")
        .run(&db.pool)
        .await
        .unwrap();
    Arc::new(db)
