...
use civitas_engine::video_utils::extract_frame_from_video;

/// Setup test environment with real civitas database
async fn setup_test_env() -> Result<Arc<DatabaseManager>> {
    ...
    let db = Arc::new(
        DatabaseManager::new(
            paths::default_civitas_data_dir()
                .join("db.sqlite")
                .to_str()
                .unwrap(),
            Default::default(),
...
#[tokio::test]
#[ignore = "requires real civitas data, run with --ignored"]
...
#[tokio::test]
#[ignore = "requires running civitas server, run with --ignored"]
