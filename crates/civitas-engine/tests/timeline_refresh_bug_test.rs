...
mod tests {
    ...
    /// TEST 9: Test civitas app filtering logic
    ///
    /// Frames from civitas app should be filtered out.
    /// This tests the filtering function directly.
    #[tokio::test]
    async fn test_civitas_app_filtering() {
        ...
        // Civitas variants should be filtered out
        assert!(
            !should_include_entry("civitas"),
            "civitas should be filtered"
        );
        assert!(
            !should_include_entry("Civitas"),
            "Civitas should be filtered"
        ...
        assert!(
            !should_include_entry("civitas-app"),
            "civitas-app should be filtered"
        );
    }

    /// TEST 10: Test that frames with all-civitas entries result in empty frame_data
    ///
    /// When all OCR entries in a frame are from civitas, after filtering,
    /// the frame_data should be empty and the frame should NOT be sent.
    #[tokio::test]
    async fn test_all_civitas_entries_results_in_empty_frame() {
        ...
        let filtered: Vec<_> = entries
            .into_iter()
            .filter(|(app_name, _)| !app_name.to_lowercase().contains("civitas"))
            .collect();

        assert!(
            filtered.is_empty(),
            "All civitas entries should be filtered out, resulting in empty frame_data"
        );
    ...
    /// TEST 11: Test mixed entries (some civitas, some other apps)
    ///
    /// When a frame has mixed entries, only non-civitas entries should remain.
    #[tokio::test]
    async fn test_mixed_entries_partial_filtering() {
        ...
        assert_eq!(filtered.len(), 2, "Should have 2 non-civitas entries");
    ...
    async fn test_empty_frame_data_should_be_skipped() {
        ...
        // Frame with empty data (all civitas filtered) - should NOT be sent
