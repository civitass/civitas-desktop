...
async fn create_test_video() -> Result<PathBuf> {
    let civitas_dir = paths::default_civitas_data_dir().join("data");

    info!("looking for monitor video in {}", civitas_dir.display());

    // Read directory and find first video containing "monitor"
    let mut entries = tokio::fs::read_dir(&civitas_dir).await?;
    ...
    Err(anyhow::anyhow!(
        "no monitor video found in civitas data dir"
    ))
}
...
#[tokio::test]
#[ignore = "requires local civitas data"]
...
async fn test_get_video_metadata() -> Result<()> {
    ...
    let metadata =
        civitas_engine::video_utils::get_video_metadata(video_path.to_str().unwrap()).await?;
