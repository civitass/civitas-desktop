...
// Regression for historical "moov atom not found" issue reports.
//
...
/// Find an explicitly configured or locally installed FFmpeg binary.
fn find_ffmpeg() -> Option<String> {
    if let Ok(path) = std::env::var("CIVITAS_TEST_FFMPEG") {
...
fn require_ffmpeg() -> String {
    find_ffmpeg().expect(
        "FFmpeg is required for this ignored integration test; install it or set CIVITAS_TEST_FFMPEG",
    )
}
...
async fn extract_frame(video_path: &str, output_path: &str) -> Result<(), String> {
    let ffmpeg = find_ffmpeg().ok_or_else(|| {
        "FFmpeg is not installed; set CIVITAS_TEST_FFMPEG to a reviewed local binary".to_string()
    })?;
...
#[tokio::test]
#[ignore = "requires local FFmpeg; uses only runtime-generated synthetic media"]
