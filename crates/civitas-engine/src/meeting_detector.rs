...
use civitas_db::{DatabaseManager, MEETING_END_REASON_AUTO_END, MEETING_END_REASON_SHUTDOWN};
use civitas_events::subscribe_to_event;
...
pub fn load_detection_profiles() -> Vec<MeetingDetectionProfile> {
    vec![
        ...
        // https://github.com/civitass/civitas-desktop/issues/2561
...
impl MeetingUiScanner {
    ...
    pub fn scan_process(&self, pid: i32, profile: &MeetingDetectionProfile) -> ScanResult {
            ...
            cidre::objc::ar_pool(|| -> Vec<String> {
                ...
                for i in 0..windows.len() {
                    if start.elapsed() >= scan_timeout {
                        debug!("meeting_scanner timeout=true");
                        break;
                    }
        ...
        let matched_signals = match scan_result {
            Ok(signals) => signals,
            Err(_) => {
                warn!("meeting_scanner ax_walk_panicked=true");
                Vec::new()
            }
        };
        ...
        debug!(
            "meeting_scanner signals={} in_call={}",
            signals_found, is_in_call
        );
    ...
    pub fn scan_process(&self, pid: i32, profile: &MeetingDetectionProfile) -> ScanResult {
        ...
        let matched_signals = match scan_result {
            ...
            Err(_) => {
                warn!("meeting_scanner uia_scan_panicked=true");
                Vec::new()
            }
        };
        ...
        info!(
            "meeting_scanner scan_completed=true signals={} in_call={}",
            signals_found, is_in_call,
        );
...
/// from the Active⇌Ending oscillation represented by the synthetic Arc/Meet
/// regression fixture without changing end-detection semantics: the grace clock keeps
    ...
    for (idx, profile) in profiles.iter().enumerate() {
        ...
        for (pid, title) in &window_titles {
            ...
            if url_match || title_match {
                // Confirms civitas saw the meeting window; pairs with the
...
/// Kept in sync with `civitas_audio`'s `AUDIO_RECENCY_WINDOW_MS`.
...
pub async fn run_meeting_detection_loop(
    ...
    detector: Option<Arc<civitas_audio::meeting_detector::MeetingDetector>>,
        ...
        if let Some(action) = action {
            match action {
                StateAction::StartMeeting { app } => {
                    ...
                    let meeting_id = match merge_candidate {
...
mod windows_live_tests {
    ...
    /// Run with: cargo test -p civitas-engine --no-default-features --lib -- windows_live_tests::test_live_zoom_detection --nocapture --ignored
    #[test]
    #[ignore]
...
mod live_tests {
    ...
    /// Run with: cargo test -p civitas-engine --lib -- live_tests::test_live_meeting_detection --nocapture --ignored
    #[test]
    #[ignore]
...
mod live_tests2 {
    ...
    /// Run with: cargo test -p civitas-engine --lib -- live_tests2::test_arc_deep_window_check --nocapture --ignored
    #[test]
    #[ignore]
