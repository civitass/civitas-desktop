...
pub fn screen_is_locked() -> bool {
    atomic_screen_is_locked(&SCREEN_IS_LOCKED)
}

fn atomic_screen_is_locked(flag: &AtomicBool) -> bool {
    flag.load(Ordering::SeqCst)
}

/// Set the screen locked state (called from capture loop when lock-screen app detected).
/// Also updates the shared flag in civitas-config so other crates (e.g. audio) can read it.
pub fn set_screen_locked(locked: bool) {
    SCREEN_IS_LOCKED.store(locked, Ordering::SeqCst);
    civitas_config::set_screen_locked(locked);
}
...
pub fn start_sleep_monitor() {
    ...
    if initial_locked {
        ...
        civitas_config::set_screen_locked(true);
    }
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_screen_unlocked(
            ...
            civitas_config::set_screen_locked(false);
            if was_locked {
                ...
                #[cfg(target_os = "macos")]
                civitas_screen::stream_invalidation::request();
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_display_reconfigured(
            ...
            #[cfg(target_os = "macos")]
            civitas_screen::stream_invalidation::request();
            civitas_audio::stream_invalidation::request();
...
fn on_will_sleep() {
    ...
    civitas_db::request_write_pause();
}
...
fn on_did_wake(handle: &tokio::runtime::Handle) {
    ...
    handle.spawn(async {
        ...
        civitas_db::request_write_resume();
...
mod tests {
    ...
    fn test_recently_woke_flag() {
        // Reset first to be hermetic: other parallel tests (e.g.
        // test_on_did_wake_sets_audio_invalidation) may leave RECENTLY_WOKE=true.
        RECENTLY_WOKE.store(false, Ordering::SeqCst);
    ...
    fn test_screen_is_locked_flag() {
        ...
        assert!(!atomic_screen_is_locked(&flag));
    ...
    async fn test_on_did_wake_sets_audio_invalidation() {
        // Clear stale flags
        let _ = civitas_audio::stream_invalidation::take();
        let _ = civitas_screen::stream_invalidation::take();
        RECENTLY_WOKE.store(false, Ordering::SeqCst);
        ...
        assert!(
            civitas_audio::stream_invalidation::take(),
            "Audio stream invalidation flag must be set after wake"
        );
        assert!(
            civitas_screen::stream_invalidation::take(),
    ...
    /// Run with: cargo test -p civitas-engine --lib -- test_screen_lock_unlock --ignored --nocapture
    #[cfg(target_os = "macos")]
    #[tokio::test]
