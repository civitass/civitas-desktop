...
use civitas_audio::core::device::DeviceType;
use civitas_db::{Speaker, TagContentType};
...
pub enum ContentItem {
    ...
    /// Selected project-root file evidence
...
pub struct ProjectFileContent {
    ...
    /// Convenience alias for generic clients and agents that read `text`.
    pub text: String,
}
...
pub(crate) async fn remove_tags(
    ...
    match state.db.remove_tags(id, content_type, payload.tags).await {
        Ok(_) => {
            state.search_cache.invalidate_all();
            state.search_cache.run_pending_tasks().await;
            Ok(JsonResponse(RemoveTagsResponse { success: true }))
        }
        Err(e) => {
    ...
    if let Some(ocr_results) = &frame.ocr_results {
        for ocr in ocr_results {
            db.insert_ocr_text(
                ...
                Arc::new(OcrEngine::default().into()), // Ideally could pass any str as ocr_engine since can be run outside of civitas
            )
            .await?;
        }
    }
    ...
    match payload.content.content_type.as_str() {
        "frames" => {
            if let ContentData::Frames(frames) = &payload.content.data {
                if !frames.is_empty() {
                    let output_dir = state.civitas_dir.join("data");
                    let time = Utc::now();
...
pub(crate) struct RawSqlQuery {
    query: String,
    /// Server-enforced response row ceiling. The SQL text does not need its
    /// own LIMIT; an outer SQLite SELECT applies this value after preparation.
    #[serde(default = "default_raw_sql_limit")]
    limit: u32,
}

fn default_raw_sql_limit() -> u32 {
    1_000
}
...
pub(crate) async fn execute_raw_sql(
    ...
    match state
        .db
        .execute_read_only_sql(&payload.query, payload.limit)
        .await
    {
        Ok(result) => Ok(JsonResponse(result)),
        Err(e) => {
            use civitas_db::ReadOnlySqlError;
            let status = match &e {
                ReadOnlySqlError::Invalid(_) => StatusCode::BAD_REQUEST,
                ReadOnlySqlError::TimedOut => StatusCode::REQUEST_TIMEOUT,
                ReadOnlySqlError::ResultTooLarge => StatusCode::PAYLOAD_TOO_LARGE,
                ReadOnlySqlError::Database(_) | ReadOnlySqlError::Worker(_) => {
                    StatusCode::INTERNAL_SERVER_ERROR
                }
            };
            error!("read-only SQL request failed: {}", e);
...
pub(crate) async fn merge_frames_handler(
    ...
    let output_dir = state.civitas_dir.join("videos");
