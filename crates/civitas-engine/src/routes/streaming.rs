...
use civitas_db::{DatabaseManager, FrameData, Order};
...
pub(crate) fn create_time_series_frame(chunk: FrameData) -> TimeSeriesFrame {
    ...
    let mut device_frames: Vec<DeviceFrame> = chunk
        .ocr_entries
        .into_iter()
        // Filter out civitas frames at display time
        .filter(|device_data| !device_data.app_name.to_lowercase().contains("civitas"))
                ...
                result = frame_rx_cache.recv() => {
                    match result {
                        Ok(hot_frame) => {
                            ...
                            // Skip civitas's own frames
                            if hot_frame.app_name.to_lowercase().contains("civitas") {
                                continue;
                            }
...
mod tests {
    use super::*;
    use civitas_db::{AudioEntry as DbAudioEntry, FrameData, OCREntry};
