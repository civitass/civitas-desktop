...
//! transcription engine (Whisper, Parakeet, Deepgram BYOK, etc.).
//!
//! Compatible with:
...
use civitas_audio::utils::ffmpeg::read_audio_from_file;
use serde::Serialize;
use serde_json::json;
use std::sync::Arc;
...
pub async fn transcribe_handler(
            ...
            "engine" => {
                let value = match field.text().await {
                    ...
                    Err(error) => {
                        return error_response(
                            StatusCode::BAD_REQUEST,
                            format!("failed to read engine field: {error}"),
                        );
                    }
                };
                let value = value.trim().to_string();
                if !value.is_empty() {
                    engine_override = Some(value);
                }
            }
    ...
    // Validate the requested engine before writing or decoding the upload. This
    // keeps invalid requests deterministic even when FFmpeg is unavailable and
    // avoids spending local CPU on a request that can never be fulfilled.
    let engine_override = match engine_override {
        Some(engine_name) => match engine_name.parse::<AudioTranscriptionEngine>() {
    ...
    let transcription_engine = if let Some(engine) = engine_override {
