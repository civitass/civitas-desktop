...
use civitas_db::{MeetingRecord, MeetingTranscriptSegment, MEETING_END_REASON_EXPLICIT_STOP};
...
fn scoped_timestamp_allowed(
    permissions: &civitas_core::pipes::permissions::PipePermissions,
    raw: &str,
) -> bool {
    if permissions.time_range.is_none() && permissions.days.is_none() {
        return true;
    }
    let Ok(timestamp) = super::time::parse_flexible_datetime(raw) else {
        return false;
    };
    let local_timestamp = timestamp.with_timezone(&Local);
    permissions.is_time_in_range(local_timestamp.hour(), local_timestamp.minute())
        && permissions.is_day_allowed(local_timestamp.weekday())
}

fn scoped_meeting_allowed(
    permissions: &civitas_core::pipes::permissions::PipePermissions,
    meeting: &MeetingRecord,
) -> bool {
    if permissions.has_window_restrictions()
        || (permissions.has_app_restrictions() && !permissions.is_app_allowed(&meeting.meeting_app))
        || !permissions.is_content_type_allowed("audio")
    {
        return false;
    }
    scoped_timestamp_allowed(permissions, &meeting.meeting_start)
}
...
pub fn emit_meeting_status_changed(status: &MeetingStatusResponse) {
    ...
    if let Err(e) = civitas_events::send_event("meeting_status_changed", status.clone()) {
...
pub(crate) async fn list_meetings_handler(
    ...
    OptionalPipePerms(pipe_permissions): OptionalPipePerms,
    ...
    if let Some(permissions) = pipe_permissions {
        meetings.retain(|meeting| scoped_meeting_allowed(&permissions, meeting));
    }
...
pub(crate) async fn get_meeting_handler(
    ...
    if pipe_permissions
        .as_deref()
        .is_some_and(|permissions| !scoped_meeting_allowed(permissions, &meeting))
    {
        return Err((
            StatusCode::NOT_FOUND,
            JsonResponse(json!({"error": "meeting not found within workflow scope"})),
        ));
    }
...
pub(crate) async fn get_meeting_transcript_handler(
    ...
    if let Some(permissions) = pipe_permissions {
        segments.retain(|segment| {
            permissions.is_content_type_allowed("audio")
                && scoped_timestamp_allowed(&permissions, &segment.captured_at)
        });
    }
...
pub(crate) async fn start_meeting_handler(
    ...
    if let Err(e) = civitas_events::send_event(
        "meeting_started",
        serde_json::json!({
            "meeting_id": id,
...
pub(crate) async fn stop_meeting_handler(
    ...
    // Emit event so triggered pipes can react
    if let Err(e) =
        civitas_events::send_event("meeting_ended", serde_json::json!({ "meeting_id": id }))
    {
...
/// `meeting_id` XOR `start`/`end` contract as the `civitas export` CLI and the
...
/// the `civitas export` CLI — same `meeting_id` XOR `start`/`end` contract.
#[oasgen]
pub(crate) async fn export_handler(
    ...
    let is_workflow = pipe_permissions.is_some();
    ...
    let server_error = |e: anyhow::Error| {
        tracing::error!("recording export failed: {e:#}");
        let message = if is_workflow {
            "recording export failed; inspect the local Civitas logs for details".to_string()
        } else {
            format!("{e:#}")
        };
        (
            ...
            .is_some_and(|path| !path.trim().is_empty())
    {
        return Err(bad_request(
            "automated workflows cannot choose an export path; omit output_path to use the guarded local exports directory"
                .to_string(),
        ));
    }

    // Explicit owner-selected output wins; otherwise name a file under the
    // guarded `<data-dir>/exports/` directory.
    ...
    // meeting_id XOR start/end, same contract as the `civitas export` CLI.
    let summary = match (body.meeting_id, body.start.is_some() || body.end.is_some()) {
        (Some(id), _) => {
            if is_workflow {
                let meeting = state
                    ...
                    .await
                    .map_err(|_| bad_request("meeting not found".to_string()))?;
                let start = crate::routes::time::parse_flexible_datetime(&meeting.meeting_start)
                    .map_err(|_| bad_request("meeting start time is invalid".to_string()))?;
                let end = meeting
                    .meeting_end
                    .as_deref()
                    .map(crate::routes::time::parse_flexible_datetime)
                    .transpose()
                    .map_err(|_| bad_request("meeting end time is invalid".to_string()))?
                    .unwrap_or_else(Utc::now);
                if end <= start || end - start > chrono::Duration::hours(2) {
                    return Err(bad_request(
                        "automated workflow exports must cover a completed range of at most 2 hours"
                            .to_string(),
                    ));
                }
            }
        ...
        (None, true) => {
            ...
            if is_workflow && (end <= start || end - start > chrono::Duration::hours(2)) {
                return Err(bad_request(
                    "automated workflow exports must cover a completed range of at most 2 hours"
                        .to_string(),
                ));
    ...
    let response_path = if is_workflow {
        std::path::Path::new(&summary.output_path)
            .strip_prefix(&state.civitas_dir)
            .map(|path| format!("Civitas data/{}", path.display()))
            .unwrap_or_else(|_| "Civitas data/exports/recording.mp4".to_string())
    } else {
        summary.output_path
    };
...
mod tests {
    ...
    fn scoped_permissions() -> PipePermissions {
        PipePermissions {
            pipe_name: "meeting-audit".to_string(),
            allow_rules: [parse_rules("App(Zoom)"), parse_rules("Content(audio)")].concat(),
            deny_rules: vec![],
            use_default_allowlist: false,
            time_range: Some((9, 0, 17, 0)),
            days: Some([0].into_iter().collect()),
            pipe_token: None,
            pipe_dir: None,
    ...
    fn meeting_at(start: &str, app: &str) -> MeetingRecord {
        MeetingRecord {
            id: 7,
            meeting_start: start.to_string(),
            meeting_end: Some("2026-07-27T11:00:00Z".to_string()),
            meeting_app: app.to_string(),
            title: Some("Planning".to_string()),
            attendees: None,
            note: None,
            detection_source: "app".to_string(),
            created_at: start.to_string(),
        }
    }

    fn local_timestamp(day: u32, hour: u32) -> String {
        Local
    ...
    #[test]
    fn workflow_meeting_scope_enforces_app_day_and_time() {
        let permissions = scoped_permissions();
        let monday_at_ten = local_timestamp(27, 10);
        let monday_at_twenty = local_timestamp(27, 20);
        let tuesday_at_ten = local_timestamp(28, 10);
        assert!(scoped_meeting_allowed(
            &permissions,
            &meeting_at(&monday_at_ten, "Zoom")
        ));
        assert!(!scoped_meeting_allowed(
            &permissions,
            &meeting_at(&monday_at_ten, "Google Meet")
        ));
        assert!(!scoped_meeting_allowed(
            &permissions,
            &meeting_at(&monday_at_twenty, "Zoom")
        ));
        assert!(!scoped_meeting_allowed(
            &permissions,
            &meeting_at(&tuesday_at_ten, "Zoom")
        ));
    }

    #[test]
    fn workflow_meeting_scope_fails_closed_on_unparseable_timestamp() {
        assert!(!scoped_meeting_allowed(
            &scoped_permissions(),
            &meeting_at("not-a-timestamp", "Zoom")
        ));
    }
