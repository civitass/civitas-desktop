...
use civitas_core::pii_removal::detect_pii_regions;
use civitas_db::TextPosition;
    ...
    match timeout(Duration::from_secs(5), async {
        ...
        match state.db.get_frame(frame_id).await {
            Ok(Some((file_path, offset_index, is_snapshot))) => {
                // Upgraded databases can contain a placeholder written by a
                // retired private-build archive feature. Consumer builds never
                // dereference non-filesystem media references.
                if file_path.starts_with("cloud://") {
                    let unavailable = json!({
                        "error": "Frame media is not available on this device",
                        "error_type": "legacy_nonlocal_media",
                        "frame_id": frame_id,
                    });
                    return Err((StatusCode::GONE, JsonResponse(unavailable)));
                }
...
/// Get frame metadata (timestamp) for deep link navigation. civitas://frame/123 → resolve to timestamp.
#[oasgen]
    ...
    if text_positions.is_empty() {
        if let Ok((_, Some(tree_json))) = state.db.get_frame_accessibility_data(frame_id).await {
            if let Some(query) = &params.query {
                ...
                text_positions = civitas_db::find_matching_a11y_positions(&tree_json, query);
...
async fn run_local_frame_ocr(image: image::DynamicImage) -> Result<(String, String), String> {
    #[cfg(target_os = "macos")]
    {
        return tokio::task::spawn_blocking(move || {
            civitas_screen::perform_ocr_apple_checked(&image, &[])
                .map(|(text, json, _confidence)| (text, json))
        })
        .await
        .map_err(|error| format!("Apple Vision OCR worker failed: {error}"))?;
    }

    #[cfg(target_os = "windows")]
    {
        return civitas_screen::perform_ocr_windows(&image, &[])
            .await
            .map(|(text, json, _confidence)| (text, json))
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        tokio::task::spawn_blocking(move || {
            civitas_screen::perform_ocr_tesseract_checked(&image, vec![])
                .map(|(text, json, _confidence)| (text, json))
        })
        .await
        .map_err(|error| format!("Tesseract OCR worker failed: {error}"))?
    }
}
...
pub async fn run_frame_ocr(
    ...
    let image = if is_snapshot {
        ...
        .await
        {
            Ok(Ok(img)) => img,
            Ok(Err(e)) => {
                error!("Failed to open snapshot for frame {}: {}", frame_id, e);
                return Err((
                    StatusCode::INTERNAL_SERVER_ERROR,
                    JsonResponse(json!({ "error": "Failed to load frame image" })),
                ));
            }
            Err(e) => {
    ...
    let (ocr_text, ocr_text_json) = run_local_frame_ocr(image).await.map_err(|error| {
        error!("On-demand OCR failed for frame {}: {}", frame_id, error);
        (
    ...
    // Store in DB before acknowledging success so the next read observes the
    // same result and a failed structured-text write cannot masquerade as OCR.
    if !ocr_text.is_empty() {
        #[cfg(target_os = "macos")]
        let engine = Arc::new(civitas_db::OcrEngine::AppleNative);
        #[cfg(target_os = "windows")]
        let engine = Arc::new(civitas_db::OcrEngine::WindowsNative);
        #[cfg(not(any(target_os = "macos", target_os = "windows")))]
        let engine = Arc::new(civitas_db::OcrEngine::Tesseract);
        state
            .db
            ...
            .map_err(|error| {
                error!(
                    "Failed to persist on-demand OCR for frame {}: {}",
                    frame_id, error
                );
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    JsonResponse(json!({ "error": "Failed to persist OCR result" })),
                )
            })?;
    }
    ...
    let text_positions = state
        ...
        .map_err(|error| {
            error!(
                "Failed to read persisted OCR positions for frame {}: {}",
                frame_id, error
            );
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                JsonResponse(json!({ "error": "Failed to read OCR result" })),
            )
        })?;
