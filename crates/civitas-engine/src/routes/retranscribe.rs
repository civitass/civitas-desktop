...
use civitas_db::{AudioChunkInfo, NewMeetingTranscriptSegment};
...
pub struct MeetingRetranscribeRequest {
    /// Optional engine override: "deepgram", "parakeet", "whisper-large-v3-turbo", etc.
    pub engine: Option<String>,
    /// Custom vocabulary for this re-transcription.
...
pub async fn drop_audio_reconciliation_chunk_handler(
    ...
    if let Err(e) = state
        .db
        .process_file_deletion_jobs(&state.civitas_dir, 100)
        .await
    {
        error!(
            "audio reconciliation chunk {} was removed but file cleanup failed: {}",
            audio_chunk_id, e
        );
        return error_response(
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("chunk removed; durable file cleanup is pending: {}", e),
        );
    }
...
pub async fn retranscribe_handler(
    ...
    for chunk in &chunks {
        ...
        let (samples, sample_rate) =
            match civitas_audio::utils::ffmpeg::read_audio_from_file(file_path) {
                Ok(result) => result,
                Err(e) => {
    ...
    for batch in batches {
        ...
        for chunk in batch {
            ...
            match tokio::task::spawn_blocking(move || {
                civitas_audio::utils::ffmpeg::read_audio_from_file(StdPath::new(&path_owned))
            })
            .await
            {
                Ok(Ok((samples, sr))) => {
