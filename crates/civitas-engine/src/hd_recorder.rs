...
use civitas_screen::monitor::SafeMonitor;
...
mod macos {
    ...
    async fn record_one_chunk(
        ...
        use civitas_screen::capture_screenshot_by_window::{
    ...
    fn start_hd_ffmpeg(out: &Path, fps: u32) -> Result<tokio::process::Child> {
        let ffmpeg = civitas_core::find_ffmpeg_path().context("ffmpeg not found")?;
