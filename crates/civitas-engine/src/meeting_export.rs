...
use civitas_core::{ffmpeg_cmd_async, find_ffmpeg_path};
...
/// meeting export and the `civitas export` CLI. Returns a summary on success.
///
...
/// Parse the device name out of a civitas audio chunk filename and report whether it is a
...
/// 0-based decoded frame index (`offset_index`). civitas's "compact" chunks are variable frame
...
mod tests {
    ...
    fn device_key_parses_name_and_output_flag() {
        let (k, out) = audio_device_key(
            "/Users/x/.civitas/data/System Audio (output)_2026-05-28_18-05-16.mp4",
        );
        assert_eq!(k, "System Audio (output)");
        assert!(out, "system audio should be detected as output");

        let (k, out) = audio_device_key(
            "/Users/x/.civitas/data/MacBook Pro Microphone (input)_2026-05-28_18-05-22.mp4",
        );
    ...
    /// Real-data smoke test: exports an actual meeting from `~/.civitas/db.sqlite` and opens the
    ...
    ///   SP_EXPORT_TEST_MEETING_ID=185 cargo test -p civitas-engine \
    ///     export_real_meeting_opens_mp4 -- --ignored --nocapture
    #[tokio::test]
    #[ignore]
    async fn export_real_meeting_opens_mp4() {
        let home = std::env::var("HOME").expect("HOME not set");
        let base = PathBuf::from(home).join(".civitas");
