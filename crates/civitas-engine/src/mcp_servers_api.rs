...
use chrono::Utc;
use civitas_connect::mcp_servers::{
...
async fn oauth_callback(
    ...
    if let Some(error) = query.get("error") {
        return html_response(
            ...
            &format!("civitas MCP OAuth failed: {}", error),
        );
    }
    let Some(state_value) = query.get("state") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "civitas MCP OAuth failed: missing state",
        );
    };
    let Some(code) = query.get("code") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "civitas MCP OAuth failed: missing code",
        );
    };
    match state.store.complete_oauth(state_value, code).await {
        Ok(server_id) if server_id == id => html_response(
            StatusCode::OK,
            "civitas MCP OAuth connected. You can close this tab.",
        ),
        Ok(_) => html_response(
            StatusCode::BAD_REQUEST,
            "civitas MCP OAuth failed: callback server mismatch",
        ),
        Err(e) => html_response(
            StatusCode::BAD_REQUEST,
            &format!("civitas MCP OAuth failed: {}", e),
        ),
    }
}
...
fn html_response(status: StatusCode, message: &str) -> Response {
    (
        ...
        format!(
            "<!doctype html><html><head><title>civitas MCP OAuth</title></head><body><p>{}</p></body></html>",
            html_escape(message)
        ),
    )
        .into_response()
