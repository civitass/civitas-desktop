...
pub fn store_path() -> PathBuf {
    civitas_core::paths::default_civitas_data_dir().join("store.bin")
}
...
fn write_store_to_with_key(
    ...
    let bytes = if encrypt {
        ...
        let ciphertext = civitas_vault::crypto::encrypt_small(&serialized, &key)
...
fn store_encryption_key() -> Result<[u8; 32]> {
    match civitas_secrets::keychain::get_key() {
        ...
        civitas_secrets::keychain::KeyResult::AccessDenied => bail!(
            "store.bin is encrypted, but keychain access was denied. Grant civitas keychain access or disable store encryption in the app, then try again."
        ),
        civitas_secrets::keychain::KeyResult::NotFound => bail!(
            "store.bin is encrypted, but the civitas keychain key was not found. Open the app once or disable store encryption, then try again."
        ),
        civitas_secrets::keychain::KeyResult::Unavailable => bail!(
