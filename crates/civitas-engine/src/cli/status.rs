...
pub async fn handle_status_command(
    ...
    // db.sqlite lives at the top of the civitas dir; the `data/` subdir
    ...
    let (frame_count, audio_count, last_timestamp) = if db_path.exists() {
        match civitas_db::DatabaseManager::new(&db_path.to_string_lossy(), Default::default()).await
        {
            Ok(db) => {
    ...
    if json {
        ...
        println!("civitas: {} {} (port {})", status_icon, status, port);
