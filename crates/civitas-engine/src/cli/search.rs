...
//! `civitas search` — query the local SQLite history without the daemon.
//!
//! Mirrors `GET /search` so AI / shell / pipe callers don't have to choose
//! between two vocabularies. Opens `~/.civitas/db.sqlite` directly; WAL
...
use crate::routes::search::{is_civitas_app, search_result_to_content_item};
...
pub async fn handle_search_command(args: &SearchArgs) -> anyhow::Result<()> {
    let base_dir = match &args.data_dir {
        ...
        None => paths::default_civitas_data_dir(),
    };
    let db_path = base_dir.join("db.sqlite");
    if !db_path.exists() {
        anyhow::bail!(
            "no civitas database at {}. run `civitas record` first.",
            db_path.display()
        );
    }
...
fn parse_content_type(s: &str) -> Result<ContentType, String> {
    match s.to_lowercase().as_str() {
        ...
        "project_file" | "project-file" | "projectfile" | "file" => Ok(ContentType::ProjectFile),
        other => Err(format!(
            "unknown content type '{}' — expected one of: all, ocr, audio, accessibility, input, memory, project_file",
            other
        )),
    }
}
...
mod tests {
    ...
    fn content_type_aliases() {
        ...
        assert!(matches!(
            parse_content_type("project-file"),
            Ok(ContentType::ProjectFile)
        ));
    ...
    fn empty_query_is_allowed_for_time_only_browse() {
        let cli = Cli::try_parse_from(["civitas", "search", "--start", "30m ago"]).unwrap();
        match cli.command {
            Command::Search(args) => {
