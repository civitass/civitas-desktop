...
//! `civitas db ...` — corruption recovery + storage cleanup.
//!
//! ## Safety model
//!
//! These commands rewrite or delete files inside `~/.civitas/`. A racing
...
//!
//!   * `~/.civitas/.db_recovery.lock` — JSON `{pid, host, started_at, op}`
...
//!     (`apps/civitas-app-tauri/src-tauri/src/main.rs`); env var
//!     `CIVITAS_IGNORE_DB_LOCK=1` is the escape hatch.
//!
...
//! 3. Same host, PID alive: refuse, point at `civitas db unlock`.
...
if you're sure no `civitas db ...` is running, run `civitas db unlock --force`.",
                        path.display(),
                    );
                }
                let _ = fs::remove_file(&path);
            ...
            LockState::HeldHere { pid, op: other } => {
                bail!(
                    "another db op is running: {} (pid {}). wait for it, or `civitas db unlock --force` if you're sure it's stuck.",
                    other,
                    pid,
                );
            }
...
will not auto-clear cross-host locks. resolve manually if needed.",
        ...
        let mut file = OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "couldn't acquire lock at {}. another `civitas db ...` may have raced us.",
                    path.display()
                )
            })?;
    ...
    fn install_signal_handlers(path: &Path, stop: Arc<AtomicBool>) {
        ...
        tokio::spawn(async move {
            ...
            let _ = fs::remove_file(&path);
            eprintln!("\ncivitas: caught signal, lock released. exiting.");
...
fn ensure_app_quit(force: bool) -> Result<()> {
    if !force && server_is_running() {
        bail!(
            "civitas is currently running (port 3030 is open). quit the app from the menubar first, or pass --force to override (only safe if you know nothing is writing to the db)."
        );
    }
    Ok(())
}
...
fn ensure_disk_space(data_dir: &Path, source_size: u64) -> Result<()> {
    ...
    if let Some(avail) = available {
        if avail < needed {
            bail!(
                "not enough disk space: need ~{} MB free, have {} MB. run `civitas db cleanup` first to reclaim old artifacts.",
                needed / 1_048_576,
                avail / 1_048_576,
            );
        }
...
fn integrity_check(db_path: &Path) -> Result<()> {
    ...
    if result == "ok" {
        ...
        eprintln!("\n  next step: quit the civitas app, then run `civitas db recover`");
        std::process::exit(1)
    }
}
...
async fn recover(data_dir: &Path, force: bool) -> Result<()> {
    ...
    println!();
    println!("    next: start civitas — it will rebuild WAL/SHM on first open.");
    println!("    once you've confirmed everything works, run:");
    println!("        civitas db cleanup --apply");
...
fn unlock(data_dir: &Path, force: bool) -> Result<()> {
    ...
    if !safe && !force {
        bail!(
            "lock looks live ({state:?}). pass --force to remove anyway. you should only do this if you're certain no `civitas db ...` is actually running."
        );
    }
