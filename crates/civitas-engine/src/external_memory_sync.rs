...
//!   `civitas-core::memories::external_sync`. Pure, no DB, easy to
//!   unit-test.
//! - The two *destination definitions* (Claude Code, Codex) live in
//!   `civitas-connect::connections::{claude_code, codex}`. They're
...
//!
//! Uses an owner-held scheduler with a start/stop/metrics/snapshot contract
//! so its background task remains observable and cancellation-safe.
...
use anyhow::Result;
use civitas_connect::connections::{load_connection, SavedConnection};
use civitas_core::memories::external_sync::{
    remove_managed_blocks_atomic, remove_owned_file, render_block_body, render_digest,
    write_atomic, write_atomic_full, Destination, MemoryEntry, SyncOutcome,
};
...
static OPERATION_GATE: OnceLock<tokio::sync::Mutex<()>> = OnceLock::new();

/// Serialize bridge writes with disconnect cleanup. Without this gate, a
/// scheduler tick could rewrite a managed block after **Stop and remove**
/// deleted it but before the connection credential was removed.
pub fn operation_gate() -> &'static tokio::sync::Mutex<()> {
    OPERATION_GATE.get_or_init(|| tokio::sync::Mutex::new(()))
}
...
impl ExternalMemorySyncScheduler {
    ...
    pub fn start(
        ...
        let dir_clone = civitas_dir.clone();
...
#[derive(Debug, Serialize)]
pub struct ExternalSyncRemoval {
    pub destination_id: &'static str,
    pub instruction_file: PathBuf,
    pub managed_block_removed: bool,
...
async fn sync_destination(
    ...
    let conn = load_connection(secret_store, civitas_dir, dest.id).await;
...
fn apply(
    ...
    // civitas-owned — no hand-edited content to preserve — so we
    // rewrite it whole.
    ...
    let entries_used = entries
        .len()
        .min(civitas_core::memories::external_sync::MAX_ENTRIES_PER_DIGEST);
...
/// Remove the local instruction-file content owned by a memory connection.
/// Unsupported connection IDs return `None`; supported IDs fail closed so the
/// caller can keep the connection configured and let the user retry cleanup.
pub fn remove_destination(
    destination_id: &str,
    credentials: &serde_json::Map<String, Value>,
) -> Result<Option<ExternalSyncRemoval>> {
    match destination_id {
        id if id == Destination::CLAUDE_CODE.id => remove_destination_with(
            &Destination::CLAUDE_CODE,
            credentials,
            resolve_claude_code_path,
        )
        .map(Some),
        id if id == Destination::CODEX.id => {
            remove_destination_with(&Destination::CODEX, credentials, resolve_codex_path).map(Some)
        }
        _ => Ok(None),
    }
}

fn remove_destination_with(
    destination: &'static Destination,
    credentials: &serde_json::Map<String, Value>,
    resolver: impl Fn(&serde_json::Map<String, Value>) -> Result<PathBuf>,
) -> Result<ExternalSyncRemoval> {
    let home = resolver(credentials)?;
    let instruction_file = destination.target_path(&home);
    let managed_block_removed = remove_managed_blocks_atomic(&instruction_file)
        .map_err(|error| anyhow::anyhow!("clean {}: {}", instruction_file.display(), error))?;
    let sidecar_removed = match destination.sidecar_path(&home) {
        Some(sidecar) => remove_owned_file(&sidecar)
            .map_err(|error| anyhow::anyhow!("remove {}: {}", sidecar.display(), error))?,
        None => false,
    };

    Ok(ExternalSyncRemoval {
        destination_id: destination.id,
        instruction_file,
        managed_block_removed,
        sidecar_removed,
    })
}

fn resolve_claude_code_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    civitas_connect::connections::claude_code::resolve_home_path(creds)
}

fn resolve_codex_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    civitas_connect::connections::codex::resolve_home_path(creds)
}
...
mod tests {
    use super::*;
    use civitas_core::memories::external_sync::{marker_end, marker_start};
    use sqlx::SqlitePool;
    ...
    async fn secret_store_with_connection(id: &str, enabled: bool) -> SecretStore {
        let pool = SqlitePool::connect(":memory:").await.unwrap();
        let store = SecretStore::new(pool, [42u8; 32]).await.unwrap();
        store
            .set_json(
                &format!("cred:{id}"),
                &SavedConnection {
    ...
    fn apply_writes_block_into_destination_file() {
        ...
        match outcome {
            SyncOutcome::Wrote { path, entries: n } => {
                ...
                assert!(
                    outer.contains("@civitas-memories.md"),
                ...
                let sidecar = dir.path().join("civitas-memories.md");
    ...
    fn apply_reports_change_when_entries_shift() {
        ...
        let sidecar = std::fs::read_to_string(dir.path().join("civitas-memories.md")).unwrap();
    ...
    #[test]
    fn remove_destination_cleans_marker_and_owned_sidecar() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let instruction_file = dir.path().join("CLAUDE.md");
        std::fs::write(&instruction_file, "# user rules\n").unwrap();

        apply(
            &Destination::CLAUDE_CODE,
            &[entry("private memory", 0.9)],
            &credentials,
            resolve_claude_code_path,
        )
        .unwrap();

        let outcome = remove_destination(Destination::CLAUDE_CODE.id, &credentials)
            .unwrap()
            .unwrap();
        assert!(outcome.managed_block_removed);
        assert!(outcome.sidecar_removed);
        let remaining = std::fs::read_to_string(&instruction_file).unwrap();
        assert!(remaining.contains("# user rules"));
        assert!(!remaining.contains("private memory"));
        assert!(!remaining.contains(&marker_start()));
        assert!(!dir.path().join("civitas-memories.md").exists());
    }

    #[test]
    fn remove_destination_ignores_other_connections() {
        assert!(remove_destination("notion", &serde_json::Map::new())
            .unwrap()
            .is_none());
    }
    ...
    async fn sync_destination_skips_when_disabled() {
        ...
        let secret_store = secret_store_with_connection(Destination::CLAUDE_CODE.id, false).await;
    ...
    async fn sync_destination_writes_when_enabled() {
        ...
        let secret_store = secret_store_with_connection(Destination::CLAUDE_CODE.id, true).await;
        ...
        match result.outcome {
            Ok(SyncOutcome::Wrote { path, entries: n }) => {
                ...
                let outer = std::fs::read_to_string(&path).unwrap();
                assert!(outer.contains("@civitas-memories.md"));
                assert!(!outer.contains("first fact"));
                // The memory bodies land in the sidecar.
                let sidecar =
                    std::fs::read_to_string(target_dir.join("civitas-memories.md")).unwrap();
