...
//! `civitas-events` bus when the OS permission state changes.
//!
//! ## Design
//!
...
use civitas_core::permissions::{check_permissions, PermissionStatus};
...
fn keychain_accessible() -> bool {
    use civitas_secrets::keychain::{get_key, is_keychain_available, KeyResult};
    ...
    if !civitas_secrets::is_encryption_requested(&civitas_core::paths::default_civitas_data_dir()) {
        return true;
    }
    match get_key() {
