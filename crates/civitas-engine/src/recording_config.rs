...
use civitas_audio::audio_manager::builder::TranscriptionMode;
use civitas_audio::audio_manager::AudioManagerBuilder;
use civitas_audio::core::engine::AudioTranscriptionEngine;
use civitas_audio::meeting_streaming::MeetingStreamingConfig;
use civitas_audio::transcription::deepgram::DeepgramTranscriptionConfig;
use civitas_audio::transcription::VocabularyEntry;
use civitas_audio::vad::VadEngineEnum;
use civitas_config::{ChannelConfig, DbConfig};
use civitas_core::Language;
use civitas_screen::PipelineMetrics;
...
pub struct RecordingConfig {
    ...
    /// True only after versioned acceptance of the separately licensed Smart
    /// PII models. Both model workers are force-disabled when false.
    pub smart_pii_model_license_accepted: bool,
    /// Consumer builds normalize this compatibility field to `local`;
    /// both PII workers run entirely on-device.
    ...
    /// Speech transcription languages.
    pub languages: Vec<Language>,
    /// Screen OCR languages. This is deliberately independent from speech so
    /// multilingual screen content does not force an audio provider setting.
    ...
    /// Require authentication for API access. When true, all requests —
    /// localhost included — must include `Authorization: Bearer <key>`
    /// (or the authenticated WebSocket subprotocol fallback).
    pub api_auth: bool,

    /// The API key for this instance (explicit local env or encrypted store).
...
impl RecordingConfig {
    ...
    pub fn from_settings(
        ...
        civitas_config::set_record_while_locked(settings.record_while_locked);

        let smart_pii_model_license_accepted = settings.smart_pii_model_license_accepted_version
        ...
        Self {
            ...
            port: settings.port,
            data_dir,
            disable_audio: settings.effective_disable_audio(),
            capture_microphone,
            ...
            async_pii_redaction: settings.async_pii_redaction && smart_pii_model_license_accepted,
            async_image_pii_redaction: settings.async_image_pii_redaction
                && smart_pii_model_license_accepted,
            smart_pii_model_license_accepted,
    ...
    pub fn to_vision_manager_config(
        ...
        VisionManagerConfig {
            ...
            languages: self.ocr_languages.clone(),
...
mod tests {
    ...
    fn settings_with(api_auth: bool) -> civitas_config::RecordingSettings {
    ...
    fn build(s: &civitas_config::RecordingSettings) -> RecordingConfig {
        RecordingConfig::from_settings(s, std::path::PathBuf::from("/tmp/sp_test"), None)
    }

    #[test]
    fn api_auth_defaults_on() {
        let c = build(&civitas_config::RecordingSettings::default());
        assert!(c.api_auth, "api_auth defaults to true for safety");
    }

    #[test]
    fn explicit_api_auth_off_is_preserved_for_loopback_api() {
        let c = build(&settings_with(false));
        assert!(!c.api_auth);
    }

    #[test]
    fn independent_audio_source_consent_flows_into_runtime_config() {
        for (microphone, system_audio) in
            [(false, false), (true, false), (false, true), (true, true)]
        {
            let settings = civitas_config::RecordingSettings {
                disable_audio: false,
                capture_microphone: Some(microphone),
                capture_system_audio: Some(system_audio),
                ..Default::default()
            };
            let config = build(&settings);

            assert_eq!(config.capture_microphone, microphone);
            assert_eq!(config.capture_system_audio, system_audio);
            assert_eq!(config.disable_audio, !microphone && !system_audio);
        }
    }

    #[test]
    fn smart_pii_workers_require_current_model_license_acceptance() {
        let unaccepted = civitas_config::RecordingSettings {
            async_pii_redaction: true,
            async_image_pii_redaction: true,
            smart_pii_model_license_accepted_version: 0,
            ..Default::default()
        };
        let config = build(&unaccepted);
        assert!(!config.smart_pii_model_license_accepted);
        assert!(!config.async_pii_redaction);
        assert!(!config.async_image_pii_redaction);

        let accepted = civitas_config::RecordingSettings {
            smart_pii_model_license_accepted_version:
                civitas_config::SMART_PII_MODEL_LICENSE_VERSION,
            ..unaccepted
        };
        let config = build(&accepted);
        assert!(config.smart_pii_model_license_accepted);
        assert!(config.async_pii_redaction);
        assert!(config.async_image_pii_redaction);
    }
    ...
    fn privacy_capture_toggles_flow_to_ui_recorder_config() {
        ...
        assert_eq!(
            ...
            civitas_a11y::ExtractionThreadPriority::Lowest
        );
    ...
    fn vision_filters_and_capture_triggers_flow_to_vision_manager_config() {
        ...
        assert_eq!(
            vision.languages,
            vec![
                civitas_core::Language::ChineseSimplified,
                civitas_core::Language::ChineseTraditional,
            ]
        );
        assert_eq!(config.languages, vec![civitas_core::Language::English]);
    }

    #[test]
    fn legacy_settings_share_speech_languages_with_ocr() {
        let settings = civitas_config::RecordingSettings {
            languages: vec!["chinese-simplified".to_string()],
            ocr_languages: None,
            ..Default::default()
        };
        ...
        assert_eq!(
            ...
            vec![civitas_core::Language::ChineseSimplified]
        );
    }
