...
/// `casey+teams1234@work.example` and `casey@work.example` should both compare
/// as `casey`.
...
async fn deduplicate_existing_speaker_aliases(db: &civitas_db::DatabaseManager) {
...
pub fn start_speaker_identification(
    db: Arc<civitas_db::DatabaseManager>,
...
async fn auto_name_input_speaker(db: Arc<civitas_db::DatabaseManager>, user_name: &str) {
...
mod tests {
    ...
    fn test_names_match_email_single_name() {
        // Calendar stores an email while the speaker has a display name.
        assert!(names_match("casey@work.example", "Casey"));
        assert!(names_match("Casey", "casey@work.example"));
        // Reverse: both as emails
        assert!(names_match("casey@work.example", "casey@company.example"));
    }

    #[test]
    fn test_names_match_plus_addressed_email_aliases() {
        assert!(names_match(
            "casey+teams1234@work.example",
            "casey@work.example"
        ));
        assert!(names_match("Casey", "casey+teams1234@work.example"));
        assert!(names_match(
            "casey.rivera+teams1234@work.example",
            "Casey Rivera"
        ));
    }
    ...
    async fn setup_db() -> civitas_db::DatabaseManager {
        let db = civitas_db::DatabaseManager::new("sqlite::memory:", Default::default())
            .await
            .unwrap();
        sqlx::migrate!("../../crates/civitas-db/src/migrations")
            .run(&db.pool)
            .await
            .unwrap();
        db
    ...
    async fn seed_speaker(
        ...
        use civitas_db::{AudioDevice, DeviceType};
    ...
    async fn test_dedup_merges_email_and_display_name() {
        ...
        // Speaker A: already named with a display name.
        seed_speaker(&db, &embedding, Some("Casey")).await;

        // Speaker B: unnamed (will be named with an email by the calendar).
        ...
        db.update_speaker_name(id_b, "casey@work.example")
            .await
            .unwrap();

        // Run the dedup — should detect voice+name match and merge B into A
        deduplicate_speaker_by_name(&db, id_b, "casey@work.example").await;

        // After merge, the email-identity entry should be gone.
        let email_speaker = db.find_speaker_by_name("casey@work.example").await.unwrap();
        assert!(
            email_speaker.is_none(),
            "email identity should have been merged away"
        );

        // The display-name identity should still exist.
        let casey = db.find_speaker_by_name("Casey").await.unwrap();
        assert!(
            casey.is_some(),
            "display name should still exist after merge"
        );
    }
    ...
    async fn test_startup_alias_cleanup_merges_plus_addressed_duplicate() {
        ...
        let embedding: Vec<f32> = vec![0.1; 512];
        let id_a = seed_speaker(&db, &embedding, Some("casey@work.example")).await;
        let id_b = seed_speaker(&db, &embedding, Some("casey+teams1234@work.example")).await;

        deduplicate_existing_speaker_aliases(&db).await;

        let base = db.find_speaker_by_name("casey@work.example").await.unwrap();
        assert!(base.is_some(), "base email identity should be kept");

        let plus = db
            .find_speaker_by_name("casey+teams1234@work.example")
            .await
            .unwrap();
    ...
    async fn test_dedup_does_not_merge_different_voices() {
        ...
        let id_a = seed_speaker(&db, &embedding_a, Some("Casey")).await;
        ...
        let _ = db.find_speaker_by_name("Casey").await.unwrap();
