...
use civitas_db::{DatabaseManager, FrameData, OCREntry};
use dirs::cache_dir;
...
impl From<civitas_db::AudioEntry> for AudioEntry {
    fn from(db_entry: civitas_db::AudioEntry) -> Self {
        Self {
...
impl FrameCache {
    pub async fn new(civitas_dir: PathBuf, db: Arc<DatabaseManager>) -> Result<Self> {
        let cache_config = CacheConfig {
            cache_dir: cache_dir().unwrap().join("civitas").join("frames"),
            ..Default::default()
        };
...
async fn extract_frame(
    ...
    let mut cmd = civitas_core::ffmpeg_cmd_async(&ffmpeg);
    cmd.args([
        "-i",
        &video_file_path,
        "-vf",
