...
//!
//! Integrates civitas-a11y capture with the server's recording loop.

use anyhow::Result;
use civitas_a11y::{ExtractionThreadPriority, UiCaptureConfig, UiRecorder};
use civitas_core::window_pattern::{self, WindowPattern};
use civitas_db::{DatabaseManager, InsertUiEvent};
...
/// `LAST_AUDIO_CAPTURE` in civitas-audio.
...
pub async fn start_ui_recording(
    ...
    // patterns (see `civitas-core::window_pattern`).
        ...
        loop {
            ...
            match handle.recv_timeout(recv_timeout) {
                Some(event) => {
                    ...
                    let is_scroll = matches!(db_event.event_type, civitas_db::UiEventType::Scroll);
                    let trigger_kind =
...
fn should_record_input_event(
    ...
    match db_event.event_type {
        civitas_db::UiEventType::Key | civitas_db::UiEventType::Text => record_keyboard_events,
        civitas_db::UiEventType::Clipboard => record_clipboard_events,
        _ => true,
    }
}
...
fn capture_trigger_kind(
    ...
    match &db_event.event_type {
        civitas_db::UiEventType::AppSwitch => {
            if is_ignored {
                None
            } else {
        ...
        civitas_db::UiEventType::WindowFocus => {
            if is_ignored {
                None
            } else {
        ...
        civitas_db::UiEventType::Click => Some(
            target
        ...
        civitas_db::UiEventType::Clipboard => Some(CaptureTrigger::Clipboard),
        ...
        civitas_db::UiEventType::Text => Some(CaptureTrigger::TypingPause),
        ...
        civitas_db::UiEventType::Scroll => None,
        // Key events fire a KeyPress trigger even when privacy settings
        // suppress storing the key row.
        civitas_db::UiEventType::Key => Some(CaptureTrigger::KeyPress),
...
mod tests {
    ...
    #[test]
    fn default_recorder_does_not_persist_keyboard_or_clipboard_content() {
        let config = UiRecorderConfig::default();
        assert!(!config.enabled);
        assert!(!config.capture_text);
        assert!(!config.capture_clipboard);
        assert!(!config.capture_clipboard_content);
        assert!(!config.capture_on_clipboard);
        assert!(!config.record_keyboard_events);
        assert!(!config.record_clipboard_events);
    }
