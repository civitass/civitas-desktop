...
use anyhow::Result;
use chrono::Utc;
use civitas_a11y::tree::TreeWalkerConfig;
use civitas_a11y::ActivityFeed;
use civitas_capture::paired_capture::{paired_capture, CaptureContext, PairedCaptureResult};
use civitas_core::window_pattern::{self, WindowPattern};
use civitas_db::DatabaseManager;
use civitas_screen::capture_screenshot_by_window::{get_excluded_sck_window_ids, WindowFilters};
use civitas_screen::frame_comparison::{FrameComparer, FrameComparisonConfig};
use civitas_screen::monitor::{list_monitors, SafeMonitor};
use civitas_screen::snapshot_writer::SnapshotWriter;
use civitas_screen::utils::capture_monitor_image;
...
pub struct CaptureParams<'a> {
    ...
    pub languages: &'a [civitas_core::Language],
}
...
pub async fn event_driven_capture_loop(
    ...
    languages: Vec<civitas_core::Language>,
    ...
    let mut walk_budget = civitas_a11y::budget::AppWalkBudget::new();
    ...
    // then suppress until success. This keeps local diagnostics readable.
    ...
    loop {
        ...
        #[cfg(target_os = "macos")]
        if civitas_screen::stream_invalidation::take() {
            info!(
                ...
                match capture_result {
                    ...
                    Ok(Err(e)) => {
                        ...
                        if consecutive_capture_errors == 1 {
                            // First failure — preserve full detail in the local log.
                            error!(
...
fn resolve_capture_metadata(
    tree_snapshot: Option<&civitas_a11y::tree::TreeSnapshot>,
    trigger: &CaptureTrigger,
...
async fn do_capture(
    ...
    walk_budget: &mut civitas_a11y::budget::AppWalkBudget,
    ...
    let tree_walk_result = tokio::task::spawn_blocking(move || {
        civitas_capture::paired_capture::walk_accessibility_tree(&config)
    })
    .await?;
...
mod tests {
    ...
    #[test]
    fn default_config_keeps_clipboard_trigger_off() {
