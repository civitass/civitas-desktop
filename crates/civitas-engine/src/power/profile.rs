...
/// Below this much free space on the data-dir volume, all capture pauses
/// (FullPause) regardless of power state or user preference. Resumes
/// automatically when space is freed.
pub const DISK_CRITICAL_BYTES: u64 = 2 * 1024 * 1024 * 1024;

/// Below this much free space a warning is logged (and surfaced via the
/// power status API) but capture continues.
pub const DISK_LOW_BYTES: u64 = 5 * 1024 * 1024 * 1024;
...
impl PowerProfile {
    ...
    pub fn for_state(state: &PowerState, user_pref: PowerMode) -> Self {
        // Disk safety overrides everything, including the user's power
        // preference: capture onto a nearly-full disk fails DB writes and
        // can wedge the whole machine. The server stays up (search/timeline
        // keep working) and capture resumes automatically once the user
        // frees space. Missing telemetry (`None`) never pauses.
...
mod tests {
    ...
    #[test]
    fn test_disk_critical_forces_full_pause_in_every_mode() {
        // A nearly-full disk breaks the DB (failed writes, WAL corruption
        // risk) and the whole machine. Capture must stop no matter what the
        // user's power preference says — the server stays up for search.
        let state = PowerState {
            disk_free_bytes: Some(DISK_CRITICAL_BYTES - 1),
            ..PowerState::default()
        };
        for mode in [
            PowerMode::Auto,
            ...
            let profile = PowerProfile::for_state(&state, mode);
            assert_eq!(
                profile.name,
                ProfileName::FullPause,
                "disk critical must pause capture in mode {:?}",
                mode
            );
            assert!(profile.capture_paused);
        }
    ...
    #[test]
    fn test_disk_at_critical_floor_does_not_pause() {
        // Exactly at the floor = still allowed; only below pauses.
        let state = PowerState {
            disk_free_bytes: Some(DISK_CRITICAL_BYTES),
            ..PowerState::default()
        };
        let profile = PowerProfile::for_state(&state, PowerMode::Auto);
        assert_eq!(profile.name, ProfileName::Performance);
    ...
    #[test]
    fn test_unknown_disk_space_does_not_pause() {
        // No disk reading (e.g. data dir not resolvable) must never pause —
        // pausing capture on missing telemetry would be a silent data loss.
