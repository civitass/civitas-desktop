...
pub struct PowerState {
    ...
    /// Free bytes on the volume holding the data dir, or `None` when the
    /// data dir wasn't provided / the volume couldn't be resolved. Filled
    /// by the power manager loop, not by `poll_power_state` (which only
    /// reads OS power telemetry).
    #[serde(default)]
...
/// Free space on the volume containing `path`, best effort. Returns `None`
/// when no mounted disk is a prefix of the (canonicalized) path.
pub fn disk_free_space(path: &std::path::Path) -> Option<u64> {
    use sysinfo::{DiskExt, System, SystemExt};
    let canonical = path.canonicalize().unwrap_or_else(|_| path.to_path_buf());
    let mut sys = System::new();
    sys.refresh_disks_list();
    // refresh_disks_list() populates stats on sysinfo 0.29, but later
    // versions split list-refresh from data-refresh — keep both so a
    // version bump can't silently make available_space() return 0
    // (which would read as disk-critical and pause capture forever).
    sys.refresh_disks();
    sys.disks()
        .iter()
        .filter(|d| canonical.starts_with(d.mount_point()))
        // Longest mount-point prefix wins ("/" vs "/System/Volumes/Data").
