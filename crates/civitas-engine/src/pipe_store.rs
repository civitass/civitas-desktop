...
use anyhow::Result;
use chrono::Utc;
use civitas_core::pipes::{PipeExecution, PipeStore, SchedulerState};
use civitas_db::DatabaseManager;
use std::sync::Arc;

/// SQLite-backed pipe store using the main civitas database.
...
impl PipeStore for SqlitePipeStore {
    ...
    async fn set_execution_running(&self, id: i64, pid: Option<u32>) -> Result<()> {
        use civitas_db::write_queue::PipeBindValue;
...
mod tests {
    use super::*;
    use civitas_core::pipes::PipeStore;
    ...
    async fn test_create_execution() {
        ...
        let id = store
            .create_execution("my-pipe", "manual", "haiku", Some("civitas"))
            .await
            .unwrap();
        assert!(id > 0);
    }
    ...
    async fn test_full_execution_lifecycle() {
        ...
        // 1. Create
        let id = store
            .create_execution("lifecycle-pipe", "manual", "claude-haiku", Some("civitas"))
            .await
            .unwrap();
