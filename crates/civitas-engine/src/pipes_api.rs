...
use axum::Json;
use civitas_connect::connections::render_context_for;
...
pub async fn run_pipe_now(
    ...
    // Keep a copy so only these declared integrations can enter its prompt.
    let required_connections = mgr
        .get_pipe(&id)
        .await
        .map(|pipe| pipe.config.connections)
        .unwrap_or_default();
    {
        let required = &required_connections;
        if !required.is_empty() {
            ...
            for conn_id in required {
                let configured =
                    civitas_connect::connections::load_connection(ss, &civitas_dir, conn_id)
                        .await
    ...
    // Include only connections explicitly declared by this workflow. MCP
    // servers remain interactive-chat-only until workflows have per-server
    // grants; never append the user's global MCP catalog here.
    let civitas_dir = mgr
        .pipes_dir()
        .parent()
        .unwrap_or(mgr.pipes_dir())
    ...
    let ss = secret_store.as_ref().map(|e| e.0.as_ref());
    let conn_ctx = render_context_for(&civitas_dir, api_port, ss, &required_connections).await;
    mgr.set_connections_context(conn_ctx);

    let result = mgr.start_pipe_background(&id).await;
    mgr.clear_connections_context();
...
/// POST /pipes/install — install a workflow from a local file or directory.
...
pub async fn list_favorites(State(pm): State<SharedPipeManager>) -> Json<Value> {
    ...
    let favorites = civitas_core::pipes::favorites::load(&dir);
...
pub async fn set_pipe_favorite(
    ...
    let dir = mgr.pipes_dir().to_path_buf();
    drop(mgr);
    match civitas_core::pipes::favorites::set(&dir, &id, body.favorite) {
