...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "Continuously sync civitas memories into Claude Code so it has long-term context across every session. CLAUDE.md gets a tiny civitas-owned marker block that `@`-imports a sibling `civitas-memories.md` sidecar — the sidecar holds the full digest and is rewritten end-to-end on each sync, while CLAUDE.md stays small and hand-editable outside the marker block. Leave home_path empty to use the default (~/.claude). For per-project memory, point home_path at a specific project's directory containing CLAUDE.md.",
    fields: &[FieldDef {
        key: "home_path",
...
impl Integration for ClaudeCode {
    ...
    async fn test(
        ...
        let probe = path.join(".civitas-write-probe");
...
mod tests {
    ...
    async fn test_creates_missing_directory_and_reports_ready() {
        ...
        assert!(!target.join(".civitas-write-probe").exists());
    }
}
