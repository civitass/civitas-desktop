...
//!   `~/.civitas/mcp_servers.json` so it survives without the secret
//!   store.
...
use civitas_core::network::{
    authorize_external_process, effective_egress_policy, EgressPolicy, EgressPurpose,
};
...
fn authorize_mcp_url_with_policy(policy: EgressPolicy, value: &str) -> Result<()> {
    let url = reqwest::Url::parse(value)?;
    policy.authorize_url(EgressPurpose::ExternalMcp, &url)?;
    Ok(())
}

fn authorize_mcp_url(value: &str) -> Result<()> {
    authorize_mcp_url_with_policy(effective_egress_policy(), value)
}
...
async fn load_file(civitas_dir: &Path) -> Result<McpServersFile> {
...
async fn save_file(civitas_dir: &Path, file: &McpServersFile) -> Result<()> {
    let path = store_path(civitas_dir);
...
impl McpServerStore {
    ...
    pub async fn list(&self) -> Result<Vec<McpServerConfig>> {
        Ok(load_file(&self.civitas_dir).await?.servers)
    }

    pub async fn get(&self, id: &str) -> Result<Option<McpServerConfig>> {
        Ok(load_file(&self.civitas_dir)
            .await?
            .servers
            .into_iter()
            .find(|s| s.id == id))
    ...
    async fn start_oauth_with_config(
        ...
        authorize_mcp_url(&discovered.authorization_endpoint)?;
        authorize_mcp_url(&discovered.token_endpoint)?;
    ...
    pub async fn complete_oauth(&self, state: &str, code: &str) -> Result<String> {
        ...
        authorize_mcp_url(&pending.token_url)?;
    ...
    async fn register_oauth_client(
        ...
        authorize_mcp_url(registration_endpoint)?;
        let response = self
            .client
    ...
    async fn refresh_oauth_token(
        ...
        authorize_mcp_url(&current.token_url)?;
...
impl StdioSession {
    async fn spawn(command: &str, args: &[String], env: &HashMap<String, String>) -> Result<Self> {
        // A user-supplied subprocess is not a network-contained local
        // transport: it can open arbitrary sockets outside Civitas. Strict
        // Local-only mode therefore blocks stdio MCP until remote features are
        // explicitly enabled.
        authorize_external_process(EgressPurpose::ExternalMcp)?;
    ...
    if let Err(e) = session
        .send(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "civitas", "version": env!("CARGO_PKG_VERSION") },
            }),
            probe_timeout,
        )
        .await
...
async fn send_mcp_notification(
    ...
    if let Err(error) = authorize_mcp_url(url) {
        tracing::debug!(
            "[mcp] notification '{}' blocked by network policy: {}",
            method,
            error
        );
        return;
    ...
    if let Err(error) = authorize_mcp_url(url) {
        tracing::debug!(
            "[mcp] notification '{}' blocked before send: {}",
            method,
            error
        );
        return;
...
async fn send_jsonrpc(
    ...
    // Settings can change while a request is prepared. Re-check at the last
    // in-process boundary before reqwest opens the socket.
    authorize_mcp_url(url)?;
    let res = req
        .send()
        .await
...
pub async fn render_context(civitas_dir: &Path, api_port: u16) -> String {
    let file = match load_file(civitas_dir).await {
        Ok(f) => f,
        Err(e) => {
...
mod tests {
    ...
    #[test]
    fn local_only_mcp_policy_allows_loopback_and_rejects_remote_http() {
        let policy = EgressPolicy::new(NetworkMode::LocalOnly);
        assert!(authorize_mcp_url_with_policy(policy, "http://127.0.0.1:8787/mcp").is_ok());
        assert!(authorize_mcp_url_with_policy(policy, "https://mcp.example.com/v1").is_err());
    }
    ...
    async fn secret_store_round_trip() {
        ...
        let ss = Arc::new(SecretStore::new(pool, [42u8; 32]).await.unwrap());
