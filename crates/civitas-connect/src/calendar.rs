...
/// Thin wrapper around `eventkit::EventsManager` with civitas-specific additions.
...
impl CivitasCalendar {
    ...
    ///
    /// civitas only reads calendars/events, but EventKit does not offer a
    ...
    fn fetch_events_with_attendees(
        ...
        // here: civitas reads existing calendar events, attendees, URLs, and
...
impl Default for CivitasCalendar {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_list_calendars() {
        if CivitasCalendar::authorization_status() != AuthorizationStatus::FullAccess {
