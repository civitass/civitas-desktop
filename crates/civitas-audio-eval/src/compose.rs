...
//!
//! VoxConverse clips are short and clean. Real civitas sessions are long
    ...
    for (idx, segment) in template.segments.iter().enumerate() {
        ...
        match segment {
            ...
            Segment::Audio {
                ...
                let (raw, source_rate) = civitas_audio::pcm_decode(&source_path)
                    .with_context(|| format!("decode audio: {}", source_path.display()))?;
                let resampled = if source_rate != COMPOSE_SAMPLE_RATE {
                    civitas_audio::resample(&raw, source_rate, COMPOSE_SAMPLE_RATE).with_context(
                        || {
                            format!(
                                "resample {} {}->{}",
                                source_path.display(),
