...
//! Compose LibriSpeech utterances into civitas-shaped diarization fixtures.
//!
//! LibriSpeech gives us clean, redistributable single-speaker utterances. This
//! binary turns those into patterns that mirror civitas usage: meeting handoffs,
...
use civitas_audio_eval::{load_utterances, LibriUtterance, RttmSegment};
use clap::Parser;
...
#[derive(Parser, Debug)]
#[command(about = "Compose civitas-shaped diarization fixtures from LibriSpeech test-clean")]
struct Args {
...
fn decode_clip(utt: &LibriUtterance, max_seconds: Option<f64>) -> Result<Clip> {
    let (raw, source_rate) = civitas_audio::pcm_decode(&utt.flac_path)
        .with_context(|| format!("decode {}", utt.flac_path.display()))?;
    let mut samples = if source_rate != SAMPLE_RATE {
        civitas_audio::resample(&raw, source_rate, SAMPLE_RATE)?
    } else {
        raw
    };
...
fn rapid_handoffs(out_dir: &Path, clips: &[Vec<Clip>]) -> Result<FixtureInfo> {
    let mut builder = FixtureBuilder::new("civitas_meeting_rapid_handoffs");
...
fn background_day(out_dir: &Path, clips: &[Vec<Clip>], long_silence: f64) -> Result<FixtureInfo> {
    let mut builder = FixtureBuilder::new("civitas_background_24_7_day");
...
fn short_backchannels(out_dir: &Path, clips: &[Vec<Clip>]) -> Result<FixtureInfo> {
    let mut builder = FixtureBuilder::new("civitas_short_backchannels");
...
fn echo_leakage(out_dir: &Path, clips: &[Vec<Clip>]) -> Result<FixtureInfo> {
    let mut builder = FixtureBuilder::new("civitas_mic_system_echo_leakage");
...
fn overlap_crosstalk(out_dir: &Path, clips: &[Vec<Clip>]) -> Result<FixtureInfo> {
    ...
    let mut builder = FixtureBuilder::new("civitas_overlap_crosstalk");
