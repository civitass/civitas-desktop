...
//! hardware testing. It replays deterministic civitas-shaped WAV/RTTM
//! fixtures through the local diarization chain or Deepgram batch output, then
//! materializes the result into a fresh civitas SQLite DB and scores the
...
use civitas_audio::{pcm_decode, resample};
use civitas_audio_eval::{load_rttm, score_pipeline, RttmSegment};
use civitas_core::Language;
use civitas_db::{DatabaseManager, NewDiarizationSegment, ReplacementAudioTranscription};
...
#[derive(Parser, Debug)]
#[command(about = "Replay civitas-shaped audio through DB/search speaker attribution")]
struct Args {
    ...
    /// Deepgram behavior. `auto` runs only when DEEPGRAM_API_KEY is present.
    ...
    #[arg(long, default_value = "civitas_meeting_rapid_handoffs")]
...
async fn run_local_predictions(fixtures: &[Fixture]) -> Result<HashMap<String, PredictionSet>> {
    ...
    let audio_crate_dir = project_dir.parent().unwrap().join("civitas-audio");
...
async fn run_deepgram_prediction(args: &Args, fixtures: &[Fixture]) -> Result<PredictionSet> {
    ...
    if !has_direct {
        if args.deepgram == DeepgramMode::Required {
            bail!("deepgram required but DEEPGRAM_API_KEY is not set");
        }
        bail!("deepgram skipped: no DEEPGRAM_API_KEY");
    }
...
async fn materialize_and_score(
    ...
    let db_path = std::env::temp_dir().join(format!(
        "civitas-pipeline-replay-{}-{}-{}-{:?}-{:?}.sqlite",
        std::process::id(),
        scenario_index,
...
fn rows_to_hypothesis(rows: &[civitas_db::AudioResult]) -> Vec<RttmSegment> {
