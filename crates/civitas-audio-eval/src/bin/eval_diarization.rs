...
//!
//! Runs the civitas diarization chain (VAD → segmentation → embedding →
...
//!
//! Drives `prepare_segments` + `EmbeddingManager` directly via `civitas-audio`'s
...
#[derive(Parser, Debug)]
#[command(about = "Score civitas diarization against an RTTM reference")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    let (samples, source_rate) = civitas_audio::pcm_decode(&args.audio)?;
    ...
    // Models live in civitas-audio's models dir (kept there so the prod
    ...
    if !segmentation_model_path.exists() || !embedding_model_path.exists() {
        anyhow::bail!(
            "missing pyannote models at {} / {}. Run civitas once to download them, \
             or copy them into crates/civitas-audio/models/pyannote/.",
