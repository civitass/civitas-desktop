...
use civitas_audio::transcription::engine::TranscriptionEngine;
use civitas_audio::transcription::verified_models::{
    ensure_verified_model_downloaded, VerifiedModel,
};
use civitas_audio::transcription::whisper::model::{
    download_whisper_model, get_cached_whisper_model_path,
};
use civitas_audio_eval::{load_utterances, score_transcription, LibriUtterance};
use clap::Parser;
use serde::Serialize;
...
fn parse_engine(name: &str) -> Result<AudioTranscriptionEngine> {
    match name {
        ...
        // `base` is not a civitas engine variant; surface clearly rather
        // than silently substituting tiny.
        "base" | "whisper-base" => anyhow::bail!(
            "whisper-base is not a civitas engine variant; use tiny / whisper-large-v3 / whisper-large-v3-turbo / parakeet"
        ),
...
/// Every engine uses Civitas' revision-pinned, SHA-256-verified downloader;
/// the eval must never bypass the production model integrity boundary.
async fn prime_model(engine: &AudioTranscriptionEngine) -> Result<()> {
    match engine {
        AudioTranscriptionEngine::Parakeet => {
            eprintln!("ensuring verified Parakeet weights are available");
            tokio::task::spawn_blocking(|| {
                ensure_verified_model_downloaded(VerifiedModel::Parakeet)
            })
            .await
            .map_err(|e| anyhow::anyhow!("parakeet download task panicked: {e}"))?
            .context("download and verify Parakeet model")?;
            Ok(())
        }
        _ => {
...
async fn run_model(
    ...
    for (idx, utt) in utterances.iter().take(take).enumerate() {
        let (samples, source_rate) = civitas_audio::pcm_decode(&utt.flac_path)
            .with_context(|| format!("pcm_decode {}", utt.flac_path.display()))?;
        let samples_16k = if source_rate != 16_000 {
            civitas_audio::resample(&samples, source_rate, 16_000)?
        } else {
            samples
        };
        ...
        let wer = civitas_audio_eval::word_error_rate(&utt.reference_transcript, &hyp);
        let cer = civitas_audio_eval::char_error_rate(&utt.reference_transcript, &hyp);
        eprintln!(
            "[{}][{}/{}] {} ({:.2}s) wer={:.3} cer={:.3}",
            spec.name,
            idx + 1,
...
mod tests {
    ...
    fn base_alias_rejected_not_silently_remapped() {
        ...
        assert!(
            err.contains("not a civitas engine variant"),
