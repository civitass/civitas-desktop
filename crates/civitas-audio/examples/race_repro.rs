...
fn model_path() -> String {
    std::env::var("CIVITAS_SPEAKER_SEGMENTATION_MODEL").expect(
        "CIVITAS_SPEAKER_SEGMENTATION_MODEL must point to a checksum-verified segmentation model",
    )
}
...
fn main() {
    ...
    match civitas_audio::speaker::create_session(&model) {
            ...
            .map(|_| {
                ...
                thread::spawn(move || {
                    for _ in 0..per_thread {
                        let _ = civitas_audio::speaker::create_session(&m);
