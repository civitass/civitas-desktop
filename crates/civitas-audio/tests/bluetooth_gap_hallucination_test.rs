...
async fn whisper_hallucination_before_after() {
    ...
    use civitas_audio::transcription::whisper::model::create_whisper_context_parameters;
    use civitas_audio::transcription::whisper::model::get_cached_whisper_model_path;
    ...
    let model_path = get_cached_whisper_model_path(&engine_config)
        .expect("ggml-tiny.bin not found in cache — run civitas once to download it, then re-run with --ignored");
    ...
    let crackle_transcript = civitas_audio::transcription::whisper::batch::process_with_whisper(
        &crackle_audio,
    ...
    let silence_transcript = civitas_audio::transcription::whisper::batch::process_with_whisper(
        &silence_audio,
        vec![Language::English],
        &mut state,
        &[],
