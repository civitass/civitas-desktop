...
mod tests {
    ...
    use civitas_audio::speaker::segment::get_segments;
    use log::LevelFilter;
    ...
    async fn test_audio_speaker_identification() {
        ...
        let multiple_speakers_path =
            std::env::var_os("CIVITAS_DIARIZATION_MULTI_SPEAKER_FIXTURE")
                .map(std::path::PathBuf::from)
                .expect(
                    "CIVITAS_DIARIZATION_MULTI_SPEAKER_FIXTURE must point to a consented synthetic fixture",
                );
        assert!(multiple_speakers_path.is_file());

        let (mut multiple_speakers_data, multiple_speakers_sample_rate) =
            civitas_audio::pcm_decode(&multiple_speakers_path)
                .expect("Failed to decode audio file");

        // reesample if not 16000
        if multiple_speakers_sample_rate != 16000 {
            multiple_speakers_data = civitas_audio::resample(
        ...
        let repeat_speaker_path =
            std::env::var_os("CIVITAS_DIARIZATION_REPEAT_SPEAKER_FIXTURE")
                .map(std::path::PathBuf::from)
                .expect(
                    "CIVITAS_DIARIZATION_REPEAT_SPEAKER_FIXTURE must point to a consented synthetic fixture",
                );
        assert!(repeat_speaker_path.is_file());

        let (mut repeat_speaker_data, repeat_speaker_sample_rate) =
            civitas_audio::pcm_decode(&repeat_speaker_path).expect("Failed to decode audio file");

        // reesample if not 16000
        if repeat_speaker_sample_rate != 16000 {
            repeat_speaker_data =
                civitas_audio::resample(&repeat_speaker_data, repeat_speaker_sample_rate, 16000)
                    .unwrap();
        }

        multiple_speakers_data.extend_from_slice(&repeat_speaker_data);
