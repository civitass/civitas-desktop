...
/// Run them locally with: cargo test -p civitas-audio --test onnx_model_test -- --ignored
#[cfg(test)]
mod tests {
    use anyhow::Result;
    ...
    use civitas_audio::vad::silero::SileroVad;
    use civitas_audio::vad::VadEngine;
    use log::LevelFilter;
    ...
    async fn test_onnx_segmentation_model_loads() -> Result<()> {
        ...
        let start = Instant::now();
        let session = civitas_audio::speaker::create_session(&segmentation_model.path)?;
    ...
    async fn test_onnx_startup_sequence() -> Result<()> {
        ...
        let _segmentation_session =
            civitas_audio::speaker::create_session(&segmentation_model.path)?;
