...
mod tests {
    ...
    use civitas_audio::core::record_and_transcribe;
    use civitas_audio::core::stream::AudioStream;
    use civitas_audio::speaker::embedding::EmbeddingExtractor;
    use civitas_audio::speaker::embedding_manager::EmbeddingManager;
    use civitas_audio::speaker::models::{get_or_download_model, PyannoteModel};
    use civitas_audio::speaker::prepare_segments;
    use civitas_audio::vad::{silero::SileroVad, VadEngine};
    use civitas_audio::{pcm_decode, AudioInput, TranscriptionEngine};
    ...
    fn required_external_fixture(variable: &str) -> PathBuf {
        let path = std::env::var_os(variable).map(PathBuf::from).unwrap_or_else(|| {
            panic!(
                "{variable} must point to a consented synthetic audio fixture outside the repository"
            )
        });
        assert!(
            path.is_file(),
    ...
    async fn test_record_and_transcribe() {
        ...
        let result = record_and_transcribe(
            ...
            Arc::new(sender),
            is_running,
            Arc::new(civitas_audio::metrics::AudioPipelineMetrics::new()),
        )
        .await;
    ...
    async fn test_audio_transcription_language() {
        ...
        let audio_path = required_external_fixture("CIVITAS_AUDIO_FIXTURE_AR");
        let (audio_samples, audio_sample_rate) =
            civitas_audio::pcm_decode(&audio_path).expect("Failed to decode audio file");

        let audio_input = AudioInput {
            data: Arc::new(audio_samples),
        ...
        let segmentation_model_path = get_or_download_model(PyannoteModel::Segmentation)
            .await
            .expect("failed to obtain checksum-verified segmentation model")
            .path;
        let embedding_model_path = get_or_download_model(PyannoteModel::Embedding)
            .await
            .expect("failed to obtain checksum-verified embedding model")
            .path;
    ...
    async fn test_stt_speed() {
        ...
        let audio_path = required_external_fixture("CIVITAS_AUDIO_FIXTURE_EN");
        let (audio_samples, audio_sample_rate) =
            pcm_decode(&audio_path).expect("Failed to decode audio file");
    ...
    async fn test_audio_stream_stop_no_race_condition() {
        ...
        // See: https://github.com/civitass/civitas-desktop/issues/3261
        setup();
