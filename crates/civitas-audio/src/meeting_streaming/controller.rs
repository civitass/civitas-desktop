...
pub fn start_meeting_streaming_loop(
    ...
    tokio::spawn(async move {
        ...
        let mut started_sub =
            civitas_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_started");
        let mut ended_sub =
            civitas_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_ended");
        let mut delta_sub = civitas_events::subscribe_to_event::<MeetingTranscriptDelta>(
            "meeting_transcript_delta",
        );
        let mut final_sub = civitas_events::subscribe_to_event::<MeetingTranscriptFinal>(
            "meeting_transcript_final",
        );
        let mut error_sub =
            civitas_events::subscribe_to_event::<MeetingStreamingError>("meeting_streaming_error");
...
async fn start_streaming_session(
    ...
    *active = Some(ActiveMeetingStream {
    ...
    let _ = civitas_events::send_event("meeting_streaming_session_started", started);
}
...
fn emit_session_ended(session: ActiveMeetingStream) {
    ...
    let _ = civitas_events::send_event("meeting_streaming_session_ended", ended);
}
...
fn emit_status(
    ...
    let _ = civitas_events::send_event("meeting_streaming_status_changed", status);
}
...
async fn readiness_error(
    ...
    match config.provider {
        ...
        MeetingStreamingProvider::DeepgramLive => {
            Some("Deepgram live transcription needs your Deepgram API key".to_string())
        }
    }
}
...
mod tests {
    ...
    // `check_and_emit_stall_notifications` calls `civitas_events::send_event`,
    ...
    async fn live_not_ready_session_keeps_background_recording_enabled() {
        ...
        let transcription_engine = Arc::new(RwLock::new(None));
        let mut active = None;
        let config = MeetingStreamingConfig::from_settings(true, "deepgram-live", None, None, None);
    ...
    #[tokio::test]
    async fn selected_deepgram_uses_direct_byok_live_when_key_available() {
