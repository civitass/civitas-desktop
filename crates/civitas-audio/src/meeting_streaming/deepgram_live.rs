...
async fn run_stream(
    ...
    request.headers_mut().insert(
        USER_AGENT,
        HeaderValue::from_static("civitas-meeting-streaming-deepgram"),
    );
    ...
    loop {
        tokio::select! {
            maybe_frame = rx.recv() => {
                ...
                if let Err(error) = civitas_core::network::authorize_url(
                    civitas_core::network::EgressPurpose::RemoteTranscription,
                    url.as_str(),
                ) {
                    warn!(
                        "meeting streaming: network policy closed Deepgram live stream: {}",
                        error
                    );
                    break;
                }
    ...
    let remote_still_allowed = civitas_core::network::authorize_url(
        civitas_core::network::EgressPurpose::RemoteTranscription,
        url.as_str(),
    )
    .is_ok();
    if remote_still_allowed {
        let _ = write
    ...
    drop(write);
    if !remote_still_allowed {
        // Once Local-only is restored, do not leave a remote socket or its
        // receive task alive while waiting for the provider to close it.
        reader.abort();
    }
    let _ = reader.await;
...
fn handle_results_event(
    ...
    if is_final {
        ...
        let _ = civitas_events::send_event("meeting_transcript_final", event);
    } else {
        ...
        let _ = civitas_events::send_event("meeting_transcript_delta", event);
    }
}
...
fn emit_error(
    ...
    let _ = civitas_events::send_event("meeting_streaming_error", event);
}
...
mod tests {
    ...
    /// Decodes an explicitly supplied synthetic fixture into the linear16 PCM
    /// bytes Deepgram expects, capped to ~10s so the live test stays quick.
    fn fixture_pcm() -> Option<Vec<u8>> {
        let path = std::env::var_os("CIVITAS_DEEPGRAM_AUDIO_FIXTURE")?;
        let reader = hound::WavReader::open(&path).unwrap_or_else(|error| {
            panic!(
                "failed to open CIVITAS_DEEPGRAM_AUDIO_FIXTURE {}: {error}",
    ...
    async fn stream_outcome(
        ...
        request.headers_mut().insert(
            USER_AGENT,
            HeaderValue::from_static("civitas-meeting-streaming-deepgram-test"),
        );
        ...
        for chunk in fixture.chunks(3200) {
            write
    ...
    async fn deepgram_live_streaming_accepts_language_params() {
        let Some(api_key) = std::env::var("CIVITAS_MEETING_DEEPGRAM_API_KEY")
            .ok()
            ...
            eprintln!(
                "skipping deepgram live smoke test: set CIVITAS_MEETING_DEEPGRAM_API_KEY (or DEEPGRAM_API_KEY) to run it"
            );
            return;
        };
        let Some(fixture) = fixture_pcm() else {
            eprintln!(
                "skipping deepgram live smoke test: set CIVITAS_DEEPGRAM_AUDIO_FIXTURE to a consented synthetic English WAV"
            );
            return;
        };

        for language in ["multi", "es"] {
            let (saw_transcript, error) = stream_outcome(&api_key, language, &fixture).await;
            assert!(
                error.is_none(),
