...
use civitas_db::{DatabaseManager, NewDiarizationSegment, Speaker};
...
pub async fn process_transcription_result(
    ...
    if result.error.is_some() || result.transcription.is_none() {
        error!("audio transcription failed; no audio result was inserted");
        return Ok(None);
    }
            ...
            .insert_audio_chunk_and_transcription(
                ...
                &civitas_db::AudioDevice {
                    ...
                    device_type: match result.input.device.device_type {
                        crate::core::device::DeviceType::Input => civitas_db::DeviceType::Input,
                        crate::core::device::DeviceType::Output => civitas_db::DeviceType::Output,
                    },
                },
                speaker_id,
            ...
            .await
        {
            ...
            Err(e) => {
                if retry < 2 {
                    ...
                    // Keep device as a structured field so local log tooling can
                    // group failures across devices.
                    error!(
