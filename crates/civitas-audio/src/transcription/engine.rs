...
use crate::transcription::verified_models::load_verified_mlx_cache;
#[cfg(any(feature = "qwen3-asr", feature = "parakeet"))]
use crate::transcription::verified_models::{
    load_verified_model, spawn_verified_download, VerifiedModel,
};
        ...
        match *config {
            ...
            AudioTranscriptionEngine::Qwen3Asr => {
                #[cfg(feature = "qwen3-asr")]
                {
                    ...
                    let load_result = tokio::task::spawn_blocking(|| {
                        load_verified_model(VerifiedModel::Qwen3Asr)
                    })
                    .await
                    ...
                    match load_result {
                        ...
                        None => {
                            warn!(
                                "verified qwen3-asr weights are not cached; transcription is disabled until the revision-pinned download completes"
                            );
                            spawn_verified_download(VerifiedModel::Qwen3Asr);
                            Ok(Self::Disabled)
                        }
                    }
                }
            ...
            AudioTranscriptionEngine::Parakeet => {
                #[cfg(feature = "parakeet")]
                {
                    ...
                    let load_result = tokio::task::spawn_blocking(|| {
                        load_verified_model(VerifiedModel::Parakeet)
                    })
                    .await
                    ...
                    match load_result {
                        ...
                        None => {
                            warn!(
                                "verified parakeet weights are not cached; transcription is disabled until the revision-pinned download completes"
                            );
                            spawn_verified_download(VerifiedModel::Parakeet);
                            Ok(Self::Disabled)
                        }
                    }
                }
            ...
            AudioTranscriptionEngine::ParakeetMlx => {
                #[cfg(feature = "parakeet-mlx")]
                {
                    ...
                    let load_result = tokio::task::spawn_blocking(load_verified_mlx_cache)
                        .await
                        .map_err(|e| {
                            anyhow!("parakeet-mlx model loading task panicked: {}", e)
                        })??;
                    match load_result {
                        Some(model) => {
                        ...
                        None => {
                            warn!(
                                "Parakeet MLX is not present as a verified local cache; no automatic download was started"
                            );
                            Ok(Self::Disabled)
                        }
                    }
        ...
        let transcription = match self {
            ...
            Self::Deepgram {
                ...
                if rms < 0.002 {
                    tracing::debug!(
                        rms,
                        "skipping Deepgram because audio is below the silence threshold"
                    );
                    Ok(String::new())
                } else {
            ...
            Self::OpenAICompatible {
                ...
                .await
                {
                    Ok(t) => Ok(t),
                    Err(e) => {
                        error!("OpenAI-compatible transcription failed");
                        Err(e)
                    }
                }
            }
