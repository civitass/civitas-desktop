        ...
        if let Some(ref client) = self.client {
            ...
            let client = Arc::new(
                Client::builder()
                    ...
                    // A loopback-compatible endpoint must not be able to
                    // redirect captured audio to a remote destination after
                    // the local-only preflight.
...
pub async fn stt(
        ...
        == AudioTranscriptionEngine::Disabled
    {
        ...
        if rms < 0.002 {
            ...
            .await
            {
                ...
                Err(_) => {
                    error!("Deepgram transcription failed; falling back to local Whisper");
                    // Fallback to Whisper
        ...
        || *audio_transcription_engine == AudioTranscriptionEngine::Parakeet
    {
        ...
        .await
        {
            Ok(transcription) => Ok(transcription),
            Err(_) => {
                error!("OpenAI-compatible transcription failed; falling back to local Whisper");
                // Fallback to Whisper
                process_with_whisper(audio, languages.clone(), whisper_state, vocabulary).await
            }
        }
...
pub async fn run_stt(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("speech-to-text failed; the segment was omitted");
            Ok(TranscriptionResult {
                input: AudioInput {
                ...
                path,
                timestamp,
                error: Some(content_free_transcription_error(&e).to_string()),
...
fn content_free_transcription_error(error: &anyhow::Error) -> &'static str {
    if error
...
mod tests {
    ...
    #[test]
    fn persisted_transcription_errors_never_echo_provider_content() {
        let error = anyhow::anyhow!(
            "https://provider.example/private?key=secret returned a private transcript"
        );
        let persisted = content_free_transcription_error(&error);
        assert_eq!(persisted, "transcription failed");
