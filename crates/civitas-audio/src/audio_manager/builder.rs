...
use anyhow::Result;
use civitas_config::ChannelConfig;
...
pub(crate) fn retain_consented_audio_sources(options: &mut AudioManagerOptions) {
    let capture_microphone = options.capture_microphone;
    let capture_system_audio = options.capture_system_audio;
    options
        .enabled_devices
        .retain(|name| match parse_audio_device(name) {
            Ok(device) => match device.device_type {
                DeviceType::Input => capture_microphone,
                DeviceType::Output => capture_system_audio,
            },
            Err(error) => {
                tracing::warn!("dropping unparseable configured audio device '{name}': {error}");
                false
            }
        });
}

/// When following system defaults, ensure each consented source type appears
/// in `enabled_devices`. Skips a source class the owner disabled and a device
/// explicitly muted via tray/API (`user_disabled_devices`).
pub(crate) async fn ensure_system_default_device_types(
    ...
    if options.capture_system_audio && !has_output {
    ...
    if options.capture_microphone && !has_input {
...
pub struct AudioManagerOptions {
    ...
    /// Allow input-device (microphone) capture.
    pub capture_microphone: bool,
    /// Allow output-device (system audio) capture.
...
impl AudioManagerBuilder {
    ...
    pub fn capture_microphone(mut self, capture_microphone: bool) -> Self {
        self.options.capture_microphone = capture_microphone;
        self
    }

    pub fn capture_system_audio(mut self, capture_system_audio: bool) -> Self {
        self.options.capture_system_audio = capture_system_audio;
        self
    }
    ...
    pub async fn build_options(&mut self) -> Result<AudioManagerOptions> {
        ...
        retain_consented_audio_sources(options);
...
mod consent_tests {
    ...
    fn retained(capture_microphone: bool, capture_system_audio: bool) -> HashSet<String> {
        let mut options = AudioManagerOptions {
            capture_microphone,
            capture_system_audio,
            enabled_devices: HashSet::from([
                "Studio microphone (input)".to_string(),
                "Meeting output (output)".to_string(),
            ]),
            ..Default::default()
        };
        retain_consented_audio_sources(&mut options);
        options.enabled_devices
    }

    #[test]
    fn manager_options_retain_only_consented_source_classes() {
        assert!(retained(false, false).is_empty());
        assert_eq!(
            retained(true, false),
            HashSet::from(["Studio microphone (input)".to_string()])
        );
        assert_eq!(
            retained(false, true),
            HashSet::from(["Meeting output (output)".to_string()])
        );
        assert_eq!(
            retained(true, true),
            HashSet::from([
