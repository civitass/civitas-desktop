        ...
        loop {
            if audio_manager.status().await == AudioManagerStatus::Running {
                ...
                if audio_manager.use_system_default_audio().await {
                    let capture_microphone = audio_manager.capture_microphone().await;
                    let capture_system_audio = audio_manager.capture_system_audio().await;
                        ...
                        if capture_microphone {
                            if let Ok(default_input) = default_input_device() {
                                ...
                                let has_correct_input = parse_audio_device(&default_input_name)
                                    .ok()
                                    .is_some_and(|d| {
                                        is_device_actively_streaming(&device_manager, &d)
                                    });
                                    ...
                                    for device_name in current.iter() {
                                        if let Ok(device) = parse_audio_device(device_name) {
                                            if device.device_type == DeviceType::Input {
                                                let _ =
                                                    audio_manager.stop_device(device_name).await;
                                            }
                                        }
                                    }
                        ...
                        if capture_system_audio {
                            if let Ok(default_output) = default_output_device().await {
                                ...
                                let has_correct_output =
                                    parse_audio_device(&default_output_name).ok().is_some_and(
                                        |d| is_device_actively_streaming(&device_manager, &d),
                                    );
                    ...
                    if let Some(new_default_input) = if capture_microphone {
                        default_tracker.check_input_changed()
                    } else {
                        None
                    } {
                        if audio_manager
                    ...
                    if let Some(new_default_output) = if capture_system_audio {
                        ...
                        && !currently_available_devices.contains(&device)
                    {
                        ...
                        match audio_manager.start_device(&device).await {
                            ...
                            Err(e) => {
                                ...
                                // fired") — keep this at warn; the monitor retries on the next
                                // two-second tick.
                                ...
                                // floods the local log with identical permission noise. Warn and
                                // let the next tick try again so we pick up the moment the user
                                // grants permission.
...
async fn run_pinned_input_fallback_sweep(
    ...
    use civitas_events::AudioDeviceFallbackEvent;
...
mod tests {
    ...
    // disabled their built-in mic from the civitas-side enabled set; macOS
    // default input is "MacBook Pro Microphone (input)" (NOT user-disabled —
    // user just didn't pin it in civitas).
