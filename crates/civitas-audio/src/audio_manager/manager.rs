...
/// corrupt, every audio chunk fails with the same error. Repeating the full
/// entry on every chunk makes the local diagnostic log unusable; once every
/// five minutes is enough to preserve the problem. Below-threshold hits still
/// go to debug!().
///
...
/// the local log during a sustained failure.
...
/// Mirrors the MeetingEvent struct from civitas-events for deserialization.
...
/// Transition `status` to `Running`, run the init future, and roll back to
/// `Stopped` if it fails. This keeps the manager's reported status honest: a
/// failed start must never leave a phantom `Running` (no receiver handles, no
/// device monitor) that makes the app — and the user — believe audio is being
/// captured when it is not. Extracted as a free function so the invariant is
/// unit-testable without standing up a full AudioManager (DB + models).
async fn run_start_guarded<F, Fut>(status: &Arc<RwLock<AudioManagerStatus>>, init: F) -> Result<()>
where
    F: FnOnce() -> Fut,
    Fut: std::future::Future<Output = Result<()>>,
{
    *status.write().await = AudioManagerStatus::Running;
    match init().await {
        Ok(()) => Ok(()),
        Err(e) => {
            *status.write().await = AudioManagerStatus::Stopped;
            Err(e)
        }
    }
}
...
impl AudioManager {
    pub async fn new(options: AudioManagerOptions, db: Arc<DatabaseManager>) -> Result<Self> {
        ...
        if options.windows_input_aec_enabled {
            info!("civitas-audio: Windows WASAPI microphone AEC enabled in settings");
        }
        if options.macos_input_vpio_enabled {
            info!(
                "civitas-audio: macOS VoiceProcessingIO (AEC) enabled in settings (default input only)"
            );
        }
    ...
    pub async fn apply_options(&self, options: AudioManagerOptions) -> Result<()> {
        ...
        *self.meeting_detector.write().await = options.meeting_detector.clone();
        ...
        super::builder::retain_consented_audio_sources(&mut options);
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        // Flip to Running, run init, and roll back to Stopped if init fails.
        // Without the rollback, a failing start_internal() (model load, device
        // enumeration, monitor start) would leave status == Running with every
        // receiver handle None — audio dead while the manager reports recording.
        run_start_guarded(&self.status, || self.start_internal()).await
    }
    ...
    pub async fn start_device(&self, device: &AudioDevice) -> Result<()> {
        let source_allowed = {
            ...
            match &device.device_type {
                crate::core::device::DeviceType::Input => options.capture_microphone,
                crate::core::device::DeviceType::Output => options.capture_system_audio,
            }
        };
        if !source_allowed {
            return Err(anyhow!(
                "{} capture is disabled by the source-level audio policy",
                match &device.device_type {
                    crate::core::device::DeviceType::Input => "microphone",
                    crate::core::device::DeviceType::Output => "system audio",
                }
            ));
        }
    ...
    pub async fn capture_microphone(&self) -> bool {
        self.options.read().await.capture_microphone
    }

    pub async fn capture_system_audio(&self) -> bool {
        self.options.read().await.capture_system_audio
    }
                ...
                let persisted_file_path = if let Some(ref out) = output_path {
                    ...
                    match write_result {
                        Ok(Ok(())) => {
                            ...
                            if !inserted {
                                // Keep the path structured so local log tooling can
                                // group the issue across different devices.
                                error!(
    ...
    pub async fn languages(&self) -> Vec<civitas_core::Language> {
...
async fn run_meeting_speaker_constraint_loop(
    ...
    let mut meeting_sub = civitas_events::subscribe_to_event::<MeetingEventData>("meeting_started");
    let mut ended_sub = civitas_events::subscribe_to_event::<MeetingEventData>("meeting_ended");
...
mod tests {
    ...
    // ── start() status-honesty invariant ───────────────────────
    // Regression: AudioManager::start() used to flip status → Running
    // *before* start_internal() ran, with no rollback on failure. A
    // failing init (model load, device enumeration, monitor start) left
    // status == Running while every receiver handle was None — audio dead
    // but the manager (and the UI) reported "recording". These tests pin
    // the contract that status reflects reality after start_guarded().

    #[tokio::test]
    async fn test_start_guarded_rolls_back_status_on_init_failure() {
        let status = Arc::new(RwLock::new(AudioManagerStatus::Stopped));

        let result = run_start_guarded(&status, || async {
            // Simulate start_internal() failing partway through init.
            Err(anyhow::anyhow!("model load failed"))
        })
        .await;

        assert!(result.is_err(), "start_guarded must surface the init error");
        assert_eq!(
            *status.read().await,
            AudioManagerStatus::Stopped,
            "status must roll back to Stopped when init fails — never leave a phantom Running"
        );
    }

    #[tokio::test]
    async fn test_start_guarded_marks_running_on_success() {
        let status = Arc::new(RwLock::new(AudioManagerStatus::Stopped));

        let result = run_start_guarded(&status, || async { Ok(()) }).await;

        assert!(result.is_ok());
        assert_eq!(
            *status.read().await,
            AudioManagerStatus::Running,
            "status must be Running after a successful start"
        );
    }
}
