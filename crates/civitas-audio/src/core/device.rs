...
impl From<civitas_db::DeviceType> for DeviceType {
    fn from(device_type: civitas_db::DeviceType) -> Self {
        match device_type {
            civitas_db::DeviceType::Input => DeviceType::Input,
            civitas_db::DeviceType::Output => DeviceType::Output,
        }
    }
}

impl From<DeviceType> for civitas_db::DeviceType {
    fn from(device_type: DeviceType) -> Self {
        match device_type {
            DeviceType::Input => civitas_db::DeviceType::Input,
            DeviceType::Output => civitas_db::DeviceType::Output,
        }
    }
}

impl From<DeviceType> for Arc<civitas_db::DeviceType> {
    fn from(device_type: DeviceType) -> Self {
...
impl From<civitas_db::AudioDevice> for AudioDevice {
    fn from(device: civitas_db::AudioDevice) -> Self {
        AudioDevice {
            name: device.name,
...
///
/// Without this filter the device monitor polls every two seconds and floods
/// the user's local log. The user also sees the phantom in the settings
/// dropdown and can pick it, only to silently get no audio. Strict-empty
/// signal — we don't blocklist by name — so it stays safe across hardware
/// revisions and locales.
#[cfg(not(all(target_os = "linux", feature = "pulseaudio")))]
...
fn source_capture_allowed(
    device_type: &DeviceType,
    capture_microphone: bool,
    capture_system_audio: bool,
) -> bool {
    match device_type {
        DeviceType::Input => capture_microphone,
        DeviceType::Output => capture_system_audio,
    }
}
...
/// bare `"default"` sentinel, or `use_system_default_audio`), enroll the
/// current default device for each independently consented source class.
/// Otherwise parse explicit device names and drop disallowed source classes.
pub async fn resolve_audio_devices_for_capture(
    ...
    if should_resolve_to_system_default_audio(configured, use_system_default_audio) {
        ...
        if devices.is_empty() {
            tracing::warn!(
                "resolve_audio_devices_for_capture: no consented default audio source found \
                 (use_system_default_audio={}, capture_microphone={}, \
                 capture_system_audio={}, configured={:?})",
                use_system_default_audio,
                capture_microphone,
...
mod consent_tests {
    ...
    fn resolve_explicit(capture_microphone: bool, capture_system_audio: bool) -> Vec<String> {
        let configured = vec![
            "Studio microphone (input)".to_string(),
            "Meeting output (output)".to_string(),
        ];
    ...
    #[test]
    fn source_consent_filters_all_four_audio_combinations() {
        assert!(resolve_explicit(false, false).is_empty());
        assert_eq!(
            resolve_explicit(true, false),
            vec!["Studio microphone (input)"]
        );
        assert_eq!(
            resolve_explicit(false, true),
            vec!["Meeting output (output)"]
        );
        assert_eq!(
            resolve_explicit(true, true),
            vec!["Studio microphone (input)", "Meeting output (output)"]
        );
    }
}
