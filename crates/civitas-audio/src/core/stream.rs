...
impl AudioStream {
    ...
    async fn start_cpal_stream(
        ...
        #[cfg(target_os = "macos")]
        {
            if macos_input_vpio && input_vpio {
                ...
                info!(
                    device = %device,
                    "civitas-audio: enabling VoiceProcessingIO (AEC) on default microphone"
                );
                ...
                if vpio_rate != original_rate {
                    info!(
                        ...
                        original_rate,
                        vpio_rate,
                        "civitas-audio: VPIO config rate differs from originally-picked config; \
                ...
                info!(
                    ...
                    channels = config.channels(),
                    "civitas-audio: using default input config for VoiceProcessingIO"
                );
            } else if macos_input_vpio && device.device_type == super::device::DeviceType::Input {
                info!(
                    device = %device,
                    "civitas-audio: macOS VPIO requested but using HAL (only system default input supports VoiceProcessingIO)"
                );
            }
        }
    ...
    async fn spawn_audio_thread(
        ...
        Ok(tokio::task::spawn_blocking(move || {
            ...
            // surfaced as `OS error -2004287480`) — CIVITAS-CLI-S2.
            ...
            if let Some(stream) = stream {
                ...
                if use_vpio {
                    info!(
                        device = %device_name,
                        "civitas-audio: VoiceProcessingIO microphone capture running (AEC initialized)"
                    );
                }
                #[cfg(target_os = "windows")]
                if use_aec {
                    info!(
                        device = %device_name,
                        "civitas-audio: WASAPI microphone capture running with AEC enabled"
                    );
                }
