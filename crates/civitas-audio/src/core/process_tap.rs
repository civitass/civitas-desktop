...
mod exclusions {
    ...
    pub const ENV_OVERRIDE: &str = "CIVITAS_AUDIO_EXCLUSIONS_PATH";
    pub const DEFAULT_RELATIVE_PATH: &str = ".civitas/audio-exclusions.json";
    ...
    mod tests {
        ...
        fn missing_file_returns_empty() {
            let (ids, mt) = read_bundle_ids(Path::new(
                "/nonexistent/civitas-audio-exclusion-test/path.json",
            ));
            assert!(ids.is_empty());
            assert!(mt.is_none());
        }
