...
use civitas_a11y::tree::{create_tree_walker, TreeSnapshot, TreeWalkerConfig};
use civitas_core::pii_removal::remove_pii;
use civitas_db::DatabaseManager;
use civitas_screen::snapshot_writer::SnapshotWriter;
...
pub struct CaptureContext<'a> {
    ...
    pub languages: Vec<civitas_core::Language>,
...
pub async fn paired_capture(
    ...
    let app_prefers_ocr = ctx.app_name.is_some_and(|name| {
        ...
        // correctly. See https://github.com/civitass/civitas-desktop/issues/2685
        n.contains("wezterm")
...
pub fn walk_accessibility_tree(config: &TreeWalkerConfig) -> civitas_a11y::tree::TreeWalkResult {
    use civitas_a11y::tree::TreeWalkResult;
    let walker = create_tree_walker(config.clone());
    match walker.walk_focused_window() {
        Ok(TreeWalkResult::Found(snapshot)) => {
            debug!(
                "tree walk: found text_len={}, nodes={}, structured_nodes={}, has_url={}, has_document={}, dur={:?}",
                snapshot.text_content.len(),
                snapshot.node_count,
                snapshot.nodes.len(),
                snapshot.browser_url.is_some(),
                snapshot.document_path.is_some(),
...
fn a11y_content_is_thin(
    snap: &civitas_a11y::tree::TreeSnapshot,
    ...
    if let Some(win) = window_name {
        ...
        {
            debug!("a11y_content_is_thin: known canvas window");
            return true;
        }
    }
    ...
    if let Some(url) = browser_url {
        ...
        {
            debug!("a11y_content_is_thin: known canvas URL");
            return true;
        }
        ...
        if MEETING_URL_PATTERNS
            ...
            .any(|pat| url_lower.contains(pat))
        {
            debug!("a11y_content_is_thin: meeting URL");
            return true;
        }
    }
    ...
    if let Some(app) = app_name {
        ...
        {
            debug!("a11y_content_is_thin: meeting app");
            return true;
        }
    }
...
fn sanitize_ocr_text_json(text_json: &str) -> String {
    ...
    let sanitized = civitas_core::pii_removal::remove_pii_from_text_json(&entries);
...
mod tests {
    ...
    async fn test_paired_capture_with_accessibility_text() {
        ...
        let snap = TreeSnapshot {
            ...
            simhash: 67890,
            truncated: false,
            truncation_reason: civitas_a11y::tree::TruncationReason::None,
            max_depth_reached: 0,
        };
    ...
    fn test_sanitize_ocr_text_json_removes_emails() {
        let json = r#"[{"text":"contact casey@work.example for info","x":"10","y":"20"}]"#;
        let result = sanitize_ocr_text_json(json);
        assert!(
            !result.contains("casey@work.example"),
    ...
    fn test_sanitize_ocr_text_json_multiple_entries() {
        let fake_key = concat!("sk-", "1234567890abcdef1234567890abcdef");
        let json = format!(
            r#"[{{"text":"user@example.com","x":"0","y":"0"}},{{"text":"safe text","x":"1","y":"1"}},{{"text":"key: {fake_key}","x":"2","y":"2"}}]"#
        );
        let result = sanitize_ocr_text_json(&json);
        assert!(
    ...
    fn test_pii_removal_on_text() {
        ...
        let text = "Contact casey@work.example or casey.rivera@personal.example for support";
        let sanitized = remove_pii(text);
        assert!(
            !sanitized.contains("casey@work.example"),
            "email 1 should be redacted"
        );
        assert!(
            !sanitized.contains("casey.rivera@personal.example"),
    ...
    fn test_pii_removal_disabled() {
        ...
        let text = "casey@work.example";
        let sanitized_text: Option<String> = Some(text).map(|t| t.to_string());
        assert_eq!(sanitized_text.as_deref(), Some("casey@work.example"));
    }
