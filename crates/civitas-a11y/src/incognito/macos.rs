...
impl MacOSIncognitoDetector {
    ...
    fn cached_incognito_result(&self, app_name: &str, window_title: &str) -> Option<bool> {
    ...
    fn check_with_cache(&self, app_name: &str, window_title: &str) -> Option<bool> {
        if let Some(result) = self.cached_incognito_result(app_name, window_title) {
            return Some(result);
        }
...
mod tests {
    ...
    fn test_cache_different_app_misses() {
        ...
        assert_eq!(
            detector.cached_incognito_result("Brave Browser", "Dog"),
            None
        );
    }
    ...
    fn test_cache_expiry() {
        ...
        assert_eq!(
            detector.cached_incognito_result("Google Chrome", "Dog"),
            None
        );
    }
}
