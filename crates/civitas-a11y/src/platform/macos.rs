...
fn check_clipboard_crash_marker() {
    CLIPBOARD_CRASH_CHECK.call_once(|| {
        let dir = civitas_core::paths::default_civitas_data_dir();
...
fn get_clipboard() -> Option<String> {
    ...
    let result = cidre::dispatch::Queue::main().sync_once(|| {
        ...
        non_empty_clipboard_text(clipboard.get_text().ok())
    });
...
fn non_empty_clipboard_text(text: Option<String>) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn test_clipboard_text_is_retained_without_touching_system_pasteboard() {
        let test_text = "civitas_clipboard_test_12345";
        assert_eq!(
            non_empty_clipboard_text(Some(test_text.to_string())),
            Some(test_text.to_string())
        );
    }

    #[test]
    fn test_clipboard_empty_or_unavailable_returns_none() {
        assert_eq!(non_empty_clipboard_text(Some(String::new())), None);
        assert_eq!(non_empty_clipboard_text(None), None);
    }

    #[test]
    fn test_clipboard_unicode_is_retained() {
        let unicode_text = "日本語テスト 🎉 émojis ñ";
        assert_eq!(
            non_empty_clipboard_text(Some(unicode_text.to_string())),
            Some(unicode_text.to_string())
        );
    }

    #[test]
    fn test_clipboard_large_content_is_retained() {
        let large_text: String = "x".repeat(100_000);
        assert_eq!(
            non_empty_clipboard_text(Some(large_text.clone())),
            Some(large_text)
        );
    }
    ...
    /// verbatim in the focused app's text field after civitas quit.
    #[test]
