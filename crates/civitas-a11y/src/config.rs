...
pub struct UiCaptureConfig {
    ...
    /// Supports `App::Title` scoping — see `civitas-core::window_pattern`.
    #[serde(default)]
...
mod tests {
    ...
    fn test_user_window_filters() {
        ...
        config.included_windows = vec!["Chrome".to_string(), "Civitas".to_string()];

        assert!(config.should_capture_target("Chrome", Some("Docs")));
        assert!(config.should_capture_target("Terminal", Some("Civitas logs")));
