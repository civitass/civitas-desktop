...
fn extract_browser_url(
    ...
    if let Some(url) = get_string_attr(window, ax::attr::document()) {
        if url.starts_with("http://") || url.starts_with("https://") {
            debug!("browser URL found through AXDocument");
            return Some(url);
        }
    }
    ...
    if app_lower.contains("arc") {
        if let Some(url) = get_arc_url() {
            debug!("browser URL found through Arc automation");
            return Some(url);
        }
    }
    ...
    if let Some(url) = find_url_in_children(window, 0, 5) {
        debug!("browser URL found through an accessibility text field");
        return Some(url);
    }

    debug!("browser URL was unavailable after all local detection tiers");
    None
}
...
fn get_arc_url() -> Option<String> {
    ...
    let output = match Command::new("osascript").arg("-e").arg(script).output() {
        Ok(o) => o,
        Err(_) => {
            debug!("Arc URL automation could not start");
            return None;
        }
    };
    ...
    if url.starts_with("http://") || url.starts_with("https://") {
        ...
        debug!("Arc URL automation returned a non-web value");
        None
    }
}
...
/// (`civitas_capture::paired_capture::walk_accessibility_tree` calls
    ...
    fn walk_focused_window_inner(&self) -> Result<TreeWalkResult> {
        ...
        if ENHANCED_MODE_CACHE.should_enable_once(pid) {
            ...
            if enhanced_ax_mode_already_enabled(&ax_app, eui_attr, ama_attr) {
                debug!(pid, "enhanced AX mode already enabled; skipping re-assert");
                ...
                debug!(
                    pid,
                    focus = ?focused_editability,
                    "enhanced AX mode limited to manual accessibility"
                );
            } else {
                ...
                debug!(pid, "enhanced AX mode enabled");
            }
        }
        if ax_mode_changed {
        ...
        if state.hit_ignored_extension {
            debug!("skipping capture: browser extension popup matched an ignored window");
            return Ok(TreeWalkResult::Skipped(SkipReason::UserIgnored));
        }
        ...
        debug!(
            ...
            has_url = browser_url.is_some(),
            has_document = document_path.is_some(),
...
fn resolve_focused_ax_app() -> Option<(Retained<ax::UiElement>, i32, String)> {
    ...
    for app in workspace.running_apps().iter() {
        ...
        debug!(pid, "focused AX app resolved through NSWorkspace fallback");
...
fn resolve_focused_window(
    ...
    if let Some(window) = ui_element_attr(app, ax::attr::main_window()) {
        debug!(pid, "focused window resolved through AXMainWindow fallback");
        return Some(window);
    }

    if let Some(window) = first_window_from_windows_attr(app) {
        debug!(pid, "focused window resolved through AXWindows fallback");
        return Some(window);
    }

    if let Some(window) = first_window_from_children(app) {
        debug!(pid, "focused window resolved through app-child fallback");
        return Some(window);
    }
