...
use civitas_a11y::{UiCaptureConfig, UiRecorder};
        ...
        Ok(handle) => {
            ...
            while start.elapsed() < Duration::from_secs(15) {
                if let Some(event) = handle.recv_timeout(Duration::from_millis(500)) {
                    ...
                    match etype {
                        "click" => {
                            if let civitas_a11y::events::EventData::Click { x, y, button, .. } =
                                &event.data
                            {
                                println!(
                        ...
                        "app_switch" => {
                            if let civitas_a11y::events::EventData::AppSwitch { name, pid } =
                                &event.data
                            {
                                println!(
