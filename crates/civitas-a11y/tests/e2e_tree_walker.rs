...
//! - A dedicated, synthetic-data test user with no personal windows open
//! - Tests will open and close real apps (TextEdit, Finder, Calculator)
//! - Tests measure CPU time and wall-clock duration
//!
//! Every test is ignored by default because it reads or changes the live GUI.
//! Owner: desktop capture maintainers.

#[cfg(target_os = "macos")]
mod e2e {
    use civitas_a11y::tree::{
    ...
    fn create_walker(max_nodes: usize) -> Box<dyn civitas_a11y::tree::TreeWalkerPlatform> {
    ...
    // =========================================================================
    #[test]
    #[ignore = "requires a dedicated synthetic-data macOS GUI session"]
    fn test_textedit_type_and_capture() {
        ...
        let test_text = "civitas accessibility test 12345";
        ...
        if let TreeWalkResult::Found(snap) = walk_result {
            ...
            if snap.app_name == "TextEdit" {
                assert!(
                    snap.text_content.contains("civitas accessibility test"),
