...
export function useSqlAutocomplete(type: "app" | "window" | "url") {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      console.error("SQL autocomplete items could not be loaded.");
    } finally {
      setIsLoading(false);
    }
  }, [type]);
...
/**
 * Fetch apps + their top windows from local civitas, grouped for the
...
export function useAppWindowTree() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      console.error("Application-window autocomplete data could not be loaded.");
    } finally {
      setIsLoading(false);
    }
  }, []);
