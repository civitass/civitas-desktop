...
interface TimelineState {
  ...
  setPendingNavigation: (
    nav: { timestamp: string; frameId?: string } | null,
  ) => void;
...
export const useTimelineStore = create<TimelineState>((set, get) => ({
  ...
  retryConnection: () => {
    ...
    set({
      error: null,
      message: currentFrames.length === 0 ? "connecting…" : null,
  ...
  loadFromCache: async () => {
    try {
      ...
      console.warn("Timeline cache could not be loaded.");
    }
  },
  ...
  connectWebSocket: () => {
    void (async () => {
      ...
      // Authenticate in the WebSocket subprotocol header; never place the key in a URL.
      const wsBase = getApiBaseUrl().replace("http://", "ws://");
      const wsUrl = `${wsBase}/stream/frames`;
        ...
        try {
          ...
          if (data === "keep-alive-text") {
            ...
            set((state) => ({
              ...
              isLoading: false,
              message:
                currentFrames.length === 0 ? "waiting for data..." : null,
            }));
            return;
          }
          ...
          if (data.type === "audio_update" && data.audio) {
            ...
            for (let i = 0; i < frames.length; i++) {
              ...
              const isDuplicate = frame.devices?.some((d: any) =>
                d.audio?.some(
                  (a: any) => a.audio_chunk_id === data.audio.audio_chunk_id,
                ),
              );
          ...
          console.error("Timeline frame data could not be parsed.");
          // OPTIMISTIC: Don't show error if we have frames
          const currentFrames = get().frames;
          if (currentFrames.length === 0) {
            set({
      ...
      ws.onerror = () => {
        ...
        if (!hasLoggedTimelineDisconnect) {
          console.warn("Timeline connection reported an error.");
        ...
        if (connectionAttempts < MAX_SILENT_RETRIES) {
          ...
          set({
            isLoading: currentFrames.length === 0,
            message:
              currentFrames.length === 0 ? "connecting to civitas..." : null,
            isConnected: false,
          });
          ...
          set({
            error:
              "Civitas’s local Timeline service could not be reached. Your local data was not removed.",
            isLoading: false,
            isConnected: false,
            message: null,
          });
      ...
      ws.onclose = (event: CloseEvent) => {
        ...
        if (event.code === 1000 && event.wasClean) {
          ...
          console.warn("Timeline connection closed unexpectedly.");
        }
      ...
      console.error("Timeline local service could not be reached.");
      set({
        isLoading: false,
        isConnected: false,
        message: null,
    ...
    const sendOrRetry = (attempt: number) => {
      ...
      if (websocket && websocket.readyState === WebSocket.OPEN) {
        ...
        requestTimeoutTimer = setTimeout(() => {
          requestTimeoutTimer = null;
          const { frames: currentFrames, pendingDateSwap: stillSwapping } =
            get();
        ...
        console.warn("Timeline connection is not ready; retrying.");
        setTimeout(() => sendOrRetry(attempt + 1), delay);
      } else {
        console.error(
          "[fetchTimeRange] WebSocket not open after 5 retries, giving up",
        );
        set({
          isLoading: false,
