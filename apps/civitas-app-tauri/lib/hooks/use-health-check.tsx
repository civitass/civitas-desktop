...
function closeHealthSocket(code = 1000, reason = "cleanup") {
  ...
  try {
    ...
    console.warn("Health connection could not be closed cleanly.");
  }
  ...
  try {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        console.error("Local service health response could not be parsed.");
      }
    };

    ws.onerror = () => {
      if (!hasLoggedDisconnect) {
        console.warn("Local service health connection reported an error."
        );
    ...
    ws.onclose = (event) => {
      ...
      if (event.code === 1000 && event.wasClean) {      } else {
        console.warn("Local service health connection closed unexpectedly.");
      }
    ...
    console.error("Local service health connection could not be created.");
