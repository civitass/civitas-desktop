 ...
 *
 * Walks the original production repro at the data layer (no React renderer):
...
describe("parallel chat production repro at the data layer", () => {
  it("preserves chat A's messages after switching to B and back", async () => {
    ...
    expect(
      (useChatStore.getState().sessions.A.messages![1] as any).content,
    ...
    expect((aDone.messages![1] as any).content).toBe(
    ...
    expect((aResumed.messages![1] as any).content).toBe(
      "Hi there! How can I help?",
    );
  ...
  it("does NOT reorder the sidebar when a backgrounded session emits text", async () => {
    // A background update must not make sidebar rows change order.
    ...
    const order = selectOrderedSessions(useChatStore.getState()).map(
      (s) => s.id,
    );
