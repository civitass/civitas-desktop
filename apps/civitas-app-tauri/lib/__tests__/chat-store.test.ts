...
/**
 * Chat-store invariants. Each test below corresponds to a concrete production
 * bug; comments name the failure mode so a future reader understands why the
 * assertion exists.
 */
...
describe("chat-store: snapshotSession", () => {
  ...
  it("does NOT bump createdAt — snapshot is plumbing, not new activity", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "A", createdAt: 100 }));
    useChatStore.getState().actions.snapshotSession("A", {
      messages: [],
      streamingText: "",
      streamingMessageId: null,
...
describe("chat-store: stable sort by createdAt", () => {
  ...
  it("orders newer createdAt first", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "old", createdAt: 100 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "new", createdAt: 200 }));
    const ordered = selectOrderedSessions(useChatStore.getState());
    expect(ordered.map((s) => s.id)).toEqual(["new", "old"]);
  });

  it("does NOT reshuffle when updatedAt changes", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "A", createdAt: 100, updatedAt: 100 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "B", createdAt: 200, updatedAt: 200 }));
  ...
  it("user-send bumps a chat to the top via lastUserMessageAt", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "older", createdAt: 100 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "newer", createdAt: 200 }));
    // User sends in the older chat — should leapfrog "newer".
    useChatStore
      .getState()
      .actions.patch("older", { lastUserMessageAt: 9_000 });
    const ordered = selectOrderedSessions(useChatStore.getState());
  ...
  it("pinned rows float above unpinned, both sorted by createdAt within group", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "p1", createdAt: 100, pinned: true }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "r1", createdAt: 200 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "p2", createdAt: 50, pinned: true }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "r2", createdAt: 300 }));
    const ordered = selectOrderedSessions(useChatStore.getState());
...
describe("chat-store: upsert preserves createdAt on re-insert", () => {
  ...
  it("keeps the original createdAt when re-upserting an existing id", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "A", createdAt: 999 }));
    expect(useChatStore.getState().sessions.A.createdAt).toBe(100);
  });
});
...
describe("chat-store: setCurrent clears unread atomically", () => {
  ...
  it("flips currentId AND clears unread on the new current in one set", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "A", unread: true }));
...
describe("chat-store: recent switcher ordering", () => {
  ...
  it("returns only chats viewed this session ordered by most recent view", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "older-viewed", createdAt: 100 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "sidebar-top", createdAt: 300 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "newer-viewed", createdAt: 200 }));
    ...
    expect(ordered.map((s) => s.id)).toEqual(["newer-viewed", "older-viewed"]);
  });

  it("returns an empty list when no chats have been viewed this session", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "a", createdAt: 300 }));
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "b", createdAt: 200 }));
  ...
  it("excludes hidden and draft chats from the switcher", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "visible", createdAt: 300 }));
    useChatStore
      .getState()
      .actions.upsert(
        baseRecord({ id: "hidden", createdAt: 200, hidden: true }),
      );
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "draft", createdAt: 100, draft: true }));
...
describe("chat-store: markUnread guards", () => {
  ...
  it("no-ops when the session is the current one", () => {
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "A", unread: false }));
    useChatStore.getState().actions.setCurrent("A");
  ...
  it("DOES mark a different session unread when nav'd away", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(baseRecord({ id: "B", unread: false }));
    useChatStore.setState({ currentId: null, panelSessionId: "A" });
...
describe("chat-store: cross-window duplicate row collapsing", () => {
  ...
  const withMessages = (
    ...
    baseRecord({
      ...
      messages: [
        ...
        ...(reply
          ? [{ id: `${id}-a`, role: "assistant", content: reply, timestamp: 2 }]
          : []),
      ] as any,
      ...over,
    });
  ...
  it("never merges pipe runs that share a templated first message", () => {
    useChatStore
      .getState()
      .actions.upsert(
        withMessages("run1", "daily digest", "a", {
          createdAt: 1_000,
          kind: "pipe-run",
        }),
      );
    useChatStore
      .getState()
      .actions.upsert(
        withMessages("run2", "daily digest", "b", {
          createdAt: 1_100,
          kind: "pipe-run",
        }),
      );
  ...
  it("keeps the visible twin rather than collapsing into a hidden one", () => {
    ...
    useChatStore
      .getState()
      .actions.upsert(
        withMessages("hiddenTwin", "shared opener", "answer", {
          createdAt: 1_000,
          hidden: true,
        }),
      );
    useChatStore
      .getState()
      .actions.upsert(
        withMessages("visibleTwin", "shared opener", "Processing...", {
          createdAt: 1_200,
        }),
      );
    const rows = dedupeSessionRecords(
      Object.values(useChatStore.getState().sessions),
    );
    expect(rows).toHaveLength(1);
