...
vi.mock("@tauri-apps/plugin-fs", () => ({
  exists: vi.fn(
    async (path: string) =>
      path === "/Users/test/.civitas/chats" || fsMock.files.has(path),
  ),
...
const CHATS_DIR = "/Users/test/.civitas/chats";
...
describe("chat-storage mutation failures", () => {
  beforeEach(() => {
    fsMock.files.clear();
    __resetChatStorageCachesForTests();
  ...
  it("propagates a filesystem delete failure so the UI cannot claim success", async () => {
    putConversation("important-chat", { updatedAt: 1 });
    vi.mocked(remove).mockRejectedValueOnce(new Error("permission denied"));

    await expect(deleteConversationFile("important-chat")).rejects.toThrow(
      "permission denied",
    );
  });

  it("rejects metadata updates when the conversation is missing or unreadable", async () => {
    await expect(
      updateConversationFlags("missing-chat", { hidden: true }),
    ).rejects.toThrow("missing or unreadable");
  });

  it("distinguishes a missing file from an unreadable existing file", async () => {
    fsMock.files.set(`${CHATS_DIR}/damaged.json`, {
      text: "{not valid json",
      mtime: 1,
    });

    await expect(loadConversationFileStrict("missing")).resolves.toBeNull();
    await expect(loadConversationFileStrict("damaged")).rejects.toThrow(
      'Conversation "damaged" exists but could not be read safely.',
    );
    await expect(loadConversationFile("damaged")).resolves.toBeNull();
  });
});
...
describe("chat-storage bounded history", () => {
  ...
  it("loads only the newest 50 conversation files for the default history view", async () => {
    ...
    expect(fsMock.reads.some((path) => path.endsWith("/chat-0.json"))).toBe(
      false,
    );
  });
  ...
  it("still searches older chats outside the initial 50", async () => {
    for (let i = 0; i < 60; i += 1) {
      putConversation(`chat-${i}`, {
        updatedAt: i + 1,
        content:
          i === 0 ? "needle from a very old conversation" : "ordinary chat",
      });
    }
  ...
  it("reports corrupt files while preserving every readable conversation", async () => {
    putConversation("readable", { updatedAt: 10 });
    fsMock.files.set(`${CHATS_DIR}/damaged.json`, {
      text: "{not valid json",
      mtime: 20,
    });
    ...
    expect(rows.map((row) => row.id)).toEqual(["readable"]);
...
describe("dedupeConversationMetas", () => {
  it("collapses two copies of the same chat, keeping the one with a real reply", () => {
    const out = dedupeConversationMetas([
      candidate("ghost", "export last 5 min of video", false, {
        createdAt: 1000,
      }),
      candidate("real", "export last 5 min of video", true, {
        createdAt: 1200,
      }),
    ]);
  ...
  it("keeps the higher message count when both copies have a reply", () => {
    const out = dedupeConversationMetas([
      candidate("short", "hi there", true, {
        createdAt: 1000,
        messageCount: 4,
      }),
      candidate("long", "hi there", true, {
        createdAt: 1100,
        messageCount: 10,
      }),
    ]);
  ...
  it("breaks ties on updatedAt when reply state and message count match", () => {
    const out = dedupeConversationMetas([
      candidate("older", "same opener", true, {
        createdAt: 1000,
        updatedAt: 1000,
      }),
      candidate("newer", "same opener", true, {
        createdAt: 1100,
        updatedAt: 5000,
      }),
    ]);
  ...
  it("normalizes whitespace and case of the first user message", () => {
    expect(
      conversationDedupKey({
        kind: "chat",
        messages: [
          { role: "user", content: "  Export  Last 5 Min\nOf Video " },
        ],
      }),
...
describe("listConversations duplicate collapsing", () => {
  ...
  it("collapses a duplicated chat into the copy that has a real reply", async () => {
    ...
    putConversation("real", {
      ...
      titleSource: "ai",
      assistantContent:
        "I've exported the last five minutes of your screen activity.",
    });
  ...
  it("keeps same-opener chats that are far apart in time", async () => {
    putConversation("morning", {
      ...
      createdAt: 1_700_000_000_000,
      content: "search civitas for what happened during this meeting",
