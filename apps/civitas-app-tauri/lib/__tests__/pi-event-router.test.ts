...
vi.mock("@/lib/chat-storage", () => ({
  ...
  loadConversationFileStrict: vi.fn(async () => null),
...
describe("pi-event-router: browser state preservation", () => {
  ...
  it("surfaces and propagates a failed background save so quit can remain open", async () => {
    seed("A", {
      messages: [
        {
          id: "u1",
    ...
    vi.mocked(saveConversationFile).mockRejectedValueOnce(
      new Error("disk full"),
    );
    ...
    expect(useChatStore.getState().sessions.A.messages).toHaveLength(1);
  });
});
      ...
      messages: [
        {
          ...
          role: "user",
          content:
            "search civitas for what happened during this meeting and summarize it",
