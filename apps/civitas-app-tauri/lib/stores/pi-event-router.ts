...
const PERSISTENCE_TOAST_COOLDOWN_MS = 15_000;

function reportPersistenceFailure(): void {
  console.error("Background chat persistence failed.");
  const now = Date.now();
  if (now - lastPersistenceToastAt < PERSISTENCE_TOAST_COOLDOWN_MS) return;
  lastPersistenceToastAt = now;
  toast({
    title: "This chat isn’t saved yet",
    description:
      "Its in-memory transcript is still available. Keep Civitas open; saving will retry automatically.",
    variant: "destructive",
  });
}
...
export function handleTerminated(payload: AgentTerminatedPayload) {
  ...
  if (store.currentId !== sid) {
    queueBackgroundPersistence(sid);
  }
}
    ...
    try {
      ...
      const offClose = await win.onCloseRequested(async (event) => {
        ...
        try {
          ...
          console.error("Pending chat saves could not be flushed on close.");
          toast({
            title: "Quit paused to protect your chat",
            description:
              "Civitas kept the window open because a conversation couldn’t be saved. Retry after checking available disk space.",
            variant: "destructive",
          });
          return;
        }
      ...
      console.error("Chat quit protection could not be registered.");
      toast({
        title: "Quit protection is unavailable",
        description:
          "Chats will still autosave, but wait for a response to finish before quitting Civitas.",
        variant: "destructive",
      });
    }
...
function queueBackgroundPersistence(sid: string): void {
  void persistBackgroundSession(sid).catch(() => {
    // `persistBackgroundSession` already reports the failure and keeps the
    // in-memory transcript intact. Recover here only because event handlers
    // cannot await the serialized save queue.
  });
}
