...
type ContentItem =
	...
	| { type: "ProjectFile"; content: { text: string; textPreview?: string | null; relativePath?: string; safeDisplayPath?: string } };
...
export function getCliPath() {
  ...
  switch (os) {
    case "windows":
      return "%LOCALAPPDATA%\\civitas\\civitas.exe";
    case "macos":
      return "/Applications/civitas.app/Contents/MacOS/civitas";
    case "linux":
      return "/usr/local/bin/civitas";
    default:
      return "civitas";
  }
}
...
export const removeDuplicateSelections = (
	...
	Array.from(selectedResults).forEach((index) => {
		...
		else if (item.type === "ProjectFile") content = item.content.text;
