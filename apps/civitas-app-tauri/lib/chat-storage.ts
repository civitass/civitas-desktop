...
export interface ConversationListOptions {
  ...
  /** Called once for each unreadable/corrupt conversation file. Valid rows are
   *  still returned so one damaged file never hides the rest of history. */
  onUnreadable?: () => void;
}
...
export async function saveConversationFile(
  ...
  // same conversation (panel autosave + a metadata update, or two
  // router-driven background saves
  ...
  // wild as an ENOENT rename failure.
...
export class ConversationFileReadError extends Error {
  constructor(id: string) {
    super(`Conversation "${id}" exists but could not be read safely.`);
...
async function readConversationFile(
  ...
  try {
    ...
    const conversation = JSON.parse(text) as ChatConversation;
    if (
      !conversation ||
      conversation.id !== id ||
    ...
    if (throwOnUnreadable) throw new ConversationFileReadError(id);
    return null;
  }
}

/** Compatibility read for list/sync surfaces that can safely skip one row. */
export async function loadConversationFile(
  id: string,
): Promise<ChatConversation | null> {
  return readConversationFile(id, false);
}

/**
 * Read-before-write and direct-open variant. Missing files still return null,
 * but an existing unreadable file throws so callers cannot mistake corruption
 * or a transient permission failure for a brand-new conversation and
 * overwrite the original.
 */
export async function loadConversationFileStrict(
  id: string,
): Promise<ChatConversation | null> {
  return readConversationFile(id, true);
}
...
function rememberConversationEntry(
  dir: string,
...
function forgetConversationEntry(dir: string, name: string): void {
  if (_orderedEntriesCacheDir !== dir || !_orderedEntriesCache) return;
  _orderedEntriesCache = _orderedEntriesCache.filter(
...
async function orderedConversationEntries(
  dir: string,
): Promise<ConversationEntry[]> {
 ...
 *
 * Rejects if the file is missing or unreadable so the caller cannot claim a
 * metadata mutation succeeded or overwrite a damaged conversation. The
 * read+write is not atomic across processes, but conflicts are functionally
 * harmless for these flags (last writer wins, no transcript loss).
 */
export async function updateConversationFlags(
  ...
  patch: Partial<
    Pick<
      ChatConversation,
      "pinned" | "hidden" | "title" | "titleSource" | "browserState"
    >
  >,
): Promise<void> {
  const conv = await loadConversationFileStrict(id);
  if (!conv) {
    throw new Error(
      "The conversation file is missing or unreadable; metadata was not changed.",
    );
  }
...
export async function migrateFromStoreBin(): Promise<void> {
  try {
    ...
    if (!history?.conversations?.length || history._migratedToFiles) {
      return;
    }
    ...
    for (const conv of history.conversations) {
      try {
        ...
        console.warn("A conversation could not be migrated to local storage.");
      }
    }
    ...
    console.warn("Conversation storage migration failed.");
  }
}
