...
export interface SourceCitation {
  ...
  // local (civitas) citation, the footer makes the row jump into the
...
function sourceCitationsFromToolCall(toolCall: ToolCallLike | undefined): SourceCitation[] {
  ...
  if (toolName === "civitas_search") {
    return [civitasToolCitation(args)];
  }
...
function civitasToolCitation(args: Record<string, unknown>): SourceCitation {
  ...
  return {
    id: stableId(["civitas-search-tool", contentType, appName, query, range]),
    kind: "civitas",
...
function bashCitations(command: string, resultText: string): SourceCitation[] {
  ...
  for (const call of extractCivitasApiCalls(command)) {
    citations.push(civitasApiCitation(call));
  }

  for (const link of extractWebLinks(command)) {
    if (isLocalCivitasUrl(link.url)) continue;
    citations.push({
      id: stableId(["web", link.url]),
      kind: "web",
...
function civitasApiCitation(call: string): SourceCitation {
  const path = extractPath(call);
  const query = extractQuery(call);
  const title = civitasTitle(path);
  const kind = civitasKind(path);
  ...
  return {
    id: stableId(["civitas", path, query]),
    kind,
    title,
    subtitle: civitasSubtitle(path, query),
    timestamp,
    query: searchText,
  };
}

function civitasKind(path: string): SourceCitationKind {
...
function civitasTitle(path: string): string {
  if (path === "/search") return "Civitas search";
...
function civitasSubtitle(path: string, query: string): string | undefined {
  const params = queryParams(query);
  if (path === "/search") {
    const parts = [
