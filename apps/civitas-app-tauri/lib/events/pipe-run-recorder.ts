...
async function finalizeBuffer(sid: string, buf: PipeRunBuffer): Promise<void> {
  ...
  try {
    ...
    console.warn("Workflow run conversation could not be saved.");
    return;
  }
  ...
  // restart (when hydrate() re-scans ~/.civitas/chats/). We use the
  ...
  try {
    ...
    console.warn("Workflow run could not be added to the sidebar.");
  }
}
