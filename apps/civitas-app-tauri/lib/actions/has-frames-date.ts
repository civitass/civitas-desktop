...
export async function listDaysWithFrames(): Promise<Set<string>> {
	...
	try {
		...
		if (!response.ok) {
			...
			console.error("Timeline day availability request failed.");
			return new Set();
		}
		const rows = (await response.json()) as Array<{ day: string }>;
		const set = new Set(rows.map((r) => r.day).filter(Boolean));		daysCache = { at: Date.now(), days: set };
		return set;
	} catch (e) {
		console.error("Timeline day availability could not be loaded.");
		return new Set();
	}
}
...
export async function hasFramesForDate(date: Date): Promise<boolean> {
	try {
		...
		if (!response.ok) {
			console.error("Timeline date availability request failed.");
			// Return false on error - let navigation proceed to try the date
			return false;
		}

		const result = await response.json();		return result.length > 0;
	} catch (e) {
		console.error("Timeline date availability could not be checked.");
		// Return false on error - let navigation proceed to try the date
		return false;
	}
}
...
export async function findNearestDateWithFrames(
	...
	try {
		...
		if (!response.ok) {
			console.error("Nearest timeline date request failed.");
			return null;
		}
		...
		console.error("Nearest timeline date could not be found.");
		return null;
	}
}
