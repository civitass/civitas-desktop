...
export interface WatchAuthorizationStatusOptions {
  ...
  /**
   * Continue polling even when `document.hidden` is true (e.g. the civitas
...
// ---------------------------------------------------------------------------
// Controller — Civitas-facing wrappers around the permission-flow plugin
// ---------------------------------------------------------------------------

/**
 * Civitas-facing alias: every OS permission we surface in onboarding,
...
function isCivitasAppBundle(
  appPath: string | null | undefined,
): appPath is string {
  if (!appPath || !appPath.endsWith(".app")) return false;
  return /civitas/i.test(appPath);
}
...
async function getCivitasAppPath(): Promise<string | null> {
  if (state.appPath !== undefined) return state.appPath;
  const appPath = await suggestedHostAppPath();
  state.appPath = isCivitasAppBundle(appPath) ? appPath : null;
...
async function restoreAlwaysOnTopWhenCivitasRefocuses(): Promise<void> {
  ...
  try {
    ...
    const unlisten = await window.onFocusChanged(async ({ payload: focused }) => {
      ...
      // Hide the drag panel when civitas is frontmost — it should only
      ...
      try {
        ...
        console.error("Window priority could not be restored after the permission flow."
        );
      }
    });
...
async function letSystemSettingsComeForward(): Promise<void> {
  try {
    ...
    await restoreAlwaysOnTopWhenCivitasRefocuses();
  } catch {
...
async function reclaimCivitasWindow(): Promise<void> {
  if (state.pendingFocusUnlisten) {
    try { state.pendingFocusUnlisten(); } catch {}
    state.pendingFocusUnlisten = null;
  }
  ...
  try {
    ...
    console.error("Civitas window could not be focused after the permission flow."
    );
  }
}
...
function watchUntilGrantedAndClose(dragPermission: Permission): void {
  ...
  state.activeWatch = watchAuthorizationStatus(
    ...
    },
    {
      ...
      pollWhenHidden: true,
      onError: (error) => {
        console.error("Permission status monitoring failed.");
      },
    },
  );
}
...
export async function requestPermissionWithFlow(
  ...
  const appPath = dragPermission ? await getCivitasAppPath() : null;
  ...
  try {
    ...
    console.error("Permission flow failed; using the system fallback.");
    await stopActiveFlow();
    await requestNativePermission(permission);
  }
}
...
export async function openPermissionSettingsWithFlow(
  ...
  try {
    ...
    console.error("Permission settings could not be opened through the primary path.");
    await stopActiveFlow();
    await openNativePermissionSettings(permission);
  }
}
