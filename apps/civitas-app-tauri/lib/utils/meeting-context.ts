 ...
 *     keep the wording in sync with crates/civitas-core/assets/pipes/meeting-summary/pipe.md
 *
 * The agent decides whether to PATCH. Empty transcript / nothing worth saving
 * → report that clearly and skip the PATCH. Useful summary → replace only the
 * Civitas-marked summary block (or append it once), preserving user-authored
 * notes through the same endpoint the autosave uses.
 */
export function buildMeetingSummarizeInstructions(
  ...
  const lines = [
    `Summarize this exact local meeting and save an evidence-grounded result back to the same record.`,
    ``,
    `meeting id: ${meetingId}`,
    `First preserve the record's current title and notes. Primary transcript source: GET "http://localhost:3030/meetings/${meetingId}/transcript"; sort rows by capturedAt and use transcript, speakerName, capturedAt, and source.`,
    `Fallback only when that transcript is empty: /search?content_type=audio bounded to the meeting start/end. Audio rows use content.transcription, not content.text.`,
    `Ignore empty fragments, obvious transcription hallucinations, and unrelated audio. Never invent a decision, owner, deadline, attendee, action, or open question.`,
    `Use this concise structure: "### Overview" (2-3 verified sentences), "### Decisions" (or "No explicit decisions captured."), "### Action items" (checkboxes; use "Unassigned" when no owner was explicit), and "### Open questions" (or "None captured.").`,
    `Wrap it exactly between "<!-- civitas-summary:start -->" and "<!-- civitas-summary:end -->", with "## Summary" immediately after the start marker.`,
    `If a marked summary already exists, replace only that marked block. Otherwise append the block after the existing notes with one blank line. Never erase, rewrite, or reorder user-authored text outside the markers.`,
    `PATCH "http://localhost:3030/meetings/${meetingId}" with the complete preserved note. Construct valid JSON carefully (for example with jq -n --arg) so quotes and newlines in existing notes cannot corrupt the request.`,
    `Refresh the title only when the current title is empty, generic ("untitled", "meeting", or just the app name), or demonstrably inaccurate. A generated title must be plain English, 5-8 words, and based on the main verified topic. Otherwise omit title.`,
    `If there is no reliable transcript, say "No reliable transcript to summarize" and skip PATCH. Do not write a placeholder. After PATCH, verify the returned meeting id is ${meetingId}.`,
  ];
