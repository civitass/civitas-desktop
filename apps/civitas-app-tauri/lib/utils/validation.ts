...
// Presets are non-secret pointers to the loopback gateway. Direct provider
// profiles and credentials are validated by Rust and stored outside settings.
...
export const aiPresetSchema = z.object({
  ...
  maxContextChars: z
    .number()
    .int()
    .min(1000, "Must be at least 1,000 characters")
    .max(2000000, "Cannot exceed 2,000,000 characters"),
});
...
export const settingsStoreSchema = z.object({
  ...
  // Audio Settings
  audioTranscriptionEngine: z
    .string()
    .min(1, "Audio transcription engine is required"),
  ...
  ocrLanguages: z.array(z.string()).nullish(),
  ...
  fps: z
    .number()
    .min(0.1, "FPS must be at least 0.1")
...
export const validateField = (
  ...
  try {
    const schemaShape = settingsStoreSchema.shape as Record<
      string,
      z.ZodTypeAny
    >;
...
// Preset validation
// `visiblePresets` should be the filtered list the user actually sees so
// migration-only presets do not block creation.
export const validatePresetName = (
  name: string,
  visiblePresets: AIPreset[],
  currentId?: string,
): FieldValidationResult => {
...
export const validateContextLength = (
  length: number,
  model: string,
): FieldValidationResult => {
  if (length < 1000) {
    return {
      isValid: false,
      error: "Context length must be at least 1,000 characters",
    };
  }
  ...
  if (length > 2000000) {
    return {
      isValid: false,
      error: "Context length cannot exceed 2,000,000 characters",
    };
  }
  ...
  if (model.includes("gpt-4") && length > 128000 * 4) {
    return {
      isValid: true,
      warning: "This context length exceeds GPT-4's recommended limit",
    };
  }
  ...
  if (model.includes("gpt-3.5") && length > 16000 * 4) {
    return {
      isValid: true,
      warning: "This context length exceeds GPT-3.5's limit",
    };
  }
