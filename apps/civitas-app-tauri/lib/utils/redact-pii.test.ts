...
describe("redactPii", () => {
  ...
  it("redacts US phone with dashes", () => {
    expect(redactPii("Call me at 234-567-8901")).toBe("Call me at [PHONE]");
  });
  ...
  it("redacts API keys", () => {
    const fakeKey = ["sk", "live", "abc123defghijklmnop"].join("_");
    expect(redactPii(`api_key=${fakeKey}`)).toContain(
      "[REDACTED]",
    );
  });
  ...
  it("redacts Unix home paths", () => {
    expect(redactPii("/Users/example/.civitas")).toBe("/Users/[USER]/.civitas");
  });

  it("redacts Windows home paths", () => {
    expect(redactPii("C:\\Users\\DavidBattaglia\\.civitas")).toBe(
      "C:\\Users\\[USER]\\.civitas",
    );
  });
