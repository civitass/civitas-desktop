...
export type WebTargetKind = "fetch" | "navigate";
...
export function isLocalCivitasUrl(url: URL): boolean {
...
export function webTargetFromUrl(url: URL, kind: WebTargetKind): WebTargetPresentation | null {
  if (isLocalCivitasUrl(url)) return null;
  return {
    url: url.toString(),
...
// Maps pi's bash curl calls to the local civitas API into a human label.
...
export function classifyCurl(cmd: string): CurlPresentation | null {
  ...
  const url = urls.find(isLocalCivitasUrl);
  if (!url) {
  ...
  if (path.startsWith("/connections/browsers/") && path.endsWith("/navigate")) {
    const body = curlBodyJson(cmd);
    if (body && typeof body.url === "string") {
      const target = webTargetFromUrlString(body.url, "navigate");
      if (target) return { label: `Opened ${target.domain} in connected browser`, webTarget: target };
    }
    return { label: "Navigated connected browser" };
  }
  if (path.startsWith("/connections/browsers/") && path.endsWith("/snapshot")) {
    return { label: "Read shared browser tab outline" };
  }
  if (path.startsWith("/connections/browsers/")) return { label: "Connected browser action" };
