...
describe("url helpers", () => {
  it("identifies the local civitas server", () => {
    expect(isLocalCivitasUrl(new URL(`${LOCAL}/search`))).toBe(true);
    expect(isLocalCivitasUrl(new URL("https://example.com"))).toBe(false);
  });
...
describe("endpointFamily", () => {
  it("maps known paths to a family chip", () => {
    ...
    expect(endpointFamily("/anything-else")).toBe("Civitas");
  });
});
...
describe("sanitizeCommand", () => {
  it("redacts the bearer token and collapses whitespace", () => {
    const fakeToken = ["secret", "123"].join("");
    const out = sanitizeCommand(
      `curl   -H 'Authorization: Bearer ${fakeToken}'   ${LOCAL}/x`,
    );
