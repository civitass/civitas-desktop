...
pub fn init_magnify_handler(app: tauri::AppHandle) {
    ...
    if Class::get("CivitasMagnifyHandler").is_none() {
        let superclass = Class::get("NSObject").unwrap();
        let mut decl = ClassDecl::new("CivitasMagnifyHandler", superclass).unwrap();
    ...
    if Class::get("CivitasScrollInterceptor").is_none() {
        ...
        let superclass = Class::get("NSObject").unwrap();
        let decl = ClassDecl::new("CivitasScrollInterceptor", superclass).unwrap();
        decl.register();
    }
}
...
pub(crate) unsafe fn attach_magnify_gesture_to_view(view: tauri_nspanel::cocoa::base::id) {
    with_autorelease_pool(|| {
        ...
        let handler_class = match Class::get("CivitasMagnifyHandler") {
            Some(cls) => cls,
            None => {
                tracing::warn!(
                    "CivitasMagnifyHandler class not registered yet — skipping gesture attach"
                );
                return;
            }
        };

        // Check if we already added our recognizer (look for CivitasMagnifyHandler target)
