...
/// Check if our own app (civitas) is still the active macOS application.
/// When the overlay panel loses focus to another civitas window (Settings,
...
/// If our own app is still active (focus moved to another civitas window
/// like Settings), we just clear without restoring — otherwise Settings/Chat
/// would lose keyboard focus. Also skip restoring if any civitas window
...
pub(crate) fn restore_frontmost_app_if_external_with_app(app: Option<&AppHandle>) {
    ...
    if is_own_app_still_active() {
        info!("Focus moved to another civitas window, clearing saved app without restoring");
        clear_frontmost_app();
        return;
    }

    // Check if Settings, Chat, Ask, or Search windows exist in Tauri's registry.
    // These are regular windows (not panels) that may be on another Space —
    // we don't want to activate a previous app and bury them.
    if let Some(app) = app {
        let non_panel_labels = ["home", "chat", "ask", "search"];
        for label in &non_panel_labels {
            if app.get_webview_window(label).is_some() {
                info!(
                    "Civitas '{}' window still open, clearing saved app without restoring",
                    label
                );
                clear_frontmost_app();
                return;
