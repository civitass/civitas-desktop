...
impl std::fmt::Display for RewindWindowId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RewindWindowId::Ask => write!(f, "ask"),
...
impl RewindWindowId {
    ...
    pub fn title(&self) -> &str {
        match self {
            RewindWindowId::Main => "civitas",
            RewindWindowId::Home => "civitas",
            RewindWindowId::Search => "search",
            RewindWindowId::Onboarding => "onboarding",
            RewindWindowId::Chat => "ai chat",
            RewindWindowId::Ask => "ask civitas",
    ...
    pub fn min_size(&self) -> Option<(f64, f64)> {
        Some(match self {
            ...
            RewindWindowId::Ask => (620.0, 600.0),
...
impl ShowRewindWindow {
    ...
    pub fn id(&self) -> RewindWindowId {
        match self {
            ...
            ShowRewindWindow::Ask => RewindWindowId::Ask,
    ...
    fn show_existing_main(
        ...
        if overlay_mode == "window" {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                // Without this, re-showing an already-created overlay when another civitas
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        // === Other windows: standard show path ===
            ...
            // Ask overlay needs panel behavior on macOS to show above fullscreen.
            // Same NSPanel recipe as Chat, except Ask is ALWAYS floating: it's a
            // summon-answer-dismiss overlay (like Search), so it doesn't consult
            // the chat-specific `chat_always_on_top` setting.
            if id.label() == RewindWindowId::Ask.label() {
                #[cfg(target_os = "macos")]
                {
                    let settings = SettingsStore::get(app)
                        .unwrap_or_default()
                    ...
                    run_on_main_thread_safe(app, move || {
                        ...
                        if let Ok(panel) = app_clone.get_webview_panel(RewindWindowId::Ask.label())
                        {
        ...
        let window = match self {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        #[cfg(target_os = "macos")]
                                        {
                                            ...
                                            let _ = app.run_on_main_thread(move || {
                                                ...
                                                // civitas window (Settings, Chat), just clear.
                                        ...
                                        #[cfg(not(target_os = "macos"))]
                                        {
                                            ...
                                            if crate::commands::any_civitas_webview_has_focus(&app)
                                                && crate::commands::main_overlay_is_visible(&app)
                                            {
                                                info!(
                                                    "main-window blur: another civitas window has focus, keep Escape registered"
                                                );
                            ...
                            if !is_focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        let _ = app.run_on_main_thread(move || {
                                            // Conditional restore: if focus moved to another
                                            // civitas window, just clear. Only activate
                                    ...
                                    #[cfg(target_os = "windows")]
                                    {
                                        ...
                                        {
                                            info!(
                                                "Main overlay blur: another civitas window has focus, keep Escape registered"
                                            );
            ...
            ShowRewindWindow::Home { page } => {
                ...
                // when Home blurs and no civitas window has focus (user left the app).
                #[cfg(not(target_os = "macos"))]
                {
                        ...
                        if let tauri::WindowEvent::Focused(focused) = event {
                            if !focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    if crate::commands::any_civitas_webview_has_focus(&app_c) {
                                        return;
                                    }
                                    if crate::commands::is_main_close_in_progress() {
                                        return;
                                    ...
                                    std::thread::spawn(move || {
                                        info!(
                                            "shortcut-sync: unregister execute (reason=home_blur_no_civitas_focus)"
                                        );
                                        let _ = crate::commands::unregister_window_shortcuts_if_generation_unchanged(
                                            app2,
                                            expected_gen,
                                            "home_blur_no_civitas_focus",
                                        );
                                    });
                                });
                            } else {
            ...
            ShowRewindWindow::Search { query } => {
                ...
                info!(query_present = query.is_some(), "creating search window");
                if let Some(q) = query {
            ...
            ShowRewindWindow::Ask => {
                #[cfg(target_os = "macos")]
                let window = {
                    // Compact Ask overlay panel. Same NSPanel recipe as Chat (created
                    // hidden, converted to panel, shown by the show_existing path when
                    // the shortcut fires) — see the Chat arm above for the rationale.
                    // Ask is always floating; there is no ask-specific on-top setting.
                    //
                    // Surface: the tray dropdown's light-glass recipe — transparent
                    // window + `UnderWindowBackground` vibrancy (desktop blur) forced
                    // to a LIGHT appearance, rounded to 20px with one soft shadow via
                    // apply_light_glass_surface below. The `/ask` webview must keep
                    // html/body transparent with a `.kg-tray-panel rounded-[20px]`
                    // root so the wallpaper shows through (see tray-dropdown.tsx).
                    let builder = self
                        .window_builder(app, "/ask")
                        ...
                        .hidden_title(true)
                        .transparent(true)
                        // Always-light glass, independent of system theme (overrides
                        // the settings-driven theme applied by window_builder).
                        .theme(Some(tauri::Theme::Light))
                        // `UnderWindowBackground` samples + blurs whatever is BEHIND
                        // the panel — the same "liquid glass floating over the
                        // wallpaper" material the tray dropdown uses.
                        .effects(tauri::utils::config::WindowEffectsConfig {
                    ...
                    // Convert to panel for fullscreen support. Only configure
                    // level + behaviors here — do NOT activate or show. The
                    // show_existing path handles that when the user presses the
                    // shortcut, matching the Chat panel creation pattern.
                    if let Ok(_panel) = window.to_panel() {
                        info!("Successfully converted ask window to panel");
                        ...
                        run_on_main_thread_safe(app, move || {
                            ...
                            if let Ok(panel) = window_clone.to_panel() {
                                // Level 1001 to appear above fullscreen apps
                                panel.set_level(1001);
                                // NonActivatingPanel (128) so clicking the ask panel
                                // doesn't activate the app (which would switch Spaces
                                // away from fullscreen apps). Preserve existing bits.
                                unsafe {
                                    let current: i32 = msg_send![&*panel, styleMask];
                                    panel.set_style_mask(current | 128);
                                }
                                ...
                                // Tray light-glass surface: forced Aqua appearance +
                                // 20px-rounded vibrancy mask + one soft shadow (the
                                // exact tray-dropdown recipe, shared helper).
                                if let Ok(ns_win) = window_clone.ns_window() {
                                    unsafe {
                                        apply_light_glass_surface(
                                            ns_win as tauri_nspanel::cocoa::base::id,
                                        );
                                    }
                                }
    ...
    /// Used when transitioning from Main to another civitas window (e.g. Home/Settings)
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    // we're transitioning to another civitas window.
                });
            }
...
/// Shared light-glass surface recipe — the tray dropdown's look, reused by the Ask
/// panel. Forces a LIGHT (Aqua) appearance so the `UnderWindowBackground`
/// NSVisualEffectView renders its light material regardless of system dark mode (the
/// builder `.theme(Light)` hint alone does not reliably flip the vibrancy material),
/// then makes the window non-opaque with ONE soft native shadow and masks the REAL
/// vibrancy — the contentView layer plus its NSVisualEffectView subviews — to a
/// 20px corner radius. The frost itself is rounded: its corners go transparent (the
/// desktop shows through) and macOS draws a single soft rounded shadow around it —
/// a floating glass card, not a docked frosted rectangle.
///
/// Callers pair this with a builder that sets `.transparent(true)` +
/// `Effect::UnderWindowBackground` and a webview whose html/body are transparent
/// with a `.kg-tray-panel` root plate (see `app/globals.css`).
///
/// Safety: must run on the main thread with a valid NSWindow pointer.
#[cfg(target_os = "macos")]
unsafe fn apply_light_glass_surface(ns_win: tauri_nspanel::cocoa::base::id) {
    use objc::{class, msg_send, sel, sel_impl};
    use tauri_nspanel::cocoa::base::{id, nil};

    // Force a LIGHT (Aqua) appearance so the vibrancy renders its light material
    // regardless of system dark mode.
    let aqua_name: id = msg_send![
        class!(NSString),
        stringWithUTF8String: b"NSAppearanceNameAqua\0".as_ptr() as *const i8
    ];
    let aqua: id = msg_send![class!(NSAppearance), appearanceNamed: aqua_name];
    if aqua != nil {
        let _: () = msg_send![ns_win, setAppearance: aqua];
    }

    // ── Float, don't dock: round the REAL vibrancy + one soft shadow ──
    // The native NSVisualEffectView is rectangular and fills the window; left
    // as-is it reads as a hard frosted rectangle behind the CSS-rounded card.
    // Make the window transparent and mask the contentView layer to the card
    // radius so the frost itself is rounded.
    ...
    if content != nil {
        ...
        let clayer: id = msg_send![content, layer];
        if clayer != nil {
            let _: () = msg_send![clayer, setCornerRadius: 20.0_f64];
            let _: () = msg_send![clayer, setMasksToBounds: true];
        }
        // Belt-and-suspenders: round the NSVisualEffectView subview's own
        // layer too — some macOS builds don't clip it via the parent mask.
        ...
        while i < count {
            ...
            if sub != nil {
                let _: () = msg_send![sub, setWantsLayer: true];
                let slayer: id = msg_send![sub, layer];
                if slayer != nil {
                    let _: () = msg_send![slayer, setCornerRadius: 20.0_f64];
                    let _: () = msg_send![slayer, setMasksToBounds: true];
                }
            }
            i += 1;
        }
...
/// Open (or toggle) the light glass menubar dropdown — the `/tray` surface (§B.2).
///
/// This is ADDITIVE: it does not replace the native tray menu; it is reached from a
/// "Dashboard" menu item (see `tray.rs`). The window is a transparent, borderless,
/// always-on-top panel with macOS `Effect::Menu` vibrancy, anchored top-right under
/// the menubar (where the tray icon lives). It uses the SAME `object_setClass`
/// NSPanel swizzle as the Search window — NOT `to_panel()`, whose
/// `Id::from_retained_ptr` causes a use-after-free SIGSEGV on close. Calling it
/// again toggles visibility.
///
/// NOTE: the on-device glass rendering / tray anchoring / crash-freeness can only be
/// verified by a real `tauri dev` build; this function compiles and follows the
/// proven Search recipe, but has not been run on a device in this change.
pub fn open_tray_dropdown(app: &AppHandle<Wry>) {
    const LABEL: &str = "tray-dropdown";
    const W: f64 = 320.0;
    // Provisional height only — the webview measures its rendered card and calls
    // `resize_tray_dropdown` so the glass hugs content (no empty frosted tail). Kept
    // close to the main page's natural height so there's no visible jump on open.
    const H: f64 = 344.0;
    ...
    // Anchor top-right, just under the menubar.
    let (x, y) = if let Ok(Some(monitor)) = app.primary_monitor() {
        ...
        let origin_y = pos.y as f64 / scale;
        (
            origin_x + (logical.width - W - 14.0).max(8.0),
            origin_y + 38.0,
        )
    } else {
        (900.0, 38.0)
    ...
    let builder = WebviewWindow::builder(app, LABEL, WebviewUrl::App("/tray".into()))
        .title("")
        .visible(false)
        .accept_first_mouse(true)
        .shadow(true)
        ...
        .position(x, y)
        .focused(true)
        // Force a LIGHT appearance so the `Effect::Menu` vibrancy renders its light
        // material — a translucent body then reads as light glass (over the dark
        // material it would read black-on-dark). The `/tray` surface is always light,
        // independent of system theme (like macOS Control Center).
        .theme(Some(tauri::Theme::Light))
        .resizable(false);

    // `UnderWindowBackground` samples + blurs the DESKTOP behind the window — the
    // strongest "liquid glass floating over the wallpaper" look (vs the denser `Menu`
    // frost). Paired with an ultra-light CSS tint + a specular rim (page.tsx) this reads
    // as delicate, highly-transparent true glass. Forced Aqua keeps it light.
    #[cfg(target_os = "macos")]
    ...
    let window = match builder.build() {
        Ok(w) => super::finalize_webview_window(w),
        Err(e) => {
            error!("open_tray_dropdown: build failed: {e}");
            return;
        }
    };

    // Dismiss the moment the panel loses key focus (click-away / app switch / Space
    // change) — the reliable native path. The JS `blur` handler is a secondary net and
    // the menubar icon toggles it closed too, so closing is never ambiguous.
    {
        let app_for_event = app.clone();
        window.on_window_event(move |event| {
            if matches!(event, tauri::WindowEvent::Focused(false)) {
                if let Some(w) = app_for_event.get_webview_window(LABEL) {
                    let _ = w.hide();
                }
            }
        });
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        run_on_main_thread_safe(app, move || {
            ...
            if let Ok(ns_win) = window_clone.ns_window() {
                let ns_win = ns_win as id;
                unsafe {
                    // Swizzle NSWindow → NSPanel (object_setClass, the crash-free path).
                    let nspanel_class: id =
                        msg_send![tauri_nspanel::raw_nspanel::RawNSPanel::class(), class];
                    object_setClass(ns_win, nspanel_class);
                    // Shared light-glass recipe (forced Aqua appearance + rounded
                    // vibrancy mask + one soft shadow) — also used by the Ask panel.
                    // See apply_light_glass_surface for the full rationale.
                    apply_light_glass_surface(ns_win);
                    // A menu-style popover: floats above normal windows, does not
                    // activate the app or switch Spaces.
                    let _: () = msg_send![ns_win, setLevel: 101_i64];
                    let current: i32 = msg_send![ns_win, styleMask];
                    let _: () = msg_send![ns_win, setStyleMask: current | 128]; // NSNonactivatingPanelMask
                    let _: () = msg_send![ns_win, setCollectionBehavior: 257_u64];
                    let _: () = msg_send![ns_win, setHidesOnDeactivate: false];
...
/// Open (or focus) the dedicated knowledge-graph map window — the `/graph-map`
/// surface. Unlike the tray dropdown this is a NORMAL resizable window (not an
/// NSPanel): an airy, chrome-light, full-window force-directed map of the user's own
/// `kg_*` graph, opened on demand from the dropdown's "View graph" entry. Forced to a
/// light appearance so the airy surface reads correctly regardless of system theme.
/// Self-contained (does not go through the `ShowRewindWindow` machinery) so it never
/// touches the Main-hiding / NSPanel-conversion logic that the app windows share.
pub fn open_graph_window(app: &AppHandle<Wry>) {
    const LABEL: &str = "graph-map";
    ...
    let (w, h) = screen_aware_size(app, 1200.0, 820.0);
    #[allow(unused_mut)]
    let mut builder = WebviewWindow::builder(app, LABEL, WebviewUrl::App("/graph-map".into()))
        .title("knowledge graph")
        .visible(true)
        .accept_first_mouse(true)
        .shadow(true)
        // Transparent + native vibrancy → the desktop shows through as light frosted
        // glass with the force-graph floating over it (the "window effect").
        .transparent(true)
        .inner_size(w.max(1040.0), h.max(720.0))
        .min_inner_size(720.0, 520.0)
        // Always-light airy surface (see KgGraphView), independent of system theme.
        .theme(Some(tauri::Theme::Light));
    ...
    #[cfg(target_os = "macos")]
    {
        builder = builder
            .hidden_title(true)
            .title_bar_style(tauri::TitleBarStyle::Overlay)
            // `UnderWindowBackground` blurs whatever is BEHIND the window (the desktop),
            // giving the frosted-glass "floating over the wallpaper" look of the sample.
            .effects(tauri::utils::config::WindowEffectsConfig {
                effects: vec![tauri::window::Effect::UnderWindowBackground],
                state: Some(tauri::window::EffectState::Active),
                ..Default::default()
    ...
    match builder.build() {
        Ok(window) => {
            let window = super::finalize_webview_window(window);
            // Force a LIGHT (Aqua) appearance so the vibrancy renders its light material
            // regardless of system dark mode (the builder `.theme()` hint is unreliable).
            #[cfg(target_os = "macos")]
            {
                let wc = window.clone();
                run_on_main_thread_safe(app, move || {
                    ...
                    if let Ok(ns_win) = wc.ns_window() {
                        ...
                        unsafe {
                            ...
                            let aqua: id =
                                msg_send![class!(NSAppearance), appearanceNamed: aqua_name];
                            if aqua != nil {
                                let _: () = msg_send![ns_win, setAppearance: aqua];
                            }
                        }
        ...
        Err(e) => {
            error!("open_graph_window: build failed: {e}");
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn window_id_label_from_str_display_roundtrip() {
        for label in [
            "main",
            "home",
            "search",
            ...
            let id = RewindWindowId::from_str(label).expect("from_str is infallible");
            assert_eq!(id.label(), label);
            assert_eq!(id.to_string(), label);
        }
    }

    #[test]
    fn ask_window_id_has_expected_identity() {
        let id = RewindWindowId::from_str("ask").unwrap();
        assert!(id == RewindWindowId::Ask);
        assert_eq!(id.label(), "ask");
        assert_eq!(id.title(), "ask civitas");
        // Ask glass-panel creation size (Ask-redesign plan: 620×600; NSPanels
        // can't be resized post-creation via plain setSize, so this IS the size).
        assert_eq!(id.min_size(), Some((620.0, 600.0)));
    }

    #[test]
    fn show_ask_maps_to_ask_window_id_with_no_metadata() {
        let show = ShowRewindWindow::Ask;
        assert!(show.id() == RewindWindowId::Ask);
        assert_eq!(show.metadata(), None);
    }

    #[test]
    fn unknown_label_falls_back_to_main() {
        let id = RewindWindowId::from_str("definitely-not-a-window").unwrap();
        assert!(id == RewindWindowId::Main);
    }
}
