...
//! Tauri commands for managing the civitas server and capture session.
//!
...
use civitas_engine::RecordingConfig;
...
fn build_config(app: &tauri::AppHandle) -> Result<RecordingConfig, String> {
    let store = load_settings_or_default(app)
        .map_err(|error| format!("cannot build capture configuration: {error}"))?;
...
pub async fn get_available_audio_devices() -> Result<Vec<AudioDeviceInfo>, String> {
    ...
    let devices = civitas_audio::core::device::list_audio_devices()
        .await
        .map_err(|e| format!("Failed to list audio devices: {}", e))?;

    let default_input = civitas_audio::core::device::default_input_device()
        .map(|d| d.to_string())
        .ok();
    let default_output = civitas_audio::core::device::default_output_device()
        .await
        .map(|d| d.to_string())
        .ok();
...
pub async fn get_available_monitors() -> Result<Vec<MonitorDevice>, String> {
    ...
    let monitors = civitas_screen::monitor::list_monitors().await;
...
pub async fn stop_capture(
    ...
    stop_capture_for_privacy(&state).await
}

/// Stop capture without stopping the local server.
///
/// Used by pause/private enforcement as the Rust-side privacy boundary. This
/// path does not depend on a hydrated webview listener, so a persisted
/// pause/private intent cannot be bypassed during boot or tray-only operation.
pub async fn stop_capture_for_privacy(state: &RecordingState) -> Result<(), String> {
    info!("Stopping capture session for pause/private enforcement");
...
pub async fn start_capture(
    ...
    crate::capture_control::capture_intent_allows_start()?;
    ...
    // concurrent `start_capture_internal` (called from spawn_civitas's
...
#[tauri::command]
#[specta::specta]
pub async fn stop_civitas(
    state: State<'_, RecordingState>,
    _app: tauri::AppHandle,
) -> Result<(), String> {
    info!("stop_civitas: stopping capture and server");
    ...
    // Shut down the server so the next spawn_civitas does a full restart
    // with fresh settings (auth key, port, etc.). Without this, spawn_civitas
    ...
    // Reset flags so the next spawn_civitas takes the full-start path
...
#[tauri::command]
#[specta::specta]
pub async fn spawn_civitas(
    state: State<'_, RecordingState>,
    app: tauri::AppHandle,
    _override_args: Option<Vec<String>>,
) -> Result<(), String> {
    info!("spawn_civitas: starting server + capture");
    ...
    if last_spawn > 0 && now_epoch.saturating_sub(last_spawn) < RESTART_COOLDOWN_SECS {
        ...
        tokio::spawn(async move {
            ...
            let port = match load_settings_or_default(&app_handle) {
                Ok(settings) => settings.recording.port,
                Err(error) => {
                    error!("deferred spawn aborted because settings are unavailable: {error}");
    ...
    let mut store = load_settings_or_default(&app).map_err(|error| {
        state.is_starting.store(false, Ordering::SeqCst);
        format!("capture cannot start while settings are unavailable: {error}")
    })?;
    crate::transcription_credentials::hydrate_recording_credentials(&mut store.recording)
        .await
        .map_err(|error| {
            state.is_starting.store(false, Ordering::SeqCst);
            format!("secure transcription credentials are unavailable: {error}")
        })?;
    let capture_suppression = crate::capture_control::capture_start_suppression_reason();
    if let Some(reason) = capture_suppression.as_deref() {
        info!("spawn_civitas: capture start suppressed by persisted intent: {reason}");
        crate::health::set_recording_status(crate::health::RecordingStatus::Paused);
    }
    ...
    // databases but catastrophic for a large real-world database: the
    // watchdog fired while migration was still healthy, both attempts raced
    // on the SQLite lock, and the app became stuck.
    //
        ...
        if server_guard.is_some() {
                ...
                .await
            {
                Ok(resp) if resp.status().is_success() => {
                    ...
                    // Server is fine — enforce the persisted capture intent.
                    drop(server_guard);
                    if capture_suppression.is_some() {
                        stop_capture_for_privacy(&state).await?;
                        state.is_starting.store(false, Ordering::SeqCst);
                        return Ok(());
                    }
    ...
    if capture_suppression.is_some() {
        info!(
            "Starting server without capture because persisted capture intent suppresses recording"
        );
    }
    ...
    // helper handles the explicit local env, settings, and encrypted secret store
    ...
    let capture_config = store.to_recording_config(data_dir);
    let (server_config, permission_gaps) = if capture_suppression.is_none() {
        let permissions_check = do_permissions_check(false);
        crate::permissions::constrain_runtime_config_to_permissions(
            capture_config.clone(),
            &permissions_check,
        )
    } else {
        let mut paused_config = capture_config.clone();
        paused_config.disable_vision = true;
        paused_config.disable_audio = true;
        paused_config.capture_microphone = false;
        paused_config.capture_system_audio = false;
        (
            paused_config,
            crate::permissions::CapturePermissionGaps::default(),
        )
    };

    if permission_gaps.screen_recording {
        warn!(
            "Screen recording permission is missing; local service will start while screen and system-audio capture remain paused"
        );
    }
    if permission_gaps.microphone {
        warn!(
            "Microphone permission is missing; local service will start while microphone capture remains paused"
        );
    }

    let server_arc = state.server.clone();
    let capture_arc = state.capture.clone();
    let capture_suppression_for_thread = capture_suppression.clone();
    ...
    // helper in `apps/civitas-app-tauri/lib/events/types.ts`).
    let app_for_pipe = app.clone();
    let pipe_agent_events = crate::agent_event_emitter::PipeAgentEventEmitter::new(app_for_pipe);
    let on_pipe_output: Option<civitas_core::pipes::OnPipeOutputLine> = Some(std::sync::Arc::new(
        move |pipe_name: &str, exec_id: i64, line: &str| {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            server_runtime.block_on(async move {
                ...
                let server = match ServerCore::start(&server_config, on_pipe_output).await {
                    Ok(s) => s,
                    Err(e) => {
                ...
                if let Some(reason) = capture_suppression_for_thread {
                    info!("Server started without capture due to persisted intent: {reason}");
                    {
                ...
                let capture = match CaptureSession::start(&server, &capture_config, true).await {
                    Ok(c) => c,
                    Err(e) => {
    ...
    match result_rx.await {
        Ok(Ok(())) => {
            info!("Civitas started successfully");
