...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            civitas_events::subscribe_to_event::<StallEvent>("meeting_streaming_audio_stall");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            civitas_events::subscribe_to_event::<StallEvent>("meeting_streaming_transcript_stall");
        while let Some(event) = sub.next().await {
...
fn handle_audio_stall(app: &AppHandle, event: StallEvent) {
    ...
    // surfaces even when the meeting window is on top of civitas. Use
    ...
    std::thread::spawn(move || {
        if let Err(e) = app_for_notify
            .notification()
            .builder()
            .title("civitas is not capturing audio")
            .body(format!(
