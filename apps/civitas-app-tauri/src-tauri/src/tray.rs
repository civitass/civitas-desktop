...
/// Flag set by the "quit civitas" menu item so that the ExitRequested
...
fn prefetch_tray_menu_data(app: &AppHandle) -> TrayMenuData {
    ...
    let (default_show, default_search, default_chat, default_ask) = if cfg!(target_os = "windows") {
        ("Alt+S", "Alt+K", "Alt+L", "Alt+J")
    } else {
        (
            "Control+Super+S",
            "Control+Super+K",
    ...
    let mut show_shortcut = if settings.show_civitas_shortcut.trim().is_empty() {
        default_show.to_string()
    } else {
        settings.show_civitas_shortcut.clone()
    };
    ...
    let mut ask_shortcut = if settings.ask_shortcut.trim().is_empty() {
        default_ask.to_string()
    } else {
        settings.ask_shortcut.clone()
    };

    if settings
        .disabled_shortcuts
        .iter()
        .any(|shortcut| shortcut == "showCivitasShortcut")
    {
        show_shortcut.clear();
    }
    if settings
    ...
    if settings
        .disabled_shortcuts
        .iter()
        .any(|shortcut| shortcut == "askShortcut")
    {
        ask_shortcut.clear();
    }
    ...
    let has_permission_issue = if onboarding_completed {
        #[cfg(target_os = "macos")]
        {
            ...
            let requirements = crate::permissions::CapturePermissionRequirements::from_settings(
                &settings.recording,
            );
            !requirements.satisfied_by(&perms)
        }
...
/// Legacy holder for a tray-owned pause timer. Pause/auto-resume is now owned
/// entirely by `capture_control` (the single authority — R2); the tray no longer
/// arms a timer here. This is kept only as a stable cancel target so
/// `capture_control::cancel_pause_timer` (invoked on every state transition) can
/// abort any timer left by an older build / in-flight upgrade path. New pauses
/// never populate it, so it is effectively always empty.
#[allow(dead_code)] // retained only as a cancel target; never constructed anymore
struct PauseTimer {
    handle: JoinHandle<()>,
}
...
/// Cancel the tray's own pause timer. `pub(crate)` so `capture_control` — the
/// single pause/private authority (R2) — can abort a stale tray timer whenever
/// it changes state, ensuring the two subsystems never both drive the shortcut
/// events (a stale tray timer could otherwise re-start capture during Private
/// mode).
pub(crate) fn cancel_pause_timer() {
...
pub(crate) fn force_tray_rebuild(app: &AppHandle) -> Result<()> {
    ...
    if let Some(tray) = app.tray_by_id("civitas_main") {
...
pub fn setup_tray(app: &AppHandle, update_item: Option<&tauri::menu::MenuItem<Wry>>) -> Result<()> {
    ...
    if let Some(main_tray) = app.tray_by_id("civitas_main") {
        ...
        // Start the menu updater only when the build exposes an update item.
...
pub fn log_tray_position(app: &AppHandle) {
    if let Some(tray) = app.tray_by_id("civitas_main") {
        ...
        error!("tray icon 'civitas_main' not found");
    }
}
    ...
    let _ = app.run_on_main_thread(move || {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            crate::window::with_autorelease_pool(|| {
                ...
                let _old = app.remove_tray_by_id("civitas_main");
                ...
                let icon = match app.path().resolve(
                    "assets/civitas-logo-tray-white.png",
                    tauri::path::BaseDirectory::Resource,
                ) {
                    Ok(path) => tauri::image::Image::from_path(path).ok(),
                    Err(_) => {
                        tauri::image::Image::from_path("assets/civitas-logo-tray-white.png").ok()
                    }
                };

                let mut builder = TrayIconBuilder::<Wry>::with_id("civitas_main")
...
fn set_autosave_name(_tray: &TrayIcon<Wry>) {
    ...
    // The tray icon position is intentionally handled by the recreate trick instead.
}
...
fn create_dynamic_menu(
    ...
    if !data.onboarding_completed {
        menu_builder = menu_builder
            ...
            .item(&PredefinedMenuItem::separator(app)?)
            .item(&MenuItemBuilder::with_id("quit", "Quit civitas").build(app)?);
    ...
    let ask_shortcut = &data.ask_shortcut;

    // --- Open civitas ---
    menu_builder = menu_builder
        .item(&MenuItemBuilder::with_id("open_app", "Open civitas").build(app)?)
        // Light glass dashboard dropdown (§B.2). Additive — the native menu stays;
        // this opens the /tray panel. A future change can move it to icon left-click.
        .item(&MenuItemBuilder::with_id("open_dashboard", "Dashboard").build(app)?)
        .item(&PredefinedMenuItem::separator(app)?);

    // --- Primary actions (most-used first) ---
    // Use native accelerators for right-aligned shortcut display (like Notion Calendar)
    let mut item = MenuItemBuilder::with_id("show_ask", "Ask");
    if !ask_shortcut.is_empty() {
        item = item.accelerator(&to_accelerator(ask_shortcut));
    }
    ...
    let version_text = match is_beta {
        true => format!("civitas v{} (Beta)", app.package_info().version),
        false => format!("civitas v{}", app.package_info().version),
    };
    menu_builder = menu_builder.item(
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("quit", "Quit civitas")
...
fn handle_menu_event(app_handle: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        "show_ask" => {
            let app = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
                let _ = ShowRewindWindow::Ask.show(&app);
                let _ = app.emit("tray-show-ask", ());
            });
        }
        "start_recording" | "stop_recording" | "toggle_recording" => {
            // Route the native tray toggle through capture_control — the single
            // pause/private authority (R2). Previously this emitted the shortcut
            // events directly AND capture_control ran its own CONTROL_STATE +
            // timer, so a native resume could silently override a persisted
            // Private/indefinite-pause intent. Going through
            // pause_capture/resume_capture keeps CONTROL_STATE authoritative and
            // cancels any stale tray timer, and re-checks private before emitting
            // a start.
            cancel_pause_timer();
            let status = get_effective_recording_status();
            let is_recording = status == RecordingStatus::Recording;
            let (optimistic, resuming) = if is_recording {
                (RecordingStatus::Paused, false)
            } else {
                (RecordingStatus::Starting, true)
            };
            set_optimistic_status(optimistic);
            let app = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if resuming {
                    if let Err(e) = crate::capture_control::resume_capture(app.clone()).await {
                        error!("tray toggle: resume_capture failed: {}", e);
                    }
                } else {
                    // Indefinite pause (no auto-resume) — matches the old toggle.
                    if let Err(e) = crate::capture_control::pause_capture(app.clone(), None).await {
                        error!("tray toggle: pause_capture failed: {}", e);
                    }
                }
            });
        ...
        id if id.starts_with("pause_") => {
            let mins: u64 = id
                ...
                .unwrap_or(15);
            // Pause via capture_control so CONTROL_STATE + a single auto-resume
            // timer are authoritative (R2). capture_control::pause_capture arms
            // its own generation-guarded auto-resume; the tray no longer keeps a
            // second PAUSE_TIMER (which could desync and re-start capture during
            // Private mode). pause_capture also cancels any leftover tray timer.
            set_optimistic_status(RecordingStatus::Paused);
            let app_for_pause = app_handle.clone();
            let duration_secs = (mins * 60) as u32;
            tauri::async_runtime::spawn(async move {
                if let Err(e) =
                    crate::capture_control::pause_capture(app_for_pause, Some(duration_secs)).await
                {
                    error!("tray pause_{mins}: pause_capture failed: {}", e);
                }
            });
            ...
            send_notify(
                "Recording paused",
                format!("civitas will auto-resume in {}.", pretty),
            );
        ...
        "update_now" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                ...
                if is_source_build(&app) {
                    ...
                    // For official builds, run the signed public update flow.
                    tauri::async_runtime::spawn(async move {
        ...
        "open_dashboard" => {
            // Toggle the light glass dropdown (§B.2).
            let app = app_handle.clone();
            let _ = app_handle.run_on_main_thread(move || {
        ...
        "quit" => {
            ...
            tauri::async_runtime::spawn(async move {
                info!("Stopping civitas recording before quit...");
                if let Some(recording_state) = app_handle_clone.try_state::<RecordingState>() {
                    ...
                    info!("Civitas server + recording stopped successfully");
                }
...
async fn update_menu_if_needed(
    ...
    let new_state = MenuState {
        shortcuts: {
            ...
            m.insert("ask".to_string(), data.ask_shortcut.clone());
            m
        },
    ...
    let tooltip: String = if has_perm_issue {
        "Civitas Desktop — ⚠️ permissions needed".to_string()
    } else if effective_status == RecordingStatus::Paused {
        // Read the countdown from capture_control's single authoritative state
        // (R2) rather than a tray-local timer.
        match crate::capture_control::pause_remaining() {
            Some(d) => format!(
                "Civitas Desktop — paused, resumes in {}",
                format_remaining(d)
            ),
            None => "Civitas Desktop — paused".to_string(),
        }
    } else {
        "Civitas Desktop".to_string()
    };
    let app_for_tooltip = app.clone();
    let _ = app.run_on_main_thread(move || {
        if let Some(tray) = app_for_tooltip.tray_by_id("civitas_main") {
    ...
    if should_update {
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            let _ = app.run_on_main_thread(move || {
                if std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    if let Some(tray) = app_for_thread.tray_by_id("civitas_main") {
