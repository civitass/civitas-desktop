...
pub async fn start_health_check(app: tauri::AppHandle) -> Result<()> {
    ...
    let mut monitoring_started_at: Option<Instant> = None;
    ...
    tokio::spawn(async move {
        loop {
            ...
            // A missing engine is intentional before the user reaches the
            // explicit engine-consent step. Keep the tray neutral, avoid local
            // health polling noise, and start the grace-period clock only when
            // capture is actually allowed. Otherwise a careful first-run user
            // would see a failed tray icon simply for reading onboarding.
            if !crate::persisted_onboarding_allows_capture(&app) {
            ...
            let monitoring_started_at = monitoring_started_at.get_or_insert_with(Instant::now);
            ...
            // `civitas-engine::permission_monitor` + capture-module emissions.
            ...
            if current_status != last_status || theme != last_theme {
                ...
                let icon_path = if is_unhealthy_icon(current_status) {
                    if theme == Mode::Light {
                        "assets/civitas-logo-tray-black-failed.png"
                    } else {
                        "assets/civitas-logo-tray-white-failed.png"
                    }
                } else {
                    if theme == Mode::Light {
                        "assets/civitas-logo-tray-black.png"
                    } else {
                ...
                let _ = app.run_on_main_thread(move || {
                    crate::window::with_autorelease_pool(|| {
                        if let Some(main_tray) = app_clone.tray_by_id("civitas_main") {
                            if let Err(e) =
            ...
            // When spawn_civitas completes, it updates last_spawn_epoch.
            ...
            if status == RecordingStatus::Recording
                && monitoring_started_at.elapsed() > NOTIFICATION_COOLDOWN
                && !in_restart_grace
            {
                if let Ok(ref health) = health_result {
                    ...
                    let woke = civitas_engine::sleep_monitor::recently_woke_from_sleep();
...
async fn show_capture_stall_notification(app: &tauri::AppHandle, system: &str) -> Result<()> {
    ...
    let payload = serde_json::json!({
        ...
        "body": format!("civitas has not received recent {} updates. this may recover on its own, but restarting usually fixes it.", system),
        "actions": [
