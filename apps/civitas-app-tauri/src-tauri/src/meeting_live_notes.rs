...
//!
//! The detector owns meeting inference. This module reacts to its stable
//! lifecycle events and to local calendar publications, then offers deliberate
//! join/open-note actions without starting capture automatically.
...
/// Calendar notifications are useful only shortly before a joinable event.
const PREWARM_LEAD: chrono::Duration = chrono::Duration::minutes(3);
...
struct CalendarPrewarmCandidate {
    key: String,
    title: String,
    meeting_url: String,
...
pub fn start(app: AppHandle) {
    ...
    // A sent prewarm is also the suppression record for the later
    // audio/UI-driven `meeting_started` toast.
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            civitas_events::subscribe_to_event::<Vec<CalendarEventSignal>>("calendar_events");
        while let Some(event) = sub.next().await {
            let current_events: Vec<CalendarEventSignal> = event
                .data
                .into_iter()
            ...
            if meeting_notifications_enabled(&prewarm_app) {
                ...
                for candidate in current_events
                    .iter()
                    .filter_map(|event| calendar_prewarm_candidate(event, now))
                {
                    // Reserve the key before sending so concurrent native and
                    // ICS publications cannot double-notify the same event.
                    let should_send = {
                        let mut guard = prewarm_suppressed.write().await;
                        guard.retain(|_, sent_at| sent_at.elapsed() < PREWARM_SUPPRESS_TTL);
                        if guard.contains_key(&candidate.key) {
                            false
                        } else {
                            guard.insert(candidate.key.clone(), Instant::now());
                            true
                        }
                    };
                    if should_send {
                        send_calendar_prewarm_notification(&prewarm_app, &candidate);
                    }
                }
            }

            let mut guard = calendar_events_for_sub.write().await;
            *guard = current_events;
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub = civitas_events::subscribe_to_event::<MeetingStartedEvent>("meeting_started");
        ...
        while let Some(event) = sub.next().await {
            ...
            // a static note pane.
            let url = format!("civitas://meeting/{meeting_id}");
            ...
            client::send_typed_with_actions(
                "meeting detected",
                format!("civitas is saving this meeting for transcription: {title}"),
                "meeting",
                Some(30_000),
                actions,
            );
...
fn forward_civitas_event(app: AppHandle, source: &'static str, target: &'static str) {
    tauri::async_runtime::spawn(async move {
        let mut sub = civitas_events::subscribe_to_event::<serde_json::Value>(source);
        while let Some(event) = sub.next().await {
...
fn calendar_prewarm_candidate(
    event: &CalendarEventSignal,
    now: chrono::DateTime<chrono::Utc>,
) -> Option<CalendarPrewarmCandidate> {
    if event.is_all_day || event.attendees.len() < 2 {
        return None;
    }
    let start = parse_rfc3339_utc(&event.start)?;
    let until_start = start.signed_duration_since(now);
    if until_start <= chrono::Duration::zero() || until_start > PREWARM_LEAD {
        return None;
    }
    let meeting_url = event_join_url(event)?;
    let title = if event.title.trim().is_empty() {
        "meeting".to_string()
    } else {
        event.title.trim().to_string()
    };
    Some(CalendarPrewarmCandidate {
        key: prewarm_key(&event.title, &event.start),
        title,
        meeting_url,
        seconds_until_start: until_start.num_seconds(),
    })
}

fn send_calendar_prewarm_notification(app: &AppHandle, candidate: &CalendarPrewarmCandidate) {
    let mut actions = vec![json!({
        "id": "join-meeting",
        "action": "join-meeting",
        "label": "join and take notes",
        "type": "meeting_join",
        "url": candidate.meeting_url.clone(),
        "primary": true,
    })];
    // Prewarm fires before the meeting row exists; the HD action uses a
    // prewarm-bound fallback and upgrades when the detector confirms the call.
    if let Some(hd) = build_hd_action(app, None) {
        actions.push(hd);
    }

    let minutes = ((candidate.seconds_until_start as f64) / 60.0).ceil() as i64;
    ...
    client::send_typed_with_actions(
        &header,
        format!("civitas is ready to transcribe: {}", candidate.title),
        "meeting",
        Some(30_000),
        actions,
    );
...
fn normalize_meeting_url(raw: Option<String>) -> Option<String> {
    ...
    let host = parsed
        ...
        .trim_end_matches('.')
        .to_ascii_lowercase();
    let known_host = host == "meet.google.com"
        || host == "zoom.us"
    ...
    known_host.then(|| parsed.to_string())
}
...
mod tests {
    ...
    #[test]
    fn prewarm_accepts_only_imminent_joinable_group_events() {
        let now = chrono::Utc::now();
        let event = CalendarEventSignal {
            title: "Design review".to_string(),
            start: (now + chrono::Duration::seconds(90)).to_rfc3339(),
            end: (now + chrono::Duration::minutes(30)).to_rfc3339(),
            attendees: vec!["owner@example.test".into(), "peer@example.test".into()],
            meeting_url: Some("meet.google.com/abc-defg-hij".to_string()),
            ..Default::default()
        };

        let candidate = calendar_prewarm_candidate(&event, now).expect("prewarm candidate");
        assert_eq!(candidate.title, "Design review");
        assert_eq!(candidate.seconds_until_start, 90);
        assert_eq!(
            candidate.meeting_url,
            "https://meet.google.com/abc-defg-hij"
        );

        let mut one_attendee = event.clone();
        one_attendee.attendees.pop();
        assert!(calendar_prewarm_candidate(&one_attendee, now).is_none());

        let mut too_early = event.clone();
        too_early.start = (now + chrono::Duration::minutes(4)).to_rfc3339();
        assert!(calendar_prewarm_candidate(&too_early, now).is_none());

        let mut already_started = event;
        already_started.start = (now - chrono::Duration::seconds(1)).to_rfc3339();
        assert!(calendar_prewarm_candidate(&already_started, now).is_none());
    }

    #[test]
    fn meeting_urls_require_https_and_an_exact_supported_host() {
        assert_eq!(
            normalize_meeting_url(Some("company.zoom.us/j/123".into())).as_deref(),
            Some("https://company.zoom.us/j/123")
        );
        assert!(normalize_meeting_url(Some("http://zoom.us/j/123".into())).is_none());
        assert!(
            normalize_meeting_url(Some("https://evil.example/?next=zoom.us/j/123".into()))
                .is_none()
        );
        assert!(normalize_meeting_url(Some("https://zoom.us.evil.example/j/123".into())).is_none());
        let credential_url = format!(
            "https://{}:{}@zoom.us/j/123",
            "fixture-user", "fixture-password"
        );
        assert!(normalize_meeting_url(Some(credential_url)).is_none());
    }
