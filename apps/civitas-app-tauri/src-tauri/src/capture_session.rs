...
use civitas_audio::audio_manager::builder::AudioManagerOptions;
use civitas_audio::core::device::resolve_audio_devices_for_capture;
use civitas_audio::core::engine::AudioTranscriptionEngine;
use civitas_audio::meeting_detector::MeetingDetector;
use civitas_audio::transcription::deepgram::{
...
use civitas_events::{send_event, PermissionEvent, PermissionKind};
...
impl CaptureSession {
    ...
    pub async fn start(
        ...
        // Re-evaluate TCC immediately before native capture starts. The server
        // remains available even when a requested source is unavailable; only
        // that source is paused for this process. Persisted settings are not
        // changed, so a relaunch after granting access restores it.
        let permissions = crate::permissions::do_permissions_check(false);
        let (effective_config, permission_gaps) =
        ...
        if permission_gaps.screen_recording {
            warn!(
                "Screen Recording permission is unavailable — screen and system-audio capture are paused"
            );
            let _ = send_event(
        ...
        if permission_gaps.microphone {
            warn!("Microphone permission is unavailable — microphone capture is paused");
            let _ = send_event(
                PermissionEvent::needed(PermissionKind::Microphone).event_name(),
                PermissionEvent::needed(PermissionKind::Microphone),
            );
        }
        ...
        // check_screen_recording_tauri() skips capture_probe on macOS 15+
        // (avoids the native TCC dialog CGWindowListCreateImage triggers).
        // Skip vision entirely when not granted; a full app relaunch after
        // granting access re-evaluates the persisted source preference.
        #[cfg(target_os = "macos")]
        let screen_recording_permitted =
            civitas_core::permissions::check_screen_recording_tauri().is_granted();
        #[cfg(not(target_os = "macos"))]
        let screen_recording_permitted = true;

        if !config.disable_vision && !screen_recording_permitted {
            warn!("Screen recording permission not yet granted — skipping VisionManager to avoid native TCC dialog; capture will start after permission is granted and Civitas relaunches");
        ...
        // --- Audio recording ---
        // Audio start is deferred ~1s so ScreenCaptureKit settles first, and runs
        // in its own task so vision/UI startup doesn't block on it. It must NOT be
        // fire-and-forget, though: a swallowed start error used to leave the UI
        // showing "recording" while audio was silently dead. So we retry once on
        // failure and, if it still fails, surface a `recording_error` event the
        // frontend can act on. (AudioManager::start now also rolls its status back
        // to Stopped on failure, so the device monitor and /health see the truth.)
        if !config.disable_audio {
            ...
            tokio::spawn(async move {
                ...
                let start_result = match audio_manager_clone.start().await {
                    Ok(()) => Ok(()),
                    Err(first) => {
                        warn!("audio manager start failed, retrying once: {}", first);
                        tokio::time::sleep(Duration::from_secs(2)).await;
                        audio_manager_clone.start().await
                    }
                };

                if let Err(e) = start_result {
                    error!("Failed to start audio manager after retry: {}", e);
                    let _ = send_event(
                        "recording_error",
                        serde_json::json!({
                ...
                } else if drm_pause && civitas_engine::drm_detector::drm_content_paused() {
        ...
        #[cfg(target_os = "macos")]
        let accessibility_permitted = civitas_core::permissions::check_accessibility().is_granted();
        #[cfg(not(target_os = "macos"))]
        let accessibility_permitted = true;

        let ui_recorder_handle = if !accessibility_permitted {
            warn!("Accessibility permission not yet granted — skipping optional UI event recording to avoid a native TCC dialog; screen/OCR and audio capture remain available");
            // Emit permission_needed event so frontend can trigger onboarding/permission flow
            let _ = send_event(
            ...
            .await
            {
                Ok(handle) => {
                    ...
                    civitas_engine::drm_detector::set_ui_recorder_stop_flag(handle.stop_flag());
                    Some(handle)
                }
                Err(e) => {
        ...
        // --- Speaker identification ---
        let _speaker_id_handle = civitas_engine::start_speaker_identification(
            server.db.clone(),
...
async fn invalidate_macos_screen_streams(reason: &str) {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(|| {
            civitas_screen::stream_invalidation::invalidate_streams();
        }),
    )
    .await;
...
fn log_capture_transcription_config(config: &RecordingConfig, options: &AudioManagerOptions) {
    ...
    {
        warn!(
            "background engine maps to Deepgram but provider credentials are incomplete — add a direct API key so batch STT can start"
        );
    }
}
