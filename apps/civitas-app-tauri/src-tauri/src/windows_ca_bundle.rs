...
//! every HTTPS request — breaking a user-approved assistant package install,
//! the assistant's provider calls, and any workflow that uses `fetch()`.
//!
//! Fix: read the Windows `ROOT` + `CA` cert stores via Crypt32, write them
//! to `%LOCALAPPDATA%\civitas\system-roots.pem`, and set
//! `NODE_EXTRA_CA_CERTS=<path>` on the process env. Child processes
//! (bun, node, the optional assistant, and workflow scripts) inherit it
...
fn install_inner() -> Result<std::path::PathBuf, String> {
    ...
    let dir = std::path::PathBuf::from(&local_app_data).join("civitas");
