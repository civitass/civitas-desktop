...
//! viewer deeplinks (`civitas://view?path=…`).
//!
...
/// the transformed string. Idempotent — already-rewritten `civitas://`
        ...
        .replace_all(body, |caps: &Captures| {
            ...
            match resolve_local_path(url) {
                Some(abs) => {
                    ...
                    format!("[{}](civitas://view?path={})", label, encoded)
                }
...
mod tests {
    ...
    fn rewrites_absolute_unix_path() {
        ...
        assert_eq!(
            out,
            "see [log](civitas://view?path=%2FUsers%2Fx%2Flog.md) here"
        );
    }
    ...
    fn rewrites_file_uri() {
        ...
        assert_eq!(out, "[x](civitas://view?path=%2Ftmp%2Fa.md)");
    }
    ...
    fn idempotent_on_viewer_links() {
        let body = "[x](civitas://view?path=%2Ftmp%2Fa.md)";
        assert_eq!(rewrite_file_links(body), body);
    }
    ...
    fn handles_multiple_links() {
        ...
        assert!(out.contains("civitas://view?path=%2Fx.md"));
        assert!(out.contains("https://y"));
        assert!(out.contains("civitas://view?path=%2Fz.json"));
    }

    #[test]
    fn rewrites_windows_drive_path() {
        let out = rewrite_file_links("[a](C:/foo/bar.md)");
        assert!(out.contains("civitas://view?path="));
    ...
    fn drops_fragment_on_file_uri() {
        ...
        assert!(out.contains("civitas://view?path=%2Ftmp%2Fa.md"));
    ...
    fn preserves_label_with_unicode() {
        ...
        assert!(out.starts_with("[📝 log](civitas://view?path="));
    }
}
