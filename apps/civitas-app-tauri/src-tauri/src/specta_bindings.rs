...
pub fn export_typescript_bindings_with(
    ...
    let generated = content
        ...
        .join("\n")
        + "\n";
    let normalized = format!("{SOURCE_HEADER}{generated}");
...
mod tests {
    ...
    fn temp_bindings_path() -> (tempfile::TempDir, PathBuf) {
        let dir = tempfile::Builder::new()
            .prefix("civitas-tauri-specta-")
            .tempdir()
            .expect("create isolated bindings directory");
        let path = dir.path().join("tauri.ts");
        (dir, path)
    }
    ...
    fn export_typescript_bindings() {
        ...
        let (_temp_dir, path) = temp_bindings_path();
        export_typescript_bindings_to(&path).expect("failed to export tauri-specta bindings");
        assert!(
    ...
    fn tauri_bindings_are_current() {
        ...
        let (_temp_dir, generated) = temp_bindings_path();
        ...
        assert_eq!(
            ...
            "lib/utils/tauri.ts is out of date.\n\
             Regenerate with:\n\
               cd apps/civitas-app-tauri && bun run bindings:generate"
        );
    }
}
