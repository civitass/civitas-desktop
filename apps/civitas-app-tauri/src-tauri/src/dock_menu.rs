...
pub fn setup_dock_menu(app_handle: AppHandle) {
    ...
    unsafe {
        ...
        extern "C" fn show_civitas(_this: &Object, _sel: Sel, _sender: id) {
        ...
        extern "C" fn dock_menu(_this: &Object, _sel: Sel, _sender: id) -> id {
            unsafe {
                ...
                // "Show civitas"
                let title = NSString::alloc(nil).init_str("Show civitas");
                let action = sel!(showCivitas:);
        ...
        let mut decl =
            objc::declare::ClassDecl::new("CivitasDockMenuDelegate", superclass).unwrap();
        decl.add_method(
            sel!(showCivitas:),
            show_civitas as extern "C" fn(&Object, Sel, id),
        );
        decl.add_method(
            sel!(openSettings:),
        ...
        if !method.is_null() {
            ...
            for sel_name in &[sel!(showCivitas:), sel!(openSettings:)] {
