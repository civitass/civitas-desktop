...
fn check_accessibility_permission() -> OSPermissionStatus {
    core_to_os_status(civitas_core::permissions::check_accessibility())
}
...
fn core_to_os_status(status: civitas_core::permissions::PermissionStatus) -> OSPermissionStatus {
    match status {
        civitas_core::permissions::PermissionStatus::NotNeeded => OSPermissionStatus::NotNeeded,
        civitas_core::permissions::PermissionStatus::NotDetermined => OSPermissionStatus::Empty,
        civitas_core::permissions::PermissionStatus::Granted => OSPermissionStatus::Granted,
        civitas_core::permissions::PermissionStatus::Denied => OSPermissionStatus::Denied,
    }
}
...
/// local database, search, graph, Timeline history, export, or settings.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) struct CapturePermissionRequirements {
    pub screen_recording: bool,
    pub microphone: bool,
}

impl CapturePermissionRequirements {
    pub(crate) fn from_settings(settings: &civitas_config::RecordingSettings) -> Self {
        let (capture_microphone, capture_system_audio) = settings.effective_audio_capture_sources();
        Self {
            // ScreenCaptureKit permission is required by visible-screen capture
            // and by macOS system-audio capture.
            screen_recording: !settings.disable_vision || capture_system_audio,
            microphone: capture_microphone,
        }
    }

    fn from_runtime_config(config: &civitas_engine::RecordingConfig) -> Self {
        Self {
            screen_recording: !config.disable_vision || config.capture_system_audio,
            microphone: !config.disable_audio && config.capture_microphone,
        }
    }

    pub(crate) fn satisfied_by(self, permissions: &OSPermissionsCheck) -> bool {
        (!self.screen_recording || permissions.screen_recording.permitted())
            && (!self.microphone || permissions.microphone.permitted())
    }
}

/// Capture sources that were requested but cannot start under the current TCC
/// state. The local service still starts so existing history and non-capture
/// features remain available.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) struct CapturePermissionGaps {
    pub screen_recording: bool,
    pub microphone: bool,
}

/// Return a process-local runtime configuration with only unpermitted capture
/// sources disabled. This never mutates the persisted user preference.
///
/// Keeping this transformation separate from the stored settings lets Civitas
/// start its local API and show prior Timeline data while macOS permission is
/// missing. On the next launch, the original preference is evaluated again and
/// the source starts automatically once the OS grant is active.
pub(crate) fn constrain_runtime_config_to_permissions(
    mut config: civitas_engine::RecordingConfig,
    permissions: &OSPermissionsCheck,
) -> (civitas_engine::RecordingConfig, CapturePermissionGaps) {
    let requirements = CapturePermissionRequirements::from_runtime_config(&config);
    let gaps = CapturePermissionGaps {
        screen_recording: requirements.screen_recording
            && !permissions.screen_recording.permitted(),
        microphone: requirements.microphone && !permissions.microphone.permitted(),
    };

    if gaps.screen_recording {
...
pub fn check_microphone_permission() -> OSPermissionStatus {
    core_to_os_status(civitas_core::permissions::check_microphone())
}
...
pub fn check_input_monitoring_permission_cmd() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        if civitas_a11y::check_input_monitoring() {
...
pub async fn request_input_monitoring_permission() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        ...
        if civitas_a11y::request_input_monitoring() {
            OSPermissionStatus::Granted
        } else {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if matches!(permission, OSPermission::Calendar) {
            tokio::task::spawn_blocking(|| {
                let cal = civitas_connect::calendar::CivitasCalendar::new();
                cal.reset();
            })
            .await
...
pub fn do_permissions_check(initial_check: bool) -> OSPermissionsCheck {
    #[cfg(target_os = "macos")]
    {
        ...
        OSPermissionsCheck {
            screen_recording: match (
                check_screen_recording_permission().permitted(),
                initial_check,
            ) {
...
fn run_self_detached(flag: &str) -> bool {
    ...
    let label = format!("team.civitas.self-{}", flag.trim_start_matches("--"));
    let result_path = format!("/tmp/civitas_self_{}_result", flag.trim_start_matches("--"));
...
// `civitas-engine::permission_monitor` which emits `permission_lost` /
...
#[cfg(test)]
mod capture_permission_tests {
    use super::*;

    fn granted_permissions() -> OSPermissionsCheck {
        OSPermissionsCheck {
            screen_recording: OSPermissionStatus::Granted,
            microphone: OSPermissionStatus::Granted,
            accessibility: OSPermissionStatus::Denied,
        }
    }

    #[test]
    fn requirements_follow_enabled_sources_not_optional_permissions() {
        let mut settings = civitas_config::RecordingSettings::default();
        assert_eq!(
            CapturePermissionRequirements::from_settings(&settings),
            CapturePermissionRequirements::default()
        );
        ...
        settings.disable_vision = true;
        settings.disable_audio = false;
        settings.capture_microphone = Some(true);
        settings.capture_system_audio = Some(false);
        assert_eq!(
            CapturePermissionRequirements::from_settings(&settings),
            CapturePermissionRequirements {
        ...
        settings.capture_microphone = Some(false);
    ...
    #[test]
    fn accessibility_never_blocks_selected_capture_sources() {
        let requirements = CapturePermissionRequirements {
            screen_recording: true,
            microphone: true,
        };
        assert!(requirements.satisfied_by(&granted_permissions()));
    }

    #[test]
    fn permission_constraints_disable_only_the_unavailable_sources() {
        let mut settings = civitas_config::RecordingSettings::default();
        settings.disable_vision = false;
        settings.disable_audio = false;
        settings.capture_microphone = Some(true);
        settings.capture_system_audio = Some(true);
        let original = civitas_engine::RecordingConfig::from_settings(
            &settings,
            std::path::PathBuf::from("/tmp/civitas-test"),
            None,
        );

        let permissions = OSPermissionsCheck {
            screen_recording: OSPermissionStatus::Denied,
            microphone: OSPermissionStatus::Granted,
            accessibility: OSPermissionStatus::Denied,
        };
        let (effective, gaps) = constrain_runtime_config_to_permissions(original, &permissions);

        assert_eq!(
            gaps,
            CapturePermissionGaps {
                screen_recording: true,
                microphone: false,
            }
        );
        assert!(effective.disable_vision);
