...
pub async fn calendar_status() -> Result<CalendarStatus, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let auth_status = CivitasCalendar::authorization_status();
...
/// Consumed by meeting live notes for local calendar enrichment and prewarm.
///
...
pub async fn start_calendar_events_publisher() {
    ...
    loop {
        ...
        if let Err(e) = civitas_events::send_event("calendar_events", items) {
...
async fn collect_calendar_events() -> Vec<CalendarEventItem> {
    #[cfg(target_os = "macos")]
    {
        ...
        let status = CivitasCalendar::authorization_status();
        if format!("{}", status) != "Full Access" && !CivitasCalendar::access_granted_this_session()
        {
            return Vec::new();
        }
        ...
        .await
        {
            ...
            Ok(Err(_)) => {
                warn!("calendar_publisher fetch_failed=true");
                Vec::new()
            }
            Err(_) => {
                error!("calendar_publisher task_panicked=true");
                Vec::new()
            }
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match tokio::task::spawn_blocking(|| {
            ...
            info!("calendar_publisher calendar_count={}", calendars.len());
            cal.get_events(1, 2)
        })
        .await
        {
...
#[cfg(target_os = "macos")]
fn calendar_event_to_item(event: civitas_connect::calendar::CalendarEvent) -> CalendarEventItem {
...
fn calendar_event_to_item_win(
    event: civitas_connect::calendar_windows::CalendarEvent,
) -> CalendarEventItem {
    let start_display = event.start_local.format("%-I:%M %p").to_string();
    let end_display = event.end_local.format("%-I:%M %p").to_string();
