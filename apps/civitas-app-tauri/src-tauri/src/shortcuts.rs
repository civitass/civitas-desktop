...
impl ShortcutConfig {
    async fn from_store(app: &AppHandle) -> Result<Self, String> {
        let store = SettingsStore::get(app)?.unwrap_or_default();
        ...
        Ok(Self {
            show: store.show_civitas_shortcut,
...
pub async fn update_global_shortcuts(
    ...
    let config = ShortcutConfig {
        ...
        // Ask flows through the store like show_chat/search — the frontend
        // saves `askShortcut`, then re-invokes this command to re-apply.
        ask: store_config.ask,
...
async fn apply_shortcuts(app: &AppHandle, config: &ShortcutConfig) -> Result<(), String> {
    ...
    register_shortcut(app, &config.show, config.is_disabled("show"), |app| {
        ...
        let _ = app.run_on_main_thread(move || {
            ...
            // next app restart.
            let settings = match load_settings_or_default(app) {
            ...
            let _ = app.emit("shortcut-show", ());
            {
                ...
                let mode = settings.overlay_mode;
    ...
    register_shortcut(
        ...
        |app| {
            let mut settings = match load_settings_or_default(app) {
                Ok(settings) => settings,
                Err(error) => {
                    error!(
                        "start-audio shortcut ignored because settings are unavailable: {error}"
                    );
                    return;
                }
            };
            let has_source_consent = settings.recording.capture_microphone == Some(true)
                || settings.recording.capture_system_audio == Some(true);
            if !has_source_consent {
                info!("start-audio shortcut ignored until an audio source is enabled in Settings");
                return;
            }
            settings.recording.disable_audio = false;
            if let Err(error) = settings.save(app) {
                error!("failed to atomically persist start-audio shortcut state: {error}");
                return;
            }
    ...
    register_shortcut(
        ...
        |app| {
            let mut settings = match load_settings_or_default(app) {
                Ok(settings) => settings,
                Err(error) => {
                    error!("stop-audio shortcut ignored because settings are unavailable: {error}");
                    return;
                }
            };
            settings.recording.disable_audio = true;
            if let Err(error) = settings.save(app) {
                error!("failed to atomically persist stop-audio shortcut state: {error}");
                return;
            }
    ...
    // Ask overlay toggle — same nspanel toggle pattern as show_chat above,
    // targeting the "ask" label/panel.
    register_shortcut(app, &config.ask, config.is_disabled("ask"), |app| {
        let app_for_closure = app.clone();
        let _ = app.run_on_main_thread(move || {
            let app = &app_for_closure;
            info!("ask shortcut triggered");
            let _ = app.emit("shortcut-show-ask", ());
            if let Some(_window) = app.get_webview_window("ask") {
                #[cfg(target_os = "macos")]
                {
                    use tauri_nspanel::ManagerExt;
                    if let Ok(panel) = app.get_webview_panel("ask") {
                        if panel.is_visible() {
                            panel.order_out(None);
                            return;
                        }
...
mod tests {
    ...
    #[test]
    fn parse_shortcut_accepts_ask_defaults() {
        // macOS/Linux default for the Ask overlay
        let mac = parse_shortcut("Control+Super+J").expect("macOS ask default must parse");
        assert_eq!(
            mac,
            Shortcut::new(Some(Modifiers::CONTROL | Modifiers::META), Code::KeyJ)
        );

        // Windows default for the Ask overlay
        let win = parse_shortcut("Alt+J").expect("Windows ask default must parse");
        assert_eq!(win, Shortcut::new(Some(Modifiers::ALT), Code::KeyJ));
    }

    #[test]
    fn parse_shortcut_accepts_sibling_summon_keys() {
        // The ask default sits in the chat=L / search=K family — all three
        // must keep parsing with the same modifier set.
        for (combo, code) in [
            ("Control+Super+L", Code::KeyL),
            ("Control+Super+K", Code::KeyK),
            ("Control+Super+J", Code::KeyJ),
        ] {
            let parsed = parse_shortcut(combo).unwrap_or_else(|e| panic!("{combo}: {e}"));
            assert_eq!(
                parsed,
                Shortcut::new(Some(Modifiers::CONTROL | Modifiers::META), code)
            );
        }
    }

    #[test]
    fn parse_shortcut_rejects_unknown_modifier_and_key() {
        assert!(parse_shortcut("Hyper+J").is_err());
        assert!(parse_shortcut("Control+Super+Kanji").is_err());
        // Empty string is not parseable — register_shortcut skips it earlier.
        assert!(parse_shortcut("").is_err());
    }

    #[test]
    fn is_disabled_maps_ask_to_frontend_key() {
        let config = ShortcutConfig {
            disabled: vec!["askShortcut".to_string()],
            ..Default::default()
        };
        assert!(config.is_disabled("ask"));
        assert!(!config.is_disabled("show_chat"));
        assert!(!config.is_disabled("search"));
    }

    #[test]
    fn is_disabled_accepts_raw_type_and_ignores_others() {
        let config = ShortcutConfig {
            disabled: vec!["ask".to_string()],
            ..Default::default()
        };
        assert!(config.is_disabled("ask"));

        let empty = ShortcutConfig::default();
        assert!(!empty.is_disabled("ask"));
    }
}
