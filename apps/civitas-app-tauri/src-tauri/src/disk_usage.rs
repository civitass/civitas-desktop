...
pub struct CachedDiskUsage {
    pub timestamp: i64,
    pub usage: DiskUsage,
    /// The civitas data directory this entry was computed for. Used to
...
pub fn get_cache_dir() -> Result<Option<PathBuf>, String> {
    ...
    Ok(Some(proj_dirs.join("civitas")))
}
...
pub async fn disk_usage(
    ...
    let data_dir = civitas_dir.join("data");
    ...
    let current_dir_key = canonical_dir_key(civitas_dir);
        ...
        if let Ok(content) = fs::read_to_string(&cache_file) {
            if content.contains("---") {
                ...
                let one_hour = 60 * 60; // 1 hour cache (reduced from 2 days)
                                        ...
                                        // `civitas_dir` defaults to "" on older cache entries — those
                                        ...
                                        // the same directory.
                let cached_key_normalized = canonical_dir_key(Path::new(&cached.civitas_dir));
                let dir_matches =
                    !cached.civitas_dir.is_empty() && cached_key_normalized == current_dir_key;
                ...
                if !dir_matches {
                    info!(
                        ...
                        cached.civitas_dir, current_dir_key
                    );
                }
            }
        }
    ...
    info!("Calculating total data size for: {}", civitas_dir.display());
    let total_data_size_bytes = directory_size(civitas_dir)
    ...
    for file_name in ["db.sqlite", "db.sqlite-wal", "db.sqlite-shm"] {
        let db_path = civitas_dir.join(file_name);
        if db_path.exists() {
    ...
    if let Ok(entries) = fs::read_dir(civitas_dir) {
    ...
    let pipes_size: u64 = {
        let pipes_dir = civitas_dir.join("pipes");
        if pipes_dir.exists() {
    ...
    let available_space = {
        ...
        let path_obj = Path::new(&civitas_dir);
        let available = sys
            .disks()
            .iter()
