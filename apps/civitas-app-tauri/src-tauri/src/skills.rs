...
//! device, copy chosen ones into the civitas skills store
//! (`<data_dir>/skills/<name>/`), and list / remove what's been imported.
//!
//! The store is the source of truth. `civitas-core`'s pi executor mirrors it
...
/// Baseline skills civitas writes itself on every session. Importing a skill
/// under one of these names would clobber them, so we reject it.
const RESERVED_SKILL_NAMES: [&str; 3] = ["civitas-api", "civitas-cli", "civitas-team"];
...
fn skills_store_dir() -> PathBuf {
    civitas_core::paths::default_civitas_data_dir().join("skills")
}
...
pub fn import_skill(source_path: String) -> Result<ImportedSkill, String> {
    ...
    if RESERVED_SKILL_NAMES.contains(&key.as_str()) {
        return Err(format!("\"{key}\" is a reserved civitas skill name"));
    }
    ...
    civitas_core::paths::copy_dir_all(&src, &dest)
...
pub fn remove_imported_skill(name: String) -> Result<(), String> {
    ...
    let chat_copy = civitas_core::paths::default_civitas_data_dir()
        .join("pi-chat")
        .join(".pi")
        .join("skills")
        .join(&key);
