...
pub fn install_user_data_dir() {
    ...
    eprintln!(
        "civitas: WebView2 user data dir source={} writable={} path={}{}",
        diagnostics.source,
...
pub fn diagnostics_snapshot_json() -> serde_json::Value {
    ...
    serde_json::json!({
        "status": if diagnostics.writable { "ok" } else { "degraded" },
        "source": diagnostics.source,
        "writable": diagnostics.writable,
        "elevation": diagnostics.elevation,
        "notePresent": diagnostics.note.is_some()
    })
}
...
fn ensure_writable_dir(path: &Path) -> Result<(), String> {
    ...
    let test_path = path.join(format!(".civitas-write-test-{}", std::process::id()));
