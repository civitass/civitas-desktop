...
#[cfg(target_os = "macos")]
async fn materialize_local_http_image(
    image_path: String,
) -> Result<(String, Option<std::path::PathBuf>), String> {
    let Ok(url) = reqwest::Url::parse(&image_path) else {
        return Ok((image_path, None));
    };
    ...
    if !is_loopback || !url.username().is_empty() || url.password().is_some() {
        return Err("live text only fetches local engine images".to_string());
    }
    if url.query_pairs().any(|(key, _)| key == "token") {
        return Err("credentials are not accepted in image URLs".to_string());
    }
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|error| format!("failed to build local image client: {error}"))?;
    ...
    let response = request
        .send()
        .await
        .map_err(|error| format!("failed to fetch local frame: {error}"))?;
    if !response.status().is_success() {
        return Err(format!(
            "local frame request failed with status {}",
            response.status()
        ));
    }
    ...
    if response.content_length().unwrap_or(0) > MAX_IMAGE_BYTES {
        return Err("local frame exceeds the 32 MiB live text limit".to_string());
    }
    ...
    let bytes = response
        .bytes()
        .await
        .map_err(|error| format!("failed to read local frame: {error}"))?;
    ...
    tokio::fs::write(&temp_path, &bytes)
        .await
        .map_err(|error| format!("failed to stage local frame: {error}"))?;
...
pub async fn livetext_analyze(
    ...
    #[cfg(target_os = "macos")]
    {
        let (image_path, staged_path) = materialize_local_http_image(image_path).await?;
