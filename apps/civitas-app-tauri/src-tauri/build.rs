...
/// `tauri-helper` discovers commands by parsing every source file and does not
/// evaluate `#[cfg(...)]`. Remove commands whose modules are absent from the
/// consumer build before its proc macros read the generated command lists.
fn filter_feature_gated_commands() {
    let e2e_build = std::env::var_os("CARGO_FEATURE_E2E").is_some();

    let manifest_dir = std::path::PathBuf::from(
        std::env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR is set by Cargo"),
    );
    let command_dir = manifest_dir.join("target").join("tauri_commands_list");
    for file_name in ["civitas_app.txt", "civitas_app_specta.txt"] {
        let path = command_dir.join(file_name);
        let Ok(contents) = std::fs::read_to_string(&path) else {
            continue;
        };
        let filtered = contents
            .lines()
            .filter(|line| e2e_build || !is_e2e_command(line))
            .collect::<Vec<_>>()
            .join("\n");
        std::fs::write(&path, format!("{filtered}\n"))
            .unwrap_or_else(|error| panic!("failed to filter {}: {error}", path.display()));
    }
}

fn is_e2e_command(path: &str) -> bool {
    path.rsplit("::")
        .next()
        .is_some_and(|name| name.starts_with("e2e_") || name == "get_e2e_seed_flags")
}
