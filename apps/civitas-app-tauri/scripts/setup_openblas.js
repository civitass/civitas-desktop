...
const config = {
	...
	windows: {
		...
		openblasSha256: 'e7595359700e8bb5a15c41af1920850b1be37078eb22813201b3d4bc5bd9227e',
		...
		openblasSha256Arm64: 'a2f0dd10028c7ac799189f06e1e1d8163370c23d14d5a995ace85bd5fdb0d374',
...
export async function setupOpenBlas({ cwd, winArch }) {
	...
	if (!(await fs.exists(path.join(cwd, config.openblasRealname)))) {
		if (winArch === 'arm64') {
			await downloadFile(config.windows.openblasUrlArm64, `${config.windows.openblasNameArm64}.zip`, {
				sha256: config.windows.openblasSha256Arm64,
				expectedBytes: config.windows.openblasBytesArm64,
				retries: 5,
				timeoutMs: 120000,
			})
			...
			await downloadFile(config.windows.openblasUrl, `${config.windows.openblasName}.zip`, {
				sha256: config.windows.openblasSha256,
				expectedBytes: config.windows.openblasBytes,
				retries: 5,
				timeoutMs: 120000,
			})
