...
describe('Settings Persistence (S10)', () => {
    ...
    it('should have privacy controls without requiring an account', async () => {
        await browser.execute(() => {
            const els = document.querySelectorAll('a, button');
            for (const el of els) {
                if (el.textContent.toLowerCase().includes('privacy')) {
                    el.click();
                    break;
                }
            }
        ...
        const pageText = await browser.execute(() => document.body.innerText.toLowerCase());
        expect(pageText).toContain('privacy');
        expect(pageText).not.toContain('sign in to continue');
    });
    ...
    it('should expose local storage and retention controls', async () => {
        await browser.execute(() => { window.location.href = '/settings'; });
        await browser.pause(1000);
        ...
        await browser.execute(() => {
            const els = document.querySelectorAll('a, button');
            for (const el of els) {
                if (el.textContent.toLowerCase().includes('storage')) {
                    el.click();
                    break;
                }
            }
        ...
        const storageText = await browser.execute(() => document.body.innerText.toLowerCase());
        expect(
            storageText.includes('storage') ||
            storageText.includes('retention') ||
            storageText.includes('disk')
        ).toBe(true);
        expect(storageText).not.toContain('sign in to continue');
    });
