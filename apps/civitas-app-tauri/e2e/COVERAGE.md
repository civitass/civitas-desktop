...
| Window lifecycle, focus, and dedupe | window-lifecycle | covered (strong; windows-system-integration, window-lifecycle) | covered (strong; window-lifecycle, focus-server) | covered (strong; window-lifecycle, focus-server) |
...
- macos: Real capture, OCR, and indexing (weak); Audio device health (gap); Updater install and rollback safety (gap).
...
| api.spec.ts | windows, macos, linux | local-api | health, audio-device-health, connections, local-api-auth | high | partial | api | 5 | Smoke coverage for local HTTP API shape and auth behavior. |
...
| focus-server.spec.ts | windows, macos, linux | local-api, window-lifecycle, tauri-command | window-lifecycle, focus-server, deeplink | medium | partial | api | 3 | Focus server opens windows and forwards deeplink args. |
