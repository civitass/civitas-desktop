...
const SIGNING_PASSWORD = "civitas-local-updater-e2e";
...
function schemeStamp(): string {
  return path.join(SIGNING, ".password-scheme-version");
}
...
function ensureSigning(): void {
  ...
  if (!existsSync(PRIVATE_KEY)) {
    ...
    if (gen.exitCode !== 0)
      throw new Error(
        "tauri signer generate failed (exit " + gen.exitCode + ")",
      );
    if (!existsSync(PUBLIC_KEY))
      throw new Error("expected public key at " + PUBLIC_KEY);
  }
...
function cmdBuild(): void {
  ...
  let tauriArgs = ["bunx", "tauri", "build", "--config", cfgE2e];
  ...
  if (!customSigning) {
    ...
    console.info(
      "[updater-local] using env signing keys — skipping local signing dir",
    );
  }
...
async function discoverBundle(): Promise<string> {
  async function newest(
    ...
    if (hits.length === 0)
      throw new Error(
        `no matching bundle in ${dir} — run updater-local:build first.`,
      );
    let best = hits[0],
      bestT = (await stat(best)).mtimeMs;
  ...
  if (process.platform === "darwin")
    return newest(path.join(root, "macos"), (n) => n.endsWith(".app.tar.gz"));
  if (process.platform === "linux")
    return newest(path.join(root, "appimage"), (n) =>
      n.endsWith(".AppImage.tar.gz"),
    );
  if (process.platform === "win32")
    return newest(
      path.join(root, "nsis"),
      (n) => n.endsWith(".nsis.zip") || n.endsWith(".exe.zip"),
    );
...
async function cmdPrepareManifest(argv: string[]): Promise<void> {
  ...
  if (!version || !bundle) {
    console.error("missing --version and/or --bundle");
    process.exit(1);
  }
...
async function cmdStageLast(argv: string[]): Promise<void> {
  ...
  const cargoToml = await readFile(
    path.join(APP_ROOT, "src-tauri/Cargo.toml"),
    "utf8",
  );
...
async function cmdServe(): Promise<void> {
  const manifestRaw = await Bun.file(MANIFEST)
    .text()
    .catch(() => null);
  if (!manifestRaw) {
    console.error(`manifest.json missing at ${MANIFEST}`);
    process.exit(1);
  }
  ...
  Bun.serve({
    ...
    async fetch(req: Request) {
      ...
      if (urlPath.startsWith("/artifacts/")) {
        let decoded: string;
        try {
          decoded = decodeURIComponent(urlPath.slice("/artifacts/".length));
        } catch {
...
switch (cmd) {
  ...
  case "setup-keys":
    ...
    break;
  case "stage-last":
    await cmdStageLast(rest);
    break;
  case "prepare-manifest":
    await cmdPrepareManifest(rest);
    break;
  case "serve":
    await cmdServe();
    break;
  default:
    console.error(
      "Usage: bun ./e2e/mock-updates/updater-harness.ts <build|setup-keys|stage-last|prepare-manifest|serve>",
    );
    process.exit(1);
}
