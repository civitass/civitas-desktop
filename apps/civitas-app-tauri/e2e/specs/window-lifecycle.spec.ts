...
describe("Window lifecycle", function () {
  ...
  it("reuses the Home handle while routing section-specific show_window requests", async () => {
    ...
    await showWindow({ Home: { page: "work-graph" } });
    await waitForWindowHandle("home", t(10_000));
    await browser.switchToWindow("home");
    await waitForWindowUrl("/home", "work-graph", t(12_000));
    const workGraphSection = await $('[data-testid="section-work-graph"]');
    await workGraphSection.waitForExist({ timeout: t(15_000) });
    await expectSingleWindowHandle("home");
