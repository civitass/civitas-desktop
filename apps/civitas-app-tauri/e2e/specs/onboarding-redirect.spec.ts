...
// gap. Every other spec runs with `CIVITAS_E2E_SEED=onboarding,...`,
...
// `CIVITAS_E2E_SEED=no-recording` so the server still starts (without
...
// 2. The login slide actually rendered (text contains "civitas" + the
...
(canRun ? describe : describe.skip)(
  ...
  function () {
    ...
    it("renders the login slide content, not a loading spinner or blank shell", async () => {
      ...
      expect(bodyText).toContain("civitas");
