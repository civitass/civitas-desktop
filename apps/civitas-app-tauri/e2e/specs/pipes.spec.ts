...
// The old Home → Pipes → Discover store surface is no longer mounted in the
// simplified desktop app. Current workflow management lives in Settings and the
// product direction is to minimize the large app in favor of Ask/search and
// endpoint/MCP surfaces, so this legacy store E2E is intentionally retired.
describe.skip('Pipes: discover → install → play', function () {
  ...
  it('navigates to Pipes section', async () => {
    ...
    // Pipes section fetches the remote store catalog from civitas.team
  ...
  it('assigns a tier-safe preset to the installed pipe (not Opus)', async () => {
    ...
    // The config may carry the preset id (e.g. "pipes" / "civitas") and/or
