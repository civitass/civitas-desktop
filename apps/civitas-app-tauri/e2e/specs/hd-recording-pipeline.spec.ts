 ...
 *
 * Run (from apps/civitas-app-tauri, against a --features e2e debug build):
 *   bun run test:e2e:hd:macos
 *   # which is: CIVITAS_E2E_SEED=onboarding bun run wdio run e2e/wdio.conf.ts \
...
describe("HD recording pipeline (high-fps capture)", function () {
  ...
  it("starts an HD session, writes a CFR mp4 chunk, and keeps indexing frames", async function () {
    ...
    // Foreground a non-Civitas window so OCR has real, non-filtered content
    // to index during the HD window (timeline/search drop "civitas" frames).
