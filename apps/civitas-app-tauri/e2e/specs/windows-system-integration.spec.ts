...
`);
  ...
  it("runs against the isolated E2E data directory instead of user app data", async function () {
    ...
    expect(E2E_DATA_DIR.toLowerCase()).not.toContain("\\appdata\\roaming\\.civitas");
...
`);
  ...
  it("keeps WebDriver and the local API reachable over localhost", async function () {
    ...
    const apiStatus = await waitForHttpStatus(apiUrl(api, "/ready"));
    const health = await fetchJson(apiUrl(api, "/health"), authHeaders(api.key));
...
`);
    ...
    const loopback = await fetchStatusWithTimeout(apiUrl(api, "/ready"));
    expect(loopback.ok).toBe(true);
    expect(loopback.status).toBeLessThan(500);

    for (const address of addresses.slice(0, 2)) {
      const status = await fetchStatusWithTimeout(`http://${address}:${api.port}/ready`);
      if (status.status > 0) {
        throw new Error(
...
`);
  ...
  it("does not show recent Windows Defender detections for civitas when Defender data is available", async function () {
    if (!isWindows) this.skip();
...
$hits = Get-MpThreatDetection -ErrorAction SilentlyContinue |
  Where-Object {
    ...
    (($_.ProcessName -like '*civitas*') -or ($_.Resources -join ' ' -like '*civitas*'))
  }
...
`);
    ...
    const [health, deviceStatus] = await Promise.all([
      fetchJson(apiUrl(api, "/health"), authHeaders(api.key)),
  ...
  it("keeps the backend alive across Windows Home close and reopen", async function () {
    ...
    const searchInput = await $(
      '//input[contains(translate(@placeholder, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "search memory")]',
    );
    ...
    try {
      ...
      const healthWhileClosed = await fetchJson(
        apiUrl(api, "/health"),
        authHeaders(api.key),
      );
      ...
      const healthAfterReopen = await fetchJson(
        apiUrl(api, "/health"),
        authHeaders(api.key),
      );
