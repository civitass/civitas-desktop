 ...
 *
 * Civitas runs a small local HTTP server (default port 11435) used for:
...
const FOCUS_SERVER_PORT = Number(process.env.CIVITAS_FOCUS_PORT ?? "11436");
...
describe("Focus server", function () {
  ...
  it("requires the per-launch token for protected local-service routes", async () => {
    const unauthenticated = await fetch(
      `http://127.0.0.1:${FOCUS_SERVER_PORT}/notifications`
    );
    ...
    const authenticated = await fetch(
      `http://127.0.0.1:${FOCUS_SERVER_PORT}/notifications`,
      {
        headers: { authorization: `Bearer ${E2E_FOCUS_TOKEN}` },
      }
    );
  ...
  it("forwards deep_link_url + args into the app via Tauri events", async function () {
    ...
    const deepLink = `civitas://timeline?timestamp=${encodeURIComponent(
