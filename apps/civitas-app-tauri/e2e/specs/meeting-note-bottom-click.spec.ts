...
describe("meeting note – bottom line is clickable", function () {
  ...
  before(async () => {
    ...
    // Auth is mandatory. On a dev box another civitas owns :3030, so the
    // app is launched with CIVITAS_PORT pointing at a free port for its own
    ...
    // the seed can never land on the real app. In CI (no CIVITAS_PORT) any
    // port is accepted.
    const expectedPort = process.env.CIVITAS_PORT
      ? Number(process.env.CIVITAS_PORT)
      : null;
