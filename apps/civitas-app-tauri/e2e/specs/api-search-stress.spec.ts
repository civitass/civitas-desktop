...
describe("Local API search and stability", function () {
  ...
  it("exposes the mandatory local API key", async function () {
    const cfg = await getLocalApiConfig();
  ...
  const queryCases = [
    ...
    ["URL text", "https://civitas.team/docs?x=1&y=2"],
    ["SQL-ish text", "'; DROP TABLE frames; --"],
    ["Windows path", "C:\\Users\\example\\Documents\\civitas"],
    ["tabs and newlines", "line one\nline two\tend"],
    ["double quotes", '"quoted phrase"'],
    ["single wildcard", "*"],
    ["FTS-ish operators", "civitas OR test NEAR app"],
    ["punctuation", "!@#$%^&*()_+-=[]{}|;:,.<>/?"],
    ...
    ["markdown link", "[civitas](https://civitas.team)"],
    ["markdown code", "`bun run test:e2e`"],
    ["email address", "test@example.com"],
    ["IPv6 address", "[::1]:3030"],
    ["percent sign", "100% match"],
    ["repeated terms", "civitas ".repeat(80)],
  ...
  const paginationAndRangeCases = [
    ...
    [
      "/search?limit=5&start_time=2999-01-01T00%3A00%3A00Z",
      "future start time",
    ],
    [
  ...
  const booleanFlagCases = [
    ...
    [
      "/search?limit=2&content_type=accessibility&on_screen=true",
      "on_screen true",
    ],
    [
      "/search?limit=2&content_type=accessibility&on_screen=false",
      "on_screen false",
    ],
  ] as const;
  ...
  const filterParamCases = [
    ...
    ["/search?limit=2&app_name=civitas.exe", "Windows executable app filter"],
    [
      "/search?limit=2&app_name=C%3A%5CProgram%20Files%5Ccivitas.exe",
      "path-like app filter",
    ],
    ["/search?limit=2&window_name=", "empty window filter"],
    ["/search?limit=2&window_name=Civitas", "window title filter"],
    [
      "/search?limit=2&window_name=%5Bdebug%5D%20%28test%29",
      "punctuated window filter",
    ],
    ["/search?limit=2&content_type=audio&speaker_ids=1", "single speaker id"],
    [
      "/search?limit=2&content_type=audio&speaker_ids=1%2C2%2C3",
      "speaker id list",
    ],
    [
      "/search?limit=2&content_type=audio&speaker_ids=abc",
      "invalid speaker id",
    ],
    ["/search?limit=2&content_type=audio&speaker_name=Casey", "speaker name"],
    ...
    ["/search?limit=2&q=test&window_name=Civitas", "query plus window"],
    [
      "/search?limit=2&q=test&content_type=accessibility&on_screen=true",
      "query plus accessibility visible",
    ],
    ...
    [
      "/search?limit=2&content_type=all&app_name=Google%20Chrome&window_name=Civitas",
  ...
  const timeRangeCases = [
    [
      "/search?limit=2&start_time=2020-01-01T00%3A00%3A00Z&end_time=2020-01-01T00%3A00%3A00Z",
      "same start and end",
    ],
    [
      "/search?limit=2&start_time=2020-01-02T00%3A00%3A00Z&end_time=2020-01-01T00%3A00%3A00Z",
      "end before start",
    ],
    ["/search?limit=2&start_time=2020-01-01", "date only start"],
    [
      "/search?limit=2&start_time=2020-01-01T00%3A00%3A00",
      "start without timezone",
    ],
    [
      "/search?limit=2&start_time=2020-01-01T00%3A00%3A00%2B02%3A00",
      "start with offset timezone",
    ],
  ...
  const readonlyEndpointEdgeCases = [
    ...
    ["/health?token=bad", "health ignores query token when bearer is valid"],
    ...
    ["/speakers/search?name=Casey", "named speaker search"],
  ...
  for (const field of healthFields) {
    it(`health response includes ${field}`, async function () {
      if (!key) this.skip();
      const res = await authedGet("/health");
      expect(res.ok).toBe(true);
      expect(typeof res.body).toBe("object");
  ...
  it("health status text is non-empty", async function () {
    if (!key) this.skip();
    const res = await authedGet("/health");
    expect(res.ok).toBe(true);
  ...
  it("OpenAPI JSON includes the search path when available", async function () {
    ...
    if (res.ok) {
      ...
      expect(Object.keys(paths).some((path) => path.includes("/search"))).toBe(
        true,
      );
    }
  });
  ...
  it("search response includes pagination metadata when successful", async function () {
    ...
    if (res.ok) {
      ...
      expect(typeof (res.body as { pagination?: unknown }).pagination).toBe(
        "object",
      );
    }
  });
  ...
  const invalidAuthCases = [
    ...
    ["random auth scheme", { Authorization: "civitas bad" }, "/search?limit=1"],
    [
      "x-api-key only",
  ...
  it("rejects unauthenticated search", async function () {
    if (!key) this.skip();
  ...
  it("rejects credentials supplied in query strings", async function () {
    if (!key) this.skip();
  ...
  it("keeps only the content-free readiness probe public", async () => {
    const res = await fetchJson(apiUrl("/ready"));
    expect(res.ok).toBe(true);
    expect(res.body).toEqual({ status: "ok" });
  });

  it("protects audio diagnostics before auth headers are available", async () => {
  ...
  it("handles 30 concurrent authenticated health requests", async function () {
    if (!key) this.skip();
    const results = await Promise.all(
      Array.from({ length: 30 }, () => authedGet("/health")),
    );
  ...
  it("handles mixed readonly API load while the UI stays responsive", async function () {
    ...
    const results = await Promise.all(
      Array.from({ length: 4 }, () => endpoints)
        .flat()
        .map((path) => authedGet(path)),
    );
    expect(
      results.filter((r) => r.status >= 500 || r.status === 0),
    ).toHaveLength(0);
