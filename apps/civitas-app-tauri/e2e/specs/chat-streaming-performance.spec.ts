 ...
 *
 * This keeps the reported "long response makes chat laggy on Mac" case under a
...
const CHAT_CPU_AVG_MAX_PERCENT = Number(
  process.env.CIVITAS_E2E_CHAT_CPU_AVG_MAX_PERCENT ?? "85",
);
const CHAT_CPU_P95_MAX_PERCENT = Number(
  process.env.CIVITAS_E2E_CHAT_CPU_P95_MAX_PERCENT ?? "120",
);
...
async function sampleAppCpuWhile<T>(
  ...
  const pid = getAppPid();
  if (!pid) {
    throw new Error("civitas app pid is not available for CPU sampling");
  }
