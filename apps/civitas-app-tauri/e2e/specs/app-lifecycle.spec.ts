...
describe("App lifecycle and UI stability", function () {
  ...
  it("round-trips localStorage", async () => {
    const ok = (await browser.execute(() => {
      localStorage.setItem("civitas-e2e-localstorage", "ok");
      const value = localStorage.getItem("civitas-e2e-localstorage");
      localStorage.removeItem("civitas-e2e-localstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
  ...
  it("round-trips sessionStorage", async () => {
    const ok = (await browser.execute(() => {
      sessionStorage.setItem("civitas-e2e-sessionstorage", "ok");
      const value = sessionStorage.getItem("civitas-e2e-sessionstorage");
      sessionStorage.removeItem("civitas-e2e-sessionstorage");
      return value === "ok";
    })) as boolean;
    expect(ok).toBe(true);
  });
  ...
  const routeCases = [
    ...
    ["/home?section=work-graph", "work-graph-route"],
    ["/home?section=pipes", "legacy-pipes-route"],
