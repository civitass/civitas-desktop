...
// The old Help sidebar section was retired when the desktop UI was simplified
// around Ask/search and settings. `FeedbackSection` still exists in source, but
// it is not mounted by the current app route tree; keeping this as an active
// E2E would force us to re-add a removed large-app surface only for CI.
describe.skip("Help section: Discord community link", function () {
