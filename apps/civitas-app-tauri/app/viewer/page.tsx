...
function ToolbarButton({
  ...
  return (
    <button
      ...
      className={
        "px-3 py-1 text-2xs tracking-wider uppercase font-mono " +
...
function ErrorState({ message, path }: { message: string; path: string }) {
  return (
    <div className="font-mono text-sm text-foreground/70 max-w-prose">
      <div className="uppercase tracking-wider text-2xs mb-2 text-foreground/40">
        couldn&apos;t open file
      </div>
...
function ImageView({ src, name }: { src: string; name: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {dims && (
        <div className="font-mono text-2xs tracking-wider uppercase text-foreground/40">
...
export default function ViewerPage() {
  ...
  const openInDefault = useCallback(async () => {
    ...
    try {
      ...
      console.error("Note could not be opened.");
    }
  }, [path]);
  ...
  const revealInFinder = useCallback(async () => {
    ...
    try {
      ...
      console.error("Note could not be revealed in the default browser.");
    }
  }, [path]);
  ...
  const copyPath = useCallback(async () => {
    ...
    try {
      ...
      console.error("Note path could not be copied.");
    }
  }, [path]);
  ...
  const copyContent = useCallback(async () => {
    ...
    try {
      ...
      console.error("Note content could not be copied.");
    }
  }, [content, detection]);
    ...
    <div className="flex flex-col h-screen bg-background text-foreground font-mono">
      ...
      >
        <div className="flex flex-col flex-1 min-w-0 select-none" data-tauri-drag-region>
          <div className="text-xs truncate font-medium" title={path} data-tauri-drag-region>
            {fileName}
          </div>
          {breadcrumb && breadcrumb !== fileName && (
            <div
              className="text-2xs tracking-wide text-foreground/40 truncate"
              data-tauri-drag-region
            >
              {breadcrumb}
            </div>
      ...
      <main ref={mainRef} className="flex-1 overflow-auto px-6 py-5">
        ...
        {content?.kind === "binary" && (
          <div className="font-mono text-sm text-foreground/70 max-w-prose">
            ...
            <button
              ...
              className="px-3 py-1 text-2xs tracking-wider uppercase font-mono border border-border bg-foreground/[0.06] hover:bg-foreground hover:text-background transition-colors"
            >
        ...
        {content?.kind === "text" && content.truncated && (
          <div
            className="font-mono text-2xs tracking-wide uppercase text-foreground/50 mb-3 px-3 py-1 border border-border bg-foreground/[0.04] flex items-center justify-between gap-3"
          >
            <span>
        ...
        {content?.kind === "text" && content.text === "" && (
          <div className="font-mono text-sm text-foreground/40 italic">
            (empty file)
          </div>
        )}
              ...
              components={{
                  ...
                  <a
                    ...
                    onClick={async (e) => {
                      ...
                      try {
                        ...
                        console.error("Viewer link could not be opened.");
                      }
                    }}
                    {...props}
                  >
                ...
                code: ({ className, children, ...rest }) => {
                  ...
                  if (!match) {
                    return (
                      <code
                        className="font-mono text-sm bg-foreground/5 px-1 py-[1px] border border-border"
                        {...rest}
                      >
                        {children}
                      </code>
