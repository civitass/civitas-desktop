...
import { ProviderSettings, searchIndex as aiSearchIndex } from "@/components/settings/provider-settings";
...
import { ProjectMemorySection, searchIndex as projectMemorySearchIndex } from "@/components/settings/project-memory-section";
...
const ALL_SETTINGS_FIELDS: IndexedSettingsField[] = [
  ...
  ...projectMemorySearchIndex.map((f) => ({ ...f, section: "project-memory" })),
...
const ALL_SETTINGS_SECTIONS: SettingsSection[] = [
  ...
  "privacy", "storage", "project-memory", "speakers",
];
...
function SettingsContent() {
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "project-memory" as const, label: "Project memory", icon: <FolderOpen className="h-4 w-4" /> },
  ...
  const renderSection = () => {
    switch (section) {
      ...
      case "project-memory": return <ProjectMemorySection />;
                  ...
                  <div className="space-y-0.5">
                    {group.items.map((item) => (
                      ...
                      >
                        ...
                        <span className={cn("text-sm truncate font-medium", section === item.id && isTranslucent ? "vibrant-sidebar-fg" : "")}>
                          {item.label}
                        </span>
                      </button>
                    ))}
...
// Settings-shaped loading skeleton — mirrors the real layout (240px source-list
// rail + content pane) so the chrome paints instantly and the hand-off to the
// loaded page is seamless, instead of a bare centered "Loading…" on a blank
// screen while the (heavy) /settings route resolves.
