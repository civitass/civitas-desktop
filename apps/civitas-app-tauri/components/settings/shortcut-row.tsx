...
const ShortcutRow = ({
  ...
  const syncShortcuts = async (updatedShortcuts: {
    ...
    await commands.updateGlobalShortcuts(
      updatedShortcuts.showCivitasShortcut,
  ...
  const handleEnableShortcut = async (keys: string) => {
    try {
      ...
      const allShortcuts: Record<string, string> = {
        showCivitasShortcut: settings.showCivitasShortcut,
        ...
        askShortcut: settings.askShortcut || "",
      ...
      switch (type) {
        case "global":
          ...
          const updatedShortcuts = {
            showCivitasShortcut: shortcut === "showCivitasShortcut" ? keys : settings.showCivitasShortcut,
            ...
            askShortcut: shortcut === "askShortcut" ? keys : (settings.askShortcut || ""),
      ...
      console.error("Keyboard shortcut could not be updated.");
      toast({
      ...
      <div className="flex items-center gap-4">
        ...
        <Switch
          ...
          onCheckedChange={async (checked) => {
            ...
            if (checked && value) {              await handleEnableShortcut(value);
            } else {              await handleDisableShortcut();
            }
          }}
        />
      </div>
