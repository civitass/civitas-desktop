...
export function AppleIntelligenceCard() {
  ...
  const checkStatus = useCallback(async () => {
    const attempt = async (): Promise<{ available: boolean; status: string } | null> => {
      ...
      console.warn("Apple Intelligence status request was rejected.");
      return null;
    };
    ...
    try {
      ...
      if (data) {
        const result = data.available ? "available" : "unavailable";        setAiStatus(result);
      ...
      try {
        ...
        console.error("Apple Intelligence status could not be checked after retry.");
        setAiStatus("unavailable");
