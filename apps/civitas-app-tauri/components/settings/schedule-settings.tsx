          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-sm font-medium text-foreground">
                Recording Schedule
                <span className="ml-1.5 text-2xs font-mono uppercase tracking-wider bg-muted text-muted-foreground px-1 py-0.5 rounded">
                  beta
                </span>
              </h3>
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1 relative h-4">
                  {HOUR_MARKERS.map((h) => (
                    <span
                      key={h}
                      className="absolute text-2xs text-muted-foreground font-mono -translate-x-1/2"
                      style={{ left: `${(h / 24) * 100}%` }}
                    >
                      {h}
                    </span>
        ...
        <div className="flex-1 relative h-7 bg-muted/40 border border-border/50">
          ...
          {rules.map((rule, idx) => {
            ...
            return (
              <div
                key={idx}
                className="absolute top-0.5 bottom-0.5 bg-foreground/20 transition-ui duration-150"
                style={{ left: `${left}%`, width: `${width}%` }}
                title={`${formatTime(rule.startTime)} - ${formatTime(rule.endTime)} (${MODE_LABELS[rule.recordMode] || "All"})`}
              >
                {width > 15 && (
                  <span className="absolute inset-0 flex items-center justify-center text-2xs text-foreground font-mono truncate px-1">
          ...
          {/* Empty state hint */}
          {rules.length === 0 && (
            <span className="absolute inset-0 flex items-center justify-center text-2xs text-muted-foreground/50 font-mono select-none">
              No recording
            </span>
          )}
        </div>
