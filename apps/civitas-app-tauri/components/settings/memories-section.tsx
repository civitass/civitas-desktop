  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <p className="text-muted-foreground text-sm mb-4">
        facts, preferences, and decisions Civitas has learned from your activity
      </p>
      ...
      {isStale && (
        <div className="flex items-start gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          <span>
            ...
            check that capture and learning are running —{" "}
            <a
              href="?section=work-graph"
        ...
        <div className="border border-border rounded-md p-3 space-y-2 bg-muted/20">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {newTags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1 px-2 py-0.5 text-2xs rounded-full border border-border bg-muted"
              >
            ...
            <Input
              ...
              placeholder="Add tag..."
              className="h-6 text-2xs w-20 px-1.5 border-dashed"
            />
          </div>
          ...
          <>
            ...
            {(showAllTags ? allTags : allTags.slice(0, 6)).map((tag) => (
              <button
                ...
                className={`inline-flex items-center gap-1 px-2 py-0.5 text-2xs rounded-full border transition-colors max-w-[150px] ${
                  activeTag === tag
            ...
            {allTags.length > 6 && (
              <button
                ...
                className="inline-flex items-center px-2 py-0.5 text-2xs rounded-full border border-dashed border-border text-muted-foreground hover:bg-muted transition-colors"
              >
        ...
        <div className="ml-auto flex items-center gap-1">
          {(
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 py-0.5 text-2xs rounded border transition-colors ${
                sortField === field
      ...
      {loading ? (
        ...
        <div className="text-sm text-muted-foreground py-8 space-y-2 text-center">
          ...
          {!debouncedQuery && !activeTag && (
            <>
              <p className="text-xs">
                memories are automatically created as civitas learns from your
                screen &amp; audio activity.
              </p>
              <p className="text-xs mt-3">
                watch learned context and next-action edges appear in the{" "}
                <a
                  href="?section=work-graph"
              ...
              >
                ...
                >
                      ...
                      <div className="text-sm text-foreground">
                        ...
                        {isLong && (
                          <button
                            ...
                            className="flex items-center gap-0.5 text-2xs text-muted-foreground hover:text-foreground transition-colors mt-1"
                          >
                            {isExpanded ? (
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                    ...
                    <Badge
                      variant="outline"
                      className="text-2xs px-1 py-0 font-normal"
                    >
                      {memory.source}
                    </Badge>
                    {editingId === memory.id ? (
                      <>
                        {editTags.filter((t) => !/^\d{4}-\d{2}-\d{2}/.test(t) && !/^\d+$/.test(t)).map((tag) => (
                          <span
                            key={tag}
                            className="inline-flex items-center gap-1 px-1.5 py-0 text-2xs rounded-full border border-border bg-muted"
                          >
                        ...
                        <Input
                          ...
                          placeholder="+ tag"
                          className="h-5 text-2xs w-16 px-1 border-dashed inline-flex"
                        />
                      </>
                    ) : (
                      <>
                              ...
                              <TooltipProvider key={tag}>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Badge
                                      variant="secondary"
                                      className="text-2xs px-1 py-0 font-normal max-w-[120px] truncate cursor-default"
                                    >
                                      {tag}
                                    </Badge>
                                  </TooltipTrigger>
