 ...
 *
 * Fields belonging to hidden sections are excluded so unavailable settings
 * do not leak into search results.
 */
...
export function scrollToSettingsField(label: string, root: ParentNode = document): void {
  ...
  const tryScroll = () => {
    ...
    // isn't jammed against the top edge. Civitas Cards render with a `border`
    ...
    return (
      <div className={cn("relative", className)}>
        ...
        {value ? (
          ...
          <kbd
            className={cn(
              "absolute right-1.5 top-1/2 -translate-y-1/2 px-1 py-0.5 text-2xs font-mono rounded border pointer-events-none",
              translucent
