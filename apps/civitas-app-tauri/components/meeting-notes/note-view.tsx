...
import { resolveAudioCapturePolicy } from "@/lib/audio-capture-consent";
...
export function NoteView({
  ...
  useEffect(() => {
    ...
    const insertDroppedImages = async (
      ...
      try {
        ...
        console.error("Dropped meeting image could not be inserted.");
        toast({
          title: "Couldn't insert image",
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Meeting summary template could not be loaded; using the fallback.",
        );
      }
      ...
      console.error("Meeting could not be summarized.");
      toast({
  ...
  const handleRetranscribe = async () => {
    ...
    try {
      ...
      console.error("Meeting could not be retranscribed.");
      toast({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error("Meeting export dialog could not be opened.");
      toast({
    ...
    try {
      ...
      // dependency) — the in-app twin of the `civitas export` CLI. Reuses
      // the running server core's DB handle. Rejects with the engine's error
      // string on failure.
      const res = await commands.exportRecording(
        meeting.id,
        null,
        null,
        target,
      ...
      console.error("Meeting could not be exported.");
      toast({
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      console.error("Meeting could not be copied.");
      toast({
        title: "Couldn't copy",
        description: String(err),
        variant: "destructive",
  ...
  return (
    <div ref={rootRef} className="relative flex h-full flex-col bg-background">
      ...
      <main className="relative min-h-0 flex-1 overflow-y-auto w-full max-w-5xl mx-auto px-5 pt-5 pb-16 sm:px-10 lg:px-16">
        <div className="flex items-center justify-between mb-9">
          <Button
            ...
            className="h-9 w-9 p-0 border border-border bg-background hover:bg-muted"
          >
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            ...
            <Pill icon={<Clock className="h-3.5 w-3.5" />}>
              ...
              {isLive || !meetingEndClock ? (
                <span className="inline-flex items-center gap-1 border border-foreground/15 bg-foreground/[0.03] px-1.5 py-0.5 text-2xs font-medium leading-none text-foreground">
      ...
      <footer className="z-30 shrink-0 border-t border-border bg-background">
        ...
        {isLive && (
          <div className="px-4 pb-1 text-center text-2xs leading-none text-muted-foreground/60">
...
function AudioHealthButton({
  ...
  const capturePolicy = resolveAudioCapturePolicy(settings);
  const liveEnabled =
  ...
  const anyAudioActive =
    isLive &&
    ((capturePolicy.microphone && inputActive) ||
      (capturePolicy.systemAudio && outputActive));
      ...
      >
        ...
        <div className="px-3 py-3">
          ...
          <AudioDeviceRow
            ...
            active={
              isLive &&
              capturePolicy.systemAudio &&
              (outputActive || outputs.length === 0)
            }
            level={outputLevel}
          />

          <div className="mt-3 grid grid-cols-[112px_minmax(0,1fr)] gap-x-3 gap-y-1.5 text-xs leading-tight">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border px-3 py-2.5">
          ...
          <div className="inline-flex shrink-0 border border-border">
            <button
              ...
              className={cn(
                "flex h-8 min-w-16 items-center justify-center gap-1.5 px-2 text-xs transition-colors hover:bg-muted",
                englishOnly
            ...
            <button
              ...
              className={cn(
                "flex h-8 min-w-14 items-center justify-center gap-1.5 border-l border-border px-2 text-xs transition-colors hover:bg-muted",
                !englishOnly
                  ? "bg-foreground text-background"
                  : "text-muted-foreground",
              )}
      ...
      {bars.map((weight, index) => {
        ...
        return (
          <span
            ...
            className={cn(
              "meeting-audio-level-bar h-full w-0.5 rounded-full bg-current transition-ui duration-150 ease-out",
            )}
            style={
              {
...
function transcriptOpenPreferenceKey(meetingId: number): string {
  return `civitas:meeting:${meetingId}:transcript-open`;
}
