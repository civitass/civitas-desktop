...
function openFile(absPath: string) {
  void commands.openNotePath(absPath);
}
...
export function Receipts({ activity }: ReceiptsProps) {
  ...
  return (
    <section className="border-t border-border pt-5">
        ...
        <ul className="border border-border divide-y divide-border mb-3">
          {urls.map((w) => (
            <li key={w.browser_url}>
              <button
                onClick={() => void openExternalUrl(w.browser_url).catch(() => {})}
        ...
        <ul className="flex flex-wrap gap-1.5 mb-3">
          {apps.map((w) => (
            <li
              ...
              className="inline-flex items-center gap-1.5 border border-border px-2 py-1 text-xs text-muted-foreground"
      ...
      {files.length > 0 && (
        <div>
          <div className="text-2xs text-muted-foreground/60 mb-1.5 px-1">
            files edited
          </div>
