...
export function MeetingNotesSection({
    ...
    const unlisten = listen<{ meetingId: number; transcript?: boolean }>(
      ...
      async (event) => {
        ...
        try {
          ...
          console.warn("Linked meeting could not be opened."
          );
          await fetchPage(0, false, appliedQueryRef.current);
        }
  ...
  const refreshUpcoming = useCallback(async () => {
    try {
      ...
      console.warn("Meeting calendar events could not be refreshed.");
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn("Automatic meeting enrichment failed.");
      }
    })();
