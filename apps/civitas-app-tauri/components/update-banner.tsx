  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (gate !== "proceed") {
        ...
        toast({
          title: "Civitas is still starting up",
          description:
            gate === "errored"
      ...
      if (os === "windows") {
        ...
        // Stop civitas before update on Windows
        try {
          await commands.stopCivitas();
        } catch (e) {
          console.warn("Local Civitas service could not be stopped before updating.");
        }
        ...
        if (!update) {
          update = await check({ endpoints: [
            "https://github.com/civitass/civitas-desktop/releases/latest/download/latest.json",
          ] } as any);
        }
        ...
        toast({
          ...
          description: "Civitas will restart automatically",
          duration: 10000,
        });
      }
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-2xs text-muted-foreground truncate">v{updateInfo.version}</div>
        </div>
      </button>
    );
  }
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 flex-1">
        ...
        <span>
          Civitas <span className="font-medium">v{updateInfo.version}</span> is ready
        </span>
      </div>
