...
import { Loader2, ChevronDown, ChevronUp, Copy, Check, X } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
function acquireLock(date: string): boolean {
  try {
    ...
    // Acquire
    localStorage?.setItem(LOCK_KEY, JSON.stringify({ date, ts: Date.now() }));
    return true;
  } catch {
...
function releaseLock() {
  try {
    ...
    // The in-memory generation guard still prevents duplicate work in this
    // component when localStorage is unavailable.
  }
}
...
function markAutoTriggered(date: string) {
  try {
    ...
    // Auto-generation remains safe through the cross-instance lock.
  }
}
...
// Same algorithm as the Rust `compute_app_usage` in civitas-apple-intelligence,
...
function computeAppUsage(
  ...
  const parsed = frames
    .map((f) => ({
      ts: new Date(f.timestamp).getTime() / 1000,
      app: f.appName,
    }))
...
- Wrong: "casey discusses api and launch plans" (starts with name, formal)
...
function loadSummary(date: string): DailySummary | null {
  try {
    ...
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<DailySummary>;
    if (
      parsed.date !== date ||
      typeof parsed.generatedAt !== "string" ||
      typeof parsed.oneLiner !== "string" ||
      !Array.isArray(parsed.timeBreakdown) ||
      !Array.isArray(parsed.keyMoments) ||
      !Array.isArray(parsed.actionItems) ||
      !Array.isArray(parsed.peopleMentioned)
    ) {
      return null;
    }
    return parsed as DailySummary;
  } catch {
    // A damaged cache must never strand the card; regeneration remains
    // available and will replace it with a validated summary.
  }
  return null;
}

function saveSummary(summary: DailySummary): boolean {
  try {
...
async function isPluggedIn(): Promise<boolean> {
  try {
    const resp = await localFetch("/power", {
      signal: AbortSignal.timeout(1500),
    });
    if (!resp.ok) return true;
    const data = await resp.json();
    return data?.state?.on_ac !== false;
  } catch {
    return true;
  }
}
...
async function fetchSummarySource(
  path: string,
  label: "audio" | "activity",
): Promise<{ data: SummarySearchItem[] }> {
  const response = await localFetch(path, {
    signal: AbortSignal.timeout(10_000),
  });
  if (!response.ok) {
    throw new Error(`${label} search returned ${response.status}`);
  }
  const payload = (await response.json()) as { data?: unknown };
  if (!Array.isArray(payload?.data)) {
    throw new Error(`${label} search returned an invalid response`);
  }
  return {
    data: payload.data.filter(
      (item): item is SummarySearchItem => !!item && typeof item === "object",
    ),
  };
}

type GeneratedSummaryFields = Pick<
  DailySummary,
  "oneLiner" | "keyMoments" | "actionItems" | "peopleMentioned"
>;

function normalizeGeneratedSummary(value: unknown): GeneratedSummaryFields {
  const record =
  ...
  const actionItems = Array.isArray(record.actionItems)
          ...
          const text =
            typeof item.text === "string"
              ...
              : typeof item.task === "string"
                ? item.task.trim()
                : "";
          const urgency: DailySummary["actionItems"][number]["urgency"] =
            item.urgency === "high" ||
            item.urgency === "medium" ||
            item.urgency === "low"
              ? item.urgency
    ...
    : [];
  const peopleMentioned = Array.isArray(record.peopleMentioned)
    ? record.peopleMentioned
        .filter(
          (item): item is Record<string, unknown> =>
            !!item && typeof item === "object",
        )
...
export function DailySummaryCard({ currentDate }: { currentDate: Date }) {
  ...
  // Check AI availability on mount and whenever the app returns to the
  // foreground. A one-shot check during engine cold-start made the feature
  // disappear for the entire session after one transient failure.
  useEffect(() => {
    const os = platform();
    if (os !== "macos") return;
    let cancelled = false;
  ...
  const generateSummary = useCallback(async (): Promise<boolean> => {
    ...
    if (!acquireLock(dateStr)) {
      setError(
        "A summary is already being generated in another Civitas window.",
      );
      return false;
    }
    ...
    try {
      ...
      const [audioResult, activityResult] = await Promise.allSettled([
        fetchSummarySource(
      ...
      if (
        ...
        throw new Error(
          "Couldn’t load your local activity. Check that Civitas is running, then retry.",
        );
      }
      const audioData =
        audioResult.status === "fulfilled" ? audioResult.value : { data: [] };
      const uiData =
      ...
      const partialDataWarning =
        audioResult.status === "rejected" ||
        activityResult.status === "rejected"
          ? "Some local activity couldn’t be loaded. This summary may be incomplete; retry when Civitas is ready."
          : null;
      ...
      if (parts.length === 0 && timeBreakdown.length === 0) {
        setError(
          partialDataWarning ??
            "No captured activity was found for this day. Choose another day or start capture, then retry.",
        );
        return partialDataWarning === null;
      }

      // ── AI inference (only if we have audio) ──────────────────────────
      let parsed: GeneratedSummaryFields = {
      ...
      if (parts.length > 0) {
        ...
        parsed = normalizeGeneratedSummary(JSON.parse(raw));
      }
      ...
      const persisted = saveSummary(newSummary);
      setSummary(newSummary);
      setIsExpanded(true);
      setError(
        !persisted
          ? "Summary ready, but Civitas couldn’t save it locally. Copy it now, then retry after checking storage access."
          : partialDataWarning,
      );
  ...
  useEffect(() => {
    const checkAutoTrigger = async () => {
      ...
      // The cross-window generation lock prevents duplicates. Mark only after
      // a completed attempt so a transient engine or storage failure does not
      // suppress every retry for the rest of the day.
      const completed = await generateSummary();
      if (completed) markAutoTriggered(todayStr);
    };

    void checkAutoTrigger();
    const interval = setInterval(() => void checkAutoTrigger(), 60000);
  ...
  const copySummary = async () => {
    if (!summary) return;
    const lines = [`# ${formatDate(summary.date)}`, "", summary.oneLiner, ""];
    ...
    try {
      ...
      setError(
        "Civitas couldn’t copy this summary. Check clipboard permission and try again.",
      );
    }
  };
  ...
  return (
    <div className="w-72 backdrop-blur-xl bg-black/60 border border-white/10 rounded-xl overflow-hidden shadow-2xl">
      ...
      {error && (
        ...
        >
          <div className="flex items-center gap-1.5">
            <span className="text-2xs leading-snug text-destructive flex-1">
              {error}
            </span>
            {!isGenerating ? (
              <button
                type="button"
                onClick={() => void generateSummary()}
                className="rounded-md px-1.5 py-1 text-2xs font-medium text-white/70 transition-colors hover:bg-white/10 hover:text-white"
              >
                Retry
              </button>
            ) : null}
                ...
                {summary.timeBreakdown.map((t, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-2xs text-white/60 w-16 truncate">
                        {t.app}
                      </span>
                      ...
                      <span className="text-2xs text-white/40 w-10 text-right">
                        {h > 0 ? `${h}h` : ""}
                        {m > 0 ? `${m}m` : ""}
                      </span>
                    </div>
              ...
              <div className="space-y-1">
                {summary.keyMoments.map((k, i) => (
                  <div key={i} className="flex gap-2">
                    <span className="text-2xs text-white/40 w-14 flex-shrink-0">
                      {k.time}
                    </span>
              ...
              <div className="space-y-1">
                {summary.actionItems.map((a, i) => (
                  <div key={i} className="flex items-start gap-1.5">
                    <span
                      className={`text-2xs mt-px ${urgencyColor(a.urgency)}`}
                    >
                      {urgencyIcon(a.urgency)}
                    </span>
                    <span className="text-2xs text-white/80">{a.text}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="px-3 py-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (!isGenerating) void generateSummary();
                }}
                className="px-2 py-0.5 text-2xs text-white/40 hover:text-white/70 transition-colors"
                title="Regenerate"
              >
                ↻
              </button>
