...
import { commands, type OSPermissionStatus } from "@/lib/utils/tauri";
...
} from "@/lib/hooks/use-timeline-selection";
...
import { TimelineEmptyStatus } from "@/components/rewind/timeline-empty-status";
import { TimelineConnectionState } from "@/components/rewind/timeline-connection-state";
import { ScreenPermissionState } from "@/components/rewind/screen-permission-state";
...
export default function Timeline({ embedded = false }: { embedded?: boolean }) {
  ...
  const screenAccessRequired = isMac && captureRequirements.screenRecording;
  ...
  }, [screenAccessRequired]);

  useEffect(() => {
    if (!screenAccessRequired) return;

    void refreshScreenPermission();
    void commands
      .getAppIdentifier()
      .then(setAppIdentifier)
      .catch(() => setAppIdentifier(null));

    const interval = window.setInterval(() => {
      void refreshScreenPermission();
    }, 3000);
    const handleFocus = () => void refreshScreenPermission();
    const handleVisibility = () => {
      if (!document.hidden) void refreshScreenPermission();
    };
  ...
  }, [refreshScreenPermission, screenAccessRequired]);

  const openScreenPermission = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description:
          "Open Privacy & Security → Screen & System Audio Recording and turn on this Civitas build.",
        variant: "destructive",
      });
    } finally {
      await refreshScreenPermission();
    }
  }, [refreshScreenPermission]);

  const relaunchForScreenPermission = useCallback(() => {
    void relaunch().catch(() => {
      toast({
        title: "Civitas couldn’t relaunch",
        description: "Quit and reopen Civitas once to activate screen capture.",
        variant: "destructive",
      });
    });
  }, []);
  ...
  const guardRefs = useMemo(
    () => ({ filters: filtersRef, scrubber: scrubberRef }),
    [],
  );
  ...
  const hasSearchHighlight = useSearchHighlight(
    (s) => s.highlightTerms.length > 0 && !s.dismissed,
  );
  ...
  const searchResultIndex = (() => {
    if (!hasSearchHighlight || searchResults.length === 0 || !highlightFrameId)
      return -1;
    return searchResults.findIndex((r) => r.frame_id === highlightFrameId);
  })();

  const inSearchReviewMode =
    hasSearchHighlight && searchResults.length > 0 && searchResultIndex >= 0;
  ...
  const { zoomLevel, targetZoom, setTargetZoom, onContainerWheel } =
    useScrollZoom({
      containerRef,
      frames,
      currentIndex,
  ...
  useEffect(() => {
    ...
    const initScreenBounds = async () => {
      try {
        const { currentMonitor, cursorPosition } =
        ...
        console.warn("Screen bounds check could not be initialized.");
      }
    };
  ...
  // Listen for navigate-to-frame events (deep link: civitas://frame/12345)
  useEffect(() => {
    ...
    const unlisten = listen<string>("navigate-to-frame", async (event) => {
      ...
      try {
        ...
        if (data?.timestamp) {
          ...
          toast({
            title: "Jumped to frame",
            description: `Opened frame ${frameId}`,
          });
          return;
        }
        setPendingNavigation(null);
        ...
        console.error("Timeline frame could not be opened.");
        setPendingNavigation(null);
        toast({
  ...
  // e.g. app opened from cold start via civitas://frame/23 — Timeline mounts late
  useEffect(() => {
  ...
  const screenPermissionMissing =
    ...
    !isPermissionGranted(screenPermission);
  const screenPermissionNeedsAttention =
    screenPermissionMissing || screenRelaunchRequired;
  const showBlockingLoader =
    isLoading &&
    !hasInitialFrames &&
    !isNavigating &&
    !error &&
    !screenPermissionNeedsAttention;
  ...
  useEffect(() => {
    if (isLoading || showBlockingLoader) {
      ...
      // Stopped loading
      if (loadingStartTimeRef.current !== null) {
        totalLoadingTimeRef.current +=
          performance.now() - loadingStartTimeRef.current;
    ...
    async (pipe?: TemplatePipe) => {
      ...
      if (pipe) {
        toast({
          ...
          description: "Running pipe with selection context",
        });
      }
    },
  ...
  useEffect(() => {
    ...
    const findDateWithFrames = async () => {
      ...
      if (!isToday && !isNavigatingRef.current) {
        ...
        if (!nearest) {
          console.warn("No recent timeline frames were found.");
          return;
        }
  ...
  const handleRefresh = useCallback(() => {
    retryConnection();
    void refreshScreenPermission();
  }, [refreshScreenPermission, retryConnection]);
  ...
  useEffect(() => {
    ...
    const unlisten = listen<{
      ...
      search_query?: string;
    }>("search-navigate-to-timestamp", (event) => {
        ...
        try {
          ...
          useKeywordSearchStore.setState({
            ...
            currentResultIndex: results.findIndex(
              (r: any) => r.frame_id === frameId,
            ),
            isSearching: false,
            error: null,
          });
        } catch (e) {
          console.error("Timeline search results could not be parsed.");
        }
      }
      ...
      >
        <div className="text-center p-8 max-w-md">
          ...
          <p className="text-sm font-mono text-muted-foreground leading-relaxed">
            The timeline is turned off in settings. Re-enable it under Display
            settings to browse your recorded history.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <TimelineProvider>
      ...
      >
        ...
        >
          {currentFrame ? (
            <CurrentFrameTimeline
              ...
              canNavigateNext={
                findNextDevice(currentIndex, -1) !== currentIndex
              }
              onFrameLoadError={() => {
              ...
              onFrameUnavailable={async () => {
                ...
                const failedFrameId =
                  frames[currentIndex]?.devices?.[0]?.frame_id;
                if (!failedFrameId) {
                ...
                try {
                  ...
                  console.error(
                    "Next valid timeline frame could not be found.",
                  );
                }
          ...
          !error &&
          frames.length === 0 &&
          (!isLoading || screenPermissionNeedsAttention) ? (
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-background via-background to-muted/20">
              {screenPermissionNeedsAttention ? (
                <ScreenPermissionState
                  ...
                  relaunchRequired={screenRelaunchRequired}
                  onOpenSettings={() => void openScreenPermission()}
                  onRelaunch={relaunchForScreenPermission}
                />
                ...
                <div className="text-center p-8 max-w-md">
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed">
                    Enable screen recording in settings to start capturing your
                    timeline.
                  </p>

                  <button
                    onClick={() =>
                      commands.showWindow({ Home: { page: "recording" } })
                    }
                ...
                <div className="text-center p-8 max-w-md">
                  ...
                  <p className="text-muted-foreground mb-6 leading-relaxed">
                    Civitas is recording your screen activity. Your timeline
                    will appear here as frames are captured.
                  </p>
        ...
        {!embedded && (
          <div className="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-background/85 via-background/30 to-transparent z-30 pointer-events-none" />
        )}

        {/* Bottom Gradient Overlay - Very subtle, only where timeline is */}
        {!embedded && (
          <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-background/80 via-background/25 to-transparent z-30 pointer-events-none" />
        )}

        {/* Top Controls */}
        <div
          ref={navBarRef}
          className={`absolute top-0 left-0 right-0 z-40 px-4 pb-4 ${embedded ? "pt-2" : "pt-[calc(env(safe-area-inset-top)+16px)]"}`}
        >
          <TimelineControls
            ...
            onChatClick={
              embedded ? undefined : () => commands.showWindow("Chat")
            }
            embedded={embedded}
            isPlaying={isPlaying}
            playbackSpeed={playbackSpeed}
        ...
        {screenPermissionNeedsAttention || (error && frames.length > 0) ? (
          <div
            className={`pointer-events-none absolute left-4 right-4 z-[46] space-y-2 ${
              embedded ? "top-14" : "top-[calc(env(safe-area-inset-top)+58px)]"
            }`}
          >
            {screenPermissionNeedsAttention ? (
              <ScreenPermissionState
            ...
            {error && frames.length > 0 ? (
              <TimelineConnectionState compact onRetry={handleRefresh} />
            ) : null}
          </div>
        ) : null}
        ...
        <div
          className={`absolute ${embedded ? "top-12" : "top-[calc(env(safe-area-inset-top)+56px)]"} right-4 z-40`}
        >
        ...
        {(() => {
          ...
          // browser_url from civitas often lacks a protocol (e.g. "github.com/foo");
          // both tauri shell.open and window.open reject/misroute schemeless inputs.
          const openableUrl = browserUrl.includes("://")
            ? browserUrl
            : `https://${browserUrl}`;
          const isHttps = openableUrl.startsWith("https://");
          return (
            <div
              className={`absolute ${embedded ? "top-[56px]" : "top-[calc(env(safe-area-inset-top)+4px)]"} left-0 right-0 z-[45] flex justify-center pointer-events-none`}
            >
              <button
                type="button"
                className="flex items-center gap-1.5 max-w-lg min-w-0 px-3 py-1 rounded-full bg-card/90 backdrop-blur-sm border border-border shadow-sm hover:bg-muted transition-colors cursor-pointer pointer-events-auto"
                ...
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    console.error("Timeline link could not be opened.");
                  }
                }}
              >
                {isHttps ? (
                  <Lock className="w-3 h-3 text-success/80 shrink-0" />
                ) : (
        ...
        >
          <SubtitleBar
            ...
            transcriptionPaused={
              health?.audio_pipeline?.transcription_paused &&
        ...
        {showBlockingLoader && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/90">
            ...
            <div className="bg-card text-foreground p-6 rounded-2xl text-center space-y-3 max-w-md mx-4">
              <h3 className="text-sm font-medium">Loading Timeline</h3>
        ...
        {error && frames.length === 0 && !screenPermissionNeedsAttention && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/90">
            {/* Close button - always visible to prevent being stuck (hidden in embedded mode) */}
            {!embedded && (
              <button
            ...
            <TimelineConnectionState onRetry={handleRefresh} />
          </div>
        )}
        ...
        {inSearchReviewMode && (
          <div
            className="absolute bottom-24 left-1/2 -translate-x-1/2 z-[45] flex items-center gap-3 px-4 py-2 rounded-full bg-card/90 backdrop-blur-sm border border-border text-foreground text-sm shadow-lg"
            onWheel={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          >
            ...
            <button
              className="px-1.5 hover:text-foreground/80 disabled:text-muted-foreground/40"
            ...
            <button
              ...
              onClick={() => navigateToSearchResult(searchResultIndex - 1)}
              title={
                isMac ? "Newer match (→ or ⌘G)" : "Newer match (→ or Ctrl+G)"
              }
            >
              &#9654;
            </button>
            <button
              className="ml-1 text-muted-foreground hover:text-foreground/80"
              ...
              <div className="text-foreground text-sm">
                {isLoading && !screenPermissionNeedsAttention ? (
                  ...
                  <TimelineEmptyStatus
                    ...
                    permissionRequired={screenPermissionMissing}
                    relaunchRequired={screenRelaunchRequired}
                  />
                )}
              </div>
