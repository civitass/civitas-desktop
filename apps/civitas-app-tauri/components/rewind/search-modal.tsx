...
function audioTranscriptions(
  ...
  return data.map((raw) => {
    const item = raw as AudioSearchEnvelope;
    const content = item.content ?? {};
    const start = content.start_time ?? 0;
    const end = content.end_time ?? start;
    return {
      timestamp: content.timestamp ?? "",
      transcription: content.transcription ?? "",
      device_name: content.device_name ?? "",
      is_input: content.device_type?.toLowerCase() !== "output",
      speaker_name:
        content.speaker_label ??
        content.speaker?.name ??
        fallbackSpeakerName,
      duration_secs: Math.max(0, end - start),
    };
  });
}
...
function frameImageUrl(frameId: number, retry?: number): string {
  const retryQuery = retry ? `?retry=${retry}` : "";
  return `${getApiBaseUrl()}/frames/${frameId}${retryQuery}`;
}

function appIconUrl(app: string): string {
  return `http://localhost:11435/app-icon?name=${encodeURIComponent(app)}`;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // only contain words from data that's already FTS-indexed. Use all
        // searchable context so screen-text suggestions are not limited to the
        // legacy OCR fallback; most screen text arrives from accessibility.
...
const FrameThumbnail = ({ frameId, alt }: { frameId: number; alt: string }) => {
  ...
  const [src, setSrc] = useState(frameImageUrl(frameId));
        ...
        <AuthenticatedImage
          ...
          onError={() => {
            if (retryCount.current < 3) {
              ...
              setTimeout(() => {
                setSrc(frameImageUrl(frameId, retryCount.current));
...
function localDateLabel(timestamp: string): string {
  const date = new Date(timestamp);
...
function InlineSearchIssue({
  ...
  return (
    <div
      role="alert"
      className="mb-3 flex items-start justify-between gap-3 rounded-md border border-border bg-muted/40 px-3 py-2 text-xs"
    >
      ...
      <button
        type="button"
        onClick={onRetry}
        className="inline-flex shrink-0 items-center gap-1 rounded-sm px-1.5 py-1 font-medium text-foreground transition-colors hover:bg-background"
      >
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className="text-xs text-muted-foreground font-mono flex-shrink-0 hover:text-foreground hover:underline"
          onClick={onNavigate}
...
export function SearchModal({ isOpen, onClose, onNavigateToTimestamp, embedded = false, standalone = false }: SearchModalProps) {
  ...
  const [transcriptionEpoch, setTranscriptionEpoch] = useState(0);
  ...
  const [frameLookupError, setFrameLookupError] = useState<string | null>(null);
  ...
  const pendingSavedQueryRef = useRef<SavedSearchSnapshot | null>(null);
  const pendingStandaloneHandoffRef = useRef(false);
  ...
  const [facetProjects, setFacetProjects] = useState<{ value: string; label: string; count: number }[]>([]);
  const [facetPeople, setFacetPeople] = useState<{ value: string; label: string; count: number }[]>([]);
  const [facetEvidenceTypes, setFacetEvidenceTypes] = useState<{ value: string; label: string; count: number }[]>([]);
  const [facetsLoading, setFacetsLoading] = useState(false);
  const [facetError, setFacetError] = useState<string | null>(null);
  const [facetEpoch, setFacetEpoch] = useState(0);
  ...
  const savedSearchSnapshot = useMemo<SavedSearchSnapshot>(
    () => ({
      ...
      appName: appFilter,
      domain: domainFilter,
      localDate: timeFilter,
      tags: query.startsWith("#") && query.slice(1).trim()
        ? [query.slice(1).trim()]
        : [],
    }),
    [query, contentFilter, appFilter, domainFilter, timeFilter],
  );

  const applySavedSearch = useCallback((saved: SavedSearch) => {
    pendingSavedQueryRef.current = saved;
    setQuery(saved.query);
    setContentFilter(saved.scope);
    setAppFilter(saved.appName);
    setDomainFilter(saved.domain);
    setTimeFilter(saved.localDate);
    setSelectedIndex(0);
    setSelectedChatIndex(0);
    setOcrOffset(0);
    setHasMoreOcr(true);
  ...
  // Async facets are normalized and aggregated by the local Rust engine.
  useEffect(() => {
    ...
    setFacetError(null);
    void fetchSearchFacets(q, -new Date().getTimezoneOffset(), controller.signal)
      .then((response) => {
        if (controller.signal.aborted) return;
        setFacetApps(response.apps.map((item) => [item.value, item.count]));
        setFacetDomains(response.domains.map((item) => [item.value, item.count]));
        setFacetTimeRanges(
          response.localDates.map((item) => ({
            label: localDateLabel(item.representativeTimestamp),
            dateKey: item.localDate,
            timestamp: item.representativeTimestamp,
            count: item.count,
          })),
        );
        setFacetProjects(response.projects);
        setFacetPeople(response.people);
        setFacetEvidenceTypes(response.evidenceTypes);
      })
      .catch((error) => {
        if (!controller.signal.aborted) {
          ...
          setFacetPeople([]);
          setFacetEvidenceTypes([]);
          setFacetError(getConsumerSearchError(error));
        }
      })
      .finally(() => {
  ...
  }, [debouncedQuery, searchEpoch, facetEpoch]);
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      pendingSavedQueryRef.current = null;
      pendingStandaloneHandoffRef.current = false;
      return;
    }
    setSelectedIndex(0);
    const windowParams = standalone
      ? new URLSearchParams(window.location.search)
      : null;
    const standaloneSavedSearch = windowParams
      ? savedSearchFromWindowQuery(windowParams.toString())
      : null;
    pendingSavedQueryRef.current = standaloneSavedSearch;
    pendingStandaloneHandoffRef.current = standaloneSavedSearch !== null;
    const initialQuery =
      standaloneSavedSearch?.query ?? windowParams?.get("q") ?? "";
  ...
  useEffect(() => {
    ...
    if (!q || q.startsWith("#") || q.startsWith("@")) {
      if (
        !q &&
        pendingStandaloneHandoffRef.current &&
        pendingSavedQueryRef.current?.query.trim()
      ) {
        return;
      }
      pendingStandaloneHandoffRef.current = false;
    ...
    const pendingSaved =
      pendingSavedQueryRef.current?.query.trim() === q
        ? pendingSavedQueryRef.current
        : null;
    pendingStandaloneHandoffRef.current = false;
    pendingSavedQueryRef.current = null;
    if (pendingSaved) {
      setAppFilter(pendingSaved.appName);
      setDomainFilter(pendingSaved.domain);
      setTimeFilter(pendingSaved.localDate);
      setContentFilter(pendingSaved.scope);
    } else {
      setAppFilter(null);
      setDomainFilter(null);
      setTimeFilter(null);
    ...
    if (pendingSaved?.localDate) {
      options.start_time = new Date(`${pendingSaved.localDate}T00:00:00`);
      options.end_time = new Date(`${pendingSaved.localDate}T23:59:59.999`);
    }
    if (pendingSaved?.appName) {
      options.app_names = [pendingSaved.appName];
    }
    searchKeywords(debouncedQuery, options);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setTagError(null);
      try {
        const response = await fetchSearchTags(tagQuery, controller.signal);
        if (controller.signal.aborted) return;
        setAllTags(response.tags.map((tag) => tag.name));
        setTagResults(
          response.frames.map((frame) => ({
            frame_id: frame.frameId,
            timestamp: frame.timestamp,
            tag_names: frame.tagNames,
        ...
        if (!controller.signal.aborted) {
          setAllTags([]);
          setTagResults([]);
          setTagError(getConsumerSearchError(error));
        }
      } finally {
        if (!controller.signal.aborted) setIsSearchingTags(false);
      }
    })();

    return () => controller.abort();
  }, [debouncedQuery, tagEpoch]);
      ...
      try {
        ...
        if (!resp.ok) {
          if (!cancelled) {
            setSpeakerResults([]);
            setSpeakerError(
              `The local speaker index returned ${resp.status}.`,
            );
          }
          return;
        }
        if (!cancelled) {
          ...
          setSpeakerResults(
            speakers.filter((speaker) => speaker.name).slice(0, isAtQuery ? 20 : 5),
          );
        }
      } catch (error) {
        if (!cancelled && !controller.signal.aborted) {
          setSpeakerResults([]);
          setSpeakerError(getConsumerSearchError(error));
        }
      } finally {
  ...
  }, [debouncedQuery, selectedSpeaker, speakerEpoch]);

  const lookupTranscriptionFrames = useCallback(
    async (
      ...
      const uniqueTimestamps = [
        ...new Set(items.map((item) => item.timestamp).filter(Boolean)),
      ];
      if (uniqueTimestamps.length === 0) {
        if (!merge) setTranscriptionFrames(new Map());
        setFrameLookupError(null);
        return;
      }
      setFrameLookupError(null);
      try {
        const response = await fetchNearestFrames(uniqueTimestamps, signal);
        const matches = new Map<
          string,
          { frame_id: number; app_name: string }
        >();
        for (const frame of response.frames) {
          if (frame.frameId !== null) {
            matches.set(frame.requestedTimestamp, {
              frame_id: frame.frameId,
              app_name: frame.appName ?? "",
            });
          }
        }
        setTranscriptionFrames((current) => {
          if (!merge) return matches;
          const next = new Map(current);
          for (const [timestamp, frame] of matches) {
            next.set(timestamp, frame);
          }
          return next;
        });
      } catch (error) {
        if (!signal?.aborted) {
          if (!merge) setTranscriptionFrames(new Map());
          setFrameLookupError(getConsumerSearchError(error));
        }
      }
    },
    [],
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!resp.ok) {
          if (!cancelled) {
            ...
            setTranscriptionError(
              `The local transcript index returned ${resp.status}.`,
            );
          }
          return;
        }
        if (!cancelled) {
          const items = audioTranscriptions(
            await resp.json(),
            selectedSpeaker.name,
          );
          setHasMoreTranscriptions(items.length >= TRANSCRIPTION_PAGE_SIZE);
          setSpeakerTranscriptions(items);
          await lookupTranscriptionFrames(
            items,
            controller.signal,
            false,
          );
        ...
        if (!cancelled && !controller.signal.aborted) {
          setSpeakerTranscriptions([]);
          setTranscriptionError(getConsumerSearchError(error));
        }
      } finally {
    ...
    return () => { cancelled = true; controller.abort(); };
  }, [selectedSpeaker, transcriptionEpoch, lookupTranscriptionFrames]);
  ...
  const loadMoreTranscriptions = useCallback(async () => {
    ...
    try {
      ...
      const items = audioTranscriptions(await resp.json(), selectedSpeaker.name);
      if (items.length < TRANSCRIPTION_PAGE_SIZE) {
        setHasMoreTranscriptions(false);
      }
      setTranscriptionOffset(newOffset);
      setSpeakerTranscriptions((previous) => [...previous, ...items]);
      await lookupTranscriptionFrames(items, undefined, true);
    } catch (error) {
      setTranscriptionError(getConsumerSearchError(error));
    } finally {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (
        ...
        e.target.closest("[data-saved-search-panel]")
      ) {
        return;
      }
      ...
      if (selectedSpeaker) {
        ...
        switch (e.key) {
          ...
          case "ArrowUp":
            ...
            break;
          case "Enter":
            if (e.shiftKey || isImeComposingKey(e)) return;
            e.preventDefault();
  ...
  const hasResults =
    ...
    tagResults.length > 0 ||
    uiEventResults.length > 0 ||
    filteredChats.length > 0;
  const showSearchError = Boolean(
    searchError &&
    !isSearching &&
    debouncedQuery &&
  ...
  const showEmpty = !showSearchError && !isSearching && !isSearchingSpeakers && !isSearchingTags && !isSearchingUiEvents && debouncedQuery && debouncedQuery.trim().length >= 3 && !hasResults && !selectedSpeaker && !isTagSearch && !isPeopleSearch && contentFilter !== "chats";
    ...
    <>
      ...
      {selectedSpeaker ? (
        <div>
          ...
          {transcriptionError && (
            <InlineSearchIssue
              ...
              onRetry={() => setTranscriptionEpoch((epoch) => epoch + 1)}
            />
          )}
          {frameLookupError && (
            <InlineSearchIssue
              title="Preview frames unavailable"
              message={frameLookupError}
              onRetry={() => {
                void lookupTranscriptionFrames(
                  speakerTranscriptions,
                  undefined,
                  false,
                );
          ...
          {speakerAppCounts.length > 1 && (
            <div className="flex gap-1.5 mb-2 overflow-x-auto scrollbar-hide pb-0.5">
              <button
                ...
                className={cn(
                  "px-2.5 py-1 text-xs rounded-full border transition-colors flex items-center gap-1.5 whitespace-nowrap shrink-0",
                  !speakerAppFilter
            ...
            <div className="flex gap-1.5 mb-3 overflow-x-auto scrollbar-hide pb-0.5">
              ...
              {speakerTimeRanges.map((range) => (
                <button
                  ...
                  className={cn(
                    "px-2.5 py-1 text-xs rounded-full border transition-colors flex items-center gap-1 whitespace-nowrap shrink-0",
              ...
              {filteredSpeakerTranscriptions.map((t, index) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "cursor-pointer rounded overflow-hidden border transition-ui duration-150",
        ...
        <>
          ...
          {isTagSearch && tagError && (
            <InlineSearchIssue
              title="Tags unavailable"
              message={tagError}
              onRetry={() => setTagEpoch((epoch) => epoch + 1)}
            />
          )}
            ...
            <div className="grid grid-cols-3 gap-3">
              {tagResults.map((frame) => (
                <div
                  ...
                  className="cursor-pointer rounded overflow-hidden border border-border hover:border-foreground/50 transition-ui duration-150"
                >
                  <FrameThumbnail
                  ...
                  <div className="p-2 bg-card">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {frame.tag_names.map((t) => (
                        <span
                          key={t}
                          className="px-1.5 py-0.5 text-2xs rounded-full bg-foreground/8 text-foreground/60"
                        >
                          {t}
                        </span>
                      ))}
          ...
          {/* Tag search empty */}
          {isTagSearch && !tagError && !isSearchingTags && tagResults.length === 0 && allTags.length === 0 && (
            <div className="py-12 text-center text-sm text-muted-foreground">
              {query.slice(1).trim()
          ...
          {isPeopleSearch && speakerError && (
            <InlineSearchIssue
              title="People unavailable"
              message={speakerError}
              onRetry={() => setSpeakerEpoch((epoch) => epoch + 1)}
            />
          )}
          ...
          {/* @ people search empty */}
          {isPeopleSearch && !speakerError && !isSearchingSpeakers && speakerResults.length === 0 && (
            <div className="py-12 text-center text-sm text-muted-foreground">
              {query.slice(1).trim()
          ...
          {!isPeopleSearch && speakerError && (
            <InlineSearchIssue
              title="People suggestions unavailable"
              message={speakerError}
              onRetry={() => setSpeakerEpoch((epoch) => epoch + 1)}
            />
          )}
          ...
          {!isTagSearch && !isPeopleSearch && facetError && (
            <InlineSearchIssue
              title="Search filters unavailable"
              message={facetError}
              onRetry={() => setFacetEpoch((epoch) => epoch + 1)}
            />
          )}

          {!isTagSearch &&
            !isPeopleSearch &&
            (facetProjects.length > 0 || facetPeople.length > 0) && (
              <div className="mb-3 flex flex-wrap gap-1.5">
                {facetProjects.map((project) => (
                  <button
                    key={project.value}
                    type="button"
                    onClick={() => setQuery(`#${project.value}`)}
                ...
                {facetPeople.map((person) => (
                  <button
                    key={person.value}
                    type="button"
                    onClick={() => setQuery(`#${person.value}`)}
                    className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                  >
          ...
          {!isTagSearch &&
            !isPeopleSearch &&
            facetEvidenceTypes.length > 0 && (
              <div
                aria-label="Evidence types in these results"
                className="mb-3 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground"
              >
                <span>Evidence</span>
                {facetEvidenceTypes.map((evidence) => (
                  <span
                    key={evidence.value}
          ...
          {showSearchError && (
            <div
              role="alert"
              className="mb-3 rounded-md border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs text-destructive"
            >
              <span className="font-medium">Search unavailable.</span>{" "}
              <span className="text-destructive/80">{searchError}</span>
            </div>
          )}
                ...
                return CHAT_BUCKET_ORDER.map((bucket) => {
                  ...
                  return (
                    <div key={bucket} className="mb-1">
                      <p className="px-1 pt-2 pb-1 text-2xs uppercase tracking-wider font-mono text-muted-foreground/50">
                  ...
                  return (
                    ...
                    >
                      <div className="relative">
                        ...
                        {groupSize > 1 && (
                          <span className="absolute top-1.5 right-1.5 px-1.5 py-0.5 text-2xs font-medium bg-black/70 text-white rounded">
                            {groupSize} frames
                          </span>
                        )}
                      </div>
          ...
          {!debouncedQuery && !isSearching && contentFilter !== "chats" && (
            <div className="py-4 px-2 space-y-4">
              ...
              {/* Suggestion chips from recent searchable activity */}
              {suggestions.length > 0 && (
                <div>
                  <p className="text-xs text-muted-foreground/60 mb-2">From recent searchable activity</p>
              ...
              {!recentChats.length && !suggestionsLoading && (
                <div className="text-center text-sm text-muted-foreground py-4">
                  type to search your local activity
                </div>
              )}
            </div>
          )}
  ...
  if (standalone || embedded) {
    ...
    return (
      <div className={cn(
        ...
        <div className={cn(
          "relative z-20 flex items-center gap-3 px-5 border-b border-border/50",
        ...
        <div className={cn(
          "px-4 py-1.5 border-t border-border/50 flex items-center justify-between text-2xs text-muted-foreground/70 font-mono",
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-4xl mx-4 bg-card border border-border shadow-2xl overflow-hidden rounded-lg isolate">
        {/* Search Input */}
        <div className="relative z-20 flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        <div className="px-4 py-2 border-t border-border bg-muted/30 flex items-center justify-between text-2xs text-muted-foreground font-mono">
          <div className="flex items-center gap-4">
            {selectedSpeaker ? (
              <>
                <span>↑↓ navigate</span>
