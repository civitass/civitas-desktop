...
describe("SearchResultStrip — rendered component (keyboard + click share this indicator)", () => {
	it("renders one dot per result and marks the active one", () => {
		...
		expect(getByTestId("search-result-dot-2").getAttribute("aria-pressed")).toBe("true");
		expect(getByTestId("search-result-dot-2").getAttribute("aria-label")).toContain(
			"current match",
		);
	});
	...
	it("uses button semantics instead of claiming the container is an unfocusable slider", () => {
		const { getByRole, getAllByRole } = render(
			<SearchResultStrip resultsLength={N} activeIndex={1} onJump={() => {}} />,
		);
		expect(getByRole("group").getAttribute("aria-label")).toContain("match 2 of 5");
		expect(getAllByRole("button")).toHaveLength(N);
	});
