...
/** Authenticated local app icon. The parent keeps its letter fallback visible on failure. */
...
export const TimelineSlider = ({
	...
	// Tracks the time of the last index change so we can tell a fast scroll burst
	// (rapid consecutive changes) from a discrete jump (keyboard, search nav).
	...
	useEffect(() => {
		...
		// During a fast scroll burst, a fresh "smooth" scroll fires on every tick
		// before the previous one finishes — the browser's compositor-driven
		// animation then fights React's index-driven re-render, producing visible
		// judder. So scroll instantly while bursting and only animate smoothly for
		// discrete jumps (>120ms since the last change). Keeps keyboard/search
		// navigation feeling smooth without the scroll-gesture jitter.
		const now = performance.now();
		const isBurst = now - lastIndexChangeAtRef.current < 120;
		lastIndexChangeAtRef.current = now;

		currentElement.scrollIntoView({
			behavior: isBurst ? "auto" : "smooth",
			block: "nearest",
			inline: "center",
		});
	...
	useLayoutEffect(() => {
		...
		// No memories → nothing to position. Cheap early-out keeps the expensive
		// reflow off the scroll hot path entirely for the common case.
		if (!timelineContentRef.current || memories.length === 0) {
			clearIfNeeded();
			return;
		}

		// The measurement reads getBoundingClientRect() on every rendered frame
		// (~120 nodes) — a forced synchronous reflow. Running it inline on every
		// appGroups change (i.e. every scroll tick) is a primary dropped-frame
		// source. Coalesce to one measurement per animation frame and cancel any
		// pending one, so a fast scroll burst measures once when it settles rather
		// than ~120 rect reads per tick. Memory diamonds tolerate the ~1-frame lag.
	...
	return (
		<div className="relative w-full" dir="rtl">
			...
			>
				...
				{allDeviceIds.length > 1 && onDeviceChange && !filtersCollapsed && (
					...
					>
						...
						<div
							className={cn(
								"flex items-center gap-1.5 overflow-hidden transition-ui duration-200 ease-out",
						...
						>
							{allDeviceIds.map((id) => (
								<button
									...
									className="rounded-full transition-ui duration-200 hover:scale-125 shrink-0"
									style={{
				...
				>
					...
					<div
						className={cn(
							"flex items-center gap-1 overflow-hidden transition-ui duration-200 ease-out",
							"bg-background/80 backdrop-blur-sm border border-border border-l-0 rounded-r-md",
						...
						<div className="absolute top-0 left-0 right-0 h-5 pointer-events-auto" style={{ direction: "ltr", zIndex: 40 }}>
							{memories.map((mem) => {
								...
								return (
									...
									>
										<div
											className="w-2 h-2 bg-foreground/50 rotate-45 hover:bg-foreground hover:scale-150 transition-ui duration-150"
									...
									<>
										...
										<div className="flex items-center gap-1.5 text-muted-foreground flex-wrap">
											...
											{usefulTags.slice(0, 3).map((t) => (
												<span key={t} className="px-1 py-0.5 bg-foreground/10 rounded text-2xs">{t}</span>
											))}
										</div>
									</>
								);
								...
								{group.dayBoundaryDate && (
									...
									>
										...
										<div
											...
											style={{ direction: 'ltr' }}
										>
											<span className="text-2xs font-medium text-primary bg-primary/15 border border-primary/25 rounded-full px-2 py-0.5">
												{group.dayBoundaryDate}
											</span>
										</div>
									</div>
								...
								{group.frames.map((frame, frameIdx) => {
									...
									return (
										<motion.div
											...
											className={cn(
												"flex-shrink-0 cursor-ew-resize rounded-t relative hover:z-50 transition-ui duration-200",
												(isSelected || isInRange) && "ring-2 ring-foreground/60 ring-offset-1 ring-offset-background",
										...
										>
											...
											{timeMarker && (
												<div
													className="absolute top-full mt-1 left-1/2 -translate-x-1/2 text-2xs font-mono text-muted-foreground whitespace-nowrap"
													style={{ direction: 'ltr' }}
												>
													{timeMarker.time}
												</div>
											...
											{shouldShowTooltip && hoveredRect && createPortal(
												...
												>
													...
													{hasTags && (
														<p className="text-primary flex items-center gap-1 mt-1 text-xs">
														...
														return (
															<>
																...
																<div className="flex flex-col gap-0.5">
																	{shown.map((ev, i) => (
																		<p key={i} className="text-muted-foreground text-xs leading-tight">
			...
			{hoveredFilterDot && createPortal(
				<div
					className="fixed z-[9999] bg-popover border border-border rounded px-1.5 py-0.5 text-2xs text-popover-foreground whitespace-nowrap shadow-lg pointer-events-none"
					style={{
