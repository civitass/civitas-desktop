	...
	const handleRetranscribe = useCallback(async () => {
		...
		try {
			...
			await showChatWithPrefill({
				context: "",
				prompt: `please read your civitas-api skill first, then retranscribe the audio from ${start} to ${end}. after retranscribing, ask me if i want to assign speaker names to the different speakers in the transcription.`,
				autoSend: true,
											...
											>
												{visible.map((pipe, i) => {
													...
													return (
														...
														>
															<button
																className="w-full h-full rounded-full border-2 shadow-md flex items-center justify-center text-2xs font-bold cursor-pointer transition-ui hover:scale-125 hover:shadow-lg"
																style={{
																	borderColor: color,
												...
												{overflow > 0 && (
													...
													>
														<button
															className="w-full h-full rounded-full bg-muted border border-border shadow-md flex items-center justify-center text-2xs font-medium text-muted-foreground cursor-pointer hover:scale-110 transition-ui"
												...
												<AnimatePresence>
													{hoveredPipeIndex !== null && (
														<motion.div
															...
															className="absolute left-1/2 -translate-x-1/2 px-2 py-0.5 rounded bg-foreground text-background text-2xs whitespace-nowrap pointer-events-none shadow z-10"
															style={{ top: -4 }}
														>
				...
				{existingTags.length > 0 && (
					<div className="flex flex-wrap gap-1 max-h-[52px] overflow-y-auto scrollbar-hide">
						{existingTags.map((t) => (
							<span
								key={t}
								className="inline-flex items-center gap-1 text-2xs px-1.5 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/25 max-w-[140px]"
							>
