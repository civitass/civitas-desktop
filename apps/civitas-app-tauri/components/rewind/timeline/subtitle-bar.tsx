...
export function SubtitleBar({ frames, currentIndex, isPlaying, onClick, transcriptionPaused, meetingApp }: SubtitleBarProps) {
		...
		return (
			<div className="flex justify-center">
				<button
					...
					className="flex items-center gap-1.5 px-2.5 py-1 bg-background/80 backdrop-blur-sm rounded-full border border-border/50 shadow-sm hover:bg-background/90 hover:border-border hover:shadow-md transition-ui duration-200 text-muted-foreground hover:text-foreground"
					title="Show captions"
				>
					<MessageSquareText className="w-3.5 h-3.5" />
					<span className="text-2xs font-medium">Captions</span>
				</button>
			</div>
		);
	}
	...
	return (
		<div
			className="transition-ui duration-200 opacity-100 cursor-pointer group max-w-2xl mx-auto"
			onClick={onClick}
		...
		>
			...
			<div className="flex justify-end mb-1">
				<button
					...
					className="px-2 py-0.5 rounded-full bg-black/60 hover:bg-black/80 text-white/70 hover:text-white text-2xs font-medium backdrop-blur-sm transition-colors flex items-center gap-1"
					title="Hide captions"
				>
					<X className="w-3 h-3" />
					<span>Hide</span>
			...
			<div
				ref={scrollRef}
				className="flex flex-col gap-1 px-3 py-2 bg-background/80 backdrop-blur-sm rounded border border-border/50 shadow-sm overflow-hidden transition-ui duration-200 group-hover:bg-background/90 group-hover:border-border group-hover:shadow-md"
			>

				{/* Transcription status indicator */}
				{transcriptionPaused && (
					<div className="flex items-center gap-1.5 text-2xs text-warning/80">
				...
				{/* CTA hint */}
				<div className={`flex items-center justify-center gap-1 transition-ui duration-200 overflow-hidden ${isHovered ? "max-h-6 opacity-100 pt-0.5" : "max-h-0 opacity-0"}`}>
					<ChevronDown className="w-3 h-3 text-muted-foreground/60" />
					<span className="text-2xs text-muted-foreground/60">Click for full transcript</span>
				</div>
			</div>
		</div>
	);
...
function SubtitleLine({
	...
	return (
		<div
			className="flex items-start gap-2 transition-opacity duration-200 ease-out motion-reduce:transition-none"
			style={{ opacity }}
		>
			{entry.is_input ? (
			...
			{isPending ? (
				...
				<span className={`text-sm text-muted-foreground flex-1 transition-ui duration-200 ${isHovered || isLookahead ? "whitespace-normal" : "whitespace-normal line-clamp-2"}`}>
					&ldquo;{text}&rdquo;
				</span>
			)}

			<span className="text-2xs text-muted-foreground/60 font-mono shrink-0 tabular-nums mt-px">
				{timeStr}
			</span>
		</div>
	);
