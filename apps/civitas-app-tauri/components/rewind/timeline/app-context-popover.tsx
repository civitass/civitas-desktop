						...
						<div className="pl-4 space-y-0.5">
							{data.topUrls.map((u, i) => {
								...
								return (
									<button
										key={i}
										className="flex items-center gap-1 text-info hover:text-info truncate w-full text-left transition-colors cursor-pointer"
										title={openableUrl}
										onClick={(e) => {
											e.stopPropagation();
											openExternalUrl(openableUrl).catch((err) => {
												console.error("Application context link could not be opened.");
											});
										}}
									>
										{domain ? (
