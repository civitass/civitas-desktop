...
export async function readRegionWithLocalOcr(base64Png: string): Promise<string> {
  const result = await commands.performOcrOnImage(base64Png);
  ...
  const performRegionOcr = useCallback(
    async (selection: SelectionRect) => {
      ...
      try {
        ...
        const extractedText = await readRegionWithLocalOcr(base64);
        ...
        toast({
          ...
          description:
            err instanceof Error
              ? err.message
              : "The selected region could not be read locally.",
          variant: "destructive",
        });
      } finally {
