	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				const available = resAvail.status === "ok" ? resAvail.data : false;				if (cancelled || !available) return;
				await commands.livetextInit(windowLabel);				if (!cancelled) {
					liveTextInitRef.current = true;
					setNativeLiveTextActive(true);
				}
			} catch (e) {
				console.warn("Native Live Text could not be initialized.");
			}
		})();
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			...
			commands.livetextAnalyze(
				...
				console.warn("Native Live Text analysis failed.");
			});
		}, 150);
