...
export function useFrameLoading(opts: {
	...
	const displayedFallbackUrlRef = useRef<string | null>(null);
	...
	const resolveEffectiveFps = useCallback((
		...
		if (serverFps > 0) {
			...
			console.warn("Timeline media rate metadata was invalid; recalibrating.");
		}
		...
		for (const candidate of commonFps) {
			...
			if (offsetIndex < maxOffset) {
				calibratedFpsCache.set(path, candidate);				return candidate;
			}
		}

		// Last resort: derive directly
		const derived = (offsetIndex + 1) / duration;
		calibratedFpsCache.set(path, derived);		return derived;
	}, []);
	...
	useEffect(() => {
		...
		const doSeek = async () => {
			...
			if (targetTime > video.duration) {
				console.warn("Timeline media seek target exceeded the segment; clamping.");
		...
		doSeek().catch((err) => {
			if (gen !== seekGenRef.current) return;
			console.warn("Native video seek failed; using the local fallback.");
	...
	useEffect(() => {
		...
		const img = new Image();
		img.onload = () => {
			const previousUrl = displayedFallbackUrlRef.current;
			displayedFallbackUrlRef.current = fetchedUrl;
			setDisplayedFallbackUrl(fetchedUrl);
			fetchedUrl = null;
		...
		void fetchAuthenticatedAssetUrl(fallbackImageUrl, controller.signal)
			.then((url) => {
				fetchedUrl = url;
				img.src = url;
			})
			.catch(() => {
				if (!controller.signal.aborted) img.onerror?.(new Event("error"));
			});
		return () => {
			controller.abort();
			img.onload = null;
	...
	useEffect(() => {
		return () => {
			if (displayedFallbackUrlRef.current) {
				URL.revokeObjectURL(displayedFallbackUrlRef.current);
				displayedFallbackUrlRef.current = null;
			}
		};
	}, []);
