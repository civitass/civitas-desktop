 ...
 *   │ (~/.civitas/chats/) │         │ listens app-wide for pi_event  │
...
import {
  useUpcomingPipes,
  type UpcomingPipe,
} from "@/lib/hooks/use-upcoming-pipes";
...
function useCollapsedPref(key: string, defaultValue = false) {
  const [collapsed, setCollapsedRaw] = useState<boolean>(() =>
    readCollapsedPref(key, defaultValue),
  );
...
function useVisibleChatSections(): {
  ...
  const liveScheduledSids = useMemo(() => {
    ...
    for (const p of runningPipes) {
      if (p.executionId !== undefined)
        set.add(pipeSessionId(p.pipeName, p.executionId));
    }
    return set;
  }, [runningPipes]);
...
export function ChatSidebar({ className, onViewAll }: ChatSidebarProps) {
  ...
  const handleCancelUpcoming = async (pipeName: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`disable returned ${response.status}`);
      }
    } catch {
      toast({
        title: "Couldn’t cancel this workflow",
        description:
          "Its schedule was not changed. Retry when Civitas is ready.",
        variant: "destructive",
      });
    }
  ...
  const hasScheduledSlice =
    upcomingPipes.length > 0 ||
    runningPipes.length > 0 ||
    upcomingError !== null;
  const [pinnedCollapsed, setPinnedCollapsed] = useCollapsedPref(
    "civitas:pinned-collapsed",
    true,
  );
  const [recentsCollapsed, setRecentsCollapsed] = useCollapsedPref(
    "civitas:recents-collapsed",
  );
  const [archivedCollapsed, setArchivedCollapsed] = useCollapsedPref(
    "civitas:closed-collapsed",
    true,
  );
  const [scheduledCollapsed, setScheduledCollapsed] = useCollapsedPref(
    "civitas:scheduled-collapsed",
  );
  const [upcomingCollapsed, setUpcomingCollapsed] = useCollapsedPref(
    "civitas:upcoming-collapsed",
  );

  const openAllCollapsed =
    recentsCollapsed && (archived.length === 0 || archivedCollapsed);
  ...
  const handleArchive = async (id: string) => {
    try {
      ...
      toast({
        title: "Chat wasn’t archived",
        description:
          "Civitas couldn’t save the change locally. The chat is still in Recents.",
        variant: "destructive",
      });
      return;
    }
    ...
    void commands.piAbort(id).catch(() => {
      // Archival is already durable; a late stream event is reconciled by the
      // hidden flag when the conversation is next hydrated.
    });
  ...
  const handleUnarchive = async (id: string) => {
    try {
      ...
      toast({
        title: "Chat wasn’t restored",
        description:
          "Civitas couldn’t save the change locally. The chat remains archived.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDeleteConfirmed = async (id: string) => {
    try {
      ...
      toast({
        title: "Chat wasn’t deleted",
        description:
          "Civitas couldn’t remove the local file. Nothing was removed; check storage access and retry.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleTogglePin = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: next ? "Chat wasn’t pinned" : "Chat wasn’t unpinned",
        description: "Civitas couldn’t save the change locally. Retry.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleRenameConfirmed = async (id: string, nextTitleRaw: string) => {
    ...
    try {
      ...
      toast({
        title: "Chat wasn’t renamed",
        description: "The title could not be saved locally. Nothing changed.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // The local file and visible row are already correct; other windows
      // reconcile from disk on their next history refresh.
    }
  };
  ...
  const handleStopRun = async (pipeName: string) => {
    try {
      const response = await localFetch(
        `/pipes/${encodeURIComponent(pipeName)}/stop`,
        {
          method: "POST",
        },
      );
      if (!response.ok) {
        throw new Error(`stop returned ${response.status}`);
      }
    } catch {
      toast({
        title: "Workflow is still running",
        description: "Civitas couldn’t stop it. Retry in a moment.",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-1">
        {hasScheduledSlice && (
          <div className="flex flex-col shrink-0">
            {upcomingError && (
              <div
                className="mx-1 mb-1 flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/30 px-2 py-1.5 text-xs text-muted-foreground"
                role="status"
                aria-live="polite"
              >
                <AlertCircle aria-hidden="true" className="h-3 w-3 shrink-0" />
                <span className="min-w-0 flex-1">
                  Upcoming schedules unavailable.
                </span>
                <button
                  type="button"
                  className="shrink-0 rounded-sm px-1 py-0.5 font-medium text-foreground transition-colors duration-150 ease-out hover:bg-muted focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:cursor-wait disabled:opacity-60"
                  onClick={() => void refetchUpcoming()}
                  disabled={upcomingRefreshing}
                  aria-label="Retry upcoming workflow schedules"
                  title={upcomingError}
                >
                  {upcomingRefreshing ? "Retrying…" : "Retry"}
                </button>
              </div>
            )}
            {upcomingPipes.length > 0 && (
...
export function CollapsedChatSidebarButton({
  ...
  useEffect(() => {
    ...
    if (pinned.length === 0 && recents.length === 0 && archived.length === 0)
      return;
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        ...
        align="start"
        sideOffset={8}
        className="w-64 p-0 rounded-xl shadow-sm"
      >
        {isLoadingChats ? (
          <div
            className="py-1"
            aria-busy="true"
            data-testid="collapsed-chat-sidebar-skeleton"
          >
          ...
          <Tabs
            ...
            className="w-full"
          >
            <TabsList className="grid w-full grid-cols-3 h-8 rounded-t-xl bg-muted/50 border-b border-border p-0">
              <TabsTrigger
                value="pinned"
                disabled={pinned.length === 0}
                className="relative h-8 rounded-tl-xl bg-transparent px-2 text-2xs font-medium shadow-none data-[state=active]:bg-background data-[state=active]:shadow-none"
              >
                Pinned
              </TabsTrigger>
              <TabsTrigger
                ...
                className="relative h-8 rounded-none bg-transparent px-2 text-2xs font-medium shadow-none data-[state=active]:bg-background data-[state=active]:shadow-none"
              >
                Recents
              </TabsTrigger>
              <TabsTrigger
                ...
                className="relative h-8 rounded-tr-xl bg-transparent px-2 text-2xs font-medium shadow-none data-[state=active]:bg-background data-[state=active]:shadow-none"
              >
                Archived
              </TabsTrigger>
            </TabsList>
    ...
    >
      ...
      <div className="absolute right-2.5 top-1/2 -translate-y-1/2 h-5 w-5 flex items-center justify-end">
        ...
        >
          ...
          >
            <DropdownMenuItem
              className="text-xs h-[30px] px-2 gap-2 rounded-none focus:bg-muted/30"
              onSelect={(e) => {
                e.stopPropagation();
            ...
            {session && (
              <>
                ...
                <DropdownMenuSeparator className="my-1 bg-border/70" />
                <DropdownMenuItem
                  className="text-xs h-[30px] px-2 gap-2 rounded-none text-destructive focus:text-destructive focus:bg-destructive/10"
                  onSelect={(e) => {
                    e.stopPropagation();
...
function CollapsibleUpcoming({
  ...
  const [collapsed, setCollapsedRaw] = useState<boolean>(() => {
    try {
      return localStorage.getItem("civitas:upcoming-collapsed") === "true";
    } catch {
      return false;
    }
  });
  const setCollapsed = (v: boolean) => {
    setCollapsedRaw(v);
    try {
      localStorage.setItem("civitas:upcoming-collapsed", String(v));
    } catch {
      // ignore
    }
  };
  return (
    <div className="flex flex-col mb-2 shrink-0">
      ...
      >
        ...
        <span className="text-2xs font-medium text-muted-foreground/60 flex-1">
          Upcoming
        </span>
        <PipeActivityIndicator
          kind="upcoming"
...
function UpcomingRow({
  ...
  return (
    <div
      className="group mx-0 flex cursor-default select-none items-center gap-2 rounded-md px-2.5 py-1 text-foreground hover:bg-muted/40 focus-within:bg-muted/40"
    ...
    >
      ...
      {/* Countdown swaps out for cancel on hover or keyboard focus. Both occupy
          the same slot, preserving the row's stable native-list rhythm. */}
      <PipeActivityIndicator
        kind="upcoming"
        label={countdown}
      ...
      <UpcomingWorkflowCancelButton
        workflowName={pipe.title || pipe.pipeName}
        onCancel={() => onCancel(pipe.pipeName)}
        testId={`upcoming-cancel-${pipe.pipeName}`}
      />
    </div>
  );
}
    ...
    <div className="flex flex-col min-h-0">
      ...
      >
        <div
          ...
          onScroll={collapsed ? undefined : onBodyScroll}
        >
...
export function SidebarChatRow({
  ...
  const now = useMinuteTick(
    !isLive && !isUnread && !isError && queuedCount === 0,
  );
  const age = formatCompactAge(activityAt, now);
  const canSwapAgeForMenu =
    !isLive && !isError && queuedCount === 0 && !isUnread && Boolean(age);
  const menuOpen = openConversationMenuId === session.id;
  return (
    <div
      className={cn(
        "group relative flex items-center gap-2 px-2.5 py-1.5 rounded-md select-none",
    ...
    >
      <button
        type="button"
        className="focus-ring min-w-0 flex-1 flex items-center gap-2 text-left rounded-md"
        onClick={() => {
          setOpenConversationMenuId?.(null);
          onSelect(session.id);
        }}
      >
        <span
          className={cn(
            "truncate flex-1 text-sm font-normal",
            isUnread
        ...
        >
          {session.streamingTitle ||
            (isConversationHistorySyncPrompt(session.title)
              ? undefined
              : session.title) ||
            "untitled"}
        </span>
  ...
  const { content, label } = ((): {
    ...
    if (isUnread) {
      return {
        content: (
          <span
            className="font-mono text-2xs leading-none text-foreground inline-flex items-center justify-center w-2.5 h-2.5"
            aria-label="Unread"
          >
            █
          </span>
