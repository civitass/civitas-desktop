...
export function useChatConversations(opts: UseChatConversationsOpts) {
  ...
  const lastPersistenceToastAtRef = useRef(0);
  const lastHistoryToastAtRef = useRef(0);
  const lastUnreadableHistoryToastAtRef = useRef(0);
  const reportHistoryFailure = useCallback(() => {
    const now = Date.now();
    if (now - lastHistoryToastAtRef.current <= 15_000) return;
    lastHistoryToastAtRef.current = now;
    toast({
      title: "Chat history couldn’t be loaded",
      description:
        "Your local files were not changed. Keep Civitas open and retry the history view.",
      variant: "destructive",
    });
  }, [toast]);
  const reportUnreadableHistory = useCallback(
    (count: number) => {
      const now = Date.now();
      if (now - lastUnreadableHistoryToastAtRef.current <= 30_000) return;
      lastUnreadableHistoryToastAtRef.current = now;
      toast({
        title:
          count === 1
            ? "One local chat couldn’t be read"
            : `${count} local chats couldn’t be read`,
        description:
          "Readable chats are still shown and no file was removed. Keep Civitas open and inspect storage before retrying.",
        variant: "destructive",
      });
    },
    [toast],
  ...
  const [showHistory, setShowHistoryRaw] = useState(() => {
    try {
      return localStorage.getItem("civitas:chat-history-open") === "true";
    } catch {
      return false;
    }
  });
  const setShowHistory = useCallback(
    (v: boolean | ((prev: boolean) => boolean)) => {
      setShowHistoryRaw((prev) => {
        ...
        try {
          localStorage.setItem("civitas:chat-history-open", String(next));
        } catch {}
        return next;
      });
    },
  ...
  const [fileConversations, setFileConversations] = useState<
    ConversationMeta[]
  >([]);
  ...
  const loadConversationMetas = useCallback(
    async (query: string) => {
      ...
      if (unreadableCount > 0) {
        reportUnreadableHistory(unreadableCount);
      }
      return conversations;
    },
    [reportUnreadableHistory],
  );

  const upsertFileConversationMeta = useCallback(
  ...
  const syncConversationTitleState = useCallback(
    async (
      id: string,
      {
        title,
      ...
      if (!resolvedTitle || !resolvedTitleSource) {
        try {
          ...
          resolvedTitle =
            resolvedTitle || loadedConversation.title?.trim() || null;
          resolvedTitleSource =
            resolvedTitleSource || loadedConversation.titleSource;
        } catch {
          return;
        }
      }
      ...
      try {
        ...
        if (session) {
          ...
          if (
            !shouldAcceptTitleSource(session.titleSource, resolvedTitleSource)
          )
            return;
        ...
        console.warn("Local chat title state could not be synchronized.");
      }
  ...
  const scheduleHistoryRefresh = useCallback(
    ...
    [refreshFileConversations, reportHistoryFailure],
  );
  ...
  // ---- saveConversation ----
  const saveConversationUnsafe = async (
    msgs: Message[],
    ...
    const convId =
      conversationId || piSessionIdRef.current || crypto.randomUUID();

    // Try to load existing conversation to preserve createdAt + title + kind.
    const { loadConversationFile } = await import("@/lib/chat-storage");
    const existing = await loadConversationFileStrict(convId);
    ...
    const hasValidPreset =
      selectedPreset && selectedPreset.provider && selectedPreset.model?.trim();
      ...
      void (async () => {
        try {
          const aiTitle = await titleCreatedByAI(
            ...
            async (partial) => {
              try {
                ...
                if (
                  !session ||
                  (session.titleSource && session.titleSource !== "fallback")
                )
                  return;
                useChatStore
                  .getState()
                  .actions.patch(convId, { streamingTitle: partial });
              } catch {}
            },
          );
          ...
          if (aiTitle) {
            ...
            const existingConv = await loadConversationFileStrict(convId);
            if (!existingConv) {
            ...
            if (
              existingConv.titleSource === "fallback" ||
              !existingConv.titleSource
            ) {
              ...
              try {
                ...
                console.warn(
                  "Generated chat title could not be written to the local store.",
                );
              }
          ...
          console.warn("Background chat title generation failed.");
        }
      })();
    }
    ...
    try {
      ...
      if (useChatStore.getState().sessions[convId]) {
        useChatStore
          .getState()
          .actions.setMessages(convId, conversation.messages as any);
      }
    } catch (e) {
      console.warn("Chat messages could not be mirrored before saving.");
    }
    ...
    try {
      ...
      console.warn("Chat title could not be synchronized.");
    }

    // Update activeConversationId in store (lightweight — no conversation data)
    if (options.syncActiveConversation !== false) {
      try {
        const { getStore, saveAndEncrypt } =
          await import("@/lib/hooks/use-settings");
        ...
        console.warn("Active chat could not be updated.");
      }
    }
  ...
  const saveConversation = async (
    ...
    try {
      await saveConversationUnsafe(msgs, options);
      lastPersistenceToastAtRef.current = 0;
      return true;
    } catch {
      // Streaming autosave retries frequently. Keep the failure visible
      // without flooding the user with a toast on every retry.
      const now = Date.now();
      if (now - lastPersistenceToastAtRef.current > 15_000) {
        lastPersistenceToastAtRef.current = now;
        toast({
          title: "This chat isn’t saved yet",
          description:
            "Keep Civitas open and check local storage access. Saving will retry automatically.",
          variant: "destructive",
        });
      }
      return false;
  ...
  const streamingSaveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const sig = `${messages.length}|${messages[messages.length - 1]?.id ?? ""}|${messages.reduce(
  ...
  const renameConversation = async (convId: string, newTitle: string) => {
    ...
    const conv = await loadConversationFileStrict(convId);
    if (!conv) return;
    ...
    try {
      ...
      console.warn("Renamed chat could not be synchronized.");
    }
    ...
    try {
      ...
      console.warn("Chat rename notification could not be sent.");
    }
  };
  ...
  const deleteConversation = async (convId: string) => {
    ...
    try {
      ...
      console.warn("Active chat state could not be cleared.");
    }
    ...
    try {
      ...
      console.warn("Chat deletion notification could not be sent.");
    }
  };
  ...
  //      the observed "loading…-on-a-paused-chat" failure mode).
  //
  ...
  const loadConversation = async (
    conv: ChatConversation | ConversationMeta,
  ) => {
    const { useChatStore } = await import("@/lib/stores/chat-store");
      ...
      if (persisted) {
        if (!store.sessions[conv.id]) {
          store.actions.upsert({
            ...
            ...(persisted.pipeContext
              ? { pipeContext: persisted.pipeContext }
              : {}),
          });
        } else {
    ...
    if (existing?.messages && existing.messages.length > 0) {
      ...
      messagesForPanel = full.messages.map((m) => ({
        ...
        ...((m as any).turnIntentId
          ? { turnIntentId: (m as any).turnIntentId }
          : {}),
        timestamp: m.timestamp,
        ...(m.displayContent ? { displayContent: m.displayContent } : {}),
      ...
      if (!store.sessions[conv.id]) {
        ...
        store.actions.patch(conv.id, {
          ...
          updatedAt: Math.max(
            store.sessions[conv.id]?.updatedAt ?? 0,
            full.updatedAt ?? 0,
          ),
  ...
  const branchConversation = async (upToMessageId: string) => {
    ...
    try {
      ...
      console.warn("Branched chat could not be synchronized.");
    }
    ...
    try {
      ...
      console.warn("Active branched chat could not be updated.");
    }
  };
