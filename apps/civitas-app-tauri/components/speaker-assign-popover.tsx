...
export function SpeakerAssignPopover({
		...
		async (name: string) => {
			...
			try {
				...
				try {
					...
					console.error("Speaker assignment propagation failed.");
					// Non-fatal — the initial assignment succeeded
				}
			} catch (error) {
				console.error("Speaker could not be assigned.");
				toast({
					title: "Error",
	...
	const handleMarkAsHallucination = useCallback(async () => {
		...
		try {
			...
			console.error("Transcript segment could not be marked as inaccurate.");
			toast({
				title: "Error",
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className="w-80 p-3 z-[9999] border border-border rounded-[var(--radius)] shadow-none" align="start">
				<div className="space-y-3">
					...
					{(speakers.length > 0 || showCreateOption) && (
						<div className="max-h-[150px] overflow-y-auto border border-border rounded-[var(--radius)]">
							{/* Existing speakers */}
