  ...
  return (
    ...
    >
      {!iconOnly && labelText && (
        <span
          className={cn(
            "text-2xs leading-none tabular-nums text-muted-foreground/70",
            labelClassName,
          )}
        >
          {labelText}
...
function PipeActivityIcon({ kind }: { kind: PipeActivityKind }) {
  ...
  return (
    <span
      className="font-mono text-2xs leading-none text-muted-foreground/60 inline-flex items-center justify-center w-2.5 h-2.5 shrink-0"
      aria-hidden
    >
      ·
    </span>
