...
export function createCivitasUrlTransform(allowedHosts: readonly string[]) {
  ...
  return (url: string): string => {
    try {
      ...
      if (parsed.protocol === "civitas:" && allowed.has(parsed.host)) {
        return url;
      }
    } catch {
...
export function civitasViewerPathFromHref(href: string): string | null {
  try {
    ...
    if (url.protocol !== "civitas:" || url.host !== "view") {
      return null;
    }
...
export async function openCivitasViewerLink(href: string): Promise<boolean> {
  const path = civitasViewerPathFromHref(href);
...
export function rewriteLocalMarkdownLinksForChat(text: string): string {
  return rewriteLocalMediaLinksForChat(text).replace(
    ...
    (match, sigil: string, label: string, rawUrl: string) => {
      ...
      return `[${label}](civitas://view?path=${encodeURIComponent(localPath)})`;
    },
  );
}
