...
import { isImeComposingKey } from "@/lib/utils/ime";
...
const AI_MODEL_UNAVAILABLE_MESSAGE =
  "This model is not available to the selected AI profile. Choose another profile in Settings → AI.";
const AI_PROVIDER_AUTH_MESSAGE =
...
const LARGE_CONTEXT_PROMPT_TAG = "civitas-large-context";
...
function normalizeConnectionForPlatform<T extends ConnectedIntegration>(
  connection: T,
  isWindows: boolean,
): T {
...
// TOOLS definition removed — search is now handled by Pi's civitas-search skill
...
function isPastedTextDoc(doc: Pick<ExtractedDoc, "name" | "ext">) {
  return (
    doc.ext === "txt" &&
    new RegExp(`^${PASTED_TEXT_DOC_BASE_NAME}(?: \\d+)?$`).test(doc.name)
  );
}
...
function extractLargeContextTask(text: string) {
  ...
  const lastParagraphTask = lastParagraph
  ...
  if (
    tail &&
    /\b(summarize|summarise|analyze|analyse|explain|extract|find|review|debug|fix|compare|list|what|why|how|tell|write|create|convert|translate)\b/i.test(
      tail,
    )
  ) {
    return tail;
...
function buildLargeContextPreview(text: string) {
  if (
    text.length <=
    LARGE_CONTEXT_PREVIEW_HEAD_CHARS + LARGE_CONTEXT_PREVIEW_TAIL_CHARS
  ) {
    return text;
  }
  ...
  const omittedChars =
    text.length -
    LARGE_CONTEXT_PREVIEW_HEAD_CHARS -
    LARGE_CONTEXT_PREVIEW_TAIL_CHARS;
  return [
...
function AppStatsBlock({ content }: { content: string }) {
  const items = content
    ...
    .map((line) => {
      ...
      return {
        app: app?.trim() ?? "",
        minutes: parseFloat(mins?.trim() ?? "0"),
      };
    })
    ...
    <MemoizedReactMarkdown
      ...
      components={{
        ...
        a({ href, children, ...props }) {
          if (
            href?.startsWith("civitas://timeline") ||
            href?.startsWith("civitas://frame") ||
            href?.startsWith("civitas://view")
          ) {
            const handleCivitasLinkClick = async (
              ...
              try {
                ...
                if (href.startsWith("civitas://frame")) {
                ...
                console.error("Civitas link could not be opened.");
              }
            };
            ...
            return (
              <a
                href="#"
                onClick={handleCivitasLinkClick}
                className="underline underline-offset-2 text-[hsl(28_35%_42%)] dark:text-[hsl(32_40%_60%)] hover:text-[hsl(28_35%_32%)] dark:hover:text-[hsl(32_40%_70%)] cursor-pointer inline"
                {...props}
              >
                {children}
              </a>
        ...
        pre({ children, ...props }) {
          return (
            <pre
              className="overflow-x-auto rounded-lg bg-[hsl(30_12%_13%)] p-3 my-2 text-xs max-w-full not-prose"
              {...props}
            >
              {children}
            </pre>
        ...
        code({ className, children, ...props }) {
          ...
          return (
            <code
              className="px-1.5 py-0.5 rounded bg-[hsl(30_12%_16%)] text-[hsl(40_28%_85%)] font-mono text-xs not-prose"
              {...props}
            >
              {content}
            </code>
...
function MessageContent({
  ...
  const sourceFooter =
    !deferSourceFooter && sourceCitations.length > 0 ? (
    ...
    !isUser && message.retryPrompt ? (
      <div className="mt-3 pt-3 border-t border-border/40 flex items-center gap-3 flex-wrap">
        ...
        <button
          type="button"
          onClick={() =>
            openFeedback(`AI error in chat: ${message.content.slice(0, 300)}`)
          }
        ...
        {hasDocs &&
          message.attachments!.map((doc, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`shrink-0 w-11 h-11 rounded-lg flex items-center justify-center text-2xs font-semibold tracking-tight ${badge.tint}`}
                >
                  {badge.label}
                </div>
  ...
  if (isUser && message.displayContent) {
    const chipMatch = message.displayContent.match(
      /^\[chip:([^|]+)\|([^\]]+)\] ([\s\S]*)/,
    );
    if (chipMatch) {
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-x-1.5 gap-y-0.5">
            <span className="inline-flex h-5 items-center gap-1 shrink-0 align-top">
              ...
              />
              <span className="text-sm font-mono font-semibold text-foreground/80 leading-5">
                {chipName}
              </span>
            </span>
            <span className="text-sm leading-5 break-words min-w-0">
              {chipText}
            </span>
          </div>
        </div>
...
function attachmentBadge(ext: string): { label: string; tint: string } {
  ...
  return {
    label: (e || "FILE").toUpperCase().slice(0, 4),
...
function getMessageIntentLabel(message: Message): string | null {
  if (
    ...
    (message.intent === "steer" || message.steeredResponse)
  ) {
...
function isPlaceholderConversationTitle(value?: string | null): boolean {
  ...
  return (
    normalized === "" || normalized === "new chat" || normalized === "untitled"
  );
}
...
function isSteeredAssistantMessage(message: Message): boolean {
  return (
    message.role === "assistant" &&
    (message.intent === "steer" || message.steeredResponse === true)
  );
}
...
function buildCollapsedSteerRenderItems(
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    const latestSteerIndex = segment.findIndex(
      (message) => message.id === latestSteer?.id,
    );
    ...
    const finalAssistant =
      (latestSteer?.turnIntentId
        ? [...assistants]
            ...
            .find(
              (message) =>
                message.turnIntentId === latestSteer.turnIntentId &&
                hasRenderableAssistantBody(message),
            )
        : undefined) ??
    ...
    const hiddenAssistants = assistants.filter((message) =>
      hiddenAssistantIds.has(message.id),
    );
    ...
    for (const message of segment.slice(1)) {
      ...
      items.push({
        type: "message",
        message,
        hideIntentLabelWhenCollapsedBy:
          isFinalAssistant && hiddenAssistants.length > 0
            ? collapsedWorkId
            : undefined,
        showActionsWhenExpandedBy:
          message.role === "user" &&
...
function collapsedSteerWorkDuration(
...
function ChatTitleMenu({
  ...
  const commitRename = async () => {
    ...
    try {
      ...
      console.warn("Chat could not be renamed.");
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      console.warn("Chat could not be deleted.");
    }
  };
...
export function StandaloneChat({
  ...
  const [showConnectBanner, setShowConnectBanner] = useState(() => {
    try {
      return (
        localStorage.getItem("civitas_connect_banner_dismissed") !== "true"
      );
    } catch {
      return true;
    }
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const measure = () =>
      setInputSectionWidth(el.getBoundingClientRect().width);
    measure();
  ...
  const tryInChatStartNewRef = useRef<(() => Promise<void> | void) | null>(
    null,
  );
  useEffect(() => {
  ...
  const [connectionChip, setConnectionChip] = useState<{
    id: string;
    name: string;
    icon: string;
  } | null>(null);
  ...
  // Tracks the input section's width so auto-suggestion chips can collapse
  // into a popover when the chat column is narrow. Updated by a ResizeObserver
  ...
  const pendingRuntimeSendRef = useRef<{
    userMessage: string;
    displayLabel?: string;
  ...
  const turnIntentMatchesText = (record: TurnIntentRecord, text: string) => {
    return (
      turnIntentTextValuesMatch(record.content, text) ||
      turnIntentTextValuesMatch(record.preview, text)
    );
  };
  ...
  const findTurnIntentForUserStart = (
    ...
    pruneTurnIntentLedger();
    const sessionTurnIntents = turnIntentLedgerRef.current.filter(
    ...
    const displayPreviewMatchesIncoming = display?.preview
      ...
      : false;
    const canUseDisplayIdentity = Boolean(
      display && (!hasIncomingText || displayPreviewMatchesIncoming),
    );
    const recordMatchesIncoming = (record: TurnIntentRecord) =>
      turnIntentMatchesText(record, text) ||
      (displayPreviewMatchesIncoming &&
        turnIntentMatchesText(record, display?.preview ?? ""));

    const byDisplayId =
      canUseDisplayIdentity && display?.turnIntentId
        ? sessionTurnIntents.find(
            (record) => record.id === display.turnIntentId,
          )
        : null;
    if (byDisplayId && recordMatchesIncoming(byDisplayId)) return byDisplayId;

    const byOptimisticUser =
      canUseDisplayIdentity && display?.optimisticUserId
        ? sessionTurnIntents.find(
            (record) => record.displayedUserId === display.optimisticUserId,
          )
        : null;
    if (byOptimisticUser && recordMatchesIncoming(byOptimisticUser))
      return byOptimisticUser;

    return (
      sessionTurnIntents.find((record) =>
        turnIntentMatchesText(record, text),
      ) ?? null
    );
  };
  ...
  const markTurnIntentConsumed = (id: string, assistantId: string) => {
    turnIntentLedgerRef.current = turnIntentLedgerRef.current.map((record) =>
      record.id === id
        ? { ...record, consumedAssistantId: assistantId }
        : record,
    );
  };
  ...
  // Chat conversations — stored as individual JSON files in ~/.civitas/chats/
  const {
    showHistory,
    setShowHistory,
    historySearch,
  ...
  const loadImageFromPath = useCallback(
    async (filePath: string) => {
      ...
      try {
        ...
        console.error("Dropped image could not be read.");
      }
    },
    [resizeImage],
  );
  ...
  const extractAndAttach = useCallback(
    async (name: string, loadBytes: () => Promise<Uint8Array>) => {
      ...
      try {
        ...
        console.error("Attached document could not be extracted.");
        toast({
    ...
    try {
      const selected = await openFileDialog({
        ...
        filters: [
          {
            name: "Attachments",
            extensions: [...imageExtensions, ...DOC_PICKER_EXTENSIONS],
          },
      ...
      console.error("File picker could not be opened.");
    }
  ...
  const handlePaste = useCallback(
    (e: React.ClipboardEvent) => {
      ...
      if (!connectionChip) {
        const parsed = parseConnectionChip(text, (id) =>
          INTEGRATION_ICON_KEYS.has(id),
        );
        if (parsed) {
          e.preventDefault();
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Discard malformed optional analytics context so it does not fail on
      // every future mount. It never contains the conversation transcript.
      sessionStorage.removeItem("pipeGenerationContext");
    }
    return () => {
      unlisten.then((fn) => fn());
  ...
  const prefillClaimsRef = useRef<
    Map<
      string,
      Array<{ windowLabel: string; timestamp: number; nonce: string }>
    >
  >(new Map());
  useEffect(() => {
    const unlisten = listen<{
      ...
      timestamp: number;
      nonce: string;
    }>("chat-prefill-claim", (event) => {
      const { dedupKey, windowLabel, timestamp, nonce } =
        event.payload || ({} as any);
      if (!dedupKey) return;
      const bucket = prefillClaimsRef.current.get(dedupKey) ?? [];
      if (
        !bucket.some((c) => c.nonce === nonce && c.windowLabel === windowLabel)
      ) {
  ...
  const openConversationLocally = useCallback(async (convId: string) => {
    const { loadConversationFileStrict } = await import("@/lib/chat-storage");
    ...
    let conv: ChatConversation | null;
    try {
      conv = await loadConversationFileStrict(convId);
    } catch {
      toast({
        title: "This chat couldn’t be opened",
        description:
          "Its local file may be damaged or temporarily unavailable. Nothing was overwritten; check storage and retry.",
        variant: "destructive",
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    if (pipeWatchIsStreaming !== undefined)
      setIsStreaming(pipeWatchIsStreaming);
  ...
  }, [currentSessionKind, currentSessionPipeContext]);
  ...
  useEffect(() => {
    ...
    if (pendingId) {
      ...
      (async () => {
        const { loadConversationFileStrict } =
          await import("@/lib/chat-storage");
        try {
          const conv = await loadConversationFileStrict(pendingId);
          if (conv) {
            loadConversationRef.current(conv);
          }
        } catch {
          toast({
            title: "This saved run couldn’t be opened",
            description:
              "Its local conversation file was left unchanged. Check storage and retry from history.",
            variant: "destructive",
          });
        }
      })();
  ...
  const hasActiveFilters =
    activeFilters.timeRanges.length > 0 ||
  ...
  const removeFilter = (
    filterType: "time" | "content" | "app" | "speaker",
    label?: string,
  ) => {
    let newInput = input;
    if (filterType === "time") {
      ...
      if (label) {
        ...
        newInput = newInput
          .replace(
            /@(today|yesterday|last[- ]?week|last[- ]?hour|this[- ]?morning)\b/gi,
            "",
          )
          .trim();
      }
      ...
      const appPattern = new RegExp(
        `@${activeFilters.appName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`,
        "gi",
      );
      ...
      const speakerPattern = new RegExp(
        `@"?${activeFilters.speakerName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"?\\b`,
        "gi",
      );
  ...
  useEffect(() => {
    ...
    const matchesBase = baseMentionSuggestions.some((s) =>
    ...
    const searchSpeakers = async () => {
      ...
      try {
        ...
        console.error("Speakers could not be searched.");
      } finally {
  ...
  const insertMention = (tag: string) => {
    ...
    if (atIndex !== -1) {
      const newValue =
        textBeforeCursor.slice(0, atIndex) + tag + " " + textAfterCursor;
      setInput(newValue);
    }
  ...
  // Keyboard listeners outlive individual renders. Read the latest steering
  // closures through refs so a shortcut never submits stale input or queue
  // state, without tearing down the listener on every render.
  const steerMessageShortcutRef = useRef(steerMessage);
  const steerQueuedPromptShortcutRef = useRef(steerQueuedPrompt);
  steerMessageShortcutRef.current = steerMessage;
  steerQueuedPromptShortcutRef.current = steerQueuedPrompt;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (isComposing || isImeComposingKey(e.nativeEvent)) {
      return;
    }
    ...
    if (isComposerSteerShortcut(e, isMac) && !showMentionDropdown) {
      ...
      if (
        ...
        void Promise.resolve(
          steerMessageShortcutRef.current(input.trim()),
        ).finally(() => {
        ...
        steerShortcutInFlightRef.current = true;
        void Promise.resolve(
          steerQueuedPromptShortcutRef.current(queuedPrompts[0]),
        ).finally(() => {
          steerShortcutInFlightRef.current = false;
        });
      }
    ...
    if (e.key === "Enter" && !e.shiftKey && !showMentionDropdown) {
      ...
      if (
        input.trim() ||
        pastedImages.length > 0 ||
        attachedDocsRef.current.length > 0
      ) {
  ...
  useEffect(() => {
    const handleComposerSteerShortcut = (event: KeyboardEvent) => {
      if (showMentionDropdown) return;
      if (isComposing || isImeComposingKey(event)) return;
    ...
    return () =>
      window.removeEventListener("keydown", handleComposerSteerShortcut, true);
  }, [
    input,
    isComposing,
    isMac,
  ...
  useEffect(() => {
    ...
    setActivePreset((prev) => {
      ...
      // unrelated update from settings, discovery, or another window).
  ...
  }, [settings.aiPresets, isSettingsLoaded, activePipeExecution]);
  ...
  const getDisabledReason = (): string | null => {
    ...
    if (!hasValidModel)
      return `No model selected in "${activePreset.id}" preset`;
    if (piStarting) return "Starting assistant…";
    return null;
  };
      ...
      if (e.key === "Escape" && !showMentionDropdown) {
        if (isLoading || isStreaming) {
          ...
          try {
            ...
            console.warn("Assistant session could not be stopped with Escape.");
          }
          setIsLoading(false);
          setIsStreaming(false);
        } else {
  ...
  const buildProviderConfig = useCallback(
    (preset?: AIPreset | null) => {
      ...
      if (!p) return null;
      // Combine the civitas search instructions with the user's preset prompt.
      ...
      const systemPrompt =
        `${buildSystemPrompt()}\n\n${presetPrompt}${connectionsCtx}`.trim() ||
        null;
      return {
  ...
  const ensureAssistantRuntime = useCallback(
    async (
      userMessage: string,
      displayLabel?: string,
      imageDataUrls?: string[],
    ): Promise<boolean> => {
      if (piInfo?.running || runtimeInstalledRef.current) return true;

      try {
        const status = await commands.getPiRuntimeStatus();
        if (status.installed && !status.installing) {
          runtimeInstalledRef.current = true;
          return true;
        }
      } catch {
        // Let piStart surface the concrete native failure if this lightweight
        // readiness probe is temporarily unavailable.
        return true;
      }

      pendingRuntimeSendRef.current = {
        userMessage,
        displayLabel,
        imageDataUrls:
  ...
  useEffect(() => {
    ...
    restartCurrentPiSession(config)
      ...
      .catch(() => {
        console.error("Assistant connection context could not be refreshed.");
        toast({
          title: "Connection changes are waiting",
          description:
            "Civitas couldn’t refresh the assistant yet. Finish the current chat or restart Civitas, then try again.",
          variant: "destructive",
        });
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    (preset: AIPreset) => {
      ...
      if (!spawnTimeFieldsChanged && (providerChanged || modelChanged)) {
        enqueuePresetSwitch(async () => {
          try {
            ...
            console.error(
              "Assistant model switch failed; restarting the session.",
            );
            try {
              await restartCurrentPiSession(providerConfig);
            } catch (err) {
              console.error("Assistant session fallback restart failed.");
            }
          }
        });
        return;
      ...
      enqueuePresetSwitch(async () => {
        try {
          ...
          console.error("Assistant preset switch failed.");
        }
      });
      // eslint-disable-next-line react-hooks/exhaustive-deps
    },
  ...
  useEffect(() => {
    ...
    const handlePiEventData = (data: any) => {
      ...
      const handleRetryDispatchFailure = (
        error: unknown,
        retrySession: string,
        ...
        if (
          ...
          piMessageIdRef.current !== retryTurnId
        ) {
          return;
        }

        console.error("Assistant retry could not be sent.");
        const userMessage =
          "Civitas couldn’t retry this response. Check your connection, then try again.";
        if (retryTurnId) {
          setMessages((prev) =>
            prev.map((message) =>
              message.id === retryTurnId
                ? {
                    ...message,
                    content: userMessage,
                    retryPrompt,
        ...
        if (!retryTurnId || piMessageIdRef.current === retryTurnId) {
          piStreamingTextRef.current = "";
          piMessageIdRef.current = null;
          piContentBlocksRef.current = [];
          setIsLoading(false);
        ...
        emitSessionActivity({
          status: "error",
          lastError:
            error instanceof Error ? error.message : "assistant retry failed",
        });
        toast({
          title: "Retry couldn’t be sent",
          description:
            "Your message is still in this chat. Check your connection and try again.",
          variant: "destructive",
        });
      };
      ...
      if (
        ...
        console.error("Assistant automatic retry failed.");
        ...
        if (quotaErrorType === "daily" || quotaErrorType === "rate") {
          ...
          if (piMessageIdRef.current) {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
                  ? { ...m, content: AI_MODEL_UNAVAILABLE_MESSAGE }
                  : m,
              ),
            );
          }
          ...
          /\bunauthorized\b/i.test(errorStr)
        ) {
          if (piMessageIdRef.current) {
            const msgId = piMessageIdRef.current;
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
                  ? { ...m, content: AI_PROVIDER_AUTH_MESSAGE }
                  : m,
              ),
            );
          }
        ...
        console.error("Assistant message processing failed.");
        emitSessionActivity({
          status: "error",
        ...
        if (piMessageIdRef.current) {
          ...
          if (quotaErrorType === "daily" || quotaErrorType === "rate") {
            ...
            console.warn("Assistant is busy; waiting for the active request.");
          } else if (
            fullError.includes("authentication_error") ||
            fullError.includes("not authenticated") ||
            /\b401\b/.test(fullError) ||
            /\bunauthorized\b/i.test(fullError)
          ) {
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
        ...
        if (
          ...
          const queuedDisplay =
            pendingDisplay ??
            consumeQueuedDisplayForStartedMessage(sidForStartedUser, text);
          const matchedTurnIntent =
            preMatchedTurnIntent ??
            findTurnIntentForUserStart(sidForStartedUser, text, queuedDisplay);
        ...
        const errMsg =
          data.message.errorMessage || data.message.error || "Unknown error";
        console.error("Language model request failed.");
          ...
          setMessages((prev) => {
            ...
            if (!content && hasNonTextBlocks) {
              ...
              if (lastErr && lastErrKind === "daily") {
                ...
                // The local gateway reports provider setup failures separately.
                content =
          ...
          if (!isPipeWatch) {
            ...
            if (followUpText.length > 500 && !followUpFiredRef.current) {
              ...
              setTimeout(() => {
                ...
                if (
                  piMessageIdRef.current &&
                  piMessageIdRef.current !== followUpTurnId
                )
                  return;
        ...
        if (!isPipeWatch) {
          ...
          if (
            pendingSteerBatchRef.current.some(
              (item) => item.sessionId === piSessionIdRef.current,
            )
          ) {
        ...
        if (
          ...
          console.warn("Assistant first request failed; retrying once.");
          if (piMessageIdRef.current && !piFirstCallRetried.current) {
            ...
            if (lastUserMsg?.content) {
              ...
              void commands
                .piPrompt(retrySession, lastUserMsg.content, null, null)
                .catch((error) => {
                  handleRetryDispatchFailure(
                    error,
                    retrySession,
                    retryTurnId,
                    lastUserMsg.content,
        ...
        if (
          ...
          console.warn("Assistant rate limited; retrying.");
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === retryTurnId
                  ? {
                      ...m,
                      content: `Rate limited — retrying in ${waitSecs}s… (attempt ${attempt}/${PI_MAX_RATE_LIMIT_RETRIES})`,
                      contentBlocks: [],
                    }
                  : m,
              ),
        ...
        if (piMessageIdRef.current) {
          ...
          if (quotaErrorType === "daily" || quotaErrorType === "rate") {
            ...
            console.warn(
              "Assistant request was already processing; waiting for completion.",
            );
            setMessages((prev) =>
              prev.map((m) =>
                m.id === msgId
      ...
      busUnregistrations.push(
        onAgentTerminated(async (payload) => {
          ...
          if (piCrashCountRef.current > MAX_CRASHES) {
            console.error(
              "Assistant crash loop detected; automatic restart stopped.",
            );
            setPiInfo(null);
            return;
          }
            ...
            if (!piStartInFlightRef.current) {
              try {
                ...
                if (result.status === "ok") {
                  ...
                  console.error("Assistant automatic restart failed.");
                  setPiInfo(null);
                }
              } catch (e) {
                console.error(
                  "Assistant automatic restart encountered an error.",
                );
                setPiInfo(null);
              }
            }
    ...
    (async () => {
      ...
      try {
        ...
        /* ignore — queue may not be initialized yet */
      }
    })();
  ...
  useEffect(() => {
    ...
    const pollExecutionStatus = async (
      ...
      try {
        ...
        if (exec.status !== "running") {
          const { parsePipeNdjsonToMessages } =
            await import("@/lib/pipe-ndjson-to-chat");
    ...
    if (stored) {
      ...
      try {
        ...
        // The payload is removed before parsing, so a malformed optional
        // workflow deep link cannot strand the chat or retry forever.
      }
    }

    // Also listen for live events (in case chat is already mounted)
    let unlisten: (() => void) | null = null;
    listen<{ pipeName: string; executionId: number; presetId?: string | null }>(
      "watch_pipe",
      (event) => {
    ...
    try {
      ...
      if (
        ...
        setFollowUpSuggestions(
          questions.filter((q: unknown) => typeof q === "string").slice(0, 2),
        );
      }
    } catch {
  ...
  async function enqueuePiMessage(
    ...
    try {
      ...
      setInput(prevInput);
      if (hadPastedImages) setPastedImages(queuedImageDataUrls);
      console.warn("Assistant follow-up could not be queued.");
    }
  }
  ...
  async function interruptActivePiTurn() {
    ...
    try {
      ...
      if (abortResult.status === "ok") {
        ...
        console.warn("Assistant could not stop before applying guidance.");
      }
    } catch (e) {
      console.warn("Assistant stop request failed before applying guidance.");
    }
  ...
  async function sendPiMessage(
      ...
      if (piStartInFlightRef.current) {
        ...
        try {
          ...
          if (result.status === "ok" && result.data.running) {
            ...
            if (isAssistantRuntimeMissingError(description)) {
              runtimeInstalledRef.current = false;
              pendingRuntimeSendRef.current = {
                userMessage,
                displayLabel,
                imageDataUrls,
            ...
            toast({
              title: "The assistant couldn’t start",
              description,
              variant: "destructive",
            });
            return;
          ...
          if (isAssistantRuntimeMissingError(e)) {
            runtimeInstalledRef.current = false;
            pendingRuntimeSendRef.current = {
              userMessage,
              displayLabel,
    ...
    if (sidNow) {
      ...
      // `draft` means "no user message ever sent". Clear it synchronously at
      // dispatch — waiting for the first auto-save leaves a window where the
      // new-chat sweep drops a session whose first turn is still streaming.
    ...
    try {
      ...
      if (prefillFrameId) {
        try {
          ...
          console.error(
            "Timeline frame could not be attached to the assistant.",
          );
        }
        setPrefillFrameId(null);
      }
      ...
      if (
        ...
        try {
          ...
          console.error("Assistant session could not be started for retry.");
        }
      }
      ...
      if (result.status === "error") {
        ...
        if (rawError.includes("already processing")) {
          ...
          // The local gateway restarts independently of the selected provider.
  ...
  async function executeSearchTool(
    ...
    try {
      ...
      if (args.window_name)
        params.append("window_name", String(args.window_name));
      if (args.speaker_name)
        params.append("speaker_name", String(args.speaker_name));
      ...
      const formatted = searchResults
        .map((result: SearchResult) => {
          ...
          } else if (result.type === "ProjectFile") {
            return `[ProjectFile] ${content.root_display_name || "project"} | ${content.safe_display_path || content.relative_path || "file"}\nkind: ${content.kind || "file"}\n${truncateText(content.text)}`;
          }
          return null;
        })
        .filter(Boolean);
  ...
  async function sendMessage(
    ...
    if (
      !piInfo?.running &&
      !(await ensureAssistantRuntime(userMessage, displayLabel, imageDataUrls))
    ) {
      return;
    }
    ...
    if (queuedDocs.length > 0) {
      ...
      const cleanLabel =
        trimmed || `📎 ${queuedDocs.map((d) => d.name).join(", ")}`;
    ...
    try {
      const taskHint =
        snapshotDocs.length > 0 && trimmed.length > 0 && trimmed.length <= 2_000
          ? trimmed
          : undefined;
      ...
      if (largeContext) {
        ...
        toast({
          ...
          description:
            "The assistant will use local chunk files instead of sending the full text inline.",
        });
      }
    } catch (e) {
      toast({
  ...
  async function queueFollowUpMessage(
    userMessage: string,
    displayLabel?: string,
  ) {
    if (
    ...
    if (hadActiveReply) {
      ...
      if (sid) {
        ...
        void commands
          ...
          .then((result) => {
            if (result.status !== "ok") {
              console.warn("Assistant guidance request was rejected.");
          ...
          .catch((err: unknown) => {
            console.warn(
              "Assistant guidance failed; reverting the pending change.",
            );
            revertFailedComposerSteer(
              batch,
              latest,
  ...
  function revertFailedComposerSteer(
    ...
    setMessages((prev) =>
      prev.filter(
        (m) =>
          !(m.id === optimisticId && m.role === "user" && m.intent === "steer"),
      ),
    );
  ...
  async function steerQueuedPrompt(prompt: PiQueuedPrompt) {
    ...
    try {
      ...
      const result = await commands.piSteerQueued(
        piSessionIdRef.current,
        prompt.id,
      );
      if (result.status !== "ok") {
    ...
    if (m.contentBlocks && m.contentBlocks.length > 0) {
      ...
      for (const block of m.contentBlocks) {
        if (block.type === "text" && block.text) {
          ...
          sections.push(
            `<details><summary>Thinking${block.durationMs ? ` (${(block.durationMs / 1000).toFixed(1)}s)` : ""}</summary>\n\n${block.text}\n\n</details>`,
          );
        }
      }
  ...
  const exportChatAsMarkdownFile = async () => {
    ...
    try {
      const filePath = await saveDialog({
        ...
        defaultPath: `civitas-chat-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}.md`,
      });
      if (filePath) {
      ...
      console.error("Chat could not be exported.");
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      console.warn("Assistant session could not be stopped.");
    }
    piStreamingTextRef.current = "";
    piMessageIdRef.current = null;
    piContentBlocksRef.current = [];
  ...
  const renderComposerUtilityMenu = () => {
    ...
    return (
      <>
        ...
        <div className="px-2 py-1 text-2xs font-medium text-muted-foreground/80 bg-muted/30 border-b border-border/40 flex items-center gap-1.5">
        ...
        <div className="px-2 py-1 text-2xs font-medium text-muted-foreground/80 bg-muted/30 border-b border-border/40">
          time
        </div>
        {STATIC_MENTION_SUGGESTIONS.filter((s) => s.category === "time").map(
          (s) => {
            const isActive = activeFilters.timeRanges.some(
              (r) => r.label === timeLabels[s.description],
            );
            return (
              <button
                key={s.tag}
        ...
        <div className="px-2 py-1 text-2xs font-medium text-muted-foreground/80 bg-muted/30 border-b border-border/40 border-t">
          content type
        </div>
        {STATIC_MENTION_SUGGESTIONS.filter((s) => s.category === "content").map(
          (s) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative flex items-center gap-3 px-4 py-3 border-b border-border/40 bg-background",
      ...
      >
        ...
        {/* New-chat affordance + shortcut chip. Shown in the floating
            ...
            spawns a fresh session, so a duplicate top-right button adds
        ...
        {!hideInlineHistory && (
          <>
            <Button
              ...
              className="relative z-10 h-7 px-3 gap-1.5 text-xs normal-case tracking-normal bg-transparent text-muted-foreground border border-border/60 hover:text-foreground hover:bg-muted/40 hover:border-foreground/30 transition-ui duration-150"
              title="New Chat"
            >
              <Plus size={14} />
              <span>New Chat</span>
            ...
            settings.showChatShortcut ? (
              <Kbd
                ...
                className="hidden sm:inline-flex font-mono text-2xs text-muted-foreground/80"
              >
      ...
      {/* The chat column remains a flex child so the same surface behaves well
          in the floating and full-window layouts. */}
      <div className="flex-1 flex min-h-0" data-browser-panel-host>
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="flex flex-col gap-3 p-2">
                      {groupedConversations.length === 0 ? (
                        ...
                        groupedConversations.map((group) => (
                          ...
                          >
                            <p className="px-2 py-1 text-2xs font-medium uppercase tracking-wider text-muted-foreground">
                              {group.label}
                            </p>
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    <Button
                                      ...
                                      className="h-6 w-6 p-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-foreground"
                                    >
            ...
            <ContextMenu>
              <ContextMenuTrigger asChild>
                <div
                  ...
                  // min-w-0 lets this flex child shrink inside narrow windows.
            ...
            "Watching pipe: X" user-bubble sentinel. */}
                    ...
                    {messages.length === 0 &&
                      ...
                      !isStreaming &&
                      !disabledReason && (
                        // Centered in the conversation viewport (the scroll container is
                        // `relative`), so the empty state sits in the middle instead of pinned
                        // to the top leaving a large blank above the composer.
                        <div className="absolute inset-0 flex flex-col items-center justify-center space-y-3 px-6 text-center">
                          ...
                          <div className="space-y-1">
                            <p className="text-sm font-medium text-foreground">
                              ask civitas about your work
                            </p>
                            <p className="text-xs text-muted-foreground max-w-xs">
                              it knows what you&rsquo;ve seen and decided —
                      ...
                      {(() => {
                        const visibleMessages = messages.filter((m) => {
                          ...
                          if (
                            !m.content &&
                            !m.contentBlocks?.length &&
                            !isSteeredAssistantMessage(m)
                          )
                            return false;
                          return true;
                        });

                        const renderItems = buildCollapsedSteerRenderItems(
                          visibleMessages,
                          {
                            canCollapseSteerWork:
                              !isLoading &&
                              !isStreaming &&
                              !piMessageIdRef.current,
                          },
                        ...
                        return renderItems.map((item) => {
                          ...
                          const shouldSuppressIntentLabel =
                            item.hideIntentLabelWhenCollapsedBy &&
                            !expandedSteerWorkIds.has(
                              item.hideIntentLabelWhenCollapsedBy,
                            );
                          const intentLabel = shouldSuppressIntentLabel
                            ? null
                            : getMessageIntentLabel(message);
                          const isSteerUserMessage =
                            message.role === "user" &&
                            message.intent === "steer";
                          ...
                          const canShowMessageActions =
                            !item.showActionsWhenExpandedBy ||
                            expandedSteerWorkIds.has(
                              item.showActionsWhenExpandedBy,
                            );
                          ...
                          const hideSupersededSteerBody =
                            isSteeredAssistantMessage(message) &&
                            Boolean(
                              nextAssistant &&
                          ...
                          const turnAggregatedCitations =
                            citationPlan.aggregatedAfter.get(message.id);
                          return [
                            <motion.div
                              key={message.id}
                              initial={{ opacity: 0, y: 10 }}
                            ...
                            >
                              ...
                              >
                                {intentLabel ? (
                                  <div
                                    className={cn(
                                      "mb-1 px-1 text-xs font-medium text-muted-foreground/70",
                                      message.role === "user"
                                        ? "text-right"
                                        : "text-left",
                                    )}
                                ...
                                {hideSupersededSteerBody ? null : (
                                  <div
                                    ...
                                    className={cn(
                                      "relative rounded-xl text-sm overflow-hidden max-w-full transition-ui",
                                      message.role === "user"
                                  ...
                                  >
                                    {editingMessageId === message.id ? (
                                      ...
                                      <MessageContent
                                        message={message}
                                        deferSourceFooter={
                                          citationPlan.deferredMessageIds.has(
                                            message.id,
                                          ) ||
                                          message.id ===
                                ...
                                shouldShowMessageActionBar ? (
                                  <>
                                    {/* Action buttons - appear on hover, outside the message box */}
                                    {editingMessageId !== message.id && (
                                      <div className="flex items-center gap-0.5 self-end mt-1 opacity-0 group-hover/message:opacity-100 group-focus-within/message:opacity-100 transition-ui duration-200">
                                        <button
                                          onClick={async () => {
                            ...
                            turnAggregatedCitations &&
                            turnAggregatedCitations.length > 0 ? (
                              <motion.div
              ...
              <ContextMenuContent className="w-56">
                <ContextMenuItem
                  ...
                  onSelect={() => void copyFullChatAsMarkdown()}
                >
                ...
                <ContextMenuItem
                  disabled={messages.length === 0}
                  onSelect={() => void exportChatAsMarkdownFile()}
                >
          ...
          {/* End of main content area with history sidebar */}
          {/* Input */}
          <div ref={inputSectionRef} className="relative bg-background">
                    ...
                    {prefillContext && (
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-foreground font-mono line-clamp-2">
                          {prefillContext.slice(0, 150)}
                          {prefillContext.length > 150 ? "..." : ""}
                        </p>
                      </div>
                    )}
                    <button
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {followUpSuggestions.map((q, i) => (
                          <Button
                            ...
                            className="h-auto rounded-md border-border/50 bg-transparent px-3 py-1 text-xs font-normal text-muted-foreground hover:border-foreground/30 hover:bg-muted/40 hover:text-foreground"
                          >
                            {q}
                          </Button>
                        ))}
            ...
            docs, then images). */}
                  ...
                  <div className="px-5 sm:px-6 py-2 border-b border-border/30 flex flex-wrap items-center gap-2">
                    {pendingDocs.map((doc) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`relative shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-2xs font-semibold tracking-tight ${badge.tint}`}
                          >
                    ...
                    {attachedDocs.map((doc, i) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-2xs font-semibold tracking-tight ${badge.tint}`}
                          >
                            {badge.label}
                          </div>
                          <div className="min-w-0 flex-1">
                            ...
                            {isPastedText ? (
                              canShowInField ? (
                                <button
                                  ...
                                  className="inline-flex max-w-full items-center gap-0.5 truncate text-2xs text-muted-foreground underline decoration-dotted underline-offset-2 hover:text-foreground"
                                >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <span className="inline-flex max-w-full cursor-not-allowed items-center gap-0.5 truncate text-2xs text-muted-foreground/70 underline decoration-dotted underline-offset-2">
                                      <span className="truncate">
                                        Show in text field
                                      </span>
                                      <ChevronRight className="h-3 w-3 shrink-0" />
                    ...
                    {pastedImages.map((img, i) => (
                      <div key={`img-${i}`} className="relative group shrink-0">
                        <button
                          type="button"
                          onClick={() =>
                            setImageViewer({ images: pastedImages, index: i })
                          }
              ...
              >
                ...
                {queuedPrompts.length > 0 && (
                  ...
                  >
                    ...
                    <TooltipProvider delayDuration={150}>
                      <div
                        ref={queuedScrollRef}
                        className="max-h-[112px] overflow-y-auto scrollbar-minimal"
                      >
                        {queuedPrompts.map((p, i) => {
                          ...
                          return (
                            ...
                            >
                              <span className="w-4 shrink-0 text-right font-mono text-2xs text-muted-foreground/50">
                                {i + 1}
                              </span>
                              <span className="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap font-mono text-sm">
                                {label}
                              </span>
                ...
                <InputGroup
                  ...
                  className={cn(
                    "flex flex-col items-stretch rounded-xl border bg-card shadow-[0_1px_2px_hsl(var(--foreground)/0.04)] ring-offset-background transition-ui focus-within:border-border/75 focus-within:shadow-[0_8px_24px_hsl(var(--foreground)/0.05)] has-[[data-slot=input-group-control]:focus-visible]:ring-0",
                    "border-border/60",
                ...
                >
                  ...
                  after the name. X (absolute, top-right) clears it. */}
                      ...
                      prefix never bleeds above the first line when scrolled. */}
                        <div className="pointer-events-none absolute left-3 right-7 top-2.5 bottom-2.5 z-10 overflow-hidden">
                          <div
                            ...
                            style={{
                              transform: `translateY(${-chipScrollTop}px)`,
                            }}
                          >
                            <IntegrationIcon
                    ...
                    <InputGroupTextarea
                      ...
                      onScroll={
                        connectionChip
                          ? (e) => setChipScrollTop(e.currentTarget.scrollTop)
                          : undefined
                      }
                      ...
                      rows={1}
                      className={cn(
                        "w-full min-h-[44px] border-0 bg-transparent px-3 text-left text-sm font-mono placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 caret-foreground resize-none overflow-y-auto scrollbar-minimal py-2.5",
                        connectionChip ? "pr-7" : "pr-3",
                      )}
                      style={{
                        maxHeight: "150px",
                        textIndent:
                          connectionChip && chipPrefixWidth
                            ? `${chipPrefixWidth + 8}px`
                            : undefined,
                      }}
                    />
                    ...
                    <AnimatePresence>
                      {showMentionDropdown && filteredMentions.length > 0 && (
                        ...
                        >
                          {["time", "content", "app", "speaker"].map(
                            (category) => {
                              const items = filteredMentions.filter(
                              ...
                              return (
                                <div key={category}>
                                  ...
                                  {items.map((suggestion) => {
                                    const globalIndex =
                                      filteredMentions.indexOf(suggestion);
                                    return (
                                      <button
                                        key={suggestion.tag}
                                        type="button"
                                        onClick={() =>
                                          insertMention(suggestion.tag)
                                        }
                                        className={cn(
                          ...
                          {isLoadingSpeakers && (
                            <div className="px-3 py-2 text-2xs text-muted-foreground flex items-center gap-2">
                              <Loader2 className="h-3 w-3 animate-spin" />
                  ...
                  <InputGroupAddon
                    align="block-end"
                    className="-mt-px flex shrink-0 items-center gap-1.5 rounded-b-xl border-t-0 bg-card px-2 pb-2 pt-1 shadow-none"
                  >
                    <Popover
                      open={appFilterOpen}
                    ...
                    >
                      <PopoverTrigger asChild>
                        <InputGroupButton
                          ...
                          variant="ghost"
                          className={cn(
                            "relative shrink-0 text-muted-foreground hover:bg-muted/50 hover:text-foreground",
                            hasActiveFilters && "bg-muted/50 text-foreground",
                          )}
                        ...
                        >
                          ...
                          {activeFilterCount > 0 && (
                            <span className="absolute -right-1 -top-1 flex h-[15px] min-w-[15px] items-center justify-center rounded-full bg-foreground px-1 text-2xs font-semibold text-background">
                              {activeFilterCount}
                            </span>
                          )}
                        </InputGroupButton>
                        ...
                        <div className="hidden sm:flex min-w-0 items-center gap-1 overflow-hidden">
                          {activeFilterLabels.slice(0, 2).map((label, idx) => (
                            <Badge
                              key={`${label}-${idx}`}
                              variant="outline"
                              className="h-6 max-w-[140px] truncate rounded-md border-border/50 px-2 text-2xs font-medium text-muted-foreground"
                              title={label}
                            >
                              {label}
                            </Badge>
                          ...
                          {activeFilterLabels.length > 2 && (
                            <Badge
                              variant="outline"
                              className="h-6 shrink-0 rounded-md border-border/50 px-2 text-2xs font-medium text-muted-foreground"
                            >
                              +{activeFilterLabels.length - 2}
                            </Badge>
                          )}
                    ...
                    <AIPresetsSelector
                      ...
                      onControlledSelect={(id, updatedPreset) => {
                        if (!id) return;
                        // Prefer the freshly-updated preset the picker just wrote — the
                        // store write is async, so re-reading settings.aiPresets here
                        // would return the OLD model and the switch would be a no-op.
                        const match =
                          updatedPreset ??
                          settings.aiPresets?.find((p) => p.id === id);
                        if (!match) return;
                    ...
                    {(() => {
                      ...
                      return (
                        <InputGroupButton
                          ...
                          className={cn(
                            "relative shrink-0 rounded-md bg-foreground text-background transition-ui duration-150",
                ...
                {showConnectBanner && (
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      ...
                      className="text-xs text-muted-foreground/70 hover:text-foreground transition-colors flex-1 text-left"
                    >
                    ...
                    <div className="flex items-center gap-1">
                      {connections
                        .filter((c) =>
                          INTEGRATION_ICON_KEYS.has(c.icon || c.id),
                        )
                        .slice(0, 8)
                        .map((c) => (
                          <button
            ...
            {/* End of max-w-4xl input wrapper */}
          </div>
        </div>{" "}
        {/* End of chat column */}
      </div>{" "}
      ...
      <AssistantRuntimeDialog
        ...
        onOpenChange={(open) => {
          setRuntimeSetupOpen(open);
          if (!open) pendingRuntimeSendRef.current = null;
        }}
        onInstalled={() => {
          runtimeInstalledRef.current = true;
          const pending = pendingRuntimeSendRef.current;
          pendingRuntimeSendRef.current = null;
      ...
      <Dialog
        open={!!imageViewer}
        onOpenChange={(open) => !open && setImageViewer(null)}
      >
        <DialogContent
          hideCloseButton
            ...
            <>
              <div className="flex items-center justify-between px-4 py-3 border-b border-border/50 shrink-0">
                <span className="text-sm font-medium text-muted-foreground">
                  {imageViewer.index + 1}/{imageViewer.images.length} Attached
                  image {imageViewer.index + 1}
                </span>
                <button
                  type="button"
