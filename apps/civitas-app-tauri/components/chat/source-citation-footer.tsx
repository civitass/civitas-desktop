...
function SourceCitationRow({ citation }: { citation: SourceCitation }) {
  ...
  const inner = (
    <>
      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded bg-surface/80 text-muted-foreground">
      ...
      <span className="min-w-0 flex-1">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          <span className="shrink-0 text-2xs font-medium capitalize leading-none text-muted-foreground/60">
            {kindLabel}
          </span>
  ...
  if (!interactive) {
    return (
      <div className="flex min-w-0 items-start gap-2 rounded-md bg-muted/25 px-2.5 py-1.5">
        {inner}
      </div>
    );
  }
  ...
  return (
    <button
      ...
      onClick={() => {
        ...
        void openExternalUrl(citation.href!).catch((error) => {
          console.warn("External citation link was rejected.");
        });
      }}
      className="flex min-w-0 items-start gap-2 rounded-md bg-muted/25 px-2.5 py-1.5 text-left transition-colors hover:bg-muted/45"
    >
      {inner}
    </button>
  );
...
function sourceIconSrc(citation: SourceCitation): string | undefined {
  if (citation.kind === "civitas") {
    return "/images/civitas.png";
  }
