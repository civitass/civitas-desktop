 ...
 *
 * These tests kill and restart civitas, so they run LAST.
 */

import {
  suite, test, summary, screenshot, fetchJson, sleep,
  HEALTH_URL, IS_WINDOWS, IS_MACOS, isCivitasRunning,
} from "./lib";
...
/** Get list of civitas-related PIDs */
function getCivitasPids(): number[] {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync([
      "powershell", "-NoProfile", "-Command",
      "Get-Process -Name 'civitas*' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Id",
    ]);
    return proc.stdout.toString().trim().split(/\r?\n/).filter(Boolean).map(Number).filter(n => !isNaN(n));
  } else {
    const proc = Bun.spawnSync(["pgrep", "-f", "civitas"]);
...
/** Find civitas executable path */
function findCivitasExe(): string | null {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command",
      "(Get-Process -Name 'civitas*' -ErrorAction SilentlyContinue | Select-Object -First 1).Path"]);
    const path = proc.stdout.toString().trim();
    return path || null;
  } else {
    const proc = Bun.spawnSync(["bash", "-c", "ps aux | grep -i civitas | grep -v grep | head -1 | awk '{print $11}'"]);
    const path = proc.stdout.toString().trim();
    return path || null;
  }
}

/** Start civitas app (best effort) */
function startCivitas(exePath: string) {
  if (IS_WINDOWS) {
...
const exePath = findCivitasExe();
console.log(`  civitas exe: ${exePath ?? "not found"}`);
...
await test("CPU not spiking on idle (10s sample)", async () => {
  if (IS_WINDOWS) {
    // Measure civitas CPU over 10s
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
      $samples = @()
      for ($i = 0; $i -lt 5; $i++) {
        $cpu = (Get-Process -Name 'civitas*' -ErrorAction SilentlyContinue | Measure-Object -Property CPU -Sum).Sum
        $samples += $cpu
    ...
    const proc = Bun.spawnSync(["bash", "-c",
      "ps aux | grep -i civitas | grep -v grep | awk '{sum+=$3} END {print sum}'"]);
...
await test("force quit + recovery (DB intact)", async () => {
  if (!exePath) {
    console.log("  (skipped: could not find civitas exe path)");
    return;
  }
  ...
  // Force kill civitas
  console.log("  force killing civitas...");
  if (IS_WINDOWS) {
    Bun.spawnSync(["powershell", "-NoProfile", "-Command",
      "Get-Process -Name 'civitas*' -ErrorAction SilentlyContinue | Stop-Process -Force"]);
  } else {
    Bun.spawnSync(["pkill", "-9", "-f", "civitas-app"]);
  }
  await sleep(3000);

  // Verify it's dead
  if (isCivitasRunning()) {
    throw new Error("civitas still running after force kill");
  }

  // Restart
  console.log("  restarting civitas...");
  startCivitas(exePath);
...
await test("disk space check (S9.6)", async () => {
  ...
  if (!isCivitasRunning()) throw new Error("app not running");
});
...
await test("clean quit leaves no orphans (S8.1)", async () => {
  ...
  const pidsBefore = getCivitasPids();
  if (pidsBefore.length === 0) {
    console.log("  (skipped: civitas not running)");
    return;
  }
  ...
  if (IS_WINDOWS) {
    ...
    Bun.spawnSync(["pkill", "-TERM", "-f", "civitas-app"]);
  }
  await sleep(10_000);

  // Check all processes exited
  const pidsAfter = getCivitasPids();
  ...
  const recovered = await waitForHealthUp(60);
  if (!recovered) {
    throw new Error("could not restart civitas after clean quit test");
  }
});
...
await test("check logs for slow DB warnings (S9.1)", async () => {
  // Look for civitas logs and check for slow insert warnings
  let logContent = "";
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
      $logDir = Join-Path $env:LOCALAPPDATA 'civitas'
    ...
    const proc = Bun.spawnSync(["bash", "-c",
      "tail -500 ~/Library/Logs/civitas/*.log 2>/dev/null || echo NO_LOG"]);
    logContent = proc.stdout.toString();
  }
