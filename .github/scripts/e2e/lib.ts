...
export const APP_NAME = process.env.CIVITAS_APP_NAME ?? "civitas";
...
const defaultArtifacts = IS_WINDOWS
  ? join(tmpdir(), "civitas-e2e", new Date().toISOString().replace(/[:.]/g, "-"))
  : join("/tmp", "civitas-e2e", new Date().toISOString().replace(/[:.]/g, "-"));
...
/** Check if civitas process is running (cross-platform) */
export function isCivitasRunning(): boolean {
  if (IS_WINDOWS) {
    const proc = Bun.spawnSync(["tasklist", "/FI", "IMAGENAME eq civitas*", "/NH"]);
    const out = proc.stdout.toString();
    return out.includes("civitas");
  } else {
