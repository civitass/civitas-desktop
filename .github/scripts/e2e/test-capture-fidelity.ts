...
const groundTruth: GroundTruth = {
  ...
  markers: {
    ...
    verify: `civitas-e2e-verify-${runId}`,
  },
  expected: {
    textEvents: 0,
    appSwitches: 0,
...
await test("CPU usage < 150%", async () => {
  const proc = Bun.spawnSync([
    "bash", "-c",
    "ps aux | grep civitas | grep -v grep | awk '{sum += $3} END {print sum}'",
  ]);
  const cpu = parseFloat(proc.stdout.toString().trim()) || 0;
  if (cpu > 150) {
    throw new Error(`CPU too high: ${cpu.toFixed(1)}%`);
  }
  console.log(`  civitas CPU: ${cpu.toFixed(1)}%`);
});
...
await test("memory < 2GB", async () => {
  const proc = Bun.spawnSync([
    "bash", "-c",
    "ps aux | grep civitas | grep -v grep | awk '{sum += $6} END {print sum}'",
  ]);
  ...
  console.log(`  civitas memory: ${rssMB.toFixed(0)} MB`);
});
