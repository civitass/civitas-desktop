...
import {
  ...
  sel, HEALTH_URL, IS_WINDOWS, IS_MACOS, isCivitasRunning,
} from "./lib";
...
if (IS_MACOS) {
  ...
  await test("tray icon visible", async () => {
    const result = await bb("find", "role:AXMenuBarItem AND name~:status", "--app", "civitas-app");
  ...
  await test("tray menu opens", async () => {
    ...
    try {
      await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "civitas-app");
      await sleep(1000);
      // Verify menu items appeared
      const items = await bb("find", "role:AXMenuItem", "--app", "civitas-app");
  ...
  await test("tray has all expected items", async () => {
    ...
    const expectedItems = ["show civitas", "settings", "quit civitas"];
  ...
  await test("tray 'show civitas' opens window", async () => {
    // Open tray menu
    await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "civitas-app");
    await sleep(500);
    ...
    // Click "show civitas"
    try {
      await bb("click", "role:AXMenuItem AND title~:show civitas", "--app", "civitas-app");
      await sleep(2000);

      // Verify window appeared
      const result = await bb("find", "role:AXWebArea", "--app", "civitas-app");
      const windows = result?.data ?? [];
      if (windows.length === 0) {
        console.log("  warning: window may not have appeared");
      }
    } catch (e: any) {
      console.log(`  (show civitas interaction failed: ${e.message})`);
    }
  });
  ...
  await test("tray menu does not fire twice (S2.6)", async () => {
    await bb("click", "role:AXMenuBarItem AND name~:status", "--app", "civitas-app");
    await sleep(500);
    const result = await bb("find", "role:AXMenuItem AND title:settings", "--app", "civitas-app");
  ...
  await test("dock icon visible (S2.1)", async () => {
    ...
    // Civitas should appear in visible processes (dock icon = not accessory mode)
    if (!apps.includes("civitas")) {
      console.log("  warning: civitas not in visible processes (may be in accessory mode)");
    } else {
...
if (IS_WINDOWS) {
  ...
  await test("civitas process running", async () => {
    if (!isCivitasRunning()) throw new Error("civitas not running");
  });

  await test("system tray area accessible", async () => {
    // Windows system tray is tricky — items may be in overflow
    // Try to find civitas in notification area
    try {
      const result = await bb("find", "name~:screenpi");
      const items = result?.data ?? [];
      if (items.length > 0) {
        console.log(`  found ${items.length} civitas element(s) in tray area`);
        return;
      }
    } catch {}
  ...
  await test("civitas window focusable via shortcut", async () => {
  ...
  await test("multiple shortcut toggles don't crash (S2.4)", async () => {
    ...
    if (!isCivitasRunning()) {
      throw new Error("civitas crashed after rapid shortcut toggles");
    }
