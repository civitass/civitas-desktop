import {
  ...
  sel, IS_WINDOWS, IS_MACOS, isCivitasRunning,
} from "./lib";

suite("app launch & health");

await test("app process running", async () => {
  if (!isCivitasRunning()) throw new Error("civitas process not found");
});
...
if (IS_MACOS) {
  ...
  await test("tray quit item", () => assertExists("role:AXMenuItem AND title:quit civitas"));
  await test("tray changelog item", () => assertExists("role:AXMenuItem AND title:changelog"));
  await test("tray show civitas", () => assertExists("title~:show civitas"));
...
if (IS_WINDOWS) {
  // Windows: verify the civitas-app process has a window handle
  // Tauri windows on Windows appear as Pane elements with class "civitas.team-sic"
  await test("tray pane exists", async () => {
    const result = await bb("find", "name~:screenpi");
    const elements = result?.data ?? [];
    if (elements.length === 0) {
      throw new Error("civitas pane/tray element not found in UIA tree");
    }
  });
}
