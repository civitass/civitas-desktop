...
await test("not blocked by Windows Defender", async () => {
  // Check if civitas is in Defender exclusions or if there are recent blocks
  const proc = Bun.spawnSync(["powershell", "-NoProfile", "-Command", `
    $threats = Get-MpThreatDetection -ErrorAction SilentlyContinue |
      Where-Object { $_.ProcessName -like '*civitas*' } |
  ...
  if (result.startsWith("BLOCKED")) {
    throw new Error(`Windows Defender blocked civitas: ${result}`);
  }
...
await test("no orphaned civitas child processes", async () => {
  // Get civitas parent PID
  const parentProc = Bun.spawnSync(["powershell", "-NoProfile", "-Command",
    "(Get-Process -Name 'civitas*' -ErrorAction SilentlyContinue | Select-Object -First 1).Id"]);
  const parentPid = parseInt(parentProc.stdout.toString().trim());

  if (!parentPid) {
    console.log("  (skipped: no civitas process found)");
    return;
  }
  ...
  console.log(`  civitas PID ${parentPid}, child processes: ${childCount}`);
