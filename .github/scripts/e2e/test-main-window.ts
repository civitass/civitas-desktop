...
if (IS_MACOS) {
  ...
  await test("civitas menu exists", () => assertExists("role:AXMenuBarItem AND title:civitas"));
  await test("edit menu exists", () => assertExists("role:AXMenuBarItem AND title:Edit"));
  await test("about menu item", () => assertExists("role:AXMenuItem AND title:About civitas"));
...
if (IS_WINDOWS) {
  ...
  await test("civitas process has window", async () => {
    const result = await bb("find", "name~:screenpi");
    const elements = result?.data ?? [];
    if (elements.length === 0) throw new Error("no civitas elements found");
  });
