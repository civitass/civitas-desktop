...
if (IS_MACOS) {
  await test("trigger onboarding", async () => {
    await click("role:AXMenuBarItem AND title:civitas");
    await sleep(500);
    try {
  ...
  await test("onboarding has skip/content", async () => {
    ...
    const keywords = ["skip", "next", "continue", "get started", "welcome", "civitas", "setup"];
...
if (IS_WINDOWS) {
  ...
  await test("civitas responsive", async () => {
    const result = await bb("find", "name~:screenpi");
    if ((result?.data ?? []).length === 0) throw new Error("civitas not found");
  });
